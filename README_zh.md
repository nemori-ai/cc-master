...
[![plugin](https://img.shields.io/badge/plugin-v0.22.0-0A7EA4)](https://github.com/nemori-ai/cc-master/releases/tag/v0.22.0)
[![ccm](https://img.shields.io/badge/ccm-v0.23.0-111827)](https://github.com/nemori-ai/cc-master/releases/tag/ccm-v0.23.0)
...
> **许可说明：**本项目源代码可见，但仅授权非商业用途。公共协议不允许重新包装销售，也不允许
> 把本项目当作公司产品或商业服务使用。详见[许可说明](LICENSING.md)。
...
它这份「靠谱」背后有实际机制：可以**推演上千遍**估算交付时间和最可能拖期的步骤；把**全机缓存 quota posture**呈现给总指挥，让调速有证据；Claude Code 在宿主支持且已有授权时还能使用账号池。Codex、Cursor 和 kimi-code 都不会自动换号。这些是决策辅助和操作护栏，不保证 provider 限额、估算或交付日期永远没有意外。
...
- 干着干着**额度快到上限**了，缓存 posture 与 selected-target advice 提醒它放慢节奏；Claude Code 可按已有授权策略选择另一账号，Codex、Cursor 和 kimi-code 则留在当前登录态。
...
源码采用 paragoge 式 `plugin/src -> plugin/dist/<host>` 模型：共享 runtime skills 放在 canonical 源里，hooks 先建 host 无关的产品契约、再落各 host 的原生实现，每个 harness 都生成自己的 adapter 产物。插件版本线是一条；release asset 按 harness 拆分，例如 `cc-master-plugin-claude-code-<version>.zip`、`cc-master-plugin-codex-<version>.zip`、`cc-master-plugin-cursor-<version>.zip` 和 `cc-master-plugin-kimi-code-<version>.zip`。

我们对「做到了什么」和「还在做什么」分得很清楚。当前 adapter 包含 Claude Code、Codex、Cursor 和 kimi-code；全局 `ccm` 进程边界让任意 origin 都能看到同一套全机 inventory、缓存 quota posture、model-policy 视图、raw worker wrapper 与 Agent Registry。kimi-code adapter 分发的是共享 skills、host-native 的 namespaced 命令与核心编排 hooks；ccm 能读取它的 managed rolling usage 窗口，但 adapter 没有自定义 subagent 角色、Workflow 等价物、batch-boundary 事件、账号池、external statusline 或 mid-flight pacing hook。Cursor IDE plugin 和 Cursor Agent CLI 是两个独立 surface：安装或登录其中一个，不证明另一个可用。board 与已登记 agent 状态落在 `ccm` 及其只读 web viewer 上。**当前产品边界以 [产品功能手册](design_docs/feature-manual.md) 为准；[cross-harness capability model](design_docs/cross-harness-orchestration-capability-model.md) 是纵向成熟度与目标设计台账**，README 不复制两者。

当前 cross-harness worker 刻意保持窄小：`ccm worker help` 解析本机目标 CLI 的真实 agent-command help；`ccm worker run` 是对 board 零副作用的 raw transport；`ccm worker dispatch` 则在一次本机 session-bound run 外包上 Agent Registry 的 create/bind/link/terminal 记账，让 runtime actor 能出现在 Web Viewer。dispatch 本身是同步命令，所以调用方仍须持有外层 background handle，而且它绝不会替父任务判定验收通过。它不是自动路由或 fallback、统一 provider API、常驻 daemon，也不提供安全认证。`ccm agent` 仍只是可观测 registry，本身不负责 spawn worker。model-policy 中的条目在有 live qualification 和 admission 证据之前都只是 candidate 与 advisory。
...
harness 兼容资料放在 [`design_docs/harnesses/`](design_docs/harnesses/)。那里是本仓校对后的本地资料源：包含从 paragoge 迁移来的 adapter 模型，也包含当前 Claude Code / Codex / Cursor / kimi-code 的真实机制结论。
...
它会探测你的操作系统和架构，下对应的 `ccm` 二进制、放进 PATH，再识别本机已安装的 harness，并把对应 adapter 包分发到每个受支持目标。每个联网下载的 release asset 在安装前都会先下载同一 release 的 `SHA256SUMS`，按精确文件名校验；清单缺失、条目缺失或 digest 不匹配都会停止安装。Claude Code 安装走 `claude` CLI（≥ v2.1.195）。Codex 安装会注册本地 Codex marketplace/plugin，命令入口走插件分发的 `$cc-master-*` 技能（如 `$cc-master-as-master-orchestrator ...`）。Cursor 安装通过 local plugin 面把 adapter 发布到 `~/.cursor/plugins/local/cc-master`。kimi-code 安装把 adapter 复制进 `$KIMI_CODE_HOME/plugins/managed/cc-master/` 并在 `installed.json` 登记（managed-dir 安装，无需 TUI 步骤）。安装器在**所有模式**下都强制需要 Node.js 22 或更高版本，包括 pin 版本和 `CC_MASTER_INSTALL_LOCAL` 本地离线安装；此外还需 `unzip` 和一个 SHA256 工具（`sha256sum` / `shasum` / `openssl`），联网安装另需 `curl` 或 `wget`。每个 harness adapter 还可能需要对应 harness 的 CLI 或 config 目录已经存在。`ccm` 引擎是**硬前置**——没有它插件就不会开工——所以安装器先把它就位。
...
- **Start / resume** — Claude Code：`/cc-master:as-master-orchestrator <目标>` 或 `/cc-master:as-master-orchestrator --resume`；Codex：`$cc-master-as-master-orchestrator <目标>` 或 `$cc-master-as-master-orchestrator --resume`；Cursor：`/as-master-orchestrator <目标>` 或 `/as-master-orchestrator --resume`（装完后请重开 Agent session，让 hooks/rules 生效）；kimi-code：`cc-master:as-master-orchestrator <目标>` 或 `cc-master:as-master-orchestrator --resume`。fresh 会先 framing/check Goal Contract 再建任务；resume 会先检查当前 revision 与 Goal Brief 再派发。
...
- **选择模型角色** — `ccm model-policy show --task <task-taxonomy> --json` 展示四路统一的 O / T1 / T2 / T3 角色与证据；candidate 是 advisory，不等于 certified 或自动选中。
- **查看 / 运行 worker** — `ccm worker help --harness <codex|claude-code|cursor-agent|kimi-code>` 读取本机目标的真实 agent-command help。只要 raw、零 board 副作用的 transport 就用 `ccm worker run`；actor 需要登记、关联并在运行时可见，就用 `ccm worker dispatch --board … --task … --idempotency-key …`。两者都透传 target-native 参数；ccm 不会替你编造 provider flags 或 fallback chain。
- **查看已登记 worker** — `ccm agent list --json` 展示 board 的 runtime roster 与 lifecycle 证据；`ccm web-viewer open` 能追踪已登记 harness 实际提供的 transcript evidence。登记与 terminal 增强可观测性，但绝不会把父任务判为验收通过。
...
- **Distill** — Claude Code：`/cc-master:distill <retro-path...>`；Codex：`$cc-master-distill <retro-path...>`；Cursor：`/distill <retro-path...>`；kimi-code：`cc-master:distill <retro-path...>`。把复盘里的候选经验蒸馏成目标项目的实际资产（纪律文档一句指针 / skill / workflow / subagent）——经用户批准的蒸馏计划把关，并走 feature-branch PR 收口（非 git 项目降级为变更草稿目录）。绝不碰 board、不调用 `ccm`。
- **`ccm account add|list|switch <email>`** — 在 Claude Code 上建备用账号池并调度，好让已有授权策略在某个窗口用紧时换号。这几条在终端里敲；token 全程 token-blind，绝不进 AI 的 context。Codex、Cursor 和 kimi-code 都不会自动换号。
...
- **Cross-harness 成熟度、历史切片与目标设计** → [Capability model](design_docs/cross-harness-orchestration-capability-model.md)
...
本项目以 [PolyForm Noncommercial License 1.0.0](LICENSE) 提供源代码；公共协议不允许
商业使用。另见[许可说明](LICENSING.md)和[名称与标识政策](TRADEMARKS.md)。
