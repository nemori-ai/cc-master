...
- **hook** 是总指挥的运行时——它熬过 compaction（重注「你是总指挥 + 这是你的 board」）、把关收尾、对过度派发软警告、感知 5h/7d 配额墙。只在结构化 JSON 解析划算处（从 JSONL 算 usage）才用 `node`，其余一律 bash（[ADR-006](adrs/ADR-006-hooks-may-use-node-js.md)）。
