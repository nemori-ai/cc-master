...
它是一个「随处可用」（ship-anywhere）的 Claude Code 插件，把任意 main-session agent 变成长周期的 **master orchestrator（总指挥）**：把目标拆成依赖图、把后台工作并行派发、在每一个等待空档里让主线程**有产出地**持续推进，并熬过反复 compaction 与跨会话续接、全程不丢线索。它**不是一个 framework**——只是命令 + 5 个 skill + hooks + 一份 board 文件。
...
这个插件 = **命令 + 5 个 skill + hooks + 一份 board 文件**，每件各有各的寿命：
...
- **skill** 是按需调阅的深度手册——跑编排循环时翻 Skill A，写 workflow 脚本时翻 Skill B，管换号号池时翻 Skill C（`account-management`：建号池 registry、选最优切入号、token 只进 vault），用 `ccm` CLI 操作 board 时翻 Skill D（`using-ccm`：命令面、board 状态机心智、写关卡纪律），把目标切成 board DAG 时翻 Skill E（`slicing-goals-into-dags`：纵切薄增量 / walking skeleton / 粒度品味）。
