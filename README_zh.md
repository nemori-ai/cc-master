...
cc-master **致力于让** Claude Code agent 化身为一个 master orchestrator，具备：

1. **异步并行多线程推进、把目标完整落地**——不是干到一半，而是一路推到目标真正落地。
2. **控制资源（token 用量）消耗的速度**——感知配额窗口（如 5h / 7d）并节流，而非盲目顶满。
3. **把握好自主决策与寻求人类用户接入的边界**——懂得哪些自己拍板、哪些必须 surface 给用户（难撤销 / 对外可见 / 方向抉择 / 终审）。
4. **目标的分解、管理、更新、规划**——边干边学，持续拆解、管理、更新计划并重规划。
5. **在资源消耗速度合理的前提下最大化实施效率的调度编排**——在燃烧速率可控的约束下，把并行与调度编排到效率最大化。
6. **根据复杂性 / 难度 / 所需时长选择合适的模型**——按一件事的复杂度、难度、预计时长为每个节点选档。

这些是**指导设计的方向目标，而非「六条已全部兑现」的声称。** 哪些能力今天已落地、哪些仍是 design-only，由单独的 gap 审计度量。完整 charter 的单一真相源在 [`design_docs/spec.md` §1.0](design_docs/spec.md)。
...
| **配额感知** | 否 | 否 | 能——感知 5h/7d 窗口，按模型档位 · WIP · 推迟节流 |
