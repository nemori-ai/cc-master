...
它是一个「随处可用」（ship-anywhere）的 Claude Code 插件，把任意 main-session agent 变成长周期的 **master orchestrator（总指挥）**：把目标拆成依赖图、把后台工作并行派发、在每一个等待空档里让主线程**有产出地**持续推进，并熬过反复 compaction 与跨会话续接、全程不丢线索。它**不是一个 framework**——只是命令 + 7 个 skill + hooks + 一份 board 文件。
...
| **能否熬过 compaction** | 否 | 否 | **能**——角色 + board 被重注 | `reinject.js`（SessionStart hook） |
...
**第 3 拍 —— 熬过一次 compaction（最难的一环）。** 长任务意味着上下文会被填满、compaction 会*整个*抹掉「我是总指挥」这件事——而 agent 没法替自己重注它，因为「它曾有个角色」这份记忆正是被抹掉的那份。`SessionStart`（含 `source:compact`）时，`reinject.js` 从 context **之外**重注：*「You are a cc-master master orchestrator. 你的 board 在 `<home>` … 按 goal 认回它 … 别重做已完成/已验收的活，先整合已完成的后台结果。」* board 带过了*进度*，hook 带过了*身份*。

**第 4 拍 —— 就绪即发，且随机应变。** `T0` 落地、在端点被验收（总指挥亲读 diff——green gate *不算*通过）。locale 叶子 `blocked → ready`，在 `wip_limit: 4` 下并行派发。然后计划撞上现实：`ar` 不只是翻译——右到左（RTL）排版要真功夫——于是总指挥把它 **escalate 成一个 `workflow`** 并重规划，而不是硬塞一个不再合身的叶子。与此同时 `usage-pacing.js` 察觉这轮已逼近 5h 配额墙，注入一条**非阻断**轻推；总指挥就此节流——降 WIP、推迟优先级最低的 locale——而非顶满硬撞。万一它想在 board 还剩 `ready` 活时收尾，`verify-board.js` 当场拦死：*「这块 board 还有 `ready` 任务……停之前先把它处理掉。」*
...
| **C5** | 在合理燃烧速率*之下*最大化吞吐 | 🟢 Live | WIP cap（~75% 利用率）+ 免费 float 并行 + `posttool-batch.js` 软警告 |
...
这个插件 = **命令 + 7 个 skill + hooks + 一份 board 文件**，每件各有各的寿命：
...
│   ├── slicing-goals-into-dags/        Skill E —— 敏捷把目标切成 board DAG
│   ├── dev-as-ml-loop/                 Skill F —— 把任务推进到验收当成 ML 优化循环
│   └── engineering-with-craft/         Skill G —— 设计/开发/测试的工程手艺（DDD/SDD/TDD/OOP）
└── hooks/
...
- **skill** 是按需调阅的深度手册——跑编排循环时翻 Skill A，写 workflow 脚本时翻 Skill B，管换号号池时翻 Skill C（`account-management`：建号池 registry、选最优切入号、token 只进 vault），用 `ccm` CLI 操作 board 时翻 Skill D（`using-ccm`：命令面、board 状态机心智、写关卡纪律），把目标切成 board DAG 时翻 Skill E（`slicing-goals-into-dags`：纵切薄增量 / walking skeleton / 粒度品味），作为执行 agent 把一个任务推进到验收时翻 Skill F（`dev-as-ml-loop`：把 dev loop 当 ML 优化过程——验收=目标函数、迭代测量、收敛即停），动手设计 / 开发 / 测试真实代码时翻 Skill G（`engineering-with-craft`：DDD/SDD/TDD/OOP 整合成五条共享根 + 工程红线——与 F 不同 plane：F 给循环形状、G 给循环里的手艺内容）。
...
2. **`SessionStart`**（`reinject.js`）在每次 compaction 后、以及 resume 时重注角色 + board——resume 时还会**报出上一轮 plan 更新未对账遗留的 `stale`/`escalated` 悬挂节点**。
3. **`Stop`**（`verify-board.js`）对**本 session** 的 active board 跑一道 node 的门（board-v2 收编自 bash·共用 board-model 数据层）（按 `owner.session_id` 过滤，所以并发编排互不干扰）。board 为空、或还剩 `ready`/`uncertain` 的活，就 **block** 住这次 Stop；当 board 看起来完成了，它会逼一次对照 goal 的**自检**——并**列出未答的 `blocked_on:user` 决策**——才放行；还有一道 fuse（连续 block 5 次）兜底，防止误判把 agent 永久焊死。
...
5. **`PostToolBatch`**（`posttool-batch.js`）在一批并行调用后，数 in_flight 任务对 board 的 `wip_limit`，过度派发时**软警告**——绝不 block，并行自由照旧。
