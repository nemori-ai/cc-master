'use strict';
// registry.js — ccm 命令 SSOT（noun → verb → spec·cli-design §3 / 契约 §三 registry.js）。
//
...
//
// 红线1 / ADR-006：node/JS only，零 npm 依赖、纯 stdlib（只 require 兄弟 board-model 取 ENUMS）。CommonJS。
// 武装闸豁免：纯声明库（无 hook 入口，只被 CLI / router require）——见 AGENTS.md §3 红线6 / §12 grep 门豁免。
...
const REGISTRY = {
  ...
  board: {
    ...
    update: {
      ...
      options: {
        ...
        'wip-limit': { type: 'string', field: 'scheduling.wip_limit', desc: 'scheduling.wip_limit（并发软上限）' },
        'owner-wip': { type: 'string', field: 'scheduling.owner_wip_limit', desc: 'scheduling.owner_wip_limit' },
  ...
  task: {
    add: {
      ...
      options: {
        ...
        type: { type: 'string', field: 'type', enum: E.taskType, desc: '任务类型（开放·未知值 warn）' },
        ...
        estimate: { type: 'string', field: 'estimate', transform: 'duration', desc: '估时 3h/90m/2d/1w' },
        ref: { type: 'string', field: 'references', transform: 'ref', multiple: true, desc: '引用 kind:ref（可重复）' },
        accept: { type: 'string', field: 'acceptance', transform: 'input', desc: '验收：一句话 DoD 或 @file' },
        role: { type: 'string', field: 'role', enum: E.role, desc: '调度角色（默认 normal）' },
        justification: { type: 'string', field: 'justification', desc: '决策理由' },
        status: { type: 'string', field: 'status', enum: E.status, desc: '初始 status（默认 ready）' },
        ...
        set: { type: 'string', transform: 'kv', multiple: true, desc: '通用设任意 ✎ 标量（path=val）' },
        'set-json': { type: 'string', transform: 'json', multiple: true, desc: '通用设任意 ✎ 对象/数组（path=json）' },
    ...
    update: {
      ...
      options: {
        ...
        'wip-limit': { type: 'string', field: 'wip_limit', desc: '本 task WIP 覆写' },
        accept: { type: 'string', field: 'acceptance', transform: 'input', desc: '验收：一句话 DoD 或 @file' },
        'add-dep': { type: 'string', field: 'addDep', transform: 'csv', multiple: true, desc: '增依赖（可重复）' },
        'rm-dep': { type: 'string', field: 'rmDep', transform: 'csv', multiple: true, desc: '删依赖（可重复）' },
        'add-ref': { type: 'string', field: 'addRef', transform: 'ref', multiple: true, desc: '增引用 kind:ref（可重复）' },
        'rm-ref': { type: 'string', field: 'rmRef', transform: 'csv', multiple: true, desc: '删引用（按 ref·可重复）' },
        parent: { type: 'string', field: 'parent', desc: '改归属（""=升为顶层）' },
        set: { type: 'string', transform: 'kv', multiple: true, desc: '通用设 ✎ 标量（path=val）' },
        'set-json': { type: 'string', transform: 'json', multiple: true, desc: '通用设 ✎ 对象/数组（path=json）' },
        log: { type: 'string', desc: '同时追一条 log' },
      },
      examples: [
    ...
    block: {
      ...
      options: {
        ...
        decision: { type: 'string', transform: 'input', desc: '--on user 时必给 decision_package（@file 或 -）' },
        log: { type: 'string', desc: '同时追一条 log' },
      },
      examples: ['ccm task block T7 --on T2', 'ccm task block T9 --on user --decision @/abs/decision.json'],
      handler: 'task.block',
    },
    'set-status': {
      summary: '通用状态转移',
  ...
  log: {
    add: {
      ...
      options: {
        ...
        ref: { type: 'string', field: 'refs', transform: 'csv', multiple: true, desc: '关联引用（可重复）' },
      },
      examples: [
  ...
  jc: {
    add: {
      ...
      options: {
        ...
        refs: { type: 'string', field: 'refs', transform: 'csv', multiple: true, desc: '佐证引用（可重复）' },
        'task-ref': { type: 'string', field: 'task_ref', desc: '关联 task' },
        set: { type: 'string', transform: 'kv', multiple: true, desc: '通用设 ✎ 标量（path=val）' },
        'set-json': { type: 'string', transform: 'json', multiple: true, desc: '通用设 ✎ 对象/数组（path=json）' },
      },
      examples: [
        'ccm jc add "选 ICU MessageFormat" --category architecture --severity high',
      ],
      handler: 'jc.add',
    },
    list: {
    ...
    resolve: {
      ...
      positionals: [{ name: 'id', required: true }],
      options: {
        status: { type: 'string', field: 'status', enum: ['upheld', 'overturned'], required: true, desc: 'upheld（维持）| overturned（推翻）' },
  ...
  cadence: {
    update: {
      ...
      positionals: [],
      options: {
        'ship-every': { type: 'string', field: 'cadence.target.ship_every', transform: 'duration', desc: 'target.ship_every（如 3h）' },
        'min-unit': { type: 'string', field: 'cadence.target.min_unit', desc: 'target.min_unit（如 "1 PR"）' },
        set: { type: 'string', transform: 'kv', multiple: true, desc: '通用设 ✎ 标量（path=val）' },
        'set-json': { type: 'string', transform: 'json', multiple: true, desc: '通用设 ✎ 对象/数组（path=json）' },
      },
    ...
    open: {
      ...
      options: {
        ...
        members: { type: 'string', field: 'members', transform: 'csv', desc: '纳入本 iteration 的 task（逗号分隔）' },
        set: { type: 'string', transform: 'kv', multiple: true, desc: '通用设 ✎ 标量（path=val）' },
        'set-json': { type: 'string', transform: 'json', multiple: true, desc: '通用设 ✎ 对象/数组（path=json）' },
      },
      examples: [
  ...
  watchdog: {
    arm: {
      ...
      positionals: [],
      options: {
        'fire-at': { type: 'string', field: 'fireAt', required: true, desc: '触发时刻（严格 ISO-8601 UTC）' },
        mechanism: { type: 'string', field: 'mechanism', enum: E.watchdogMechanism, required: true, desc: 'cron|loop|monitor|shell（降级链）' },
...
//   通配逃生口。供 registry.test.mjs 断言「每个 writer=agent 经 CLI 的 FIELDS 字段都有 CLI 入口」。
//   返回 { fields:Set<string>, hasSet:boolean, hasSetJson:boolean }。
function WRITABLE_FIELDS_COVERED() {
  const fields = new Set();
  let hasSet = false;
  let hasSetJson = false;
  for (const noun of Object.keys(REGISTRY)) {
    for (const verb of Object.keys(REGISTRY[noun])) {
      const spec = REGISTRY[noun][verb];
...
module.exports = { REGISTRY, ALIASES, WRITABLE_FIELDS_COVERED };
