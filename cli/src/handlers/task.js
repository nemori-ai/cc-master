'use strict';
// handlers/task.js — task noun handler（add/show/list/update/start/done/block/set-status/rm·cli-design §3.2）。
//
// 照搬 log.js 范式：每 verb 一个 handler(ctx) → exitCode；写 verb 用 _common.runWrite（resolve / mutate / render
//   三回调），读 verb 用 runRead（resolve / compute / render）。buildFields 把 parsed flags 按 registry 的
//   field/transform 映射成 mutation 入参。handler **直接 require leaf 模块**（mutations / render / registry），
...
function applyOps(board, sets, setJsons) {
  let b = board;
...
// ── 内部：若给了 --log 则追一条 log（与主写动作同一笔落盘·log.js 范式外延）。
function maybeLog(board, ctx, summaryFallback) {
  const logMsg = ctx.values && ctx.values.log;
  if (logMsg === undefined) return board;
  return mutations.appendLog(board, { summary: typeof logMsg === 'string' ? logMsg : summaryFallback });
}
...
function add(ctx) {
  ...
  return runWrite(ctx, {
    mutate: (board) => {
      ...
      let next = mutations.addTask(board, args);
    ...
    render: (next, c, { dryRun }) => {
      ...
      if (c.flags.json) {
        const t = (next.tasks || []).find((x) => x && x.id === id);
...
function update(ctx) {
  ...
  return runWrite(ctx, {
    mutate: (board) => {
      const id = ctx.positionals[0];
      const { fields, sets, setJsons } = buildFields(ctx.values, spec, { stdin: ctx.stdin });
      let next = mutations.updateTask(board, id, fields);
      next = applyOps(next, sets, setJsons);
...
function _transitionVerb(ctx, toStatus, label) {
  const id = ctx.positionals[0];
  return runWrite(ctx, {
    mutate: (board) => {
      // done verb 还可带 --artifact / --verified（先转移盖时间戳，再覆写产物字段）。
      let next = mutations.transition(board, id, toStatus, { force: ctx.flags.force });
      if (toStatus === 'done') {
        const fields = {};
    ...
    render: (next, c, { dryRun }) => {
      ...
      return dryRun ? `[dry-run] 将 ${label} task: ${id} (→ ${toStatus})` : `task ${id} → ${toStatus}`;
    },
  });
}
function start(ctx) {
  ...
  return runWrite(ctx, {
    mutate: (board) => {
      ...
      if (decisionRaw !== undefined) {
        const text = io.readInputSpec(decisionRaw, { stdin: ctx.stdin });
        let parsed = text;
        try { parsed = JSON.parse(text); } catch (_e) { /* 非 JSON → 留原文（lint 会按形状校验） */ }
        args.decisionPackage = parsed;
      }
      let next = mutations.blockTask(board, id, args);
...
function setStatus(ctx) {
  const id = ctx.positionals[0];
  const toStatus = ctx.positionals[1];
  return runWrite(ctx, {
    mutate: (board) => {
      let next = mutations.transition(board, id, toStatus, { force: ctx.flags.force });
...
function rm(ctx) {
  ...
  return runWrite(ctx, {
    mutate: (board) => {
      ...
      if (!exists) {
        const e = new Error(`task not found: ${id}`);
        e.errKind = 'NotFound';
        throw e;
      }
      ...
      if (!b.owner || typeof b.owner !== 'object') b.owner = { active: true, session_id: '', heartbeat: '' };
    ...
    render: (next, c, { dryRun }) => {
      if (c.flags.json) {
        return render.renderTaskList(next.tasks || [], { json: true });
      }
...
function show(ctx) {
  return runRead(ctx, {
    ...
    render: (task, c) => render.renderTaskDetail(task, { json: !!c.flags.json, color: c.flags.color }),
  });
}
...
function list(ctx) {
  return runRead(ctx, {
    compute: (board) => {
      ...
      const statusFilter = v.status === undefined
        ? null
        : (Array.isArray(v.status) ? v.status : [v.status]);
    ...
    render: (tasks, c) => render.renderTaskList(tasks, { json: !!c.flags.json, color: c.flags.color }),
  });
}

module.exports = { add, show, list, update, start, done, block, setStatus, rm };
