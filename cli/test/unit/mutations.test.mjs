// mutations.test.mjs — P5.1·board v2 写 SSOT（纯函数·零 IO）契约门。
//
// mutations.js 是写入关卡管线的「mutate(raw,args)→next」纯函数环节（cli-design §5 步骤 4）：
...
//
// 范式照搬 board-model.test.mjs：.mjs + node:test + node:assert/strict，经 createRequire 加载 CJS。
...
const REL = 'cli/src/mutations.js';
const PATH = join(ROOT, REL);

test('mutations.js exists in cli/src (CLI package, requirable)', () => {
  assert.ok(existsSync(PATH), 'mutations.js exists in cli/src');
});

const m = require(PATH);
const model = require(join(ROOT, 'cli/src/board-model.js'));
...
test('boardUpdate updates goal / scheduling / git; stamps heartbeat; does NOT mutate input', () => {
  ...
  const b = m.boardUpdate(orig, { goal: 'new goal', wipLimit: 8, ownerWip: 3, branch: 'feat/x', worktree: '/ww' });
...
test('addTask appends a task with defaults (status ready, deps [], created_at stamped)', () => {
  ...
  const t = b.tasks.find((x) => x.id === 'T9');
...
test('addTask honors explicit fields incl deps, type, executor, references', () => {
  const b = m.addTask(baseBoard(), {
    id: 'T9', deps: ['T0', 'T1'], type: 'development', executor: 'subagent', handle: 'bg-1',
...
test('updateTask overwrites flexible fields; stamps heartbeat; pure', () => {
  ...
  const t = b.tasks.find((x) => x.id === 'T1');
...
test('updateTask never changes id; unknown task → NotFound throw', () => {
  ...
  const t = b.tasks.find((x) => x.title === 'x');
  assert.equal(t.id, 'T1', 'id is immutable via updateTask');
  assert.throws(() => m.updateTask(baseBoard(), 'NOPE', { title: 'x' }), (e) => e.errKind === 'NotFound');
});
...
test('transition illegal with {force:true} is allowed (越闸)', () => {
  ...
  const t = b.tasks.find((x) => x.id === 'T0');
...
test('transition idempotent from===to is legal (no throw)', () => {
  ...
  assert.equal(b.tasks.find((x) => x.id === 'T1').status, 'ready');
});

test('transition unknown task → NotFound', () => {
  assert.throws(() => m.transition(baseBoard(), 'NOPE', 'done', {}), (e) => e.errKind === 'NotFound');
});
...
test('addJc appends a judgment_call with status pending_review + raised_at stamped', () => {
  const b = m.addJc(baseBoard(), {
    id: 'J1', summary: 'chose X over Y', category: 'architecture', severity: 'high',
    decision: 'adopt X', rationale: 'standard', impact: 'wide', refs: ['commit abc'], task_ref: 'T0',
  });
...
test('resolveJc sets status upheld/overturned + note; unknown → NotFound', () => {
  let b = m.addJc(baseBoard(), { id: 'J1', summary: 's', category: 'architecture', severity: 'low' });
  ...
  assert.throws(() => m.resolveJc(b, 'NOPE', { status: 'upheld' }), (e) => e.errKind === 'NotFound');
});
...
test('cadenceOpen opens an iteration {id, started_at, status:open, goal, deadline, members}', () => {
  const b = m.cadenceOpen(baseBoard(), 'I1', { goal: 'ship slice', deadline: '2026-06-05T14:00:00Z', members: ['T0', 'T1'] });
...
test('cadenceShip sets status=shipped WITHOUT checking member completeness (that is lint BIZ-CADENCE-SHIPPED)', () => {
  ...
  const it = b.cadence.iterations[0];
  assert.equal(it.status, 'shipped', 'mutations only flips status; member completeness is lint job');
  assert.ok(ISO.test(it.shipped_at));
});

test('cadenceShip unknown iteration → NotFound', () => {
  assert.throws(() => m.cadenceShip(baseBoard(), 'NOPE', {}), (e) => e.errKind === 'NotFound');
  assert.throws(() => m.cadenceOpen(baseBoard(), 'I1', {}) && m.cadenceShip(baseBoard(), 'I1', {}), (e) => e.errKind === 'NotFound');
});

// ── watchdog ─────────────────────────────────────────────────────────────────────────────────────
test('watchdogArm writes full object {armed_at, fire_at, mechanism, job_id, checklist}', () => {
  const b = m.watchdogArm(baseBoard(), { fireAt: '2026-06-24T13:00:00Z', mechanism: 'cron', jobId: 'job-1', checklist: ['drain'] });
...
test('applySet creates intermediate objects for nested ✎ dotpath', () => {
  ...
  assert.equal(b.tasks.find((x) => x.id === 'T1').observability.source, 'task-notification');
});
...
test('applySet REFUSES board 🔒 fields (schema/goal/owner/git/tasks) with errKind=Validation', () => {
  for (const p of ['schema', 'goal', 'owner.active', 'owner.session_id', 'git.branch', 'tasks']) {
    assert.throws(() => m.applySet(baseBoard(), p, 'x'), (e) => {
...
test('applySet REFUSES task 🔒 fields (id/status/deps/parent) with errKind=Validation', () => {
  for (const p of ['tasks[T1].id', 'tasks[T1].status', 'tasks[T1].deps', 'tasks[T1].parent',
    'tasks.T1.status']) {
    assert.throws(() => m.applySet(baseBoard(), p, 'x'), (e) => {
      assert.equal(e.errKind, 'Validation', `${p} → Validation`);
      return true;
...
test('applySet refuses replacing a whole task object (tasks[<id>]) — Validation', () => {
  assert.throws(() => m.applySet(baseBoard(), 'tasks[T1]', { id: 'T1' }), (e) => e.errKind === 'Validation');
});

test('applySet on unknown task id throws NotFound (after 🔒 gate passes)', () => {
  assert.throws(() => m.applySet(baseBoard(), 'tasks[NOPE].hitl_rounds', 1), (e) => e.errKind === 'NotFound');
});

test('applySet invalid/empty dotpath → Validation', () => {
  assert.throws(() => m.applySet(baseBoard(), '', 'x'), (e) => e.errKind === 'Validation');
});
...
test('applySetJson parses JSON string into a flexible field', () => {
  ...
  assert.deepEqual(b.tasks.find((x) => x.id === 'T1').output_schema, { type: 'object', ok: true });
});

test('applySetJson accepts an already-parsed object/array', () => {
  const b = m.applySetJson(baseBoard(), 'tasks[T1].dep_pins', { T0: 'sha256:abc' });
  assert.deepEqual(b.tasks.find((x) => x.id === 'T1').dep_pins, { T0: 'sha256:abc' });
});

test('applySetJson bad JSON string → Validation', () => {
  assert.throws(() => m.applySetJson(baseBoard(), 'meta.x', '{bad json'), (e) => e.errKind === 'Validation');
});

test('applySetJson honors 🔒 gate too (refuses deps)', () => {
  assert.throws(() => m.applySetJson(baseBoard(), 'tasks[T1].deps', '["X"]'), (e) => e.errKind === 'Validation');
});
