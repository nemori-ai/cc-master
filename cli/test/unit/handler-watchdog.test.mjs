// handler-watchdog.test.mjs — P5.2·watchdog noun handler（handlers/watchdog.js）契约门。
//
// watchdog.js 照 log.js 范式：arm（runWrite + mutations.watchdogArm + render）、disarm（runWrite + watchdogDisarm·幂等）、
...
const wdHandler = require(join(SRC, 'handlers', 'watchdog.js'));
...
test('watchdog arm replaces an existing watchdog (whole-object write)', () => {
  const boardPath = mkBoardHome({
    watchdog: { armed_at: '2026-06-23T00:00:00Z', fire_at: '2026-06-23T01:00:00Z', mechanism: 'cron', job_id: 'old' },
  });
  const ctx = mkCtx(boardPath, {
...
test('watchdog disarm sets watchdog to null (no residue)', () => {
  const boardPath = mkBoardHome({
    watchdog: { armed_at: '2026-06-24T10:00:00Z', fire_at: '2026-06-24T12:00:00Z', mechanism: 'cron' },
  });
  const ctx = mkCtx(boardPath);
...
test('watchdog disarm hints external job-id cleanup when one existed', () => {
  const boardPath = mkBoardHome({
    watchdog: { armed_at: '2026-06-24T10:00:00Z', fire_at: '2026-06-24T12:00:00Z', mechanism: 'cron', job_id: 'cron-xyz' },
  });
  const ctx = mkCtx(boardPath);
...
test('watchdog status reads an armed watchdog (human field list)', () => {
  const boardPath = mkBoardHome({
    watchdog: { armed_at: '2026-06-24T10:00:00Z', fire_at: '2026-06-24T12:00:00Z', mechanism: 'cron', job_id: 'cron-abc', checklist: '查后台' },
  });
  const ctx = mkCtx(boardPath);
