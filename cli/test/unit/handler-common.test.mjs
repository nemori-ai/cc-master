// handler-common.test.mjs — P5.2·共享读 / 写关卡 runner（handlers/_common.js）契约门。
//
// _common.js 是写入关卡管线（cli-design §5）的执行体：runWrite（resolve → 锁 → 读 → mutate → lint → 拒/写）、
//   runRead（resolve → 算 → render）、buildFields（flag → FIELDS dotpath 映射 + --set/--set-json 收集）。
//   本测试用 mkdtemp 临时 home + 真 leaf（io/discover/mutations/lint-core），断言：
...
const common = require(join(SRC, 'handlers', '_common.js'));
const io = require(join(SRC, 'io.js'));
const mutations = require(join(SRC, 'mutations.js'));
const { REGISTRY } = require(join(SRC, 'registry.js'));
const EXIT = io.EXIT;
...
function mkBoardHome({ sid = 'sid-test', goal = 'a goal', tasks = [] } = {}) {
...
function mkCtx(boardPath, { values = {}, flags = {}, positionals = [], sid = 'sid-test' } = {}) {
  const outBuf = [];
  const errBuf = [];
  const ctx = {
...
test('runWrite happy: board atomically written + render out', () => {
  ...
  const code = common.runWrite(ctx, {
    mutate: (board) => mutations.appendLog(board, { summary: ctx.positionals[0], kind: 'note' }),
    render: (next, c, { dryRun }) => (dryRun ? 'preview' : 'written'),
  });
...
test('runWrite lint hard error → VALIDATION, board NOT written', () => {
  ...
  const code = common.runWrite(ctx, {
    ...
    mutate: (board) => { const b = structuredClone(board); b.goal = 12345; return b; },
...
test('runWrite --dry-run: full lint runs but board NOT written', () => {
  ...
  const code = common.runWrite(ctx, {
    mutate: (board) => mutations.appendLog(board, { summary: 'dryrun entry' }),
    render: (next, c, { dryRun }) => (dryRun ? 'preview-out' : 'written-out'),
  });
  assert.equal(code, EXIT.OK);
...
test('runWrite --force overrides hard error and writes', () => {
  ...
  const code = common.runWrite(ctx, {
    mutate: (board) => { const b = structuredClone(board); b.goal = 999; return b; },
    render: () => 'forced-write',
  });
  assert.equal(code, EXIT.OK);
...
test('runWrite warnings surfaced to stderr but do not block write', () => {
  ...
  const ctx = mkCtx(boardPath);
  const code = common.runWrite(ctx, {
    mutate: (board) => mutations.addTask(board, { id: 'TW', type: 'development' }),
  ...
  const onDisk = JSON.parse(readFileSync(boardPath, 'utf8'));
  assert.ok(onDisk.tasks.find((t) => t.id === 'TW'));
});
...
test('runWrite does NOT catch mutation throw (bubbles to router)', () => {
  ...
  assert.throws(
    () => common.runWrite(ctx, {
      ...
      mutate: (board) => mutations.transition(board, 'NOPE', 'in_flight', {}),
      render: () => 'x',
    }),
    (e) => e.errKind === 'NotFound',
  );
});
...
test('runRead happy: resolves board, computes, renders', () => {
  ...
  const code = common.runRead(ctx, {
    compute: (board) => board.tasks.filter((t) => t.status === 'ready'),
    render: (tasks) => `ready=${tasks.length}`,
  });
  assert.equal(code, EXIT.OK);
