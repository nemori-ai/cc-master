// handler-cadence.test.mjs — P5.2·cadence noun handler（handlers/cadence.js）契约门。
//
// cadence.js 四 verb：update / open / ship（runWrite）+ status（runRead）。本测试用 mkdtemp 临时 home
...
const cadence = require(join(SRC, 'handlers', 'cadence.js'));
...
function mkBoardHome({ tasks = [doneTask('T0'), doneTask('T1')], cadenceObj } = {}) {
...
test('cadence update honors --set (generic flexible-path escape hatch)', () => {
  const boardPath = mkBoardHome();
  const ctx = mkCtx(boardPath, { values: { 'ship-every': '3h', set: ['cadence.note=experimental'] } });
  const code = cadence.update(ctx);
  assert.equal(code, EXIT.OK);
  const onDisk = JSON.parse(readFileSync(boardPath, 'utf8'));
...
test('cadence open --json renders the new iteration', () => {
  const boardPath = mkBoardHome();
  const ctx = mkCtx(boardPath, { values: { goal: 'g', members: 'T0' }, flags: { json: true }, positionals: ['I2'] });
  const code = cadence.open(ctx);
  assert.equal(code, EXIT.OK);
  const parsed = JSON.parse(ctx.outBuf.join(''));
  assert.equal(parsed.ok, true);
...
test('cadence ship marks iteration shipped when members are all done+verified', () => {
  const boardPath = mkBoardHome({
    cadenceObj: {
      ...
      iterations: [{ id: 'I1', started_at: '2026-06-24T08:00:00Z', deadline: '2026-06-24T14:00:00Z', goal: 'g', members: ['T0', 'T1'], status: 'open' }],
    },
  });
...
test('cadence ship → VALIDATION (3) when a member is not done+verified (BIZ-CADENCE-SHIPPED hard gate)', () => {
  ...
  const boardPath = mkBoardHome({
    ...
    cadenceObj: {
      ...
      iterations: [{ id: 'I1', started_at: '2026-06-24T08:00:00Z', members: ['T0', 'T1'], status: 'open' }],
    },
  });
  const before = readFileSync(boardPath, 'utf8');
  const ctx = mkCtx(boardPath, { positionals: ['I1'] });
  ...
  assert.ok(ctx.errBuf.join('').includes('BIZ-CADENCE-SHIPPED'), 'lint report names the rule on stderr');
});

test('cadence ship on a non-existent iter-id throws NotFound (router → exit 5)', () => {
  const boardPath = mkBoardHome({
    cadenceObj: { target: { ship_every: '3h' }, iterations: [{ id: 'I1', started_at: '2026-06-24T08:00:00Z', members: ['T0'], status: 'open' }] },
  });
...
test('cadence status renders target + iterations (human)', () => {
  const boardPath = mkBoardHome({
    cadenceObj: {
      target: { ship_every: '3h', min_unit: '1 PR' },
      iterations: [{ id: 'I1', started_at: '2026-06-24T08:00:00Z', goal: 'slice 1', members: ['T0', 'T1'], status: 'open' }],
    },
  });
  const ctx = mkCtx(boardPath);
...
test('cadence status --json returns the cadence object', () => {
  const boardPath = mkBoardHome({
    cadenceObj: { target: { ship_every: '3h' }, iterations: [{ id: 'I1', started_at: '2026-06-24T08:00:00Z', members: [], status: 'open' }] },
  });
