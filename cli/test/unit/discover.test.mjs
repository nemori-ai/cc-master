// discover.test.mjs — P5.1·板 / home 解析（CLI 发现层·设计稿 §6 + 契约 §三）契约门。
//
// discover.js 是 CLI 的发现层：把「现在该读 / 写哪块 board」按确定性优先级解成 {boardPath, board}。
...
//   依赖反转：自带 boardMatches（不 require hooks/hook-common·守红线）。红线6：只读 owner.session_id，绝不写。
//
// 用 mkdtempSync 造临时 home + XDG state，写多块假 board.json，设 env，断言全优先级路径；afterEach 清临时 dir。
...
const D = require(join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'src', 'discover.js'));
...
function writeBoard(home, name, { active = true, sessionId = '', goal = 'a goal' } = {}) {
...
test('exposes the contracted surface', () => {
  for (const fn of ['resolveHome', 'boardMatches', 'pointerPath', 'readPointer', 'writePointer', 'deletePointer', 'resolveBoard']) {
    assert.equal(typeof D[fn], 'function', `discover exports ${fn}`);
  }
});
...
test('resolveHome: --home wins over everything', () => {
  assert.equal(D.resolveHome({ homeFlag: '/explicit/home', env: { CC_MASTER_HOME: '/x' } }), '/explicit/home');
});
...
test('resolveHome: nothing found → throw .errKind=NotFound', () => {
  ...
  try {
    process.chdir(empty);
    assert.throws(() => D.resolveHome({ env: {} }), (e) => e.errKind === 'NotFound');
  } finally {
    process.chdir(origCwd);
  }
});
...
test('pointerPath: ($XDG_STATE_HOME||~/.local/state)/cc-master/boards/<sid>.path', () => {
  const xdg = mkXdg();
  assert.equal(D.pointerPath('sid-1', { XDG_STATE_HOME: xdg }), join(xdg, 'cc-master', 'boards', 'sid-1.path'));
});
...
test('resolveBoard: sid anchor (no pointer) → boardMatches exact', () => {
  ...
  const xdg = mkXdg();
  const { boardPath } = D.resolveBoard({ sid: 's-bbb', env: { XDG_STATE_HOME: xdg, CC_MASTER_HOME: home } });
...
test('resolveBoard: no sid, multiple active disambiguated by goalSubstr', () => {
  ...
  const b2 = writeBoard(home, '02.board.json', { active: true, goal: 'refactor the database layer' });
