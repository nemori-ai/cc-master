// help.test.mjs — P5.2·help 渲染层（help.js）契约门。
//
// help.js 导出 printHelp(out, registry, noun?, verb?) + printVersion(out)（技术栈契约 §二 help.js）。
...
//     · printVersion：'ccm <ver>'，<ver> 形如 \d+.\d+.\d+（fallback 到 plugin.json·cli/package.json 尚未建）。
...
const help = require(join(SRC, 'help.js'));
...
test('printHelp task lists all task verbs (incl ls alias mark)', () => {
  ...
  const t = c.text();
  for (const verb of ['add', 'show', 'list', 'update', 'start', 'done', 'block', 'set-status', 'rm']) {
