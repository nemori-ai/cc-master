// registry.test.mjs — P5.2·命令 SSOT（registry.js）契约门 + 反漂移门。
//
// registry.js 是 CLI 命令面的单一真相源（noun → verb → spec）。本测试钉死：
...
const { REGISTRY, ALIASES, WRITABLE_FIELDS_COVERED } = require(join(SRC, 'registry.js'));
const model = require(join(SRC, 'board-model.js'));
...
test('REGISTRY covers all 6 namespaces with all their verbs', () => {
  ...
  for (const noun of Object.keys(EXPECTED)) {
    assert.deepEqual(
      Object.keys(REGISTRY[noun]).sort(),
      EXPECTED[noun].slice().sort(),
      `noun ${noun} verbs`,
    );
  }
});
...
test('positionals carry name + required:bool', () => {
  for (const noun of Object.keys(REGISTRY)) {
    for (const verb of Object.keys(REGISTRY[noun])) {
      for (const p of REGISTRY[noun][verb].positionals) {
...
test('options carry type string|boolean and optional metadata', () => {
  for (const noun of Object.keys(REGISTRY)) {
    for (const verb of Object.keys(REGISTRY[noun])) {
      const opts = REGISTRY[noun][verb].options;
      for (const flag of Object.keys(opts)) {
        ...
        if (o.multiple !== undefined) assert.equal(typeof o.multiple, 'boolean', `${where} multiple`);
        if (o.required !== undefined) assert.equal(typeof o.required, 'boolean', `${where} required`);
    ...
    for (const verb of Object.keys(REGISTRY[noun])) {
      ...
      for (const flag of Object.keys(opts)) {
        ...
        if (o.transform !== undefined) {
          assert.ok(KNOWN.has(o.transform), `${noun} ${verb} --${flag} transform=${o.transform} unknown`);
        }
      }
    }
  }
...
const EXEMPT = {
  board: new Set(['owner', 'tasks', 'log', 'judgment_calls', 'cadence', 'watchdog', 'meta', 'git', 'scheduling']),
  task: new Set([
    'id', // 经 `task add <id>` 的 positional 写入（🔒·非 flag·绝不 --set）
    'status', 'deps', 'started_at', 'finished_at', 'created_at',
    'blocked_on', 'decision_package',
  ]),
};
...
test('anti-drift: common dedicated flags are registered as writable fields', () => {
  const covered = WRITABLE_FIELDS_COVERED();
  for (const f of ['title', 'description', 'type', 'executor', 'handle', 'estimate', 'references', 'role', 'justification', 'artifact', 'verified', 'goal']) {
