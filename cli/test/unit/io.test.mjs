// io.test.mjs — P5.1·io.js（运行时 IO 工具箱）契约门。
//
// 钉死：EXIT enum、isTTY、resolveColor 优先级矩阵全覆盖、paint、readInputSpec（- / @file / 字面量）、
//   writeFileAtomicSync（内容完整 + temp 清理）、jsonOk/jsonErr 壳、parseDuration（happy + 坏）。
// CJS 经 createRequire 加载（照搬 board-model.test.mjs 范式）。
...
const IO = 'cli/src/io.js';

test('io.js exists in cli/src (CLI package, requirable)', () => {
  assert.ok(existsSync(join(ROOT, IO)), 'io.js exists in cli/src');
});

const io = require(join(ROOT, IO));

// ── EXIT enum ────────────────────────────────────────────────────────────────────────────────────
test('EXIT enum carries the agreed distinct codes', () => {
  assert.deepEqual(io.EXIT, {
    OK: 0, ERROR: 1, USAGE: 2, VALIDATION: 3, LOCKED: 4, NOT_FOUND: 5,
  });
});
...
test('resolveColor ① NO_COLOR non-empty → false (highest, overrides everything)', () => {
  ...
  assert.equal(io.resolveColor({
    stream: ttyStream, argv: ['--color'], env: { NO_COLOR: '1', FORCE_COLOR: '1', TERM: 'xterm' },
  }), false);
  assert.equal(io.resolveColor({ stream: noTtyStream, argv: [], env: { NO_COLOR: 'anything' } }), false);
});
...
test('resolveColor ② --no-color → false (over FORCE_COLOR & TTY, NO_COLOR unset)', () => {
  assert.equal(io.resolveColor({
    stream: ttyStream, argv: ['--no-color'], env: { FORCE_COLOR: '1', TERM: 'xterm' },
  }), false);
});

test('resolveColor ② --color → true (over FORCE_COLOR=0, TERM=dumb, non-TTY)', () => {
  assert.equal(io.resolveColor({
    stream: noTtyStream, argv: ['--color'], env: { FORCE_COLOR: '0', TERM: 'dumb' },
  }), true);
});

test('resolveColor ② --no-color beats --color when both present (no-color earlier in chain)', () => {
  assert.equal(io.resolveColor({ stream: ttyStream, argv: ['--color', '--no-color'], env: {} }), false);
});
...
test('resolveColor ③ FORCE_COLOR beats TERM=dumb', () => {
  assert.equal(io.resolveColor({ stream: noTtyStream, argv: [], env: { FORCE_COLOR: '1', TERM: 'dumb' } }), true);
});
...
test('resolveColor ⑤ falls back to isTTY (no overrides at all)', () => {
  ...
  assert.equal(io.resolveColor({ stream: ttyStream, argv: [], env: { TERM: 'xterm-256color' } }), true);
});
...
test('writeFileAtomicSync: writes full content and leaves no temp residue', () => {
  ...
  try {
    ...
    assert.deepEqual(entries, ['board.json'], `dir should hold only board.json, got ${JSON.stringify(entries)}`);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('jsonErr: {ok:false,exit,error,violations}; violations defaults to []', () => {
  assert.deepEqual(
    JSON.parse(io.jsonErr({ exit: 5, error: 'boom' })),
    { ok: false, exit: 5, error: 'boom', violations: [] });
  const v = [{ rule: 'GRAPH-CYCLE', level: 'hard', task: 'T1', message: 'cycle' }];
  assert.deepEqual(
    JSON.parse(io.jsonErr({ exit: 5, error: 'lint failed', violations: v })),
    { ok: false, exit: 5, error: 'lint failed', violations: v });
});
...
test('parseDuration: bad input throws with .errKind="Usage"', () => {
  for (const bad of ['', 'h', '3', '3x', 'abc', '3 hours', '-2h', '0h', '3hm', null, 42, undefined]) {
    assert.throws(() => io.parseDuration(bad), (e) => {
      assert.equal(e.errKind, 'Usage', `bad input ${JSON.stringify(bad)} → .errKind Usage`);
      return true;
