// handler-board.test.mjs — P5.2·board noun handler（handlers/board.js）契约门。
//
// 用 mkdtemp 临时 home + 真 leaf（mutations / render / discover / lint / graph）+ 临时板，端到端验证每 verb：
...
const boardHandler = require(join(SRC, 'handlers', 'board.js'));
...
function mkBoardHome({ tasks = [], goal = 'board handler test', extra = {} } = {}) {
...
function mkCtx({ boardPath, home, values = {}, flags = {}, positionals = [], sid = 'sid-bh', env } = {}) {
  const outBuf = [];
  const errBuf = [];
  const v = { ...values };
...
function task(id, over = {}) {
  return { id, status: 'ready', deps: [], type: 'development', executor: 'subagent', handle: 'h', title: `task ${id}`, ...over };
}
...
test('board graph: human prints topo order + ready', () => {
  const { boardPath } = mkBoardHome({ tasks: [task('T1'), task('T2', { deps: ['T1'], status: 'blocked' })] });
  const ctx = mkCtx({ boardPath });
...
test('board critical-path: human prints chain + makespan + weight_source', () => {
  const { boardPath } = mkBoardHome({
    tasks: [task('T1', { estimate: { value: 3, unit: 'h' } }), task('T2', { deps: ['T1'], estimate: { value: 2, unit: 'h' } })],
  });
  const ctx = mkCtx({ boardPath });
...
test('board next: human lists ready ids; blocked-by-dep not ready', () => {
  const { boardPath } = mkBoardHome({ tasks: [task('T1'), task('T2', { deps: ['T1'], status: 'ready' })] });
  const ctx = mkCtx({ boardPath });
...
test('board init: creates a new board file in home (owner.active:true / session_id:"")', () => {
  ...
  const onDisk = JSON.parse(readFileSync(join(home, files[0]), 'utf8'));
...
test('board init --dry-run does not create any file', () => {
  ...
  const code = boardHandler.init(ctx);
  assert.equal(code, EXIT.OK);
  assert.equal(readdirSync(home).filter((n) => n.endsWith('.board.json')).length, 0, 'no file written');
...
test('board update: --wip-limit coerces to number; --branch/--worktree land git', () => {
  const { boardPath } = mkBoardHome();
  const ctx = mkCtx({ boardPath, values: { 'wip-limit': '6', 'owner-wip': '2', branch: 'feat/x', worktree: '/ww' } });
  const code = boardHandler.update(ctx);
  assert.equal(code, EXIT.OK);
  const onDisk = JSON.parse(readFileSync(boardPath, 'utf8'));
...
test('board update: no flags → throws Usage (router maps to exit 2)', () => {
  const { boardPath } = mkBoardHome();
  const ctx = mkCtx({ boardPath });
  assert.throws(() => boardHandler.update(ctx), (e) => e.errKind === 'Usage');
});
...
test('board show: no active board → throws NotFound (router maps to exit 5)', () => {
  ...
  assert.throws(() => boardHandler.show(ctx), (e) => e.errKind === 'NotFound');
});
