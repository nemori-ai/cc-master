// cli.test.mjs — ccm CLI 端到端契约门（P5.3·设计稿 §11 P5.7 退出码矩阵 + JSON 契约 + 别名 + 板缺失）。
//
// 真二进制冒烟：spawnSync(process.execPath, [BIN, ...args]) 跑 cli/bin/ccm.js（**不用 execFileSync**·契约 §一.5），
...
const BIN = join(HERE, '..', '..', 'bin', 'ccm.js');
...
function seedBoard(home, { goal = 'e2e goal', tasks = [], log = [] } = {}) {
...
function runCcm(args, { home, sid = SID, input } = {}) {
...
test('board init --goal → 板文件生成（exit 0）', () => {
  ...
  const b = readBoard(join(home, files[0]));
...
test('task add → 0 + 板含该 task；start → done 状态机走通', () => {
  ...
  assert.ok((b.tasks || []).some((t) => t.id === 'T1'), 'board should contain T1');
  assert.equal(b.tasks.find((t) => t.id === 'T1').status, 'ready');
  ...
  assert.equal(b.tasks.find((t) => t.id === 'T1').status, 'in_flight');
  assert.ok(b.tasks.find((t) => t.id === 'T1').started_at, 'started_at stamped');

  const done = runCcm(['task', 'done', 'T1', '--verified', '--artifact', '/abs/out.md'], { home });
  assert.equal(done.status, 0);
  b = readBoard(boardPath);
  const t1 = b.tasks.find((t) => t.id === 'T1');
...
test('非法状态转移（task done 从 ready 直接 done）→ 3（VALIDATION）', () => {
  const { home } = mkHome();
  seedBoard(home, { tasks: [{ id: 'T1', status: 'ready', deps: [], created_at: '2026-06-24T08:00:00Z' }] });
...
test('task rm <id> --yes → 0 + 板不再含该 task', () => {
  ...
  assert.ok(!(b.tasks || []).some((t) => t.id === 'T2'), 'T2 should be gone');
  assert.ok((b.tasks || []).some((t) => t.id === 'T1'), 'T1 should remain');
});
...
test('--dry-run 不落盘（add 后板仍无该 task）', () => {
  ...
  const b = readBoard(boardPath);
  assert.ok(!(b.tasks || []).some((t) => t.id === 'TDRY'), 'dry-run must NOT persist');
...
test('task list --json → data 是任务数组（数据进 stdout）', () => {
  const { home } = mkHome();
  seedBoard(home, { tasks: [{ id: 'T1', status: 'ready', type: 'development', deps: [], created_at: '2026-06-24T08:00:00Z' }] });
...
test('task block --on user 经 stdin 喂 decision_package（-）', () => {
  const { home } = mkHome();
  const boardPath = seedBoard(home, { tasks: [{ id: 'T1', status: 'ready', type: 'development', deps: [], created_at: '2026-06-24T08:00:00Z' }] });
  const decision = JSON.stringify({
    question: '选 A 还是 B？',
    why_it_matters: '影响后续架构',
    options: [{ id: 'A', summary: 'A 方案' }, { id: 'B', summary: 'B 方案' }],
    recommendation: 'A',
    prepared_at: '2026-06-24T10:00:00Z',
    freshness: { inputs_hash: 'abc', checked_at: '2026-06-24T10:00:00Z' },
  });
  const r = runCcm(['task', 'block', 'T1', '--on', 'user', '--decision', '-'], { home, input: decision });
  // 退出码可能 0（lint 过）或 3（decision_package 形状不全被 BIZ-AWAITING 挡）——两者都证明 stdin 被读到了。
  assert.ok(r.status === 0 || r.status === 3, `block via stdin should be 0 or 3, got ${r.status}; stderr=${r.stderr}`);
  if (r.status === 0) {
    const b = readBoard(boardPath);
    const t1 = b.tasks.find((t) => t.id === 'T1');
