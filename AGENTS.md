...
├── scripts/                  ← 带外 **dev-only** 脚本：sync-plugin-dist / check-plugin-dist-sync / sync-codex-skills / eval-trigger / eval-benchmark / skill-lint（仅开发本仓用、repo 根调用，**不随 plugin 分发**；裸路径在此正确）。运行时带外脚本（codex-review / view webview）住 `plugin/src/skills/master-orchestrator-guide/canonical/scripts/`（随 skill 分发，见上；旧 cc-usage.sh 已退役 → `ccm usage`）
...
**hook 双端锁步（抗漂移硬约束，HOOKPAR-DEC / ADR-028）**：`plugin/src/hooks/<hook>/CONTRACT.md`（每个双端 `implemented` 的业务 hook 一份）是该 hook **host-neutral 业务规则 SSOT**——业务规则枚举 + 注入 taxonomy + 武装语义 + 降级行为（三分类学 `event-unavailable` / `protocol-capability-gap` / `host-convention-divergence`，后者必带 `tracked_by`）。`implementations/<host>/` 是这份 CONTRACT 在各自协议下的**投影**，不是独立 spec。**`plugin/src/hooks/<hook>/` 下任何业务逻辑改动（判定分支 / 阈值 / 状态转移 / 注入文案的语义而非措辞），若该 hook 在 `hooks.yaml` 声明双端 `implemented`，必须在同一 PR 要么同步改另一端的等价实现，要么在该 hook 的 CONTRACT.md「降级行为」一节显式声明本次改动为何只影响一端**（协议不支持 / host 无等价事件 / 有意暂缓 + tracking 链接）——同「`ccm`⟷`using-ccm` 锁步」同型，改了一端不声明就是让另一端悄悄过期。落地物：① 每条业务规则一个 `rule-<id>`，在其「PARITY anchors」一节登记 `required_hosts`，`// PARITY: rule-<id>` 结构锚点须在每个 required host 的实现文件里能 grep 到（`tests/content/hook-injection-contracts.test.mjs` 机械核验，只证明「两端都声明了这条规则」，不证明判定逻辑本身等价）；② parity 矩阵是生成物——`scripts/gen-hook-parity-matrix.sh` 从全部 CONTRACT.md 的「降级行为」节汇总渲染 `design_docs/hook-parity-matrix.md`（只读、类比 `plugin/dist` 的 source-to-artifact 纪律，`--check` 模式接进 `run-tests.sh`，改了 CONTRACT.md 忘重新生成矩阵会被拦）；③ 行为级 parity test（`tests/hooks/test_parity-fixtures.sh`）用同一份 host-neutral fixture stdin（Claude-Code-native 形状，Codex launcher 天然可归一化消费）跑两端真实现，断言判定落在同一等价类——这是唯一能拦住「两端字符串锚点都对、判定逻辑不同」这类分叉（`hook-common.js` 的 `ctx.normalized` 是这条归一化桥接收敛的单点，纯附加视图、零业务行为变化）。硬卡点：新增 lint 脚本对比 PR diff 里 touch 到的 `implementations/<host-A>/` 与 `implementations/<host-B>/` 文件集合，若只有一侧被 touch 且该 hook 双端 `implemented`，检查其 CONTRACT.md 是否同 PR 也被 touch（存在性检查，非语义检查——PR reviewer 手动判断声明是否合理）。
...
- **发版（release）——两条独立版本线，各自 tag、各自触发**（决策快照 [`adrs/ADR-022-version-line-decoupling.md`](adrs/ADR-022-version-line-decoupling.md)·本段是它的操作纲领·不复述正文）。`ccm`（独立 TS 引擎/CLI·ADR-014）与 cc-master plugin 的**版本号 + 发版触发 + changelog 节奏完全解耦**，互不绑定。**未来发版者先回答「这次该 bump 哪条线」**：
  ...
  - 机制细节如下：
  - **plugin 线（裸 `vX.Y.Z` tag·延续历史·手动门）**——版本号同步改两处 manifest：`plugin/src/.claude-plugin/plugin.json` + `plugin/src/.claude-plugin/marketplace.json`（都要 bump，漏一个会版本不一致）+ `CHANGELOG.md`（`[Unreleased]` 定版为 `[x.y.z] — YYYY-MM-DD`，顶部留空 `[Unreleased]`）；合并进 main 后打**裸** `vx.y.z` tag（`gh release create vx.y.z --target main`）→ 触发 `.github/workflows/plugin-release.yml`（同一 plugin tag 下打 per-harness asset：`cc-master-plugin-claude-code-<tag>.zip` / `cc-master-plugin-codex-<tag>.zip`）。发版前手动跑 `bash run-tests.sh`（须 `ALL TESTS PASSED`）+ `bash scripts/check-plugin-dist-sync.sh`（须无 `plugin/dist` diff）+ `claude plugin validate plugin/dist/claude-code`（须 `Validation passed`）。**版本文件归属 plugin 线**：`plugin.json` + `marketplace.json`（+ `CHANGELOG.md`）。
...
| 改一个双端 `implemented` hook 的业务逻辑 / 查它的降级行为声明 / 跑行为级 parity test | 先改对应 `plugin/src/hooks/<hook>/CONTRACT.md`（HOOKPAR-DEC / ADR-028 SSOT），再改两端实现或在其「降级行为」节声明分叉；`scripts/gen-hook-parity-matrix.sh` 生成 [`design_docs/hook-parity-matrix.md`](design_docs/hook-parity-matrix.md)（只读汇总）；[`tests/hooks/test_parity-fixtures.sh`](tests/hooks/test_parity-fixtures.sh)（行为级）+ [`tests/content/hook-injection-contracts.test.mjs`](tests/content/hook-injection-contracts.test.mjs)（结构级 PARITY anchors）+ `scripts/check-hook-parity-touch.sh`（PR-diff 存在性检查）+ [`adrs/ADR-028-hook-parity-contract-and-normalization.md`](adrs/ADR-028-hook-parity-contract-and-normalization.md) |
