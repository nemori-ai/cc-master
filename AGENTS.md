...
├── website/                  ← 项目官网（Astro 7 静态站 + Tailwind 4，中英双语 i18n〔EN 默认 + /zh/〕，Cloudflare 静态部署）：landing 十 section + 精选 docs（深链 GitHub）；设计 token 与 ccm web-viewer 同源（OKLCH mission-control）；`node_modules`/`dist`/`.wrangler`/`assets-src` gitignored；截图/视频由 `website/tools/capture.mjs` 对真实 `ccm web-viewer` 产出
...
- **website**（官网，`website/`）——Astro 7 静态站：landing 文案集中在 `src/i18n/ui.ts`（EN）与 `ui.zh.ts`（ZH），改文案双语同步；设计 token 唯一来源是 `src/styles/tokens.css`（从 ccm web-viewer `styles.css` 移植，改 viewer 配色时同步）；docs 页走 content collection（`src/content/docs/{en,zh}/`，frontmatter 有 zod 校验，深链 GitHub 的路径必须真实存在）；截图/视频资产由 `website/tools/capture.mjs`（对真实 `ccm web-viewer` + `tests/fixtures/board.example.json` 播种）+ `optimize-images.mjs` 再生产，原始大 PNG 落 gitignored 的 `assets-src/`；构建 `pnpm -C website build`，部署 Cloudflare Workers Static Assets（`wrangler.jsonc` + `pnpm wrangler deploy`）；**自定义域名 `cc-master.vibecoding.icu` 经官方 `cf` CLI 管理（`cf workers domains update`——不在 wrangler.jsonc 写 routes，workers.dev 默认路由保留）**。
