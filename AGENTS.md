...
│   │   ├── knowledge/        ← skill knowledge graph authored source root；K0 仅有 CONTRACT 骨架，能力以 `skill-knowledge contract` 为准
...
- **skill knowledge graph**（`plugin/src/knowledge/`）——面向维护者的 authored graph source root，不是 runtime skill，也不是 `plugin/dist` 生成物；当前 rollout stage 是 K0，目录为空 inventory 属显式 debt。先用 `node scripts/skill-knowledge.mjs contract --json` 读取真实能力，再用 `check --stage K0 --json` 检查；`compile/report/path/explain/change` 在 K0 只会 fail closed（exit 10），不得把 declared 当 implemented，也不得用 examples 冒充已盘点 source。
