...
│   ├── orchestrating-to-completion/  ← SKILL A：编排方法论（魂）+ references/ + scripts/（运行时带外脚本 codex-review + view webview，随 skill 分发，prose 用 `${CLAUDE_SKILL_DIR}` 引用；配速信号已收口进 `ccm usage`、退役了旧带外 cc-usage.sh）
...
├── scripts/                  ← 带外 **dev-only** 脚本：eval-trigger / eval-benchmark / skill-lint（仅开发本仓用、repo 根调用，**不随 plugin 分发**；裸路径在此正确）。运行时带外脚本（codex-review / view webview）住 `skills/orchestrating-to-completion/scripts/`（随 skill 分发，见上；旧 cc-usage.sh 已退役 → `ccm usage`）
