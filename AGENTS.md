...
它**不是**：agent framework / library，不是某个 LLM API 的包装，不依赖 agent-teams 或 scheduled routines（见 §3 红线 5）。它是 **commands + 5 skills + hooks + 一个 board 文件**的薄编排层。
...
- **六条 design 红线**——hooks 只用 bash+node/JS（红线1，ADR-006）/ board narrow waist / 五 skill 不重叠 / 指挥不演奏 / ship-anywhere / 所有 hook 武装后才激活（dormant-until-armed，红线6，ADR-007）。SSOT 在本文 **§3**（每条带 grep/CI 卡点）。
...
3. **五个分发 skill 各自自洽、互不重叠。** SKILL A（`orchestrating-to-completion`）= 主线编排**决策**（含 pacing 决策 + 一张**已切好**的 DAG 怎么排期）；SKILL B（`authoring-workflows`）= 脚本内写法；SKILL C（`account-management`）= 换号号池**机制层**（号池 / 选号 / 切号 / vault token 安全）；SKILL D（`using-ccm`）= **ccm CLI 操作手册**（board 操作机制层：读写 board / 状态机 verb / 三档字段 / footgun + 全量 command-catalog）；SKILL E（`slicing-goals-into-dags`）= **敏捷切 DAG 方法论**（怎么把目标**切**成 board DAG:纵切薄增量 / walking skeleton / 粒度品味 / 价值风险排序——「切」先于 A 的「排」，A 在分解点引用 E）。**决策 + 排期归 A;切分方法论归 E;机制按域分流**——换号机制归 C、board 操作机制归 D，A 在相应决策点单向引用 C/D/E 而不复述其内容。`using-ccm` 与 `ccm` 命令面的锁步抗漂移约束见 **§6**。
   → 决策快照：[`adrs/ADR-005-two-skills-separation.md`](adrs/ADR-005-two-skills-separation.md)（两 skill 分离原则现扩到五个，account-management / using-ccm / slicing-goals-into-dags 经 curating 闸纳入·不变其精神）· 硬卡点：跨界/复述在 PR review 拦——"编排决策 + DAG 排期" 归 A，"workflow 脚本怎么写" 归 B，"号池怎么管 / 选号切号 / token 安全" 归 C，"怎么用 ccm 操作 board" 归 D，"怎么把目标切成 DAG" 归 E。
...
本仓**分发**五个 skill：A（编排决策 + DAG 排期）、B（workflow 写法）、C（`account-management`·换号号池机制层）、D（`using-ccm`·ccm CLI 操作手册 / board 操作机制层）、E（`slicing-goals-into-dags`·敏捷切 DAG 方法论）——**互不重叠**（红线 3）：A = orchestrator 做什么（含换号**决策** + 已切好的 DAG 怎么排期），B = workflow 脚本怎么写，C = 号池怎么管 / 怎么选号切号 / token 怎么安全（换号**机制**），D = 怎么用 ccm 读写 board（board 操作**机制**），E = 怎么把目标**切**成 board DAG（纵切 / walking skeleton / 粒度品味，「切」先于 A 的「排」）。另有**四个项目自用、不随插件分发**的 dev meta-skill（住 `.claude/skills/`，不在 `skills/`），终端用户装插件时看不到它们——**造 / 评 / 治三件套**（skillsmith / curating / grounding）+ 它们**上游**的一个需求发现 skill：
