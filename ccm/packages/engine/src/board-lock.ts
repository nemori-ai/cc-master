// board-lock.ts — board 写入并发保护（轻量 advisory 文件锁·ADR-013 §2.5）。
//
...
//
// T1 port 注：原 CJS 源的 require('fs')/require('crypto') 换成 ESM node import；module.exports 换成命名导出。
//   锁机制、stale 阈值、token 形态、错误文案逐字保持（零行为变化）。
...
export function acquire(boardPath: string, opts: LockOptions = {}): string {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (!err || err.code !== 'EEXIST') throw e; // 非「被占」错误（权限/路径）→ 直接抛
      // 被占：检查是否 stale（崩溃遗留）。stale → 偷锁（unlink）后立即重试，不耗 retry 配额。
      const meta = readLockMeta(lockPath);
      const ts = meta && Number.isFinite(meta.ts) ? (meta.ts as number) : null;
  ...
  throw new Error(
    `LOCK_TIMEOUT: ${lockPath} 被占用超过 ${retries} 次重试（可能另一个写者在写，或锁未被释放——必要时手动删 .lock）`,
  );
}

// release(boardPath, token) → true（删了我们自己的锁）/ false（不是我们的锁 或 已不存在·advisory 归属保护）。
export function release(boardPath: string, token: string): boolean {
  const lockPath = lockPathFor(boardPath);
  const meta = readLockMeta(lockPath);
...
export function withLock<T>(boardPath: string, fn: () => T, opts: LockOptions = {}): T {
...
export function isLocked(boardPath: string, opts: LockOptions = {}): boolean {
