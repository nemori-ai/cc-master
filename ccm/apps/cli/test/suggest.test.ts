// suggest.test.ts — suggest.ts（did-you-mean·Damerau-Levenshtein）契约门。
//
// 钉死：suggestSimilar('baord',['board','task']) 含 'board'；完全无关输入返回空；按距离升序；
//   相邻换位（transposition）一步距离；空输入/空候选边界。
//
// T2a port 注：原 .mjs 经 createRequire 加载 CJS（cli/test/unit/suggest.test.mjs），改成正常 ESM import
//   ported 的 .ts 源（NodeNick `.js` specifier→tsx 重映射到 .ts）。per-module 粒度覆盖保留。

import assert from 'node:assert/strict';
import { test } from 'node:test';
import { damerauLevenshtein, suggestSimilar } from '../src/suggest.js';

const S = { suggestSimilar, damerauLevenshtein };
...
test("suggestSimilar('baord', ['board','task']) includes 'board'", () => {
  ...
  assert.ok(
    !out.includes('task'),
    `'task' should not be suggested for 'baord', got ${JSON.stringify(out)}`,
  );
});
...
test('results are sorted by ascending edit distance', () => {
  ...
  // 第一个的距离不大于最后一个的距离（升序）。
  const d0 = S.damerauLevenshtein('lst', out[0] as string);
  const dN = S.damerauLevenshtein('lst', out[out.length - 1] as string);
