...
test('board init --capabilities：安装路径也保持协商只读，不触发 statusline auto-install', () => {
  const root = mkdir();
  const configDir = join(root, 'Missing Claude Config 配置');
  const masterHome = join(root, 'Missing CC Master Home');
  const r = runCcm(['board', 'init', '--capabilities', '--json', '--no-input'], configDir, {
    extraEnv: {
      // Use a production-like lexical location outside tmp/repo/worktree markers. The in-process
      // router only consumes this path for its installed-vs-dev decision; no executable is needed.
      CCM_BIN: join('/', 'opt', 'ccm-installed-path-test', 'ccm'),
      CC_MASTER_HOME: masterHome,
    },
  });
