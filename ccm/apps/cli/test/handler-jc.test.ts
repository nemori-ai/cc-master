// handler-jc.test.ts — P5.2·jc handler（handlers/jc.ts）契约门。
//
// jc.ts 是 judgment_calls 自决台账 handler（add/list/show/resolve），照 log.ts 范式。本测试用 mkdtemp 临时板
...
import * as jcHandler from '../src/handlers/jc.js';
...
function mkBoardHome({ judgment_calls }: { judgment_calls?: unknown[] } = {}): string {
...
test('jc add allocates next id sequentially (J1 exists → J2)', () => {
  const boardPath = mkBoardHome({
    judgment_calls: [
      { id: 'J1', summary: 'old', status: 'upheld', raised_at: '2026-06-24T09:00:00Z' },
    ],
  });
...
const SEED_JC = [
  {
    ...
    severity: 'high',
    category: 'architecture',
    raised_at: '2026-06-24T09:00:00Z',
  },
  {
    id: 'J2',
    summary: 'drift B',
    ...
    severity: 'critical',
    category: 'drift',
    raised_at: '2026-06-24T09:30:00Z',
  },
  {
    id: 'J3',
    summary: 'misalign C',
    ...
    severity: 'low',
    category: 'other',
    raised_at: '2026-06-24T10:00:00Z',
  },
];
...
test('jc show on non-existent id → NOT_FOUND', () => {
  ...
  let caught: { errKind?: string } | null = null;
  try {
    jcHandler.show(ctx);
  } catch (e) {
    caught = e as { errKind?: string };
  }
...
test('jc resolve sets status upheld + note + resolved_at', () => {
  const boardPath = mkBoardHome({ judgment_calls: SEED_JC });
  const ctx = mkCtx(boardPath, {
    values: { status: 'upheld', note: '事后看是对的' },
    positionals: ['J1'],
  });
  const code = jcHandler.resolve(ctx);
  assert.equal(code, EXIT.OK);
  const onDisk = JSON.parse(readFileSync(boardPath, 'utf8'));
  const j = onDisk.judgment_calls.find((x: { id: string }) => x.id === 'J1');
...
test('jc resolve overturned (no note) works', () => {
  ...
  const code = jcHandler.resolve(ctx);
  assert.equal(code, EXIT.OK);
  const j = JSON.parse(readFileSync(boardPath, 'utf8')).judgment_calls.find(
    (x: { id: string }) => x.id === 'J3',
  );
