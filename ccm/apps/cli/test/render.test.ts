// render.test.ts — ccm CLI 渲染层（人读 vs --json 双形态）契约门。
//
// render.ts 只产字符串：human 默认无色（color=true 才 paint）、--json 必产合法 JSON、表格无边框 grep 可过滤。
...
//
// T2a port 注：原 .mjs 经 createRequire 加载 CJS（cli/test/unit/render.test.mjs），改成正常 ESM import
//   ported render.ts；真实 analysis 句柄由 `@ccm/engine` 的 analyzeGraph 算（原 require board-graph-core.js）。

import assert from 'node:assert/strict';
import { test } from 'node:test';
import { analyzeGraph } from '@ccm/engine';
import * as R from '../src/render.js';

// ANSI 转义探测正则（任何 \x1b[…m 都算上色痕迹）。
// biome-ignore lint/suspicious/noControlCharactersInRegex: \x1b(ESC) 是 ANSI 上色探测的字面所需（测试探测 render 是否上色）。
const ANSI_RE = /\x1b\[[0-9;]*m/;

// 真实板（精简自 board.example.json 的形状）—— 多 status / type / executor 覆盖渲染分支。
function sampleBoard(): any {
  return {
    schema: 'cc-master/v2',
    ...
    tasks: [
      ...
      },
      {
        ...
        started_at: '2026-06-05T12:18:00Z',
      },
      { id: 'T3', status: 'ready', deps: ['T0'], type: 'doc-alignment', title: 'doc align' },
      {
        ...
        blocked_on: 'user',
        title: 'Split the PR?',
        decision_package: { ask_type: 'decision', question: 'One PR or two?' },
      },
    ],
  };
}

// 由 board-graph-core（引擎）真算一个 analysis 句柄（验证 render 吃真实句柄形态）。
function realAnalysis(board: any) {
  return analyzeGraph(board);
}

// 解析 json render 的统一壳 → 返回 data。
function parseData(s: string): any {
...
test('renderBoardSummary human color=true: 出现 ANSI 转义', () => {
  const out = R.renderBoardSummary(sampleBoard(), {
    color: true,
...
test('renderTaskList human color=false: 无 ANSI + 每行可被 grep（含 id）', () => {
  ...
  // 表头含列名
  assert.match(lines[0] as string, /ID/);
  assert.match(lines[0] as string, /STATUS/);
  assert.match(lines[0] as string, /TITLE/);
});
...
test('renderTaskList color=true 时上色但去色后仍含 id（对齐不被 ANSI 撑乱）', () => {
  ...
  // biome-ignore lint/suspicious/noControlCharactersInRegex: \x1b(ESC) 去色后验对齐不被撑乱（ANSI 字面所需）。
...
test('renderGraph json: 句柄 → impact/rollup/nesting 三表面正确 + 现有字段不变', () => {
  ...
  assert.equal(
    data.impact.T0.count,
    data.impact.T0.descendants.length,
  ...
  assert.equal(
    data.rollup.owners.M1.total,
    2,
    'M1 直接子 = M1.a, M1.b（G1 的 parent 是 M1.a 不计入）',
  );
  assert.deepEqual(
...
test('renderCriticalPath 含环: 标不可算', () => {
  const plain = {
    criticalPath: { chain: [], makespan: null, weight_source: 'cycle', cycle: ['A', 'B', 'A'] },
  };
  const out = R.renderCriticalPath(plain, { color: false });
...
test('renderLintReport json: {ok, violations:[{rule,level,task?,message}]}', () => {
  ...
  const hard = data.violations.find((v: any) => v.level === 'hard');
  assert.equal(hard.rule, 'FMT-ID');
  assert.equal(hard.task, 'X');
  const warn = data.violations.find((v: any) => v.level === 'warn');
...
test('renderLintReport human PASS-with-warnings: PASS header + warn 行', () => {
  const out = R.renderLintReport(
    { errors: [], warnings: [{ rule: 'FMT-TIME', message: 'x' }] },
    { color: false },
  );
  assert.match(out, /PASS/);
  assert.match(out, /\[warn\] FMT-TIME/);
