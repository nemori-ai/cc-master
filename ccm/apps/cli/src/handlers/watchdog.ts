...
export function disarm(ctx: Ctx): number {
  ...
  return runWrite(ctx, {
    mutate: (board) => {
      const b = board as { watchdog?: WatchdogLike };
      priorJobId = (b && b.watchdog && b.watchdog.job_id) || null;
    ...
    render: (next, c, { dryRun }) => {
      ...
      const hint = priorJobId
        ? `（已删整对象，请去清理外部调度 job-id=${priorJobId}）`
        : '（已删整对象）';
      return prefix + hint;
    },
  });
...
export function status(ctx: Ctx): number {
  return runRead(ctx, {
    compute: (board) => {
      const b = board as { watchdog?: WatchdogLike | null };
      return (b && b.watchdog) || null;
    },
    render: (wd, c) =>
      renderWatchdog(wd as WatchdogLike | null, { json: !!c.flags.json, color: c.flags.color }),
  });
}
...
function renderWatchdog(
  wd: WatchdogLike | null | undefined,
