// handlers/cadence.ts — cadence noun handler（节奏 / iteration 收口·cli-design §3 namespace cadence）。
//
// 照 handlers/log.ts 的范式（每 verb 一个 handler(ctx)→exitCode；写 verb 用 _common.runWrite，
...
interface CadenceLike {
  target?: { ship_every?: unknown; min_unit?: unknown };
  iterations?: IterLike[];
  [k: string]: unknown;
}
...
function applySets(board: BoardArg, sets: SetOp[], setJsons: SetOp[]): BoardArg {
  let b = board;
...
export function update(ctx: Ctx): number {
  ...
  return runWrite(ctx, {
    mutate: (board) => {
      ...
      let next = mutations.cadenceUpdate(board as BoardArg, args);
      next = applySets(next, sets, setJsons);
      return next;
    },
    render: (next, c, { dryRun }) => {
      const nb = next as { cadence?: CadenceLike };
      const cadence = nb.cadence || {};
...
export function open(ctx: Ctx): number {
  ...
  return runWrite(ctx, {
    ...
    mutate: (board) => {
      ...
      const args = {
        goal: fields.goal as string | undefined,
        deadline: fields.deadline as string | undefined,
        members: fields.members,
      };
      let next = mutations.cadenceOpen(board as BoardArg, iterId, args);
      next = applySets(next, sets, setJsons);
      return next;
    },
    render: (next, c, { dryRun }) => {
...
export function ship(ctx: Ctx): number {
  return runWrite(ctx, {
    mutate: (board) =>
      mutations.cadenceShip(board as BoardArg, ctx.positionals[0] as string, {
        force: ctx.flags && ctx.flags.force,
      }),
    render: (next, c, { dryRun }) => {
      const iter = findIter(next, ctx.positionals[0]);
      if (c.flags.json) return render.jsonString(iter || null);
...
// ── cadence status ──────────────────────────────────────────────────────────────────────────────
// 读 verb：render.ts 无专属 cadence renderer（它只产字符串、不算节奏）——故 human 形态在此 handler 内拼字符串
//   （goal/target 一行 + 每 iteration 一行），json 形态用 render.jsonString 裹整个 cadence 对象（形状稳定）。
export function status(ctx: Ctx): number {
  return runRead(ctx, {
    compute: (board) => {
      const b = board as { cadence?: CadenceLike };
      return (b && b.cadence) || {};
    },
    render: (cadence, c) => {
      if (c.flags.json) return render.jsonString(cadence);
      return renderStatusHuman(cadence as CadenceLike);
    },
  });
}

// ── 本地 human 渲染小工具（render.ts 不含 cadence renderer·见 status 注释）─────────────────────────
function renderTargetLine(
  target: { ship_every?: unknown; min_unit?: unknown } | undefined,
): string {
  const t = target && typeof target === 'object' ? target : {};
  const se = formatShipEvery(t.ship_every);
  const mu =
    t.min_unit !== undefined && t.min_unit !== null && t.min_unit !== '' ? t.min_unit : '-';
  return `ship_every=${se} min_unit=${mu}`;
}

// ship_every 既可能是 {value,unit}（经 --ship-every duration transform）也可能是字符串（手搓 / --set）。
function formatShipEvery(se: unknown): string {
  if (se === undefined || se === null || se === '') return '-';
  if (typeof se === 'object' && (se as { value?: unknown }).value !== undefined) {
    const o = se as { value?: unknown; unit?: unknown };
    return `${o.value}${o.unit || ''}`;
  }
  return String(se);
}

function findIter(board: unknown, iterId: string | undefined): IterLike | undefined {
  const b = board as { cadence?: CadenceLike };
  const iters = b && b.cadence && Array.isArray(b.cadence.iterations) ? b.cadence.iterations : [];
  return iters.find((it) => it && it.id === iterId);
}

function renderIterLine(iter: IterLike | undefined): string {
  if (!iter || typeof iter !== 'object') return '(无此 iteration)';
  const members =
    Array.isArray(iter.members) && iter.members.length ? iter.members.join(', ') : '-';
  const goal = iter.goal !== undefined && iter.goal !== null && iter.goal !== '' ? iter.goal : '';
  return `${iter.id} [${iter.status || '?'}]${goal ? ` ${goal}` : ''} (members: ${members})`;
}

function renderStatusHuman(cadence: CadenceLike | undefined): string {
