...
**3) 临界路径（Critical path，agent 心算估计）**：你**心算估计**的那条通往目标的最长依赖链（沿 `deps` 看哪条串行链最长）。注意这是估计而非机器算出的 CPM——board 上**没有**机器算出的 float 字段；要真算 float / ES-EF-LS-LF 请走带外脚本（属 ship-anywhere，不进 hook）。把它如实呈现为估计，别当成精确的零 float 链。
