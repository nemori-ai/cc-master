...
- **Unanswered user decisions**: list the `title` of every task with `blocked_on:"user"` — these are decisions the orchestrator is waiting on the user for. Surface them prominently (a long-running orchestration must never silently sit on an un-answered user gate).
