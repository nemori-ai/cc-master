---
description: '渲染一份 cc-master board 摘要——进度、阻塞、临界路径、以及等待用户拍板的决策。'
---
...
- **临界路径（Critical path）**：零 float 的 `deps` 链（通往目标的最长依赖链）。
...
- **预算快照（Budget snapshot）**：注意主线可跑 `scripts/cc-usage.sh`（带外，非 hook）取一个 5h/7d 的 usage 信号——`five_hour.used_tokens` / `burn_rate_per_min` / `window_remaining_min` 以及 `seven_day.used_tokens`——在 pacing 决策前读出相对滚动配额窗口的 已用 / burn / 剩余。若你跑了它就把数字呈现出来；否则指向该命令。
