...
1. **认准要可视化的那块 board。** Board 集中住在 cc-master home 下的 `boards/` 子目录，以 `<timestamp>-<pid>.board.json` 命名。home 解析：`$CC_MASTER_HOME` 优先，否则 `$CLAUDE_CONFIG_DIR/cc-master`（`CLAUDE_CONFIG_DIR` 默认 `$HOME/.claude`）——全局、用户级、不再 per-project。列出 `<home>/boards/`，读取每一块 `owner.active` 为 `true` 的 board。
   - 若恰好只有一块 active，就用它。
