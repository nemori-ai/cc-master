...
3. **处理孤儿 `in_flight`**：旧 session 派发的后台任务，其 handle 随旧 session 一起失效——**绝不当它「还在飞」干等**。把每个 `in_flight` 当孤儿，走 `orchestrating-to-completion` 的端点验收 + content-hash 判定（细节在该 skill 的 `references/resume-verify.md` §孤儿 in_flight 续接，**此处不复述**）：产物已落地且端点验过 → 标 `done`/`verified`；否则降回 `ready`/`stale` 重新派发拿新 handle。
