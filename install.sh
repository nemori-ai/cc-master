#!/usr/bin/env bash
# install.sh — 一条命令把 `ccm` 引擎二进制 + cc-master 插件装到本机（版本可指定）。
#
# 这是 **dev-only / 面向终端用户的安装器**——从 repo 根维护、随 GitHub 仓库托管，
# 用户用 curl 直接拉来跑（不随 plugin 分发、不是 hook，故用裸 shell + 可联网，
...
usage() {
  sed -n '2,40p' "$0" 2>/dev/null | sed 's/^# \{0,1\}//' >&2
  exit 0
}
...
while [ $# -gt 0 ]; do
  case "$1" in
    --version) VERSION="${2:-}"; [ -n "$VERSION" ] || die "--version 需要一个值（如 v0.10.0）"; shift 2 ;;
...
resolve_version() {
  ...
  if [ -n "$LOCAL_SRC" ]; then
    ...
    [ -n "$zip" ] || die "本地源 $LOCAL_SRC 里找不到 cc-master-plugin-*.zip（无法推断版本，请加 --version）。"
    local base="${zip##*/}"; base="${base#cc-master-plugin-}"; base="${base%.zip}"
    printf '%s\n' "$base"; return
  fi
  # 联网模式：GitHub API 取最新 release tag。
  local tag
  tag="$(http_get "$GITHUB_API/repos/$REPO/releases/latest" \
         | grep -m1 '"tag_name"' \
         | sed -E 's/.*"tag_name"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/' || true)"
  [ -n "$tag" ] || die "无法从 GitHub API 取最新 release tag（仓库无已发布 release？可用 --version 指定，或用 CC_MASTER_INSTALL_LOCAL 本地源）。"
  printf '%s\n' "$tag"
}
...
TAG="$(resolve_version)"                   # 如 v0.10.0
BIN_ASSET="ccm-${PLATFORM}"                # 如 ccm-darwin-arm64
ZIP_ASSET="cc-master-plugin-${TAG}.zip"

log "目标平台：${PLATFORM}    版本：${TAG}"
if [ -n "$LOCAL_SRC" ]; then log "源：本地目录 ${LOCAL_SRC}"; else log "源：GitHub release（${REPO}）"; fi
...
# ── ① 装 ccm 二进制 ─────────────────────────────────────────────────────────────────────────────────
log "① 安装 ccm 引擎二进制 …"
fetch "$BIN_ASSET" "$TMP/ccm" "$GITHUB/$REPO/releases/download/$TAG/$BIN_ASSET"
chmod +x "$TMP/ccm"
mkdir -p "$PREFIX"
...
fetch "$ZIP_ASSET" "$TMP/plugin.zip" "$GITHUB/$REPO/releases/download/$TAG/$ZIP_ASSET"
...
# ── 收尾 ────────────────────────────────────────────────────────────────────────────────────────────
ok "完成 ✓  ccm（${TAG}）+ cc-master 插件都装好了。"
