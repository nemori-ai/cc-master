...
GOOD='{"schema":"cc-master/v1","meta":{"template_version":1},"goal":"g","owner":{"active":true,"session_id":"sess-x"},"git":{"worktree":"/w","branch":"b"},"tasks":[{"id":"T0","status":"done","deps":[]},{"id":"T1","status":"ready","deps":["T0"]}]}'
...
H="$(make_project)"
seed_board "$H" "other" true "sess-other" '{"schema":"cc-master/v1","goal":"g","owner":{"active":true,"session_id":"sess-other"},"tasks":[{"id":"T0","status":"ready","deps":["NOPE"]}]}'
...
# ── (h) HARD FAIL: invalid JSON (R1) → injects a non-blocking PostToolUse report naming R1 ───────────
...
printf '{"schema":"cc-master/v1","owner":{"active":true,"session_id":"sess-x"},"tasks":[{"id":"T0"' > "$H/broken.board.json"
...
assert_contains "$HOOK_OUT" "R1" "(h) names the violated rule (R1 invalid JSON)"
...
# run lint, reporting R1. This is the codex-caught bug: isArmed used to JSON.parse-and-continue on the
...
H="$(make_project)"
printf '{"schema":"cc-master/v1","owner":{"active":true,"session_id":"sess-x"},"tasks":[{"id":"T0"' > "$H/mine.board.json"
run_lint "Write" "$H/mine.board.json" "sess-x" "$H"
...
assert_contains "$HOOK_OUT" "R1" "(h2) names R1 even when the broken board is the SOLE active board (codex bug)"
...
H="$(make_project)"
printf '{"schema":"cc-master/v1","owner":{"active":true,"session_id":"sess-other"},"tasks":[{"id":"T0"' > "$H/theirs.board.json"
...
# ── (i) HARD FAIL: dangling dep (R4a) + bad status (R3c) → report names BOTH + the offending task ────
H="$(make_project)"
seed_board "$H" "armed" true "sess-x" "$GOOD"
seed_board "$H" "bad" true "sess-x" '{"schema":"cc-master/v1","goal":"g","owner":{"active":true,"session_id":"sess-x"},"git":{"worktree":"","branch":""},"tasks":[{"id":"T0","status":"done","deps":[]},{"id":"T3","status":"in_flght","deps":[]},{"id":"T7","status":"ready","deps":["T5"]}]}'
run_lint "Edit" "$H/bad.board.json" "sess-x" "$H"
assert_eq 0 "$HOOK_RC" "(i) dangling+badstatus → rc 0"
assert_contains "$HOOK_OUT" "R4a" "(i) names dangling-dep rule (R4a)"
assert_contains "$HOOK_OUT" "R3c" "(i) names bad-status rule (R3c)"
...
# ── (j) HARD FAIL: cycle (R4c) → report names the cycle rule ─────────────────────────────────────────
H="$(make_project)"
seed_board "$H" "armed" true "sess-x" "$GOOD"
seed_board "$H" "cyc" true "sess-x" '{"schema":"cc-master/v1","goal":"g","owner":{"active":true,"session_id":"sess-x"},"git":{"worktree":"","branch":""},"tasks":[{"id":"A","status":"ready","deps":["B"]},{"id":"B","status":"ready","deps":["A"]}]}'
run_lint "Write" "$H/cyc.board.json" "sess-x" "$H"
assert_contains "$HOOK_OUT" "R4c" "(j) names the cycle rule (R4c)"
assert_not_contains "$HOOK_OUT" '"decision":"block"' "(j) never blocks"
rm -rf "$H"

# ── (k) HARD FAIL: tasks not an array (R2f) ──────────────────────────────────────────────────────────
H="$(make_project)"
seed_board "$H" "armed" true "sess-x" "$GOOD"
seed_board "$H" "notarr" true "sess-x" '{"schema":"cc-master/v1","goal":"g","owner":{"active":true,"session_id":"sess-x"},"git":{"worktree":"","branch":""},"tasks":{"oops":1}}'
run_lint "Write" "$H/notarr.board.json" "sess-x" "$H"
assert_contains "$HOOK_OUT" "R2f" "(k) names tasks-not-array rule (R2f)"
rm -rf "$H"

# ── (k2) HARD FAIL: a task MISSING deps (R3d) — deps is a required narrow-waist field, not a flexible
...
H="$(make_project)"
seed_board "$H" "armed" true "sess-x" "$GOOD"
seed_board "$H" "nodeps" true "sess-x" '{"schema":"cc-master/v1","goal":"g","owner":{"active":true,"session_id":"sess-x"},"git":{"worktree":"","branch":""},"tasks":[{"id":"T0","status":"done","deps":[]},{"id":"T1","status":"ready"}]}'
run_lint "Write" "$H/nodeps.board.json" "sess-x" "$H"
assert_eq 0 "$HOOK_RC" "(k2) missing deps → rc 0 (never blocks)"
assert_contains "$HOOK_OUT" "R3d" "(k2) names the missing-deps rule (R3d)"
...
# ── (l) WARN-ONLY: blocked_on dangling (R5b) + bad timestamp (R6a) → PASS, but warns; rc 0; no block ─
H="$(make_project)"
seed_board "$H" "armed" true "sess-x" "$GOOD"
seed_board "$H" "warns" true "sess-x" '{"schema":"cc-master/v1","goal":"g","owner":{"active":true,"session_id":"sess-x"},"git":{"worktree":"","branch":""},"tasks":[{"id":"T1","status":"in_flight","deps":[],"started_at":"12:18Z"},{"id":"T9","status":"blocked","deps":[],"blocked_on":"T8"}]}'
run_lint "Write" "$H/warns.board.json" "sess-x" "$H"
assert_eq 0 "$HOOK_RC" "(l) warn-only → rc 0"
assert_contains "$HOOK_OUT" "R5b" "(l) warns on dangling blocked_on (R5b)"
assert_contains "$HOOK_OUT" "R6a" "(l) warns on bad timestamp format (R6a)"
...
H="$(make_project)"
seed_board "$H" "free" true "sess-x" '{"schema":"cc-master/v1","goal":"g","owner":{"active":true,"session_id":"sess-x"},"git":{"worktree":"","branch":""},"my_custom_top":42,"weird":{"nested":true},"tasks":[{"id":"T0","status":"done","deps":[],"artifact":"x","mechanism":"sub-agent","handle":"bg-1","whatever_i_want":["a","b"],"notes":"free text"}]}'
...
H="$(make_project)"
seed_board "$H" "minimal" true "sess-x" '{"schema":"cc-master/v1","goal":"g","owner":{"active":true,"session_id":"sess-x"},"git":{"worktree":"","branch":""},"tasks":[{"id":"X","status":"ready","deps":[]}]}'
...
H="$(make_project)"
seed_board "$H" "armed" true "sess-x" "$GOOD"
seed_board "$H" "arch" false "sess-x" '{"schema":"cc-master/v1","goal":"g","owner":{"active":false,"session_id":"sess-x"},"tasks":[{"id":"T0","status":"ready","deps":["GHOST"]}]}'
