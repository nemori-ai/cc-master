...
// ════════════════════════════════════ E 组：WV3 view-model-first dashboard/detail rail ════════════════════════════════════
test('E1 — view.html: board poll 优先读取 /view-model.json，/board.json 只在 fallback 路径读取', () => {
  const src = read(VIEW);
  assert.match(src, /fetchViewModelBoard\(\)/, 'poll 入口调用 view-model-first fetch helper');
  assert.match(src, /serverUrl\('\/view-model\.json'\)/, '存在 /view-model.json fetch');
  assert.match(src, /serverUrl\('\/board\.json'\)/, '保留 /board.json compatibility fallback');
  assert.match(src, /return\s+fetchBoardFallback\(\)/, 'view-model 不可用时才 return 到 board fallback');
  assert.match(
    src,
    /async function fetchViewModelBoard\(\)[\s\S]*serverUrl\('\/view-model\.json'\)[\s\S]*return\s+fetchBoardFallback\(\)/,
    'fetchViewModelBoard 函数体内先尝试 /view-model.json，再落到 /board.json fallback',
  );
});

test('E2 — view.html: layout freshness 优先使用 /view-model.json rev.topologyHash', () => {
  const src = read(VIEW);
  assert.match(src, /function topologyRev\(board\)/, '存在 topologyRev(board) helper');
  assert.match(
    src,
    /rev\.topologyHash[\s\S]*return\s+['"]vm:['"]\s*\+\s*rev\.topologyHash/,
    'topologyRev 优先返回 view-model rev.topologyHash',
  );
  assert.match(src, /return\s+['"]local:['"]\s*\+\s*structSig\(board\)/, '老 server fallback 才本地计算 structSig');
  assert.match(src, /const sig = topologyRev\(b\)/, 'apply() 用 topologyRev 判定 fresh layout');
  assert.doesNotMatch(src, /const sig = structSig\(b\)/, 'apply() 不再每次 board poll 都本地扫 topology');
});

test('E3 — view.html: graph 搜索会收窄可见集并标出匹配节点', () => {
  const src = read(VIEW);
  assert.match(src, /function deriveSearchMatches\(tasks, query\)/, '存在搜索匹配派生函数');
  assert.match(src, /function deriveFocusIds\(an, matchIds\)/, '存在搜索 focus 可见集派生函数');
  assert.match(src, /focusIdsRef\.current/, 'render/buildGraph 接入 focusIdsRef');
  assert.match(src, /searchMatchesRef\.current/, 'render/buildGraph 接入 searchMatchesRef');
  assert.match(src, /className: 'graphnav'/, 'graph canvas 内有实际搜索导航 toolbar');
  assert.match(src, /className: 'searchbox'/, '搜索输入存在');
  assert.match(src, /searchMatch:\s*matchSet\.has\(t\.id\)/, '匹配节点被写入 node data');
  assert.match(src, /if \(data\.searchMatch\) cls\.push\('match'\)/, '匹配节点有可见高亮 class');
});

test('E2 — view.html: insights rail renders board-level dashboard modules from the view model when present', () => {
  const src = read(VIEW);
  for (const needle of [
    'BoardDashboard',
    'status counts',
    ...
    assert.match(src, new RegExp(needle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')), `dashboard includes ${needle}`);
  }
});

test('E3 — view.html: detail rail has board-v2 task field sections', () => {
  const src = read(VIEW);
  for (const needle of [
    'acceptance',
    'references',
    ...
    assert.match(src, new RegExp(needle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')), `detail rail includes ${needle}`);
  }
});

// ════════════════════════════════════ F 组：WV5 multiboard + peers UI wiring ════════════════════════════════════
test('F1 — view.html: serverUrl keeps local token gate and propagates selected board query', () => {
  const src = read(VIEW);
  assert.match(src, /const ACCESS_TOKEN = INITIAL_SEARCH\.get\('token'\)/, 'viewer still reads token from URL');
  assert.match(src, /const ACTIVE_BOARD_FILE = INITIAL_SEARCH\.get\('board'\)/, 'viewer reads selected board query');
  assert.match(src, /u\.searchParams\.set\('token', ACCESS_TOKEN\)/, 'serverUrl keeps token on API fetches');
  assert.match(src, /u\.searchParams\.set\('board', ACTIVE_BOARD_FILE\)/, 'serverUrl propagates board query to API fetches');
  assert.match(src, /function boardSwitchUrl\(row\)/, 'board switch URL helper exists');
});

test('F2 — view.html: UI polls /boards.json and renders active/current board selector', () => {
  const src = read(VIEW);
  assert.match(src, /async function fetchBoardRoster\(\)/, 'boards roster fetch helper exists');
  assert.match(src, /serverUrl\('\/boards\.json'\)/, 'boards roster uses /boards.json');
  assert.match(src, /const \[boardsInfo, setBoardsInfo\]/, 'Board component keeps boards roster state');
  assert.match(src, /setInterval\(pollBoards, 4000\)/, 'boards roster polls independently');
  assert.match(src, /function BoardSelector\(\{ boardsInfo \}\)/, 'BoardSelector component exists');
  assert.match(src, /className: 'boardselect'/, 'BoardSelector renders compact header selector');
  assert.match(src, /window\.location\.assign\(boardSwitchUrl\(next\)\)/, 'selector switches via refreshed token URL');
});

test('F3 — view.html: UI polls /peers.json and renders priority/workload/remaining/heartbeat freshness', () => {
  const src = read(VIEW);
  assert.match(src, /async function fetchPeerRoster\(\)/, 'peers roster fetch helper exists');
  assert.match(src, /serverUrl\('\/peers\.json'\)/, 'peers roster uses /peers.json');
  assert.match(src, /const \[peersInfo, setPeersInfo\]/, 'Board component keeps peers roster state');
  assert.match(src, /setInterval\(pollPeers, 4000\)/, 'peers roster polls independently');
  assert.match(src, /function PeersDashboard\(\{ peersInfo \}\)/, 'PeersDashboard component exists');
  for (const needle of ['peerPriority', 'peerWorkload', 'peerRemaining', 'fmtHeartbeat', 'heartbeat', 'remaining', 'workload']) {
    assert.match(src, new RegExp(needle), `peers dashboard includes ${needle}`);
  }
  assert.match(src, /peersInfo\.available === false/, 'peers unavailable state renders gracefully');
  assert.match(src, /no active peers/, 'empty peers state renders gracefully');
});
