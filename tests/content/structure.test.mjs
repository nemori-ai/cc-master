...
test('hooks.json registers all 6 hook scripts across 5 events via plugin-root paths', () => {
  ...
  assert.ok(h.hooks.SubagentStop, 'SubagentStop registered');
  ...
  for (const s of ['bootstrap-board.sh', 'verify-board.sh', 'reinject.sh', 'subagent-stop.sh', 'posttool-batch.sh', 'usage-pacing.js']) assert.match(all, new RegExp(s));
