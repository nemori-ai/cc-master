...
- **怎么办**（归 A 镜头 7·决策）：这是一个典型的 `blocked_on:"user"` 决策——**范围（砍 scope）/ 期限（延 deadline）/ 加资源（换号补配额）三选一** surface 给用户。本文只教「怎么识别这个张力（读哪两个字段）」；surface 动作 + decision_package 采访包归 `master-orchestrator-guide`。
