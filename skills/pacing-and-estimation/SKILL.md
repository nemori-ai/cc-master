...
>
> **职责边界**：**消费机制**归本 skill；**决策**归 `master-orchestrator-guide`（「量力而行」/「目标即依赖图」/「该问就问」三镜头 + 决策程序 + `references/cost-decisions.md` 的换号决策锚）；**命令怎么敲 / account 操作**归 `using-ccm`；**走廊数学 / 估算算法的实现 SSOT** 在 ccm 引擎 `@ccm/engine`（本 skill 不复述数学）。
...
- **别自己重算引擎已经算好的东西**（DRY）：`ccm usage advise` 已把账户 `used_percentage` + `resets_at` + effective-N 喂进引擎走廊数学（`@ccm/engine` 的 `usage/pacing.ts`），你**读它的 `verdict` 字段**即可，不必自己拿百分比重算走廊上下沿。
