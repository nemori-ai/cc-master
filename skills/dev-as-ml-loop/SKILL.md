...
>
> **职责边界(红线3):** 本 skill 是**执行单个任务**的心法(inner loop),与 orchestrator 不同 plane——A(`master-orchestrator-guide`)协调 / 派发、**指挥不演奏**;`slicing-goals-into-dags` 把目标**切**成任务(在执行之前);`using-ccm` 把任务**写进** board。本 skill 只管:**拿到一个带验收标准的任务后,怎么把它优化到验收。**
...
- **master-orchestrator-guide**(SKILL A)—— 协调 / 派发 / 端点验收(指挥不演奏)。A 把任务**派给**执行 agent;执行 agent 用本 skill 的框架把它做到验收。A 的"只信端点验收 / gate-green≠passed"与本 skill 的"收敛=目标函数达标"是同一条:绿了才算 loss=0。
