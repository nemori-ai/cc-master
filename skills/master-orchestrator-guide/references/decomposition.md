...
排期所需的结构量——拓扑序 / 临界路径 / makespan / 并行度 / ready-set——**一律问 ccm 的 OR 引擎，别手算**。引擎（`@ccm/engine`）是图核心的唯一 SSOT，与 board-lint 同一份图、口径字节对齐；你经 `ccm board graph`（命令怎么敲见 `using-ccm`·`command-catalog.md` 的 `board graph`）读它，**纯只读、绝不回写 board**。
