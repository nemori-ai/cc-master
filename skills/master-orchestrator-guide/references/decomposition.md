...
> **服务愿景：C4**（拆解 / 管理 / 更新 / 规划）**· C5**（在资源预算内高效调度）。**何时读：** 拿到一张**已切好**的 DAG 要给它排期时——临界路径 / float / 并行度是什么、为何把资源压临界链、粒度与每节点契约。**「怎么把目标切成这张 DAG」不在这里**（纵切薄增量 / walking skeleton / 粒度品味）——那是 `slicing-goals-into-dags`（E）：**切先于排**。
>
...
- **每条依赖边都是债务**（魂镜头 2 的硬规则）：默认全并行，逐边举证——除非能指名一个被下游直接消费的具体上游产物（artifact / hash），否则删掉那条边。「先做 X 当安全网」「按这个顺序更稳妥」是顺序习惯，不是数据依赖。（规则 SSOT 在魂、CPM / float 论证 SSOT 在此，勿互抄。）
...
**资源决策 —— 把最强资源压临界链**：临界路径上的**难实现**用强模型 + 双 reviewer + 你紧盯；高 float 任务配便宜资源、塞进空隙里跑。这里的「资源」含每节点的**模型档位（model tier）**——档位事实、四档相对成本、按难度选档、以及为何*主线*绝不切模型，全在 `pacing-and-estimation`（H）的 model-tiers；本文只给「把稀缺档位压临界链」这条排期判断，**不复述档位语义**。

---

## 3. 用 `ccm board graph` 机器算（主力路径）

排期所需的结构量——拓扑序 / 临界路径 / makespan / 并行度 / ready-set——**一律问 ccm 的 OR 引擎，别手算**。引擎（`@ccm/engine`）是图核心的唯一 SSOT，与 board-lint 同一份图、口径字节对齐（ADR-012/014）；你经 `ccm board graph`（命令怎么敲见 `using-ccm`（D）·`command-catalog.md` 的 `board graph`）读它，**纯只读、绝不回写 board**（红线 2）。
...
**时间输入从哪来**——`criticalPath.makespan` 要 measured 才有小时数；要**预测**这片的 ETA / 查进度偏差，用 `ccm estimate`（ETA / 临界路径 / EVM / 风险）+ `ccm baseline`（EVM 计划基线）。**这两者的输出怎么读、forecast / EVM / confidence / 风险该不该信、coverage 低时怎么降低信任——全归 `pacing-and-estimation`（H），本文不复述**；本文只说：把 estimate 的 ETA / risk 当排期的时间输入，靠数据排程、不靠手感。
...
- **心算够用**——拓扑平凡时：小图 / 单链 / 浅依赖临界链一眼可见；只需粗判 fan-out 值不值（`parallelism` 明显 ≈1 或 ≫1）。**在平凡图上仪式性跑一遍 CLI 是 busywork**（不解锁依赖、不降风险、不产 artifact、不验假设——过不了 fill-work 准入测试·违镜头 4），别为「显得严谨」而跑。
...
> **字段怎么落进 board**（`acceptance` / `estimate` / `deps` / `executor` 怎么写、撞哪条校验规则）见 `using-ccm`（D）的 `board-model-guide.md`——本文只给排期判断，不教 ccm 命令与字段取值。
