...
> **何时读：** 你要懂 `ccm account` 这套换号号池**为什么这么设计**——号池模型、录号机制 why、选号判据、vault 安全局限时（换号是最重的 pacing lever，它的底层容量就是这套号池）。
> **这是概念叙事（why），不是实现，也不是决策。** 命令**怎么敲**见 `command-catalog.md` 的 namespace account；**算法 / vault / 切号的实现** 在 ccm 引擎里（本文不复述实现代码）；换号**决策**（何时换 / 值不值 / 谁拍板 / 绝不自授权）归 `master-orchestrator-guide` skill。本文只讲「读手册的人也该懂的那条 why」。
