...
J_top: agent 要对 board 做读写时，**用 ccm 操作得对**——status 经合法生命周期 verb 转移（不当字段赋值、不踩 `ready→done`）、🔒 字段走专属命令（不 `--set`）、`ccm` 缺则降级 Write+lint，不绕写关卡、不踩 footgun、不伪造审计轨迹（如造一个没 `started_at` 的 done）。
...
baseline_reference:
  ...
  expected_uplift: 把 strict_dim 从 floor 推过去——一上手就走对的 verb（status 用 start/done/block/set-status、先 start 再 done）、改 ✎ 字段才 `--set tasks[ID].field`、`ccm` 缺时降级而非硬卡、撞闸读 exit code 纠偏而非 `--force` 硬推。
