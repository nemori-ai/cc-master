...
> **分发 skill(随插件 ship)。** `ccm` 是 cc-master 的 board 命令行——board 数据模型 SSOT 的**唯一写入关卡**(ADR-013/014)。本 skill 是它的**操作手册**:不是"该编排什么"(那是 master-orchestrator-guide),而是"既然要动 board,怎么用 ccm 动得对"。
...
- [references/command-catalog.md](references/command-catalog.md) —— 全量命令面:14 个 namespace（board/task/log/jc/cadence/watchdog/baseline/policy/peers/usage/estimate/account/statusline/upgrade）每条命令的签名 / positional / flag 表 / 例子 / `--json` 输出形状。其中 `upgrade`（自升级 ccm 二进制 + 插件）/ `statusline`（self-contained 状态行）是非 board 操作。
...
- **master-orchestrator-guide**(SKILL A) —— 决策层:该编排什么、怎么拆 DAG、何时阻塞等用户、何时换号。本 skill 是它的"手怎么动"。注:**board 协议 SSOT 是 `ccm` 引擎 `@ccm/engine` 的 board-model**(enums / 字段元数据 / 不变式 / 状态机);A 的 board 协议 reference 是 orchestrator 侧的**派生叙事**(home 解析 / snapshot / supersession 等长程协议 narrative)、**非 SSOT**——二者本 skill 都不复述,只给操作视图。
