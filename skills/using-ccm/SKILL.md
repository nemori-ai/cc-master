---
name: using-ccm
description: 'Use when you (orchestrator/agent) read or mutate a cc-master board through the ccm CLI —— 当你要用 ccm 操作 board:建板 / 加改任务 / 起跑·完成·阻塞 / 查 ready·DAG·临界路径 / 记 judgment_call·log / 收 cadence iteration / arm watchdog 时。它是 ccm 的操作手册:命令面 + board 状态机心智 + 写入关卡纪律(status 走生命周期 verb 不用 --set、🔒 字段走专属命令、ccm 缺则降级 Write)。Triggers: 敲 ccm task/board/jc/cadence/log/watchdog 命令、"怎么把任务标 done / 加依赖 / 查临界路径 / 阻塞等用户"、任何 board 写操作、ccm 报 exit 3 / illegal transition / load-bearing 字段被拒。Do NOT use when 你在决定"该编排什么 / 怎么拆 DAG / 何时换号"(那是 orchestrating-to-completion)、写 workflow 脚本(authoring-workflows)、管号池与 vault(account-management)——本 skill 只管"怎么用 ccm 这把工具",不管"该让 ccm 做什么"。'
---
...
>
> **职责边界(红线3):** 决策归 A、workflow 脚本归 B、号池机制归 C;**怎么用 ccm 这把工具操作 board** 归本 skill。A 在它的决策点说"现在把 T7 标完成 / 派发 T3 / 阻塞等用户",**怎么落成 ccm 命令**翻到这里。

---

## 何时翻开本 skill

你要对 board 做**任何**读或写——建板、加 / 改任务、起跑 / 完成 / 阻塞、查 ready 集 / DAG / 临界路径、记 judgment_call 或 log、开 / 收 cadence iteration、arm watchdog——就用 ccm,用法看这里。全量命令 / flag / `--json` 形状在 [references/command-catalog.md](references/command-catalog.md);本文给的是**心智 + 纪律 + 热路径**,让你不必逐条 `--help` 也敲得对。
...
board 字段分三档(完整定义是 board 协议 SSOT,见 SKILL A `orchestrating-to-completion` 的 board 协议 reference:`${CLAUDE_PLUGIN_ROOT}/skills/orchestrating-to-completion/references/board.md`;这里只给**操作规则**):
...
- [references/command-catalog.md](references/command-catalog.md) —— 全量命令面:6 个 namespace 每条命令的签名 / positional / flag 表 / 例子 / `--json` 输出形状。
- **orchestrating-to-completion**(SKILL A) —— 决策层:该编排什么、怎么拆 DAG、何时阻塞等用户、何时换号。本 skill 是它的"手怎么动";它的 board 协议 reference(`${CLAUDE_PLUGIN_ROOT}/skills/orchestrating-to-completion/references/board.md`)是 board **协议 SSOT**(schema / status enum / 三档字段 rationale),本 skill 不复述、只给操作视图。
