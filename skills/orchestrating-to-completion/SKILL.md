...
board 是指挥为一场长任务存的持久 save file——一张带状态的任务依赖图。它一身二用：① 跨 compaction 存活的记忆，② hook（一个 shell，对 agent context 与内建 `Task` 工具都失明）唯一能读的窗口。**你的 board 文件才是单一真相源**（内建 `Task*` 工具至多是个非权威的 in-session 草稿镜像）；每个 turn 你经 `ccm` 命令改它（写关卡；`ccm` 缺则 `Write` 整个文件——它很小），并在决策程序 step 7 flush 它。
