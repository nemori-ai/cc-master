...
| `references/cost-and-pacing.md` | Choosing a model tier per node + why the main thread stays on one model (prompt-cache); pacing a long run against the 5h/7d quota window — sense via `scripts/cc-usage.sh`, levers: downgrade model / lower WIP / defer float. |
