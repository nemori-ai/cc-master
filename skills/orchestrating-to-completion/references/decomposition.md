...
**资源决策**：把最强的资源压在临界路径任务上（opus 实现 + 双 reviewer + 编排者紧盯）；高 float 任务配便宜资源、塞进空隙里跑。这里的"资源"包含每节点的**模型档位（model tier）**——强档（Opus/Fable）上临界链，便宜档（Haiku/Sonnet）上机械性的 float。四档表、相对输出成本、以及为何*主线*绝不切模型，都在 `references/cost-and-pacing.md` 里。
