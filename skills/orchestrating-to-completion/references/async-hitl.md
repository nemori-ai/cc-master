...
- **`enter_cmd` 自带 home 选择器**：discuss 在新终端起、未必继承自定义 `CC_MASTER_HOME`——home 非默认时把 `--home <绝对路径>` 写进 `enter_cmd`（**路径加 shell 引号**防含空格 home 被截断；默认 home 则裸 `/cc-master:discuss <node-id>`），让复制按钮产物不依赖 env。生成规则 SSOT 在 `board.md` §decision_package。

### 消化 `.decision.md` sidecar（in recon step 1）

recon 时扫 awaiting-user 节点：发现 board home 同目录有对应 `<board-stem>--<node-id>.decision.md` sidecar（`<board-stem>` = 当前这块板的文件名去 `.board.json` 后缀——带 board-stem 是为在共享 home 多 board 下防撞 / 防误消化别的板的 sidecar；node id 既要拼进此文件名，挂 decision_package 的节点 id 应 path-safe `[A-Za-z0-9._-]`、非 `.`/`..`，discuss 落 sidecar 前会 guard 校验）→
