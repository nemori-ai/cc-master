...
- **resume（`--resume` / `-c`）与 compaction 都保留原 `session_id`**——`SessionStart` 分别以 `source:"resume"` / `"compact"` 触发、`session_id` 不变。故武装与 reinject **跨 resume / compaction 照常工作**，无需特殊处理。
- **全新独立会话（无 `--resume`，必拿新 `session_id`）对别人的 active 板按设计休眠**——这是红线 6 防跨会话污染，**不是**续跑失效。
- **未盖 `session_id`（空串 `""`）的 active 板由下一个 active 会话收养**（对称 degrade）——空 board sid 会被任何非空 stdin sid 字面对不上而**永久孤儿化**，故空 board sid 也降级武装；但 board sid **非空且 ≠ 本会话 sid** 仍休眠（红线 6 防线不退化）。
