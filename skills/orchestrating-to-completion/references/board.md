...
- **写时即校验（`ccm` 命令·首选路径）**：经 `ccm` 命令改 board 时，写入关卡在**落盘前**就跑全套校验——有 hard error 直接 `exit 3` 拒绝落盘，坏 board 根本写不进去（无需事后再 lint）。
- **自动兜底（PostToolUse lint hook·`Write` 降级路径）**：你走 `Write` / `Edit` 降级改 active board 后，lint hook（经 `spawn ccm`）写盘后自动校验；若发现结构 / 语法 / 格式错，会注入一条点名「违了哪条规则 + 哪个字段 / task + 怎么修」的非阻断提示——**看到就当回合修掉，别带病往下跑**。
- **手动（随时跑 lint 脚本）**：当你用 **`Bash`（`sed` / `echo` / `cat >` / 脚本）改了 board**（lint hook 看不见这类编辑、且绕过了 `ccm` 写关卡——尽量别这么干，除非 `ccm` 不可用），或任何想确认 board 健康的时刻，主动跑：
