...
- soft-observed 读法（同 `wip_limit` 模式）：`verify-board.sh` best-effort 读 top-level `wakeup`——有 `in_flight` 且无 `wakeup`（或非对象）→ 注入 watchdog 提醒；**已有 `wakeup` 对象 → 当作「已 armed」、静默不提醒**（present = armed）。
- **退役 watchdog 必须从 board 移除 / 清空 `wakeup` 对象（不只 CronDelete 那个 job）**：因为 hook 见任何 root `wakeup` 对象就当 armed 而静默，**陈旧的 `wakeup` 残骸会让 hook（与 compaction 后的你）误判仍有 watchdog armed**——于是下一次「有可能静默失败的 in_flight」等待时，本该发出的 watchdog 提醒被静默掉，重开静默失败盲区。**不变式：当前无 watchdog armed 时，`wakeup` 必须 ABSENT**——退役 = CronDelete job **且** 删 `wakeup` 对象，两件一起做（[[Finding #56]]）。
...
```json
{
  ...
  "wip_limit": 4,
  "wakeup": {
    "armed_at": "12:30Z", "fire_at": "13:15Z", "mechanism": "cron", "job_id": "cron-9f",
