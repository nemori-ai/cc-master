...
**本质**：你跑长任务时的"存档文件"——一张带状态的**任务依赖图（task dependency graph）**。它身兼两职：① 跨 compaction 存活的记忆，② hook 唯一能读到的那扇编排状态窗口（hook 是个 shell——它读不到 agent 的 context，也读不到内建的 `Task` 工具）。
...
> **故本文是 orchestrator 侧的派生叙事视图**（协议 narrative + 长程操作纪律：home 解析 / snapshot·flush / supersession / 柔性边故事）——**board 协议的 canonical SSOT（enums / 字段元数据 tier / 不变式注册表 / 状态机）是 `@ccm/engine` 的 board-model**（ADR-013/014），本文与它冲突时以引擎为准。字段取值的操作视图 + 全部校验规则速查见 `using-ccm` 的 `board-model-guide.md`。
...
- **存储 = 单一真相源的 board 文件（每编排一份命名文件）**：**board 变更只走 `ccm` 命令**——`ccm` 是唯一写入关卡（持锁 / 落盘前校验不变式 / 守状态机 / 盖 derived 字段；怎么用见 `using-ccm` skill）。**直接 file-edit（`Write` / `Edit` / `sed` / `echo` / `cat >`）会被 board-guard PreToolUse hook 拦**（ADR-025·手改绕过写关卡、静默腐蚀 deps 图 / 状态机 / 窄腰）；markdown 视图按需生成。校验这次写盘合不合契约的**机械关卡就是 `ccm`**（`ccm` 命令写时即校验，有 hard error 直接拒绝落盘）；PostToolUse lint hook 只作事后 backstop——见下方「board lint」段。
...
- **不存的反例（诚实劝退）**：① 纯 shell node 的 token——shell 无 token 概念（标 `shell-no-tokens` 即可）；② **delta 反推 per-node token**（node start/finish 各读一次账户 `used_percentage` 取差值）——**并发污染结构性不可行**（多 node 在飞时 delta 把它们全混在一起、无法归因到单 node；且账户口径只给百分比无绝对 token 分母）。delta 是**账户级 pacing** 工具（见 `pacing-and-estimation` skill 的 usage-signals），口径与用途与 per-node 度量正交，**勿混用**。

#### account 柔性边 —— `accounts[]`（换号消费追踪，纯 agent-shaped）

> **来源迁移（A2 T6）**：effective-N（pacing 的「还能序列消费几份配额」）的**权威来源已从 board 迁到号池 registry `accounts.json`**——`usage-pacing.js` 现在**只读** `${CC_MASTER_HOME:-$HOME/.claude/cc-master}/accounts.json`（数非 active 且 token 未过期的可切入备号 + 当前在用号）算 effective-N，**既不读 board 的 `num_account` 也不读 board 的 `accounts[]`**。accounts.json 与 board **正交**（独立的用户级、跨编排 registry，号池怎么建 / 怎么算见 `using-ccm` 的 `references/account-pool.md` + ccm `account` 引擎）。故 board 的 `accounts[]` / `num_account` **不再是 soft-observed**——hook 已不读它们，它们退回**纯 agent-shaped**（agent 想留作续跑/复盘的消费轨迹台账可留，hook 一概不读）。
...
- **top-level `accounts`**（数组，optional，默认 / 缺省 = 缺省）：**纯 agent-shaped 柔性边**——hook **完全不读**。当你握多份配额并随 pacing 用换号 lever 序列消费时，可用它给**续跑 / 复盘**留一笔可读的消费轨迹（哪些备号已烧穿 / 当前活跃哪号）；但它**不驱动任何 hook 行为**——effective-N 由 `usage-pacing.js` 从 `accounts.json` 现算（见上「来源迁移」）。schema（agent 自由塑形）：
  ...
  - **谁填、何时填（可选）**：orchestrator 若要留消费轨迹，可在换号那一拍（决策程序里 surface「换号」并执行换号机制前后）顺手记一笔。纯 prose 纪律，零 hook、零脚本，**省了也不影响 pacing**（pacing 的 effective-N 看 registry、不看这里）。换号 lever 的编排决策序列在 `references/cost-decisions.md`；选号 / 切号机制在 `using-ccm`（D·account-pool.md）+ ccm `account` 引擎。
  ...
  - **安全检查 —— ✅（关键）**：`accounts[]` **只存账号标识 + 消费状态，绝不存任何 token / 凭证值**——board 入不入版本控制都一样：bearer token 全程只活在 vault + 换号脚本子进程 env，**永不落 board / 永不 commit**（token 安全纪律实现 SSOT 在 ccm 引擎 `@ccm/engine/account/vault.ts`·概念叙事见 `using-ccm` 的 `references/account-pool.md`）。
...
- **`decision_package`**（per-task 柔性边，挂在 `blocked_on:"user"` 节点上）：master 在 idle / 创建 awaiting-user 节点时为该节点预备的一份采访包（agent-shaped，on-board，webview 可直接渲染富决策卡）。canonical 契约（字段名 / 枚举值钉死，勿改）：
  ...
  - **lint 强制（R8·board-lint-core）**：上面这些「钉死」不是纯君子协定——board-lint 用一条 awaiting-user 完整性规则给采访闭环兜底（背景：曾出现 awaiting-user 节点不带 / 带不全 decision_package、工具链全绿放行，等到新 session 跑 `/cc-master:discuss` 才发现没包、讨论开不起来——C1「board 完整性零机制保障」典型）。判定口径 = `isAwaitingUser`（与 webview / discuss 两端对齐）：节点 `blocked_on === "user"` 且 `status ∈ {blocked, in_flight}`。
    ...
    - **红线 2 不破**（R8 校验的是 agent-shaped 柔性边、**decision_package 仍不进 narrow waist**）：① 行为型 hook（reinject / verify-board / posttool-batch / usage-pacing）仍**不读** `decision_package`，编排行为不依赖它；② board-lint 是**校验器**不是行为 hook，且它本就对 agent-shaped 字段合法性 hard-error（R5b `blocked_on` 是先例）；③ PostToolUse 的 board-lint hook **绝不 `decision:block`**（只软提示），故 R8a hard error 不卡你写盘、只在 CLI / `run-tests.sh` 端点闸真红。R8 只查「awaiting-user 这一既有契约位上的柔性边」的存在 + 形状，不要求任何别的柔性边存在（守 silent-on-unknown）。
...
- **绝不进硬 waist（红线 2）**：它**不是** pinned 的 `schema`（那是 hook 契约 / 窄腰，content 测试断言 === `'cc-master/v2'`，动它 = 动窄腰）。`meta.template_version` 与「waist 协议版本」正交，是纯 agent-shaped 字段、**零 hook 读它做分支**。若未来有 hook 想据它分支行为，那一刻它就从 agent-shaped 升为 soft-observed，须按下方「硬 waist vs soft observed」走人审——本变更不引入任何这种读取。
...
- **未盖 `session_id`（空串 `""`）的 active 板保持休眠**（红线 6：不武装不相关 session）——合法续跑因 resume / compaction 保留 `session_id`、板带原 `session_id` 故照常匹配武装；异常的 blank 板（bootstrap 在缺 sid 的 stdin 上建板）由**显式 re-arm**（重跑 `as-master-orchestrator` → bootstrap 重盖 `session_id`）认领。不对称收养空 board sid：那会武装任意不相关 session、破红线 6，故只认显式 re-arm。SSOT 见 ADR-007 §2.3 / §4.5。board sid **非空且 ≠ 本会话 sid** 同样休眠。
- **`as-master-orchestrator --resume` = 显式跨 session re-arm（ADR-009）**——和上面那条平台 `--resume`（保留 sid，是 Claude Code 的 resume）**不是一回事**：这是用本插件的命令、让一个**全新 session** 显式**接管**一块别的 session 的（或已归档的）board。bootstrap（唯一武装豁免 hook，ADR-007 §2.5）会把选定旧板的 `owner.session_id` **盖成新 sid**、`owner.active` 置 `true`（**可复活 `/stop` 归档的板**，`false → true`），并**保留 `tasks` / `log` / `goal` / `git`**。这是经 `as-master-orchestrator` + 用户显式 `--resume` 授权的合法武装形态（区别于被拒绝的「隐式自动收养空板」），落在红线 6 精神内——SSOT 在 ADR-009，对「显式 vs 隐式」与「复活归档板为何合规」的论证去那里读，此处不复述。
...
- **✎ `runtime.*` 是 hook-owned 例外（ADR-020）** —— IDNUDGE 等周期 hook 经 `ccm board set-param` 带锁写 `board.runtime.*`（如 `last_identity_remind`）。你走 `ccm` 命令改 board 天然保留它（`ccm` 做字段级合并、不整盘覆写）；你自己**永不写 `runtime.*`**（那是 hook 的簿记区）。
...
- **写时即校验（`ccm` 命令·唯一写路径）**：经 `ccm` 命令改 board 时，写入关卡在**落盘前**就跑全套校验——有 hard error 直接 `exit 3` 拒绝落盘，坏 board 根本写不进去（无需事后再 lint）。这是 board 变更的唯一正路。
- **写关卡硬化（board-guard PreToolUse hook·ADR-025）**：直接 file-edit board 在**执行前**就被 deny——`Write` / `Edit` / `MultiEdit` 目标 board 文件、或 `Bash` 用 `sed` / `echo` / `tee` / `cat >` 手改它，都会被拦并注一条 `<directive source="board-guard">` 提醒改用 `ccm` verb。把「只走 `ccm`」从纪律硬化为机制，从源头挡绕关卡的手改。
- **事后 backstop（PostToolUse lint hook·经 `spawn ccm`）**：万一有 `Bash` 手改绕过 guard 的启发式检测溜进来（shell 写检测不可判定·偏假阴），写盘后的 lint hook 兜一道——注一条点名「违了哪条规则 + 哪个字段 / task + 怎么修」的非阻断提示，**看到就当回合改用 `ccm` 修掉，别带病往下跑**。
- **手动（随时跑 lint 脚本）**：任何想确认 board 健康的时刻，主动跑：
