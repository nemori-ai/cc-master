...
| **C2** 控制 token 消耗速度 | 5 | `pacing-and-estimation` skill（消费层）· `cost-decisions`（换号决策锚） | dispatch 的 "reserve budget+WIP first" 备注 | Stop (H8 usage-pacing) "[cc-master pacing] 5h 配额临界 ... pace 杠杆（见 orchestrating-to-completion / cost-and-pacing）" → 镜头 5（减速侧）; "5h 配额欠用 + 临 reset" / "未用满的额度…会永久蒸发" / "可加速" → 镜头 5 + pacing-and-estimation 加速侧 lever; "按 N 份可序列消费的配额理想节奏" / "切到下一份配额" → 镜头 5 + pacing-and-estimation 的 effective-N 缩放（多账号理想节奏·effective-N 由 hook 从 `accounts.json` 号池算，非起跑 flag）; "7d 配额硬总闸 ... 暂停 dispatch 新节点" → 镜头 5（7d 总闸收紧）+ 决策程序 dispatch §(f) + 镜头 7（surface 用户） |
