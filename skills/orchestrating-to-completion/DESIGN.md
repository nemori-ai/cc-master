...
- **形状蕴含**：(强 process, 强 cognitive) → Craft C。SKILL.md 以命名锚为主干（七镜头 / 红线 / Rationalization Table / Red Flags），配决策程序 dot-graph 作确定性牙齿，**不是编号清单**——价值在 agent 内化心智模型 + 服从那张 graph 的控制流，而非重放步骤。reinject 友好（每回合整篇重注，越短越好），深细节全下沉 `references/`。
