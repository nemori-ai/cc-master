...
## 切出 token 抢救（switch-out rescue·补 vault↔官方存储单向失同步）

> **病根（Finding #72·根因级）**：cc-master vault 是官方共享凭证存储的**一次性快照副本**（录号那刻从官方 keychain「Claude Code-credentials」拷来）。换号的 token 数据流本来是**单向**的——切入号方向**闭环**（refresh → writeback 切入号 vault → 覆写官方存储），但**缺反向回流**：运行中的 claude 在 access token 临近过期（~8h）时**自己惰性 refresh**（无重启换号架构正依赖此行为），服务端**轮转 refresh token**（旧的吊销、发新的），claude 把新 blob 写回**官方存储**（`account=$USER`）、却**绝不回流 cc-master vault**（claude 不知 vault 存在）；切出时也只写 token-blind 配额快照、**不抢救 token**。于是切出号 vault 停在「切入那刻 writeback 的值」、其 refresh token 早被 claude 自主轮转吊销 → **下次切入用它即 refresh 失败（rc=4）**。号池里「用过的号」逐个变切不进的死号。

> 注意与上一节「轮转后回写失败的 token 抢救」的**区别**：那节治的是**切入时 cc-master 自己 refresh** 轮转后**回写 vault 失败**（NEW_BLOB 是唯一副本、抢救到 recovery 文件）；本节治的是**切出号被 claude 自主 refresh** 轮转、新 token 只进官方存储、cc-master 全程没参与那次 refresh——两者都是「轮转后新 token 丢失同步」，但触发路径不同，合起来才补全 vault↔官方存储双向新鲜。

**修：切出 token 抢救（`rescue_switchout_token`）**——在覆写官方存储成新号**之前**（此刻官方存储 `account=$USER` 还是切出号被 claude 更新到最新的 blob），把它读出来 `writeback` 回**切出号**的 vault，补上缺失的「官方存储 → 切出号 vault」反向流，与既有「切入号 vault → 官方存储」形成闭环。机制要点：

- **时机**：换号锁内、`overwrite_official_stores` **之前**（必在覆写前读官方存储——覆写后官方存储就是切入号、再读不到切出号最新 blob）。
- **读取源**：官方存储当前完整 blob——mac 主路径 `security find-generic-password -w -s "Claude Code-credentials" -a "$USER"`（与录号捕获源同·完整 blob 只在 keychain·credentials.json 的 refreshToken 在 mac 上可能空），非 mac fallback `credentials.json`。校验三必需字段（含**非空 refreshToken**·缺则跳过——抢救一个没 refreshToken 的 blob 无意义）。
- **身份 guard（安全关键·防 mislabel）**：官方存储 `account=$USER` 的 blob 属于谁，看 `~/.claude.json` `oauthAccount.emailAddress`——须 == 切出号 email，否则官方存储当前不是切出号（用户手动 `/login` 切过？）→ **绝不**把别号 token 写进切出号 vault（会污染号池），跳过；读不到身份也保守跳过（不冒险）。复用 [录号身份 guard](#agentic-录号的安全闭环) 同一范式。
- **token-blind**：官方 blob 经管道喂 node 规整、经 `writeback_vault` 的 keychain argv / file printf 写·绝不回显 / 进 registry（同 `NEW_BLOB` 纪律）。
- **best-effort·绝不阻断换号**：任一步失败（无切出号 / 读不到官方完整 blob / 身份不匹配 / 回写失败）→ 跳过 + 非致命提示，切入照常（失败最坏 = 下次切回该号需 `--refresh`，与修复前现状同）。
- **幂等无害**：即便端点不轮转 refresh token，把官方最新 blob 写回 vault 也无害；若轮转则修复 bug——故修复**不依赖**「是否轮转」这一 Anthropic 服务端行为的确定性（该环节待真账号 dogfood 坐实，见 Finding #72）。
