...
- **机制硬闸（`switch-account.sh` 校验）**：`switch-account.sh` 在真正覆写凭证三存储**之前**，经进程边界 `ccm policy show --json` 读 active board 的 policy；`autonomous_account_switch == "deny"` → **拒绝切号、exit 4、打日志**（在 board.log 留痕「机制层拦下一次越权换号」）。哪怕 orchestrator 忘了规则 / 把建议层合理化掉，机制层也拦住、且**越界响亮**（loud failure，不是静默放行）。
