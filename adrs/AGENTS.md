...
| **ADR-035** | [Goal Contract lifecycle](ADR-035-goal-contract-lifecycle.md) | Accepted（用户授权实施并交付 PR·2026-07-15） | raw request 只作需求证据；以 optional observed `goal_contract` + ccm-managed append-only Goal Brief + `ccm goal set/confirm/amend/show/check` 在 DAG 前固化 normalized goal，并由 skill/ccm/hook 分层防漂移；不扩大 narrow waist，legacy board 诚实降级。 |
| **ADR-036** | [declared delivery and dependency truth](ADR-036-declared-delivery-dependency-truth.md) | Accepted（用户批准 D1–D4·2026-07-14） | 以三个 ✎ flexible 字段分离 candidate-complete / target-delivered / dependency-qualified；历史 board exact legacy，新 contract 只启 declared，strict 仅只读 dry-run；本地 exact containment + reviewed reconciliation + non-git digest + edge-scoped expiring user waiver，零隐式网络，窄腰不动。 |
| **ADR-037** | [adapter-derived harness capability protocol](ADR-037-adapter-derived-harness-capability-protocol.md) | Proposed | ccm 的 harness adapter registry 成为受支持 harness 的唯一 exhaustive inventory；消费者按小型、版本化 capability protocol 组合 adapter facts，unsupported 显式声明，避免 parallel target lists 静默漏端。 |
| **ADR-038** | [Git-native skill knowledge graph](ADR-038-git-native-skill-knowledge-graph.md) | Accepted（用户批准完整方案·2026-07-23；**K3-00 graph-first / skill-as-artifact amendment 2026-07-24**） | canonical Markdown span + module-sharded strict JSON + stable marker + typed change set；composition `consumes` + candidate_analysis derived admit；multipane graph 在 final host Markdown 验 point→point directed diameter ≤3，critical primary 更短；Node in-memory compiler，数据库不作 v1 canonical store。 |
| **ADR-039** | [换号归机器：orchestrator 不感知自主换号](ADR-039-capacity-switching-belongs-to-the-machine.md) | Accepted（用户拍板·2026-07-30） | 换号整条移出编排器的 lever 清单——配额压力下只剩同一份配额内腾挪，到边界即停派 + `blocked_on:"user"`；唯一例外是用户直接命令（且不构成下次自决资格）。收窄 ADR-016：`policy.autonomous_account_switch` 字段/闸/缺省全留，语义从「授权 agent 自主换号」改为「后台自动换号是否放行」。知识层退役 2 点（`capacity.account-switch-{gate,sequence}`）/ 新增 2 点（`capacity.switch-is-not-your-lever` / `capacity.post-switch-continuity`）。ccm 引擎、hook、board schema 一字不动；诚实记账：全机统一切号仍待 ccm 侧收口（当前执行者仍是 per-session `usage-pacing` hook）。 |
