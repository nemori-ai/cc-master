...
| **ADR-038** | [Git-native skill knowledge graph](ADR-038-git-native-skill-knowledge-graph.md) | Accepted（用户批准完整方案·2026-07-23） | canonical Markdown span + module-sharded strict JSON + stable marker + typed change set；multipane graph 在 final host Markdown 验 point→point directed diameter ≤3，critical primary 更短；Node in-memory compiler，数据库不作 v1 canonical store。 |
