...
| **ADR-017** | [多-orchestrator 协调](ADR-017-multi-orchestrator-coordination.md) | Accepted（用户拍板·2026-06-29）| 多块板同时活跃时的 orchestrator 间**协调感知层**：跨板**只读花名册**（`ccm peers` 读 verb·扫 home 全 active 板出 goal / owner / 进度，不抢锁）+ board 新增 ✎ `coordination` 块（agent-shaped·hook 不读·红线2 不破）。协调走**感知 + 板级优先级 + 机械 floor**（换号协调溶解进 LOADBAL 选号、配速协调走各板独立感知），**砍掉**显式通信通道（无 message-passing·避免分布式复杂度）。Source: 2026-06-29「多-orchestrator 协调」需求发现（用户拍板砍通信通道）。 |
...
| **ADR-029** | [`ccm web-viewer` namespace](ADR-029-ccm-web-viewer-namespace.md) | Accepted（用户 scope change·2026-07-08·实现待 WV12/WV13/WV8） | board web viewer 的正式入口改为 `ccm web-viewer`（不是 `ccm view`，也不是 `/cc-master:view` / `$cc-master-view`）；ccm owns home-scoped lifecycle（start/open/status/list/stop/restart + internal `serve --state`、`<home>/services/web-viewer/` service files、pid + `/_ccm/health` stale 检测、token redaction、restart 新 token、扫描 `<home>/boards/` 并在 viewer 内列出/切换 boards；`--board`/`--goal` 只设初始 selection，不按 board 创建独立 service）；viewer 继续只读、127.0.0.1、token-gated、viewer/data routes GET-only、零外网、无 board writes、路径 containment。旧 plugin command / Codex skill 只允许短期迁移提示，后续删除并由 projection 生成 dist。Source: WV11-WEBVIEWER-DESIGN scope change。 |
