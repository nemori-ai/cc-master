...
- 你要给一个新 / 现有 skill 落一份设计宪法（分发 skill 用 `.design/DESIGN.md`）。
...
## `.design/DESIGN.md` 设计宪法

每个站得住的 skill 配一份 6 段设计宪法（本仓统一用 6 段），回答「这 skill 是什么 / 为什么」，**设计先于实现**——任何对 SKILL.md 的实质改动先更新对应设计文档。分发 skill 放 `skills/<name>/.design/DESIGN.md`，保持 runtime 根目录只放 agent 会读的文件；dev-only skill（住 `.claude/skills/`）可保留根部 `DESIGN.md`。六段：① one-liner ② craft 自分类 ③ value triad（plugin / agent / human 三视角）④ 责任边界（IN / OUT 移交表 / boundary heuristic）⑤ 触发与反例 ⑥ 演化锚。
...
| 给这 skill 写设计宪法 | `.design/DESIGN.md` 6 段模板 | 见 reference |
...
- **[`references/design-md-template.md`](references/design-md-template.md)** — 6 段设计宪法模板 + 每段填写指引。
