...
两者分开的收益：改 skill 前先在设计宪法把设计意图更新清楚（一次思考），SKILL.md 只承接「怎么用」的落地（一次执行）。设计宪法留下设计演化的痕迹，SKILL.md 留下行为契约的痕迹——各自单一真相源。

**路径约定：** 分发 skill（`skills/<name>/`）的设计宪法放在 `skills/<name>/.design/DESIGN.md`，同目录的轻量 J 放在 `skills/<name>/.design/OBJECTIVE.md`。`.design/` 是 co-located dev 文档，release 打包时剔除，不给用户 agent 读。dev-only skill（`.claude/skills/<name>/`）不随插件分发，可继续用根部 `DESIGN.md` / `OBJECTIVE.md`。

---

## 6 段模板（可直接复制）

把下面整段复制到分发 skill 的 `.design/DESIGN.md`（dev-only skill 可复制到根部 `DESIGN.md`），逐段填实。`<...>` 是待填占位。
...
直接抄 `cc-master-skillsmith` 的 craft 两轴诊断结论。这一段是 **设计宪法和 skillsmith 的接缝**：curating 判「要不要建」，skillsmith 判「建出来该长什么形状」，DESIGN.md 的 §2 把后者的结论钉在案，让维护者改 body 前先知道目标形状。**curating 自己不做 craft 诊断**（那是 skillsmith 的领地），只在设计宪法留一格转录结论。
...
对应 scoresheet D2（bounded-context）+ 重叠检测。OUT-of-scope 移交表的每一行，应该和这个 skill 的 `description` 里的 `Do NOT use …` 反例一一对应——设计宪法的边界表是 description 反例对子的设计依据。boundary heuristic 那一句是边界的「速判公式」，要能判边缘场景，不能是「看情况」这种废话。

### § 5 触发与反例

Recognition cues 写**症状**（agent / 维护者会怎么描述这个处境），不写 workflow。Counter-examples 是边界的反面，每条要指明「该去哪」。Pre-flight gate 是硬门——任一不满足就 STOP 并把缺口反馈上游，不是「尽量满足」。这一段是 description 触发逻辑的设计来源：先在设计宪法想清触发/反例，再凝练成 frontmatter 的 `Use when / Do NOT use` 对子。
...
| **`.design/DESIGN.md`**（分发 skill）/ **`DESIGN.md`**（dev-only skill） | 是什么 / 为什么 / 边界 / 演化 | 改这 skill 的维护者 | **本 skill（curating）的设计宪法模板** |
| **`.design/OBJECTIVE.md`**（分发 skill）/ **`OBJECTIVE.md`**（dev-only skill） | 成功 = 什么（可度量契约） | 跑 eval 的人 / 迭代者 | `grounding-skill-evals`（J 怎么声明） |
