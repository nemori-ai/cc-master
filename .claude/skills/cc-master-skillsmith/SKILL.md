...
### description 是路由器，不是摘要

`description` 决定 agent 什么时候会读这个 skill。它必须保留**何时用 / Triggers / Do NOT use /
职责边界**这些路由信息，且中文为主。不要把它改成英文短摘要，也不要因为某个 host adapter
是 `stub` 或 `partial` 就写成维护者说明（例如“Codex adapter stub for X”）。

adapter `partial/` 的 description 要从 canonical description 做语义 delta：保留触发条件和排除
条件，只替换 host-specific 能力边界。adapter `stub/` 也不是空壳摘要：它要告诉 agent 何时触发
这个 stub、正在阻止误用哪套 host 机制、缺哪类能力才能解除 stub。
...
动 delta 之前先做一遍静态自查。机械子集（frontmatter 引号 / 必备字段 / description 路由密度 /
adapter payload 唯一性 / 死链）已由
...
- **`references/body-content-types.md`** — body 只装的 4 类内容（触发 / 命名锚 /
  ...
  reference / 一层深 / no orphan）+ **分发 skill 的受众 / 自包含两条额外纪律**
  （写给用户 agent 的第二人称 prompt·无维护者自留物·无代号·repo-无关 self-contain；
  dev meta-skill 豁免）。
...
- **`.claude/skills/grounding-skill-evals/references/track-ab-discipline.md` §7** — 演化频率
  ...
  「改多快 + 怎么回退」。Skill knowledge 改图协议在 `design_docs/skill-knowledge-graph/`，不归本 skill。
