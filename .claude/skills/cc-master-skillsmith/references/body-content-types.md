...
- [分发 skill 的两条额外纪律（受众 + 自包含）](#分发-skill-的两条额外纪律受众--自包含)
- [4 层写作分配表](#4-层写作分配表)
...
> cc-master 特例：SKILL A（`master-orchestrator-guide`）由 SessionStart hook
...
## 分发 skill 的两条额外纪律（受众 + 自包含）

上面 4 类管「装什么」；这一节管**分发 skill**（住 `skills/`、随插件 ship 给用户
agent 的那些）额外要守的「怎么写」。**只约束分发 skill**——dev meta-skill（住
`.claude/skills/`，如本 skill 三件套 + `requirement-elicitation`）不分发给终端用户，
这两条不约束它们**自身**（它们的 body / reference 可以引用 repo、可以带维护者视角、
可以用仓内代号）。判据一句话：你写的这个 skill 会不会随插件装到用户机器上？会 →
两条都要守；不会（dev 工具）→ 豁免。**注意别把这两条误伤成「meta-skill 教你怎么写
分发 skill 的那段指导也要 self-contain」**——受约束的是**产出的分发 skill**，不是教
学用的 dev reference 本身。

- **受众纪律**——分发 skill 的 SKILL.md + references 是**写给正在编排自己目标的
  用户 agent 的 prompt**：第二人称、imperative、agent-as-actor。**不夹带写作者 /
  维护者的自留物**——文档自述（「本文回答…」「本节展开…」）、投递机制旁白（「这段每次
  compaction 全文重注」）、设计理由旁白（「之所以这么设计是因为…」）、给评审的 meta
  注、纯装饰性自述 TOC。这些是维护者视角，属于分发 skill 的 `.design/DESIGN.md`（或 dev-only skill 的 `DESIGN.md`）/ 提交说明 / 本类 dev
  reference，不属于用户 agent 干活时读的运行时 prose。（长 reference 顶部的
  `## Contents` 锚点 TOC 是 agent 扫描用的 skip map，属功能不属自述，保留。）

- **自包含纪律**——分发 skill 服务用户 agent、**不服务 cc-master 开发者**，所以要
  **repo-无关 self-contain** 且 **无代号**：
  - **不引用 `skills/` 外的仓库文件**——`design_docs/` / `adrs/` / `hooks/` /
    `README` / `AGENTS.md` / `CHANGELOG` 不随插件分发，装机后是死链。跨文件引用只指向
    随插件 ship 的 `skills/` 内路径，且用 `${CLAUDE_PLUGIN_ROOT}` /
    `${CLAUDE_SKILL_DIR}` 绝对引用（裸相对路径会相对用户 cwd 解析、找不到）。
  - **无代号**——别用 `ADR-016` / `Finding #38` / charter `C3` / hook 内部编号 /
    「SKILL A」/「镜头 7」这类**仓库内部代号**去压缩一个概念。用户 agent 没有这套
    代号表，代号对它就是空指针。概念要么用它的**名字**讲——这正是「命名锚」的用法：
    `指挥不演奏` 是名字（可留），`SKILL A` 是代号（要展开成它的名字或换 `skills/`
    内路径引用）——要么直接引一条 `skills/` 内的文件路径。

这两条与 4 类正交：4 类管内容归哪一类，这两条管**同一段内容的措辞与自包含**。仓级
SSOT 与机械卡点（`skill-lint.sh` / `plugin validate` 的 grep 门）在 AGENTS.md
§6 / §12。
...
**若这是分发 skill（住 `skills/`）：SKILL.md body + 每个 reference 再各过一遍受众 +
自包含。** 逐段问：措辞是不是写给用户 agent 的第二人称 prompt（没夹维护者自述 / 投递
机制 / 设计理由旁白 / 给评审的 meta 注）？有没有引用 `skills/` 外的 repo 文件、或用
`ADR-NNN` / `Finding #NN` / 「SKILL 字母」/ 「镜头 N」这类代号压缩概念？命中就改——
维护者视角挪去分发 skill 的 `.design/DESIGN.md`（或 dev-only skill 的 `DESIGN.md`），代号展开成名字或换成 `skills/` 内路径引用。dev meta-skill
不受这两条约束。
