...
成熟的评测 / 优化方法论有一整套重型机器：评测侧的多字段机器可读裁决 schema、自动 holdout-gap 检测 / 偏置缓解套件、多维 rubric；优化侧的带自动搜索 + 多候选 filter 的优化引擎、多维优化前沿、统计显著性 / 方差分析预检、自动化的 predicted-delta 契约。**本仓小 portfolio 体量撑不起也不需要那套引擎。**
