...
J（objective function）= 这个 skill「成功 = 什么」的机器可读答案。分发 skill 落在 `skills/<name>/.design/OBJECTIVE.md`；dev-only skill（`.claude/skills/<name>/`）可保留根部 `OBJECTIVE.md`。**本仓的 J 是轻量的**：一句话 `J_top` + 一段 `baseline_reference`（没这 skill 的 agent 会怎样退化）+ 1-2 个不可回退的 `strict_dims`。就这么多——不要膨胀成重型 J 不变量集那种 schema。
...
| 你要做 | 用什么 | 去读 |
|---|---|---|
| 给一个 skill 声明 / 修 J | 写 `.design/OBJECTIVE.md` 或 dev-only 根部 `OBJECTIVE.md`（J_top + baseline_reference + strict_dims） | [`references/objective-function.md`](references/objective-function.md) |
...
- **`references/objective-function.md`**——轻量 J 写法 + `.design/OBJECTIVE.md` / `OBJECTIVE.md` 的 schema（§5.4）+ `J_top` / `baseline_reference` / `strict_dims` 怎么填的判据与正反例。
