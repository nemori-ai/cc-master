...
> **`scripts/` 不进 `hooks/`（红线 1，ship-anywhere）：** 这三个脚本都依赖 `uv` + Python 3.12 + `claude`/`codex` CLI，**只许进 `scripts/`（带外手动 / 编排调用），绝不进 `hooks/`**（hook 必须纯 bash，无 runtime，在 Bedrock/Vertex/Foundry 都能跑）。将来本 skill 若要补新工具脚本，同样只进 `scripts/`。
...
- **`references/track-ab-discipline.md`**——怎么跑现有三脚本（eval-trigger / eval-benchmark / codex-review）+ 怎么读判决 + generator≠judge（codex 当第二评委）。
