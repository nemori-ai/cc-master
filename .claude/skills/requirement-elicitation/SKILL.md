...
- **编排层面的事**（怎么把已批准的 goal 拆图、派发、驱动到完成）→ `master-orchestrator-guide`；其中 workflow 脚本怎么写 → `authoring-workflows`。
...
> 这一节只点边界，**不复述**编排的 how——「前台对话 ∥ 后台执行」的调度机制、async-HITL、p95 hedging 全在 `master-orchestrator-guide`（镜头 7 + `references/async-hitl.md`）。本 skill 管「挖什么需求、为什么」，那个 skill 管「怎么把对话当 async worker 调度」。
