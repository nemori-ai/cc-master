...
| [`walkthrough.md`](walkthrough.md) | 叙事化走一遍完整闭环（webview 富卡 → 复制命令 → `/cc-master:discuss D1` → freshness-check → 写 `.decision.md` → master recon 消化），每步标明预期现象 + 对应 smoke 断言。 |
...
- **STEP 4** —— 模拟讨论结束：写结构合法的 `D1.decision.md` sidecar，且断言 board 逐字节未变（discuss 不碰 board，单写者纪律）。
- **STEP 5** —— master 消化：recon 拾取 sidecar → 纯 node 解析 → 选定 option id 属于 board 节点 `decision_package.options`（证明 master 用得上）。
