...
| 文件 | 作用 |
|---|---|
| [`fixture.board.json`](fixture.board.json) | 一块合法的 `cc-master/v1` board，含一个 `status:"blocked", blocked_on:"user"` 决策节点 `D1`，挂一份完整合法的 `decision_package`（`ask_type:"decision"`、3 个 options）。能被 `view-server.js` 加载、被 `view.html` 的 `DecisionCard` 渲染成富决策卡。字段严格对齐 [`board.md` §decision_package](../../skills/master-orchestrator-guide/references/board.md) 契约。 |
