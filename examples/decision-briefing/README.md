...
| 文件 | 作用 |
|---|---|
| [`fixture.board.json`](fixture.board.json) | 一块合法的 `cc-master/v1` board，含一个 `status:"blocked", blocked_on:"user"` 决策节点 `D1`，挂一份完整合法的 `decision_package`（`ask_type:"decision"`、3 个 options）。能被 `ccm web-viewer open --board ...` 打开并渲染成富决策卡。字段严格对齐 [`board.md` §decision_package](../../skills/master-orchestrator-guide/references/board.md) 契约。 |
...
- **STEP 4b** —— webview 历史区：smoke 仍起 legacy viewer payload、`node` HTTP GET `/decisions.json`，断言返回 D1 的 2 条（round 顺序对、`tldr` 抽取对、`node_id` 对）——卡片「💬 已讨论 N 次」据此渲染。正式人工入口是 `ccm web-viewer open`。
- **STEP 5** —— master 消化：recon 拾取**最新**那份（round 2）→ 纯 node 解析 → 选定 option id 属于 board 节点 `decision_package.options`（证明 master 用得上）。

相关产物：[`commands/discuss.md`](../../commands/discuss.md)（discuss 命令）· [`board.md`](../../skills/master-orchestrator-guide/references/board.md)（字段契约 SSOT）· [`async-hitl.md`](../../skills/master-orchestrator-guide/references/async-hitl.md)（准备 / 消化纪律）· `view.html` 的 `DecisionCard`（富卡渲染）。
