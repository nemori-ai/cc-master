...
这一步是整个设计的**入口取舍**：拒绝「按钮直接拉起 session」（要 webview 长 POST 端点，撞红线 1/2/5），改用纯客户端剪贴板 + 复制命令——零红线代价，`view-server.js` 零改动。

> **smoke 对应**：STEP 1 末尾断言 `enter_cmd` 含 `/cc-master:discuss D1`——即用户点「复制命令」会拿到的那条。富卡的视觉渲染本身靠这一步人验（webview 在浏览器里）。
...
谈完，它把结论整理成一份**决策文档 sidecar**，写到 **`<board-home>/<board-stem>--D1.decision.md`**（`<board-stem>` = board 文件名去 `.board.json` 后缀——带 board-stem 防共享 home 下多板撞名 / 误消化）：
...
> **smoke 对应**：STEP 4 用脚本生成一份**结构合法**的 `<board-stem>--D1.decision.md`（frontmatter 4 字段 + 四个 `##` 段）模拟那场讨论的产物，并断言 **board 文件与 fixture 逐字节相同**——硬证 discuss **没碰 board**（单写者纪律不破）。

---

## D. master 消化 —— recon 拾取 → 解析 → 用上这份数据 → replan

master 在下一次 recon / idle（决策程序 step 1）扫 awaiting-user 节点，发现 board home 同目录有 `<board-stem>--D1.decision.md`，按 [`async-hitl.md`](../../skills/orchestrating-to-completion/references/async-hitl.md) §消化纪律：
...
> **smoke 对应**：STEP 5 模拟 recon 拾取——纯 node 解析 sidecar（禁 jq/python），断言：frontmatter `node_id` 对得上、四字段齐、`inputs_hash_at_decision` 与决策包内一致、四个 `##` 段都在、能解析出非空 **TL;DR**，且**选定 option id（`opt-sidecar-index`）属于 board 节点 `decision_package.options` 之一**——这最后一条是「master 真能用上这份数据」的硬证：解析出来的结论不是悬空字符串，而是能接回 board 上那张选项表、据此 replan 的可执行决策。
>
...
| **模拟讨论结束** | STEP 4 | discuss 写结构合法 sidecar、**不碰 board**（单写者） |
| **master 消化** | STEP 5 | recon 拾取 → 解析 → 选定 option 属于 board options（用得上） |
...
| R5 ship-anywhere | 复制命令是客户端 clipboard 非 POST；无新运行时依赖；零联网 |
