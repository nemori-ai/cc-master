...
- **Bulk discuss** — Claude Code: `/cc-master:bulk-discuss`; Cursor: `/bulk-discuss`; Codex: `$cc-master-bulk-discuss`; kimi-code: `cc-master:bulk-discuss`. Walks *every* decision waiting on you in one sitting, inside the orchestrator's own session: each one explained in plain language (no bare node ids), with option cards to pick from. Your answers are collected, summarised for a final look, then reconciled into the board in a single pass. `--list-only` just shows how many are queued.
