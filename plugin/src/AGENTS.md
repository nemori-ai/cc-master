...
content-summary: |
  cc-master plugin source tree 入口。定义 paragoge-style source-of-truth：skills 使用 SAP，hooks 使用 PHIP，skill knowledge graph（K1 pilot）使用独立 authored source root，dist 只由 sync 生成。
---
...
| `knowledge/` | Skill knowledge graph 的 authored maintainer source root；当前 **K1 pilot** 已落真实 inventory（**3 modules / 9 points**），不是 runtime skill、也不进 `plugin/dist` |
...
- 修改 `knowledge/` 前先读取 `node scripts/skill-knowledge.mjs contract --json` 的 capability registry；K1 pilot 已实现 `change` / `check` / `contract` / `explain` / `path` / `report`；`compile`、`check --host` 或 `check --base`、`report --host` 仍 exit 10。四 host fixture probe 已交付，**不等于** CLI host integration。不要复制 `design_docs/skill-knowledge-graph/examples/` 来伪装真实 inventory。
