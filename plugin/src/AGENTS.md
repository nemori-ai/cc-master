...
| `knowledge/` | Skill knowledge graph 的 authored maintainer source root；K2 已落 8 runtime skills / full portfolio inventory，K3-00 起 `dev-as-ml-loop` 走 graph-first composition；并由 `compile` / `materialize` 投影到 `plugin/dist/<host>/knowledge/` |
...
- 修改 `knowledge/` 前先读取 `node scripts/skill-knowledge.mjs contract --json` 的 capability registry；K2 已实现 `change` / `check` / `compile` / `contract` / `explain` / `path` / `report`，K3-00 另实现 `materialize`（graph-first composition）。四 host fixture probe 已交付，**不等于** CLI host integration。`check --host|--base` / `report --host` 仍 exit 10。不要复制 `design_docs/skill-knowledge-graph/examples/` 来伪装真实 inventory。
