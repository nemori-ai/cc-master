...
> **落地**:每个纵切片 = 一个带 `--accept`(自己的 DoD)的 `ccm task`,片之间的真实数据依赖用 `--deps` 连。横切那种"一整层"节点往往**给不出自己的 DoD**——难端点验收,这本身就是切错的信号。
...
`motivation_conflict`（主体：行为约束） —— 删掉后 agent 会倾向于按技术层横切目标(先数据模型、再 API、再 UI),因为这“感觉”更严谨,实际上把并行度锁到 1、把可用价值推到最后。

主体是「必须纵切、不许横切」的硬规则，横切是感觉更严谨的默认近路，动机完美者知道规则就会照做。

## 为什么它随模型变强而更重要

模型越强,越能给横切编出技术上真实、听起来专业的理由(具体的返工成本、迁移路径、技术债分析)——这些理由不是编造的谎言而是货真价实的工程考量,只是被错误地当成了跳过纵切的理由,论证质量本身让人更难识破。

## 失败形态

任务列表拆得很细、每个任务都带 --accept,但其中一个“大”任务本质是“把 schema 定完整”——形式上符合“每片有 DoD”的要求,实质上那个 DoD 是“整层完工”而非“一个用户能摸到的能力”,纵切已经名存实亡。
