...
2. **用户澄清**——一次前台问题，可提前备好（prefetch）、与后台并行。
3. **真实端点 dogfood / 可逆有限实验**——切一个 walking-skeleton 薄片真跑一遍接触现实，胜过纸上推演。
4. **异质 reviewer**——换一个模型家族的第二视角。**注意：它抓契约 / 同族盲区，不算真实外部证据**——只是阶梯里偏内部的一档，顶替不了 3 / 5 / 6。用「多找几个 agent 内部 review」冒充外部验证，正是本纪律要防的那种自欺。
...
`capability_gap`（双重性质·方法部分补不回来，它才是承重结构） —— agent 知道该找外部证据校准假设，但赶进度时会用『多拉一个 agent 来 review』这类成本低、看着像验证的动作替代真正的外部证据，并记成『已核实』——这正是原文点名要防的自欺。

主体是 falsification-first 的方法与「三条全中才校准」判据加成本阶梯选法，缺了就不会挑该验哪个假设、用哪档手段。

## 边界

本点只管『什么时候该校准、该用哪一档手段落定』这套判据和成本阶梯，不管每一档手段本身具体怎么执行（怎么切薄片验证、怎么问用户、怎么读文档），也不管三条触发闸各自的定义细节。

## 失败形态

最隐蔽的违反是流程看着走全了：承重假设列了、还真找了个第二视角 review 了一遍，报告写得像做过外部验证——但用来『验证』的始终是同族模型的内部推断，从未真的接触代码之外的现实，假设其实还是没被真正证伪过。
