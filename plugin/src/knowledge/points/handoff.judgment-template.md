...
`motivation_conflict`（主体：行为约束） —— 删掉后,agent 在切 session 前(累、context 快耗尽、怕丢掉攒了一身的 mental context)会把“为稳妥起见多写点”包装成负责任,把已有的结构化状态原样倒一遍写进交接文档,制造一份会过期的第二真相。

主体是「绝不把 board 已装下的内容复抄进 handoff」，累了直接 dump board 明显更省力，大段文字都在拆这个自我说服。

## 边界

只有当接手方客观上读不到那份结构化状态(不同工具链、状态本身损坏或不可达)时,才需要在文档里补状态而非只给指针;只要接手方能正常读到,复述就没有正当理由。

## 为什么它随模型变强而更重要

模型越强,写出来的状态转写就越流畅、越像“有内容的分析”,而不是干巴巴的字段罗列——恰恰是这份文笔,把原样倒一遍伪装成了言之有物的叙事,让写者和读者都更难分辨这段话是补充判断,还是又一份会过期的真相。

## 失败形态

六段模板的标题结构都对、看起来完全合规,但“关键判断”或“下一步”段落里,内容其实还是把某个任务的状态/依赖用完整句子转述了一遍——形式上在正确位置,实质上仍是既有结构化状态的复述,只是没有被单独列成一段清单而已。
