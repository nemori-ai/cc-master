...
`motivation_conflict`（主体：行为约束） —— 删掉后 agent 在缺外部验证通道时,要么直接断言假设成立继续往下做(编造信心),要么无限期挂起等用户上线(永久停摆),两种都比五步协议更省事、更容易被自己接受。

主体是无通道时「绝不编造已验、绝不无限阻塞、绝不静默改下游」，诚实记未知并走 amendment 明显比顺手声称没问题更费力。

## 边界

只适用于“承重判断”且当下确实无外部通道可查证的情形;一旦有低成本的外部核验手段(哪怕只是一次探测请求),就该直接用,不套用这套“记未知+可逆推进”的流程。

## 为什么它随模型变强而更重要

模型越强,越能为一个未验证的假设编出一段技术上站得住脚的论证(引用真实的工程道理、给出貌似合理的置信区间),这种论证足够精细、足以说服审阅者甚至自己,而不是单纯因为强模型产出更多假设。

## 失败形态

agent 确实按格式写了一条记录标注假设,措辞却已经带上“经评估应可行”这类定性结论,而不是老实标注“未验证”——形式上履行了记录未知,实质上已经把编造的信心塞了进去。
