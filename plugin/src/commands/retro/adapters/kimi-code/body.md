...
- **默认目录**（未传 `--out` 时）：
  - 若项目根下**已存在** `design_docs/` 目录——落到 `<项目根>/design_docscc-master:retros/`。
...
3. **调度与估算质量**——WIP 峰值 vs `scheduling.wip_limit`、关键路径实际耗时 vs 若有 `estimate` 字段的对比、log 里是否出现过 `usage advise` 的 throttle/switchcc-master:stop 类 verdict 记录、是否有 hedge/重派的实例。
