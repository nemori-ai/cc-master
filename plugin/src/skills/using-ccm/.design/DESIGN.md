...
- 用 ccm `account` 操作号池：录号 / 删号 / 列号 / 无重启换号（命令操作面 + `account-pool.md` 概念叙事——实现 SSOT 在 ccm 引擎，决策归 A）。
...
| 不归本 skill | 归谁 |
|---|---|
| 该编排什么 / 怎么拆 DAG / 何时阻塞等用户 / 何时换号（**决策**） | `master-orchestrator-guide`（A） |
| board 协议 **canonical SSOT**（enums / status enum / 字段 tier / 不变式 / 状态机） | `@ccm/engine` 的 board-model（A 的 board.md 是**派生叙事**、非 SSOT） |
| workflow 脚本怎么写（脚本 DSL，非 ccm CLI） | `authoring-workflows`（B） |
| 号池 / 选号 / 切号 / vault 的**算法与实现 SSOT** | ccm 引擎 `@ccm/engine/account`（D 只给操作面 + `account-pool.md` 概念叙事） |
| 换号**决策**（何时换 / 谁拍板 / 绝不自授权）+ pacing 配速估算消费 | `master-orchestrator-guide`（A·决策）· `pacing-and-estimation`（消费） |
...
- **状态机转移表 / 三档字段 / lint 规则在 `@ccm/engine` 改了** → 同步心智锚 2/3 + `references/board-model-guide.md` 的**操作视图与校验规则全集速查**（**canonical SSOT 在 `@ccm/engine` board-model**；A 的 board.md 是派生叙事，本 skill 只跟操作视图、不复述协议）。
