...
{{CODEX_REVIEW_SCRIPT_GUIDANCE}}

**跨模型族二审的收益不对称，判断"该多认真对待这次 verdict"时把这个考虑进去**——用更强模型审弱模型的产出收益最大；反过来让明显更弱的模型审明显更强模型的产出，不但收益薄，弱 reviewer 的"纠错"建议还可能把原本正确的产物改错。所以：
- 产出方模型档位 ≤ codex 侧默认档位、或两者相当时，认真对待 `needs-attention`，按下面四档正常核对。
- 产出方模型档位明显强于 codex 侧默认档位时，`needs-attention` 判定的可信先验要下调——它更可能落进下面第④档（noise），核对时多留一分怀疑，但**仍要逐条核对、不能因为"档位差"就整体跳过**。
- 这条只调整"核对时带几分怀疑"，**绝不动摇 silent-pass-through 守卫**——空 review / 调用失败依旧一律未通过，无论两端模型档位如何。

**一条 finding 的说服力不是它的正确性证据。** 一段写得斩钉截铁、条理分明的 `needs-attention` 描述，和一段潦草的描述，在"这条 finding 是否属实"这件事上权重相同——你核对时只认产物上可复核的事实（这一行代码到底做了什么、这个契约到底怎么写的），不因为 reviewer 措辞坚决就少查一步、也不因为措辞含糊就直接判噪声。
