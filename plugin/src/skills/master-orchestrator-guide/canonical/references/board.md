...
>
> **本文是 orchestrator 侧的派生叙事视图，不是协议 SSOT。** board 协议的 canonical SSOT（narrow-waist schema / status enum / 字段三档 🔒👁✎ / `parent` 不变式 / 全部 FMT·GRAPH·BIZ 校验规则 / 状态机）归 **ccm 引擎的 board-model**；它的**操作视图**（每个字段何时填什么、撞哪条规则、一次写对不撞 exit 3）在 {{USING_CCM_BOARD_MODEL_POINTER}}。**本文不复述 schema / enum / 字段三档 / 校验规则的细节**——要查字段取值或规则去 `using-ccm`，本文只讲 orchestrator 特有的长程操作纪律。
...
> **lint 引擎 SSOT = ccm 引擎，规则全集速查在 `using-ccm`**。下面几道自检的规则逻辑都不在 plugin 里——经**进程边界** `spawn ccm board lint` 取裁决。**全部 FMT / GRAPH / BIZ 规则逐条速查在 {{USING_CCM_BOARD_MODEL_POINTER}}**——本文不复述规则清单。
