...
- 读就绪：`ccm next`；查图 / 临界路径：`ccm board`；append-only 记账：`ccm log add`；自驱决策记录：`ccm jc`；节奏 iteration 收口：`ccm cadence`。
...
- executor 字段怎么写进 board（各值必填字段、选择决策树）见 {{USING_CCM_BOARD_MODEL_POINTER}}。
