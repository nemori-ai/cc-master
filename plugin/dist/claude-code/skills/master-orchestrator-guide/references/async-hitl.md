...
1. **按 `job_id` CronDelete 清掉待发 job**（若机制是 CronCreate 这类会重复 fire 的），免得它在已无事可 reconcile 时反复把你叫醒（白烧拍）；
...
工具降级链（CronCreate → ScheduleWakeup → Monitor → background-shell `until` 兜底）+ 各自的可用性 / cache-warmth 心智；每档先拿真实 handle 再 arm在 `dispatch.md` §派发卫生（watchdog/liveness 维度）。
