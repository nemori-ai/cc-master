...
> **一次 drill 的闭环：** 你只是在回答或写 routing record、且本页八段已经给齐所需判断时，就在本页闭环并停止下钻。末尾 owner 地图是缺信息时的深挖索引，不是读完本页后的续读清单；不得仅因场景出现 fan-out / join、workflow、runtime terminal 或端点验收字样就再开 reference。只有 routing record 之外确实还要计算 lane / liveness、读取动态 target 事实、执行精确 `ccm` 命令，或实际运行端点验收步骤时，才继续 drill，并记录本页缺的是哪项证据。
...
从全机 inventory 中选精确 `harness + surface`，先确认它确实可调用、能返回可 recon 的 handle、对目标 workspace 有所需权限，再比较任务适配度与容量。此 host 当前能用于发车或追踪的机制包括：sub-agent（经 `run_in_background`）/ Workflow 工具 / 后台 shell（`until` 轮询）。目标 CLI 的真实调用形状只看本次解析出的 [using-ccm worker help](${CLAUDE_PLUGIN_ROOT}/skills/using-ccm/references/command-catalog.md#worker-help)；不要凭记忆复制 provider flags，也不要把 `ccm` 当成 model / effort 参数翻译层。
...
统一模型事实与证据分层从 `pacing-and-estimation` 的模型事实页读取；selected-target 的 surface / model / quota / binding 解释只按 [pacing-and-estimation 目标事实口径](${CLAUDE_PLUGIN_ROOT}/skills/pacing-and-estimation/references/cross-harness-target-facts.md)。这些页面拥有动态事实，你不要在本页或 board 复制 provider 型号、窗口、价格与 quota catalog。精确查询和写入语法查 `using-ccm`（`${CLAUDE_PLUGIN_ROOT}/skills/using-ccm/references/command-catalog.md`）。
...
精确 command / field / status verb 只查 `using-ccm`（`${CLAUDE_PLUGIN_ROOT}/skills/using-ccm/references/command-catalog.md`），不要从本文复制一套命令表。派后立即在 routing record 留下 agent、task、attempt 的关联与 handle provenance；三者可关联，不能合并成一个状态。
...
| 动态 provider / model / quota 事实、证据层级、freshness 与 selected-target binding | `pacing-and-estimation`；入口为 [pacing-and-estimation 目标事实口径](${CLAUDE_PLUGIN_ROOT}/skills/pacing-and-estimation/references/cross-harness-target-facts.md) | 读取或解释当前事实时；不要把 catalog 抄回这里 |
| `ccm` flags、JSON、board 字段与生命周期 verb | `using-ccm`（`${CLAUDE_PLUGIN_ROOT}/skills/using-ccm/references/command-catalog.md`） | 真正敲命令或写 board 时 |
