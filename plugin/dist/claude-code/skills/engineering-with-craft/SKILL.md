...
- [deepens_to: 不变式即锚](./SKILL.md#ccm-k-point-craft-invariant-anchor) <!-- ccm:k:edge edge:craft.foundation.shared-spine-to-invariant-anchor -->
...
它跑在三个尺度上：①**顶层敏捷**（目标纵切薄增量 / walking skeleton / 按 cadence 交付·见 slicing-goals-into-dags）；②**片内有序**（每薄片端到端走 SDD→DDD→OOP→TDD 四棒）；③**任务内迭代**（片里每个 dev 任务按 propose→measure→adjust 逼近验收·见 dev-as-ml-loop）。dev-as-ml-loop 的迭代循环在每一棒**内部**跑，与接力次序不同尺度、不打架。
