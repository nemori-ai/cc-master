...
| `watchdog` | board 顶层 | `ccm watchdog arm / disarm` | Stop hook 会在有 `in_flight` 任务却无健康 watchdog 时提醒你 arm；只有带 nonblank `job_id` 且未过期的 canonical `watchdog` / legacy `wakeup` 才会静默这条提醒。 |
...
| `job_id` | **必填 nonblank string**；真实外部调度句柄（如 CronCreate 返回的 id）；用于追踪、recon 与退役，不能省略或填空白。没有真实 handle 就不要 arm |
...
| `FMT-POLICY` | warn | `policy` 非对象，或 `autonomous_account_switch` 不在 `{allow, deny}` | 用 `ccm policy set --autonomous-account-switch=allow\|deny`；写入须有真实用户授权 |
