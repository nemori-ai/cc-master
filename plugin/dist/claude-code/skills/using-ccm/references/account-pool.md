...
> **何时读：** 用户直接让你做账号操作（录号 / 续期 / 切号 / 查池），你要懂 `ccm account` 这套号池**为什么这么设计**——号池模型、录号机制 why、选号判据、vault 安全局限。
> **这是概念叙事（why），不是实现。** 命令**怎么敲**见 `command-catalog.md` 的 namespace account；**算法 / vault / 切号的实现** 在 ccm 引擎里（本文不复述实现代码）。
> **换哪份配额不是编排器的决策。** 当前用哪个号由后台容量管控层在整机范围统一决定；编排侧在配额边界上该守什么，见 `${CLAUDE_PLUGIN_ROOT}/skills/master-orchestrator-guide/references/cost-decisions.md`。本页讲的是「用户让你动这套东西时，你该懂的那条 why」。
...
选号是**引擎的机械动作**，不是谁的判断题：需要切号时，引擎从所有**非 active 且 token 未过期**的号中挑一个**预计可用配额最优**的切入。判据（权重 / 阈值是引擎可 env 覆写的常量·实现在 ccm 引擎的选号逻辑、本文只蒸馏 why）：
...
- **临到期降权 + 边界**：token 临近到期 → 降权（不归零·还能用、只是该续期）；无历史新号视作满血最优先；全池无双窗口健康号（每个都 5h 或 7d 逼顶）→ `NONE_ALL_EXHAUSTED`，**没有号可切、也不空切**——这时唯一正确的结果是停下来让用户知道全池已到边界（全池 5h 墙 / 7d 健康时同样是 stop）。
...
> **这是纵深防御的安全网、不是硬锁。** agent 有 shell，理论上能绕过；价值在「让擅自换号从一句合理化变成要主动绕闸、且每次都在 log 留痕」。**写这个开关的权属用户**：`ccm policy set` 非 TTY 必须带 `--user-authorized`，而那个标记只能由用户给——agent 自己签发就是 self-grant 越权。缺省是 `allow` 也不构成「所以可以自己切」的许可：它放行的是后台机制，不是 agent 的自主动作。容量边界上编排侧该守什么，见 `${CLAUDE_PLUGIN_ROOT}/skills/master-orchestrator-guide/references/cost-decisions.md`。
...
- [Knowledge atlas](../../master-orchestrator-guide/SKILL.md#ccm-k-skill-master-orchestrator-guide)
- [Module module:ccm.account-pool](./account-pool.md#ccm-k-module-ccm-account-pool)
- [contrasts_with: 换号不在你的 lever 清单里](../../master-orchestrator-guide/references/cost-decisions.md#ccm-k-point-capacity-switch-is-not-your-lever) <!-- ccm:k:edge edge:ccm-account-vault-not-switch-decision -->
