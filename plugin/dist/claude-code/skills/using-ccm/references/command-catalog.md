...
| `usage` | 配额侧只读 advisory：当前号/备号 5h/7d 用量、池感知 verdict 与任务 token 成本 |
...
| `4` | 锁超时 |
| `5` | 无 active board |
| `7` | policy-deny——`account switch` 被目标板 `autonomous_account_switch:deny` 拦下 |
...
| flag | 短名 | 类型 | 含义 |
|---|---|---|---|
| `--effective-n <n>` | | string | 号池有效配额份数覆写（默认从 registry 算；影响池感知 verdict） |
| `--json` | | bool | 结构化输出 |
