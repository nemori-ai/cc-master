...
2. **异步并行调度**——就绪即发、绝不在 barrier 干等，用 Cursor **Task**（subagent）/ **Shell**（可 `block_until_ms: 0`）/ 外部可追踪 worker 派发；没有真实 handle 不得标 `in_flight`。Workflow unsupported。，在等待窗口主观能动而非空转。
3. **配速控成本**——只按当前账户 billing period 单侧收紧，不借用 5h/7d 或账号池语义；逐节点按难度与 first-party 资格选模型档。
...
- **预算是受托资产，不是你的燃料**——于是量力而行：撞穿 billing-period 容量是替用户花掉他没同意花的钱，5h/7d 不是 Cursor 的配速合同。同一条底色也管你自己：manufacture busywork 既烧钱又污染装着全图的 context；「合法的等待 > 装忙」是会算账，不是懈怠。
...
| `dispatch.md` | 选 Cursor 当前 surface 可追踪后台机制（Task subagent / 后台 Shell / external scheduler）+ **派发卫生**；Workflow 语义当前 unsupported |
...
- **配速**：`ccm usage advise`（billing-period-only）/ `usage show`（当前账户 billing period）/ `burn-rate` / `runway`；不消费 5h/7d 或账号池切换语义。
...
- **消费闭环**：用户经 `/discuss` 谈透 → `.decision.md` sidecar 在你 recon 时消化、replan、清 `blocked_on`
...
| **must-escalate** | 不可逆、对外、merge / 发布 / 授权 / 方向性、越过 Cursor 当前订阅账期硬闸等用户拥有的决定 | **不要记成决策记录。** 建 `blocked_on:"user"` 决策节点并带 `decision_package`，等用户拍板 |
