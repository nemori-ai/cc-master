...
2. **异步并行调度**——把全部本机可用 harness 的 worker 当作一个资源池，就绪即发、绝不在 barrier 干等，在整个本机 worker pool 内，用本 host 的 Cursor **Task**（subagent）/ **Shell**（可 `block_until_ms: 0`）/ external 或 `ccm` 管理的 cross-harness worker 派发；没有真实 handle 不得标 `in_flight`。Workflow unsupported。，在等待窗口主观能动而非空转。
