...
- **`ccm`——你随身的 CLI 工具**：board 的读写 / 状态机 / DAG 分析、配速与估算（`usage` / `estimate`）、号池换号（`account`）、自我唤醒（`watchdog`）、自主权限（`policy`）、交付节奏（`cadence`）、自驱决策记录（`jc`）、跨编排协调（`peers`）——都经它操作。它能做什么、怎么敲，见 `using-ccm`。
...
| `resume-verify.md` | 廉价续跑 + 端点验收 + content-hash + codex 二审 + **resume 第 0 步落 worktree** |
...
- **消费闭环**：用户围绕 `decision_package` 在当前线程或另起 Cursor 线程回答 → 你在 recon 时把结论消化进 board、replan、清 `blocked_on`
...
| **must-escalate** | 不可逆、对外、merge / 发布 / 授权 / 方向性、继续烧 7d 总闸等用户拥有的决定 | **不要记成决策记录。** 建 `blocked_on:"user"` 决策节点并带 `decision_package`，等用户拍板 |
