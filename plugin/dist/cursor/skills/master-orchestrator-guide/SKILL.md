...
2. **异步并行调度**——把全部本机可用 harness 的 worker 当作一个资源池，就绪即发、绝不在 barrier 干等，在整个本机 worker pool 内，用本 host 的 Cursor **Task**（subagent）/ **Shell**（可 `block_until_ms: 0`）/ external 或 `ccm` 管理的 cross-harness worker 派发；没有真实 handle 不得标 `in_flight`，workflow planning / runtime 边界只按 `worker-routing.md`，在等待窗口主观能动而非空转。
...
| `dispatch.md` | 展开 Cursor 当前 surface 的可追踪后台机制、并行控制与**派发卫生**；workflow planning / runtime 边界只按 `worker-routing.md` |
