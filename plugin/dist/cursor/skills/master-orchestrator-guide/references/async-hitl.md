...
1. **清掉对应 wakeup handle**：若是后台 Shell，就停止/确认退出并记录 exit；若是外部 scheduler，就禁用 cron/systemd/CI job；若没有真实 handle，就不要写成已 armed；
...
后台 Shell → 外部 scheduler + 手动 recon → manual recon；没有 CronCreate/ScheduleWakeup 时不要伪造在 `dispatch.md` §派发卫生（watchdog/liveness 维度）。
