...
| `policy` | 兼容读取 board policy；Cursor account switch 永久不可用，stored allow 不产生能力 |
...
| `usage` | Cursor 当前订阅账期只读 advisory 与任务 token 成本；无账号池 / 自动换号 |
...
| `--session-id <id>` | | string | 指定 session（特权调用者注入；Cursor host adapter 负责传入当前 session id，缺失时按 board 发现降级） |
...
| `4` | 锁超时 |
| `5` | 无 active board |
| `7` | 授权拒绝——`dependency waive` 缺显式 `--user-authorized`；亦保留跨 host policy-deny 兼容码，Cursor 没有可执行的 account-switch 路线 |
...
## namespace policy（Cursor 兼容读取）

`ccm policy show --json` 可以诊断旧板保存的 `autonomous_account_switch`，但 Cursor adapter 的 account switch 永久不可用：stored `allow` 不创建 candidate、不授权 login/logout/credential mutation，也不改变 `effective_n=1`。不要在 Cursor 下把 policy 翻成 `allow`；若需收紧旧板，只能在用户明确授权下写 `deny`。
...
| flag | 短名 | 类型 | 取值 | 含义 |
|---|---|---|---|---|
| `--accounts <v>` | | enum | `current` | Cursor 只消费当前登录态；不要用 `all` 构造备号容量 |
...
| flag | 短名 | 类型 | 含义 |
|---|---|---|---|
| `--effective-n <n>` | | string | Cursor 固定按 `1` 解读；该 flag 不得产生换号候选或扩容 |
| `--json` | | bool | 结构化输出 |
...
- positional：无
- 行为：当前 Cursor 账户按 **billing_period（~30d）** 算一个配额%-burn-rate（Δused%/Δtime·账户权威·`window-elapsed` = 已用% ÷ 账期已逝小时·%/h）；读 Cursor dashboard `GetCurrentPeriodUsage`，信号不可得 → `available:false`（exit 0·诚实降级·非 exit 1）。`burn_pct_per_hour` 不可算 → `null`。不要期望 5h/7d 双窗 burn-rate 字段。
- flags：
...
`data.policy` 可能含历史 `autonomous_account_switch`，但 Cursor 的运行时 effective capability 始终是 no-switch。不要根据 stored `allow` 执行 account 命令。
...
```jsonc
{
  ...
  "allocation": {
    "pressure": { "headroom_pct": 15, "quota_model": "cursor-billing-period", "band": "warn" },
...
{ "available": true, "accounts_scope": "current", "effective_n": 1,
  "billing_period": { "used_percent": 5.5, "remaining_percent": 94.5, "resets_at": "2026-08-01T00:00:00Z" },
  "source": "cursor-dashboard" }
```

备号列表与 switch eligibility 不属于 Cursor 输出合同。
...
`hold` 静默；`throttle` 减速；`stop_billing_period` 进入账期硬停决策。Cursor 自动换号永久禁止。
