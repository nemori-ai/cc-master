...
| `task-id` | ① **in-session subagent（Task tool 派生·harness `origin`）**：value = subagent 的 agentId，**必须同时 `--transcript <父 session 转录绝对路径>`**——viewer 由「父转录 + agentId」派生 `subagents/agent-<agentId>.jsonl` 实时流（配方与反模式见 command-catalog 的 agent bind 节；错登成 `session-id:<agentId>` 或漏 transcript → stream 恒为无源）② 其他以 task 粒度跟踪、只有 transcript 可查的派发 | `--transcript` 记 transcript 路径引用（绝不内嵌内容） |
