...
**为什么 id/status/deps/parent 是 🔒？** 这四个字段被 hook、图算法、readySet、lint 机器读取。手改绕过写关卡，会造成悬挂引用、环、lint 拦不住的非法态转移——后果是 board 说谎，所有下游消费者（viewer / resume / hook）沿着错误输入跑。**这正是为什么 board 变更根本不给手改的路**：直接 file-edit 目标 board（编辑工具或 shell 重定向/文本替换手改）会被 Cursor PreToolUse board-guard hook **当场 deny**。手改绕过写关卡会静默腐蚀 deps 图 / 状态机 / 窄腰——机制层直接不给你这条路。
...
| `observability` | 后台任务完成 / recon 时，从 Cursor subagent result、后台 Shell session、cloud task 或外部 run 的可得 telemetry 抄取 | 可选遥测；缺失优雅降级，不影响派发逻辑 |
...
| `watchdog` | board 顶层 | `ccm watchdog arm / disarm` | Cursor hook 可以在 Stop / SessionStart 等事件里注入提醒；只有带 nonblank `job_id` 且未过期的 canonical `watchdog` / legacy `wakeup` 才算健康并静默提醒。hook 不能凭空创建未来唤醒：不要把提醒当成 durable timer，它只能在 Cursor 再次触发相关事件时发声。 |
...
- [next: status 八态语义与转移判断](./board-model-guide.md#ccm-k-point-ccm-board-status-semantics)
- [requires: 三档字段操作规则](../SKILL.md#ccm-k-point-ccm-field-tiers)
...
- [Knowledge atlas](../../../knowledge/atlas.md)
- [Module module:ccm.board-model.lifecycle](../../../knowledge/modules/ccm.board-model.lifecycle.md#ccm-k-module-ccm-board-model-lifecycle)
- [next: executor 五种语义与选择树](./board-model-guide.md#ccm-k-point-ccm-board-executor-choice)
- [requires: status 是状态机不是赋值字段](../SKILL.md#ccm-k-point-ccm-status-state-machine)
<!-- ccm:k:nav:end -->
...
- [Knowledge atlas](../../../knowledge/atlas.md)
- [Module module:ccm.board-model.lifecycle](../../../knowledge/modules/ccm.board-model.lifecycle.md#ccm-k-module-ccm-board-model-lifecycle)
- [next: acceptance 怎么写好](./board-model-guide.md#ccm-k-point-ccm-board-acceptance)
- [routes_to: status 八态语义与转移判断](./board-model-guide.md#ccm-k-point-ccm-board-status-semantics)
<!-- ccm:k:nav:end -->
...
- [next: agents［］ 登记簿语义](./board-model-guide.md#ccm-k-point-ccm-board-agents-registry)
- [routes_to: artifact/verified 真完成](./board-model-guide.md#ccm-k-point-ccm-board-artifact-verified)
- [requires: planning/routing 是 opt-in 合同](../SKILL.md#ccm-k-point-ccm-planning-opt-in)
- [routes_to: status 是状态机不是赋值字段](../SKILL.md#ccm-k-point-ccm-status-state-machine)
<!-- ccm:k:nav:end -->
...
- [Module module:ccm.board-model.contracts](../../../knowledge/modules/ccm.board-model.contracts.md#ccm-k-module-ccm-board-model-contracts)
- [routes_to: artifact/verified 真完成](./board-model-guide.md#ccm-k-point-ccm-board-artifact-verified)
- [deepens_to: jc 何时建与 severity](./board-model-guide.md#ccm-k-point-ccm-board-jc-judgment)
<!-- ccm:k:nav:end -->
...
- [Knowledge atlas](../../../knowledge/atlas.md)
- [Module module:ccm.board-model.lifecycle](../../../knowledge/modules/ccm.board-model.lifecycle.md#ccm-k-module-ccm-board-model-lifecycle)
- [next: estimate 怎么估](./board-model-guide.md#ccm-k-point-ccm-board-estimate-judgment)
- [routes_to: status 八态语义与转移判断](./board-model-guide.md#ccm-k-point-ccm-board-status-semantics)
<!-- ccm:k:nav:end -->
...
- [Knowledge atlas](../../../knowledge/atlas.md)
- [Module module:ccm.board-model.lifecycle](../../../knowledge/modules/ccm.board-model.lifecycle.md#ccm-k-module-ccm-board-model-lifecycle)
- [next: deps 怎么连](./board-model-guide.md#ccm-k-point-ccm-board-deps-linking)
- [routes_to: status 八态语义与转移判断](./board-model-guide.md#ccm-k-point-ccm-board-status-semantics)
<!-- ccm:k:nav:end -->
...
- [Knowledge atlas](../../../knowledge/atlas.md)
- [Module module:ccm.board-model.lifecycle](../../../knowledge/modules/ccm.board-model.lifecycle.md#ccm-k-module-ccm-board-model-lifecycle)
- [next: blocked_on 怎么选](./board-model-guide.md#ccm-k-point-ccm-board-blocked-on)
- [routes_to: status 八态语义与转移判断](./board-model-guide.md#ccm-k-point-ccm-board-status-semantics)
<!-- ccm:k:nav:end -->
...
`blocked_on: "user"` 的节点是给用户的「采访包」——用户点开 `/discuss` 时靠 `decision_package` 里的内容理解上下文、做决策。**没有包 = discuss 开不起来**，所以 lint 对这类节点做 `BIZ-AWAITING` hard 校验：
...
high / critical jc 在 Cursor Stop hook / recon 提醒回前台时会被显眼提示——用户不必主动去查，hook 会告知；缺 hook 时由你在 recon 中主动检查。
...
- [Module module:ccm.board-model.contracts](../../../knowledge/modules/ccm.board-model.contracts.md#ccm-k-module-ccm-board-model-contracts)
- [routes_to: artifact/verified 真完成](./board-model-guide.md#ccm-k-point-ccm-board-artifact-verified)
- [next: cadence/iteration 节奏](./board-model-guide.md#ccm-k-point-ccm-board-cadence)
<!-- ccm:k:nav:end -->
...
- [Module module:ccm.board-model.contracts](../../../knowledge/modules/ccm.board-model.contracts.md#ccm-k-module-ccm-board-model-contracts)
- [routes_to: artifact/verified 真完成](./board-model-guide.md#ccm-k-point-ccm-board-artifact-verified)
- [deepens_to: parent/owner 嵌套语义](./board-model-guide.md#ccm-k-point-ccm-board-parent-owner)
<!-- ccm:k:nav:end -->
...
- [Knowledge atlas](../../../knowledge/atlas.md)
- [Module module:ccm.board-model.contracts](../../../knowledge/modules/ccm.board-model.contracts.md#ccm-k-module-ccm-board-model-contracts)
- [next: watchdog 何时 arm](./board-model-guide.md#ccm-k-point-ccm-board-watchdog)
- [routes_to: status 是状态机不是赋值字段](../SKILL.md#ccm-k-point-ccm-status-state-machine)
<!-- ccm:k:nav:end -->
...
Cursor 下 watchdog 先是 board 里的 liveness 契约：记录“什么时候该回来 recon、回来查什么、超时如何处置”。它不等于已经存在一个 Cursor-native 的 CronCreate/ScheduleWakeup 工具。需要自动唤醒时，降级到 background Shell floor 或外部调度，并把句柄写进 board。
...
Cursor hook 可以在 Stop / SessionStart 等事件里注入提醒；只有带 nonblank `job_id` 且未过期的 canonical `watchdog` / legacy `wakeup` 才算健康并静默提醒。hook 不能凭空创建未来唤醒：不要把提醒当成 durable timer，它只能在 Cursor 再次触发相关事件时发声。
...
| `loop` | 仅当当前 Cursor 环境有已验证且已创建的 recurring automation；`job_id` 写 automation id | 下一项 |
| `monitor` | 可续查的外部状态 watcher；`job_id` 写 watcher / run id | 下一项 |
| `shell` | Cursor Shell（`block_until_ms: 0`）+ AwaitShell / notify_on_output；`job_id` 写真实 shell id | 没有真实 handle 就不要 arm，改记 blocked / recon 状态 |
...
| `job_id` | **必填 nonblank string**；真实 wakeup handle：后台 Shell id、外部 scheduler job id；用于追踪、disarm / recon，不能省略或填空白。不要伪造 CronCreate / ScheduleWakeup job id。。没有真实 handle 就不要 arm |
...
Cursor 下尤其要防“纸面 disarm”：如果 watchdog 记录的是外部 scheduler 或 background Shell，`ccm watchdog disarm` 只更新 board，不保证外部机制已停。每次 disarm 都要同步处理真实 scheduler / shell，并在 log 里写清结果。
...
**过期 wakeup 的 self-heal：** 如果 board 有 `wakeup` 但 `fire_at` 已过期（比现在早），Cursor Stop hook / recon 会把它当「未 armed」处理，重新提醒你选择真实可用的 wakeup handle。这是对「arm 后忘了退役」的自愈机制。
...
- [Knowledge atlas](../../../knowledge/atlas.md)
- [Module module:ccm.board-model.contracts](../../../knowledge/modules/ccm.board-model.contracts.md#ccm-k-module-ccm-board-model-contracts)
- [deepens_to: artifact/verified 真完成](./board-model-guide.md#ccm-k-point-ccm-board-artifact-verified)
- [operationalizes: namespace watchdog](./command-catalog.md#ccm-k-point-ccm-cmd-watchdog)
<!-- ccm:k:nav:end -->
...
- [Knowledge atlas](../../../knowledge/atlas.md)
- [Module module:ccm.board-model.contracts](../../../knowledge/modules/ccm.board-model.contracts.md#ccm-k-module-ccm-board-model-contracts)
- [next: 决策树与反模式深化](./board-model-guide.md#ccm-k-point-ccm-board-antipatterns)
- [requires: status 是状态机不是赋值字段](../SKILL.md#ccm-k-point-ccm-status-state-machine)
<!-- ccm:k:nav:end -->
...
- [routes_to: artifact/verified 真完成](./board-model-guide.md#ccm-k-point-ccm-board-artifact-verified)
- [routes_to: status 八态语义与转移判断](./board-model-guide.md#ccm-k-point-ccm-board-status-semantics)
- [deepens_to: FMT/GRAPH/BIZ 规则速查（操作视图）](./board-model-guide.md#ccm-k-point-ccm-board-validation-rules)
<!-- ccm:k:nav:end -->
...
| `FMT-POLICY` | warn | 历史 `policy` 非对象，或值不在 `{allow, deny}` | 只作兼容诊断；Cursor no-switch 不因 stored `allow` 改变 |
...
- [Module module:ccm.board-model.contracts](../../../knowledge/modules/ccm.board-model.contracts.md#ccm-k-module-ccm-board-model-contracts)
- [routes_to: artifact/verified 真完成](./board-model-guide.md#ccm-k-point-ccm-board-artifact-verified)
- [next: 交付 DDL 四态判断](./board-model-guide.md#ccm-k-point-ccm-board-deadline-ddl)
<!-- ccm:k:nav:end -->
