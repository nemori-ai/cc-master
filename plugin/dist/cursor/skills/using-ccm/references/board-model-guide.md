...
| `watchdog` | board 顶层 | `ccm watchdog arm / disarm` | Cursor hook 可以在 Stop / SessionStart 等事件里注入提醒，但它不能凭空创建一个未来唤醒。不要把 hook 提醒当成 durable timer；它只能在 Cursor 再次触发相关事件时发声。 |
...
`blocked_on: "user"` 的节点是给用户的「采访包」——用户在当前 Cursor 线程或另起 Cursor 线程里靠 `decision_package` 的内容理解上下文、做决策。**没有包 = 用户决策入口缺上下文**，所以 lint 对这类节点做 `BIZ-AWAITING` hard 校验：
...
| `job_id` | 真实 wakeup handle：后台 Shell id、外部 scheduler job id；用于 disarm / recon 清理。不要伪造 CronCreate / ScheduleWakeup job id。 |
