...
把一个 `blocked_on:"user"` 节点从「干问题」升级成「**对着准确且仍有时效的完整依据，做一次高质量决策**」的闭环。它接在上面 prefetch / recon 两条纪律上——不复述、只补两个具体动作（准备 / 消化）；讨论可在当前 Codex 线程或另起 Codex 线程承载；master 不亲自替用户做决定，只把 `decision_package` 作为上下文 surface 给用户，等回答后 recon 消化。采访包 / sidecar 协议（字段、枚举、sidecar 结构）的 SSOT 在 `board.md` §`decision_package`，此处只讲方法论。
...
1. **清掉对应 wakeup handle**：若是后台 terminal session，就停止/确认退出并记录 exit；若是外部 scheduler，就禁用 cron/systemd/CI job；若是 Codex app automation，就停止或更新 automation；若没有真实 handle，就不要写成已 armed；
...
> **不变式：当前无 watchdog armed 时，`board.wakeup` 必须 ABSENT。** 别在 board 上留陈旧残骸——一个已退役 watchdog 的 `wakeup` 对象不是无害遗物，它是一面骗 hook 与未来的你"还在守着"的假旗。正常路径下多半是某个 `in_flight` 先正常完成、harness 先把你叫回来——这时 watchdog 成了一个待发的空响，记得在那次重入里就**两件一起做**（CronDelete job **且** 清掉 `wakeup` 对象）。（陈旧 `wakeup` 会重开静默失败盲区。）

后台 terminal session → Codex app thread automation（若当前环境可用）→ 外部 scheduler + `codex exec resume` → Codex cloud status loop → manual recon；没有同名 cron/wakeup agent 工具时不要伪造在 `dispatch.md` §派发卫生（watchdog/liveness 维度）。
