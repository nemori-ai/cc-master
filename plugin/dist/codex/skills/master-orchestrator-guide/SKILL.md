...
2. **异步并行调度**——把全部本机可用 harness 的 worker 当作一个资源池，就绪即发、绝不在 barrier 干等，在整个本机 worker pool 内，用本 host 的 Codex subagents / 后台 terminal / cloud 或 `ccm` 管理的 cross-harness worker 派发；没有真实 handle 不得标 `in_flight`，在等待窗口主观能动而非空转。
...
| 「**Codex 7d 已越过当前 policy 硬边界，但这个节点在临界路径上；我先派了再说。**」 | 临界路径不是绕过不可逆消耗边界的理由。把「是否继续消耗 7d 配额」建成 `blocked_on:"user"` 决策；旧的 ship 意图不是续耗 standing 授权，hook 的非阻断也不是可忽略的 FYI。只继续收敛与验收在飞工作。 |
...
- 你正要在 Codex 7d 已越过当前 policy 硬边界时 dispatch 新节点，却没有先把「是否继续消耗」surface 给用户——你在替用户跨不可逆消耗边界。
...
| `dispatch.md` | 从本机全池选跨 harness worker + 选 Codex 当前 surface 可追踪后台机制（subagent / 后台 terminal / cloud task / external scheduler / automation）+ **派发卫生**；API 会话先发现 deferred subagent 工具；Workflow 语义当前 unsupported |
...
- **可视化** — 用 `ccm web-viewer open` 打开浏览器只读 DAG viewer；需要指定初始板时用 `ccm web-viewer open --board <board-path>`。旧 `$cc-master-view` 已删除。
...
- **消费闭环**：用户围绕 `decision_package` 在当前线程或另起 Codex 线程回答 → 你在 recon 时把结论消化进 board、replan、清 `blocked_on`
...
| **must-escalate** | 不可逆、对外、merge / 发布 / 授权 / 方向性、继续越过 Codex 7d 硬闸等用户拥有的决定 | **不要记成决策记录。** 建 `blocked_on:"user"` 决策节点并带 `decision_package`，等用户拍板 |
