...
| `watchdog` | board 顶层 | `ccm watchdog arm / disarm` | Codex hook 可以在 Stop / SessionStart 等事件里注入提醒，但它不能凭空创建一个未来唤醒。不要把 hook 提醒当成 durable timer；它只能在 Codex 再次触发相关事件时发声。 |
...
**estimate 缺失时的降级：** CPM 用默认 unit（工期排序仍运行，但 makespan 是 `weight_source: "mixed"`，精度降低）。estimate 存在是给 CPM 喂时长，缺失不阻塞调度。
...
**judgment_call（jc）是「自决诚实台账」**——凡 orchestrator 或执行者做了超过被明确授权范围的决策，就建一条 jc 记录。它的存在价值：用户审计 / resume 后快速了解「谁在这里自己做了什么决定」。

**何时建 jc：**

| 场景 | 要不要建 jc |
|---|---|
| 在两个技术方案之间做了选择，没有提前和用户确认 | **建** |
| 依赖版本 / API 漂移，选择了兼容策略 | **建** |
| 发现 spec 和实现有偏差，自己判断了 | **建** |
| 执行明确的既定方案，无自由裁量 | **不建** |
...
| severity | 判断标准 | 示例 |
|---|---|---|
| `low` | 影响局部、可逆、风险小 | 函数命名风格选择 |
| `medium` | 影响多个模块、反转有成本 | 选用某个库替代另一个 |
| `high` | 影响架构或核心路径、反转代价大 | 换接口协议、改存储方案 |
| `critical` | 不可逆、影响系统整体、或涉及安全 / 合规 | 选择不可撤销的外部依赖 |
...
Codex hook 可以在 Stop / SessionStart 等事件里注入提醒，但它不能凭空创建一个未来唤醒。不要把 hook 提醒当成 durable timer；它只能在 Codex 再次触发相关事件时发声。
...
| `job_id` | 真实 wakeup handle：后台 terminal session id、外部 scheduler job id、Codex app automation 名称或 cloud task id；用于 disarm / recon 清理 |
...
**「done 真语义」三要素（BIZ-DONE-VERIFIED，目前 reserved·暂不强制）：** `status=done` ∧ `verified=true` ∧ `artifact` 非空。这是完整意义上的「真的做完并验了」。目前 lint 不强制（reserved 级别），但操作上推荐三要素同时满足。
...
```bash
# ❌ 错误（exit 3: illegal transition: ready → done）
ccm task done T3

# ✅ 正确（两步）
ccm task start T3          # ready → in_flight，盖 started_at
ccm task done T3 --verified --artifact /abs/output.md   # in_flight → done，盖 finished_at
```
...
| `BIZ-DONE-VERIFIED` | reserved | done 真语义（`status=done` ∧ `verified=true` ∧ `artifact` 非空）——登记在册、lint 暂不强制 | 推荐三要素同时满足：`task done --verified --artifact /abs/...`——见 [L 节](#l-referencesartifactverified-语义) |
