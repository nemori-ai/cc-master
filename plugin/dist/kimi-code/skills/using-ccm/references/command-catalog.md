...
| `policy` | 兼容读取 board policy；kimi-code account switch 永久不可用，stored allow 不产生能力 |
...
| `account` | 账号池 namespace 不对 kimi-code 可用：`ccm account add` / `refresh` / `list` / `switch` 统一走 `NotImplemented`；kimi-code 也无 CLI 配额信号，`ccm usage` 恒 `available:false`。 |
| `statusline` | statusline namespace 仅作兼容告知：kimi-code 无外部命令式 status-line hook；`statusline install/uninstall` 为 `NotImplemented`；kimi-code 无任何 CLI 用量源，`ccm usage` 恒 `available:false`。 |
...
| `--session-id <id>` | | string | 指定 session（特权调用者注入；kimi-code host adapter 负责传入当前 session id，缺失时按 board 发现降级） |
...
| `4` | 锁超时 |
| `5` | 无 active board |
| `7` | 授权拒绝——`dependency waive` 缺显式 `--user-authorized`；亦保留跨 host policy-deny 兼容码，kimi-code 没有可执行的 account-switch 路线 |
...
- `quota status --machine-wide` 默认只读所有受支持 target scope 的**本机缓存投影**；加 `--refresh` 才按需经各 harness 的 live collector best-effort 填充 observation 缓存后再读，让冷缓存即使没有 monitor daemon 也不必全是 unknown。两者 JSON 根都是 `ccm/machine-quota-status/v1`，`summary.decisions[]` 给 target-bound posture，`readings[]` 给可得的百分比与 reset 事实；unknown / stale / missing 必须原样保留。Codex target 只有 `codex-cli + seven_day`；Cursor 的 `cursor-ide-plugin` 与 `cursor-agent-cli` 是两个独立 target，不能互补；Kimi Code 投影 `kimi-cli + five_hour` 与 `kimi-cli + seven_day`，信号来自只读 `kimi-usages-api` current login。
...
`cursor-agent-cli + billing_period`，任一 surface 的信号不能补齐另一条；Kimi Code 投影
`kimi-cli + five_hour` 与 `kimi-cli + seven_day`，由 `kimi-usages-api` 只读当前登录。
...
- positional：无
- 行为：经引擎**带锁**把 `owner.active` 翻 `false`（停用即休眠·全套 hook 对它休眠）；**非破坏**——`tasks`/`log`/`goal`/`git` 全留（审计留痕·文件不删）。给 kimi-code adapter 的 stop / handoff / resume 流程一条**走单写者带锁管线**的归档路径，替代手编辑 board JSON 翻 active。幂等：已 `false` 再 archive 仍 `false`（无副作用）。日后可经 `ccm`/kimi-code resume + board recon 复活。孤儿 / rollup 检查归调用方。
- flags：
...
> 让本机装了 ccm 的用户用 CLI 直接升级**两件解耦的发布物**：① **ccm 二进制**（per-OS Node SEA·随 GitHub `ccm-v*` 线发布）；② **cc-master 插件/入口面**（按 harness adapter 分发）。kimi-code 下 `upgrade plugin` 用本机已安装的 cc-master 包刷新本地 kimi-code marketplace/plugin 注册，更新的是 `plugin/dist/<harness>` 内的 skills/hook 打包资产与命令入口；它不触碰 Claude marketplace。**非 board 操作**——不读/写 board。
>
...
`data.policy` 可能含历史 `autonomous_account_switch`，但 kimi-code 的运行时 effective capability 始终是 no-switch。不要根据 stored `allow` 执行 account 命令。
...
```jsonc
{
  ...
  "allocation": {
    "pressure": { "headroom_pct": 15, "quota_model": "kimi-no-quota-signal", "band": "warn" },
...
信号可用时 `data.refresh_hint` 为 `null`；信号不可用（`available:false`）且成因是某 harness 的短命 token
过期（这类凭证由 harness 自己刷新、ccm 全程只读）时，它带一个 `{reason, recoverable, command, remedy, recheck}`
对象：`recoverable:true` 时 `command` 是让该 harness 自行刷新 token 的可执行命令、`remedy` 是「运行它 →
重跑 recheck」的完整人读步骤、`recheck` 是重新查询 usage 的命令；不可自恢复（网络 / 401 / API 变更）时
`recoverable:false` 且 `command` / `remedy` 为 `null`。ccm 只出提示、绝不替你刷新或写任何凭证。
