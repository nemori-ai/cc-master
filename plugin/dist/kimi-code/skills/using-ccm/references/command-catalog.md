...
> `cursor-agent-cli` 各读自己的 current-login `billing_period` 与 named pools，不能跨 surface 或跨池互补。Claude 的账号 registry snapshot
