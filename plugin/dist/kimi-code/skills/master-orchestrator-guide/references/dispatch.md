...
- **后台机制**（「你怎么真跑你负责执行的那些」）—— 你此刻实际抄起哪个工具把活跑起来：kimi-code Task subagent / 后台 Bash 任务 / 外部 scheduler 或 CI job。它回答「怎么让它真的动起来」。
...
- **`workflow`** —— board 字段可保留这个 executor 值表达“结构化 fan-out/fan-in 工作”，但 kimi-code adapter 当前不支持 Claude Code Workflow API。需要多叶子确定性控制时，用多个 Task / 后台 Bash 组合实现，并记录每个 handle；不要调用 `Workflow`。
...
executor 值定了“谁负责”；在 kimi-code 下，真把活跑起来用这些 **agent runtime 可用**机制，按可追踪性优先：

- **Task subagents** —— 默认并行派发方式。用 Task tool 启动（`coder`（默认）/ `explore`（只读）/ `plan` / `general` + Agent Swarm），把返回的 subagent id 记录为 handle。没有真实返回值，就不要把 board task 标成 `subagent` / `in_flight`。
- **后台 Bash** —— 适合长跑 shell 命令、watch loop、构建/测试、轮询外部谓词。用 Bash 后台任务，后续轮询任务输出；把 Bash 任务 id、命令、工作目录、停止条件、recon 方法写进 board。不要把“我发起了命令”当完成，只有 session 退出并经端点验收后才折回 `done`。
