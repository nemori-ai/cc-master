...
- **host**：`kimi-code`
- **usage profile**：kimi 无 CLI 配额信号：`ccm usage` 恒 `available:false` / `signal:null`，不读任何窗口百分比、不出 `throttle`、`switch`、`stop_5h`、`stop_7d` 或 reset verdict；缺信号即保持不可判，自动换号永久禁止。
