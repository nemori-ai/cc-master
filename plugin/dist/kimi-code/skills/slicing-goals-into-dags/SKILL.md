...
- **master-orchestrator-guide** —— 切好的图怎么**排期**(CPM / float / 临界路径 / 并行度计算,在它的 board 协议 reference)、怎么**派发**(按 kimi-code adapter 已验证的后台机制选择（内置 subagent 角色 / 后台 Bash 任务；无 Workflow）)。本 skill 是"切",它是"排 + 派"。
