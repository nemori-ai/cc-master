...
// PARITY: rule-verify-board-tag-protocol — ADR-018 标签协议在 kimi-code 侧的本地等价包装（无共享 hook-common
...
// 会话级连续 block 计数，达阈值即强制放行 + strong advisory。kimi Stop: launcher 把 kind:block 映射为
// hookSpecificOutput.permissionDecision="deny"（agent 续跑一次 + reason 注入·probe 证实）；kimi 每个 stop
// cycle 内建单次续跑护栏，本 FUSE 跨 cycle 计数防止连续 deny 卡死整个 session（stop_allow_until 释放阀仍在）。
const FUSE = 5;
function fuseSidecarPath(home, sessionId) {
  const name = sessionId ? `.kimi-code-${sessionId}.stopfuse` : '.kimi-code-nosession.stopfuse';
...
function main() {
  ...
  block(`cc-master kimi-code Stop continuation required:\n${notes.join('\n')}`);
}
