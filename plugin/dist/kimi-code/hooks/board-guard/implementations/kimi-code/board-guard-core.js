...
// PARITY: rule-board-guard-directive-tag — ADR-018 标签协议（ambient/advisory/directive）在 kimi-code 侧的
...
function commandWritesBoard(command, home) {
  ...
  const writes = name === 'ccm'
    ? command.redirectionTargets.some((target) => segmentTouchesRealBoard([target], home))
...
function main() {
  ...
  // kimi-code tool surface (kimi-code.md §6, Claude-shaped): Bash | Write | Edit | MultiEdit.
  // No apply_patch on this host. Write/Edit/MultiEdit → path check; Bash → write-heuristic.
