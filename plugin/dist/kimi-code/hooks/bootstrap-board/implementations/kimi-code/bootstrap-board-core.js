...
function parseInvocation(prompt) {
  ...
  // kimi native plugin commands are namespaced `cc-master:<command>` (kimi-code.md §4). When the user
  // types `cc-master:as-master-orchestrator <goal>`, the UserPromptSubmit hook sees that literal prefix
  // on the first content-block's text; the command body path injects the sentinel handled below.
...
function writeSessionState(home, sessionId, boardPath, invocation, mode) {
  ...
  const state = {
    schema: 'cc-master-kimi-code-session/v1',
    harness: 'kimi-code',
...
function resumeBoard(home, boardsDir, flags, sessionId, invocation) {
  ...
  say(
    'context',
    [
      `cc-master resume: armed kimi-code orchestration board at ${boardPath}`,
...
try {
  ...
  say('context', `cc-master kimi-code bootstrap failed: ${error && error.message ? error.message : String(error)}`);
}
