...
- **原生 `/loop`·`ScheduleWakeup`（仍不直接引入）**：动态自步调（ScheduleWakeup）Bedrock/Vertex/Foundry 不支持、`/loop` 会话 7 天过期——撞 **ship-anywhere** 硬约束。cc-master 本就事件驱动（后台一完成 harness 自动唤醒重入），不需定时轮询；`/loop` 唯一真场景（等 harness 追踪不到的外部状态：CI / 远程队列 / 审批超时）用**已有的"后台 shell"积木消解**（`until <就绪>; do sleep N; done` 丢进 `run_in_background`，完成后 harness 通知重入），更贴事件驱动、完全 ship-anywhere。详见整合设计文档。
