...
| **SessionStart**（startup/resume/compact） | 扫 home；若存在任何活跃 board，则重注 orchestrator **角色** + home 路径 + 活跃 board 短清单（文件名 + goal）。它**不**指向某个固定 board 路径——agent 凭 goal 认回自己的 board，继续编排循环。【扛 compaction / 跨会话续命】 |
