...
validation.json      # 完整候选图、result hash、四 host projection witness、patch dry-run
apply.patch          # 工具从 accepted/candidate bytes 生成
runtime-candidate/   # validate 期 ignored 临时根：overlay 后的 candidate repo + per-host dist
```
...
change begin --op <type> --scope ... --base <git-ref>
  ...
  → any host projection / link / hop / budget failure → fail closed（不写 accepted、不 finalize ledger）
  → change apply <workspace>
  → re-run the same validate gate；only then atomically replace scoped accepted files
  → finalize immutable change record only after accepted tree is complete
```

Host coverage 与条件边（validate 硬门）：

- `full`：对该 host 跑完整 candidate sync + compile `--check` + surface verifier + H1–H4 + router budget。
  **overall `full` 当且仅当**该 host 上每个 skill 的 `host_coverage.state` 都是 `full`（不得把
  full+partial / full+stub / partial+stub 虚报成 overall full）。
- `partial`：只对真实投影子图计 coverage——full skill 的全部 module ∪ partial skill 的
  `covered_modules`；stub/unsupported skill 不进入 point/edge/entry/hop/budget 分母。
- `stub` / `unsupported`：必须 abstain——`mode` 为 `stub` 或 `unsupported`，`point_anchors=0`，
  `enabled_edges=0`，H1–H4 witness 显式 `abstained:true`；不得生成或声称虚假 point surface。
- 路径权威：repo / workspace / candidate / runtime-candidate / scope file 的 locate 链禁止任何
  symlink；realpath 必须严格落在真实允许根内。cleanup 只能删除本次成功创建且再次验证为
  workspace 内普通目录的 runtime root；all-stub 不需要 runtime root 时不得清理未知路径。
- 失败见证：未执行的门必须 `ok:false` + `witness.skipped:true`，且不得出现在冒充成功的
  `executed_checks`；合法 stub abstention 与 skipped failure 不得混用。
- 条件边：计入某 route class 当且仅当 `when` 命中且 `avoid_when` 未命中。K1 在无显式 cue context
  时，candidate runtime witness 必须声明
  `conditional_route_policy: "enabled_by_default-only"`，并且**只**把
  `runtime.enabled_by_default: true` 的 authored navigation edge 送进默认 route class；
  不得静默按「忽略 when/avoid_when 的全量 union」计 hop。若编译器仍把
  `enabled_by_default: false` 的边发射进 final nav，validate 必须 fail closed。
...
## 12. Dev meta-skill 架构（K1-09 DELETE 裁决）

### 12.1 准入结论：**REJECTED / DELETE**

公平 baseline 下对候选 `governing-skill-knowledge` 的 Counterfactual Probe 结论是
**weak × weak = DELETE**（证据摘要：[`../eval/governing-skill-knowledge-round1.md`](../eval/governing-skill-knowledge-round1.md)）。

- Probe A weak：正式规范 + CLI contract + `skill-knowledge.mjs change|check|contract`
  已覆盖 health / typed op / witness；独立 meta-skill 不增加不可替代能力面。
- Probe B weak：Round-1 with-skill Track B 未达 independent strict green。
- 因此**不得**在 `.claude/skills/` 保留该候选，也不得在 AGENTS 路由里宣称“已准入”。

维护者旅程仍以本目录正式规范为准（specification / schemas / cli-contract / examples），
不经过独立 meta-skill。

### 12.2 边界（无独立 skill 时的 owner）
...
root `AGENTS.md` 必须反映真实磁盘：runtime portfolio **8**；dev/meta **9**
