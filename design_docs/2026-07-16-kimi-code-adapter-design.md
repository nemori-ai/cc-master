...
- **quota collector**：`machine-wide-quota.ts:60` `TARGETS` **不加** kimi-code 条目（无配额面）；`ccm usage`/`ccm harness` 已对 `signal:null` 优雅降级（`available:false`），不伪造窗口。将来 kimi 若出 CLI 配额面再加。
...
| `ccm-quota-account` | **gap** `protocol-capability-gap` | 只读配额 + 号池 | `ccm usage advise` 无信号即 `available:false`；`ccm account *` NotImplemented | 无 CLI 配额面；号池绑 Claude OAuth → unsupported |
| `usage-pacing-midflight` | **gap** `event-unavailable`+无信号 | mid-flight 配速采样 | Stop-only pacing，且 kimi 无配额信号 → 基本静默 | 无 PostToolBatch + 无配额面 |
...
| `machine-wide-quota-notification` | **gap** | 全机配额通知 | 无 kimi 配额 target | 无 TARGETS 条目 |
...
- **quota**：`ccm-quota-account` + `machine-wide-quota-notification` 加 kimi-code `protocol-capability-gap`/无 target。
...
5. kimi 配额面若出现 → `machine-wide-quota.ts` TARGETS + `readCurrentUsage` 真信号。
