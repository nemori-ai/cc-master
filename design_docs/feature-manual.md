...
**横切：Goal Contract 生命周期与防漂移 ✅（Unreleased）** —— fresh 入口把原始参数 / issue 当需求证据，建 `assurance:pending` skeleton，不直接复制到 `board.goal`；agent 先 framing，再以 `ccm goal set|confirm|amend|show|check` 管理当前 revision。复杂背景落为 `$CC_MASTER_HOME/goals/` 下的 immutable Goal Brief，board 保存 ref + SHA-256。resume/compaction/周期提醒/完成闸按当前 revision 重对齐；hook 只做 lifecycle/integrity 守卫，语义漂移判断仍归 skill。legacy board 保持兼容。
...
## 4. board 引擎——`ccm`（12 namespace · 52 handler；`status-report` / `web-viewer` 为新增设计）
...
### 4.6 status-report —— 静态可程序化 board 状态报告 📐

| verb | 做什么 | 用户价值 | 状态 |
|---|---|---|---|
| `status-report render` | 一次性计算 board 状态报告；`--json` 输出稳定 `ccm/status-report/v1` schema，不写 artifact | 把原先 agent 临场写的 status prose 变成可测试、可复现的数据产品 | 📐 |
| `status-report write` | 计算并原子写 `<home>/reports/status-report/boards/<board-stem>.status-report.json` | web viewer / CLI / 未来客户端共用同一份派生报告，不污染 board | 📐 |
| `status-report show` | 用户入口：复用 fresh artifact，缺失/陈旧时按需刷新；人类输出从同一 JSON 派生 | 终端里看进度、阻塞、临界路径、决策、风险、下一步，不再依赖 prompt-time agent prose | 📐 |
| `status-report watch` | 可选周期写报告的 headless loop；viewer 不依赖它才能正确 | 没开浏览器也能持续产出静态状态 artifact | 📐 |

报告内容覆盖：progress summary、blocked-on-user、in-flight、ready、done、异常状态、critical path、lint/over-scheduling/usage health、risks、next actions。刷新判定用 `board_hash` / `topology_hash` / `advisory_hash` / `input_hash` / `expires_at`，只写 `<home>/reports/status-report/`，**绝不写 board**。Web viewer 的 board 页面新增 Status 子模块，通过 `/status-report.json?board=<file>` 读取同一 schema。

### 4.7 web-viewer —— 本地只读 board app 📐

| verb | 做什么 | 用户价值 | 状态 |
|---|---|---|---|
| `web-viewer open` | 打开 / 启动 ccm 托管的本地 board viewer app；默认复用 home-scoped service | 浏览器里看 board / DAG / status，不需要知道 plugin 路径或后台脚本 | 📐 |
| `web-viewer start/open/status/stop/restart` | 管 home-scoped viewer service 的 PID、token、URL、state file、stale cleanup | viewer 生命周期可检查、可停止、可恢复，不绑某个 agent session | 📐 |

目标 UI 是 **Vite + React + TypeScript 等工业化前端 app stack 的 build artifact**，由 `ccm web-viewer` service 在 `127.0.0.1` 本地托管；手写静态 HTML shell 只允许作为短期 bootstrap/prototype/smoke harness，不能作为验收目标。服务边界：ccm 提供 board list / board data / DAG read model / status-report JSON endpoints，frontend app 消费这些 API；UI 不复制 board/status 模型、不写 board。运行时不联网（无 CDN / remote font / telemetry），token-gated，release packaging 需证明 assets 能从 repo/dev build 与 SEA/release artifact 被正确托管。

推荐工作流：`build-web-apps` 用于 frontend app/UI redesign/browser QA；`build-web-data-visualization` 用于 DAG/dashboard/critical path/graph layout/accessibility/visual regression；`figma` 只在用户授权设计系统或高保真稿时使用，不是阻塞前置。

---

## 5. 命令——分发 Command + ccm CLI surface
...
| `ccm status-report show` 📐 | `ccm` 生成的静态 board 状态报告（目标/进度/任务分组/临界路径/健康检查/预算快照/风险/下一步），同一 JSON schema 供 CLI 与 viewer 读取 | 终端里一眼看清"干到哪、卡哪、等不等我、配额剩多少"，但不再靠 agent 现场写 prose；旧 `/cc-master:status` / `$cc-master-status` 已删除，后续只应出现在历史语境 |
| `ccm web-viewer open` 📐 | 本地 web viewer app 渲 board DAG + Status 模块（只读、127.0.0.1、token-gated、零联网；生命周期归 `ccm web-viewer start/open/status/stop/restart`；目标 UI 是 ccm 托管的 Vite/React/TS 等 build artifact，非长期手写静态 shell） | **浏览器里实时看 AI 在干嘛，像盯一块活的项目看板** |
