...
>
> 末次全量同步：2026-06-26（基于 `board-v2-redesign` worktree，版本 0.10.0 + 未提交的成本轴增量）。
...
## 2. 总指挥的大脑——7 个分发 Skill（A–G）+ 决策程序

skill 是按需调阅的深度手册，各自自洽、互不重叠。A 给指挥本身、B–E 给指挥的不同决策面、F–G 给**被派去干活的执行 agent**。**全部 ✅已落地**（0.10.0；A/B 老牌、C 在 0.8.0、D/E/F/G 为 0.10.0 新增）。
...
| **C. account-management** | 指挥 | 换号号池机制层（registry + vault token + 选号 + 无重启换号） | 攒备用账号池，配额烧穿时自动切号续命，token 全程不进 AI 视野 |
| **D. using-ccm** | 指挥 | `ccm` CLI 操作手册 + board 模型/字段/46 条校验规则速查 | 底层保证 AI 写"项目看板"时走一道带校验的关卡，不写脏、不自相矛盾 |
...
## 3. 运行时脊柱——6 个 Hook + 武装闸

hook 是 cc-master 的牙齿。**全部 ✅已落地**（board-v2 已从 v1 的 6 份脆弱 awk 副本收敛为 5 个 node hook 共用 `hook-common.js` 武装地基 + 1 个 bash bootstrap）。
...
> 两点机制底色（README 该懂）：① **唯一能硬 block 的只有 verify-board（Stop 闸）**，其余 5 处都只软推、绝不阻断——"指挥不演奏 / 引擎不替它思考"红线在 hook 层的落点。② **ADR-018 标签化注入协议（`<ambient>`/`<advisory>`/`<directive>`）目前还没落进 hook 代码**（grep 零命中）——它是 AGENTS.md §13 立的作者侧约定，标 📐 渐进迁移。

---

## 4. board 引擎——`ccm`（11 namespace · 45 verb）
...
| `board lint` | **46 条规则**（30 FMT + 7 GRAPH + 9 BIZ）体检整板，硬错退 3 | 写错当场拦、不埋雷 | ✅ |
...
### 4.2 jc / cadence / watchdog / baseline / policy —— 治理与节奏
...
- **无重启换号 + 号池序列储备（SKILL C，0.8.0 起）✅**：任一时刻全机器仅 1 个活跃账户，号池 N 个账户是序列储备；换号=覆写官方三存储 + 运行中 claude 惰性 re-read 接管（不重启进程、board 不归档）。effective-N 缩放：握 N 份配额时单 5h 窗要快 N 倍消耗才不浪费。**凭证写已加跨进程锁**（🔨 本分支：anchor 到机器全局，防并发换号交错损坏三存储）。
- **成本轴偿付力仪表（🔨 本分支）**：%-burn-rate（流速）/ %-runway（续航）/ %-cost-to-complete（还要花多少，回答"这目标装得下吗"）——见 §4.3/§4.4。
...
cc-master **致力于让** 任意会话 agent 化身指挥并具备六项能力。这是 **aspirational 北极星、非验收单**；诚实兑现状态本身是工程成熟度的体现。SSOT 在 `spec.md §1.0`，gap 审计在 `vision-landing-tracker.md`（last-audited 2026-06-11，部分已被 ADR-013/014/015 推进）。

| 能力 | 兑现 | 真 gap 一句话 |
|---|---|---|
| **C1** 异步并行多线程、完整落地 | 并行 ✅ + 完整闸 ✅(verify-board) | board 完整性零机制保障、靠 agent 自觉 |
| **C2** 控制消耗速度 | sensing ✅ · pacing 决策 🔨→✅(引擎收口) | 原"传感器真但 loop 从不调它"，正被引擎化关闭 |
| **C3** 自主 vs 人类边界 | 🔨(行为红线靠端点守) | Stop 闸不分"未答终审"与"等上游" |
| **C4** 分解/管理/更新/规划 | board ✅ · 分解方法 🔨 · 临界路径已机器算 | supersession/重规划无事务一致性 |
| **C5** 资源下最大化效率 | scheduler 🔨(手跑决策程序) | 单侧兜底→双侧走廊正补过调度侧 |
| **C6** 按复杂度/难度/**时长**选模型 | lever ✅ · 选档 🔨 · **duration 维 📐→🔨** | duration 维曾蒸发，正由 ccm estimate 接回 |
...
| **多-orchestrator 协调（ADR-017 草稿）** | M 个指挥共享号池配额；两层=AIMD 机械底座（收敛公平份额、抗震荡）+ **L-agentic 智能体推理层**（读 peer 的 goal 推理相对价值、主动礼让）；channel = 各板写自己的 `coordination` 块、ccm 跨板聚合 | 📐 仅设计（ADR-017 草稿、号段已预留，住 gitignored plans/） |
| **Hook→Agent 标签协议（ADR-018）** | `<ambient>`/`<advisory strength>`/`<directive>` 三类 + P1–P6 作者纪律；"没有中性注入"；按 tag 分配注意力、可追溯 | 🔨 Accepted-rollout（作者侧已落 AGENTS §13；读者侧 SKILL A 须先跑 pressure baseline；hook 代码尚未迁） |
