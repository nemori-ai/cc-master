...
| `web-viewer start/status/list/stop/restart` | 管 viewer service 的 PID、token、URL、state file、stale cleanup | viewer 生命周期可检查、可停止、可恢复，不绑某个 agent session | 📐 |
...
| `ccm web-viewer open` 📐 | 本地 web viewer app 渲 board DAG + Status 模块（只读、127.0.0.1、token-gated、零联网；生命周期归 `ccm web-viewer start/status/list/stop/restart`；目标 UI 是 ccm 托管的 Vite/React/TS 等 build artifact，非长期手写静态 shell） | **浏览器里实时看 AI 在干嘛，像盯一块活的项目看板** |
