...
cc-master 不是一个替 agent 做决定的中央调度框架。行动者始终是运行在 Claude Code、Codex 或 Cursor 中的 agent；`as-master-orchestrator` 把当前主会话初始化成 master orchestrator，插件在运行时提供行为指导和通知，独立安装的 `ccm` 提供可验证的状态、进程和只读决策事实。

产品由三层组成：

1. **cc-master plugin**：三 origin 的 commands / skills / rules / hooks，把 agent 的身份、决策程序、恢复和通知接进各 harness。
...
支持的 origin 是 **Claude Code、Codex、Cursor IDE plugin**；可被 `ccm worker` 直接启动的本机 headless worker 是 **Claude Code CLI、Codex CLI、Cursor Agent CLI**。Cursor IDE 与 Cursor Agent 是两个独立 surface，不混为一个客户端。
...
| `using-ccm` | CLI 操作手册、board 模型、状态机、Agent Registry 和 **82 条**校验规则派生视图 | ✅ |
...
fresh board 以 pending skeleton 起步。原始 prompt 和 GitHub issue 是需求证据，不是 canonical goal；orchestrator 通过 `ccm goal set|confirm|amend|show|check` 维护带 revision 的 Goal Contract，复杂上下文用不可变、哈希校验的 Goal Brief。resume、compaction 和完成闸都重对齐当前 revision。

### 4.2 Board ✅

board 是单一可写编排事实源；所有写入经 `ccm` 带锁关卡。`@ccm/engine` 维护字段 tier、状态机、DAG、临界路径和 **82 条 FMT / GRAPH / BIZ 规则**。`done` 必须同时有 `verified:true` 和非空 artifact；retry 会归档旧 attempt 证据并清空当前 attempt。
...
派发顺序是 create starting agent → 实际 spawn/worker/native dispatch → bind handle/link task → task 进入 `in_flight`。agent terminal 只说明进程生命周期结束，绝不自动等于 task 通过验收。
...
- 发现已安装的 Claude Code、Codex、Cursor Agent 可执行文件；
- 展示目标 CLI 的真实 help / 参数面；
- 原样透传 target-native argv，避免 ccm 复制一套会漂移的参数 schema；
- 启动 session-bound worker、捕获结构化结果和 lifecycle evidence；
...
### 6.1 一个生产者、三 origin 消费 ✅

`ccm` 是 quota 事实唯一生产者。各 origin 的 bootstrap 向本机服务注册/订阅；hook 从 cached posture 生成 pre-context 和 coordination notification，使任意 origin 的 orchestrator 能看到本机全部已支持 harness 的 provider-scoped 状态。hook 不抓 provider token、不自行重算权威结论，也不以 agent 数量猜 quota。
...
所有 hook 都遵守 dormant-until-armed：只有当前 session 经 `as-master-orchestrator` 接管 active board 后才工作。Claude Code、Codex、Cursor 由共享 CONTRACT + host adapter 保持业务语义 parity。
...
viewer 已是交付能力，不是设计稿。它由 `ccm web-viewer start|open|status|stop|restart|serve` 管理，监听 localhost、token-gated、运行时零外网，前端资产随 ccm SEA 内嵌并在 upgrade/install 后 reconcile。页面提供 Graph / Board / List / Timeline、Goal Contract、planning/routing badges、DecisionCard、peers/inbox、Agent Registry roster/inspector、task links、filters 和 shareable URL state。UI 是只读消费者，不能写 board。
...
- plugin 对 Claude Code、Codex、Cursor 分别生成 host-native artifact；共享版本号不代表三 host 的 API 形状完全相同。
...
- Claude/Codex/Cursor provider CLI 会演化，因此 ccm 透传真实参数并暴露真实 target help；只把稳定的发现、生命周期、evidence 和 board contract 放在 ccm 层。
