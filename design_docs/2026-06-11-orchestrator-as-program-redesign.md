...
4. **最大且最廉的赢**：把 loop 的**异步事件**（sub-agent 完成、并行批解析）接到**自动通知**（`SubagentStop` / `PostToolBatch`），把「agent 得记得 integrate / 查过调度」（脆弱 prose，compaction 后最易丢）变成「运行时主动告诉它」（确定性通知）——bash 即可、不动 narrow waist。
...
**curating 判定：不拆。** 6 愿景留在 `orchestrating-to-completion` 当镜头；`authoring-workflows` 正交（一种机制的脚本写法）、按约束不动。再设计只做**定点镜头加固** + **让方法论显式引用新 hook 通知**（如「运行时会在 `SubagentStop` 通知你某 sub-agent 完成——那时去 integrate+端点验收」），让 skill 与 hook 协同而非各说各话。
...
- **H6 · `SubagentStop` → 后台完成自动通知主线 integrate+端点验收**〔闭 C1/C5 阶段5〕
  *核实*：SubagentStop 在 sub-agent 完成时触发，**能注入 `additionalContext`**（且可叠 block）。
  *做法*：hook 读 board，注入「sub-agent〈agent_type〉完成 → 去 integrate 它对应的 task + 端点验收（gate-green≠passed）」。把「等待窗口记得收割 done」（prose、compaction 后最易丢）变成运行时主动通知。**这是阶段5「Interrupt/notify」从空白到有的关键一笔，也是首轮调研漏报、二轮坐实的最强新机会。**
...
| 层 | delta | 类型 | 红线 | 优先 |
|---|---|---|---|---|
| **hooks** | 新增 `SubagentStop` hook（H6 integrate 通知） | 新增·纯 bash | ✅ 合规 | **P0** |
...
| **skills** | `orchestrating-to-completion` 定点加固：镜头5 引用 H5/H6/H8 通知、镜头7 引用 H3、C6 接 duration 维 | 改·prose | ⚠️ 纪律段改前跑 pressure baseline | P1 |
...
- **第一波（P0，全不动 waist · bash 为主）**：H6（SubagentStop integrate 通知）+ H5（PostToolBatch 过调度警告）+ H3（Stop 列未答 user 决策）。这三笔补上阶段5 空白 + C5/C3 的对称缺口，**杠杆最高、成本最低、零红线风险**。
