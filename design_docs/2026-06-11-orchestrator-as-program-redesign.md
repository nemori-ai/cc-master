...
1. **skill 体系不拆**（curating 判定）：6 愿景是「指挥一个长 horizon 目标」**一个** bounded context 的 6 个镜头，全在 `master-orchestrator-guide`。拆成 6 个 skill = 装饰 + overlap + 破坏 reinject。
...
**curating 判定：不拆。** 6 愿景留在 `master-orchestrator-guide` 当镜头；`authoring-workflows` 正交（一种机制的脚本写法）、按约束不动。再设计只做**定点镜头加固** + **让方法论显式引用新 hook 通知**（如「运行时会在 `PostToolBatch` 提醒你过调度——那时收一收并行」），让 skill 与 hook 协同而非各说各话。（原拟用 `SubagentStop` 做「sub-agent 完成 → integrate+端点验收」的通知，已评估并移除——`additionalContext` 不达父线、与内建通知冗余，见 §5.1。）
...
| **skills** | `master-orchestrator-guide` 定点加固：镜头5 引用 H5/H8 通知、镜头7 引用 H3、C6 接 duration 维 | 改·prose | ⚠️ 纪律段改前跑 pressure baseline | P1 |
