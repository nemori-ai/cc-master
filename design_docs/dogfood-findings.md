...
| 92 | `ccm watchdog arm` 缺真实 `job_id` 仍成功，board 对象存在与真实唤醒句柄脱钩 | should-fix(静默失败安全网假武装·candidate 已修) | arm mutation nonblank handle 闸 + warn-only legacy 诊断 + 三 host fail-closed + disarm 两字段 ABSENT |
| 94 | `done + verified` 只证明 candidate acceptance，不能证明 target delivery；status-only deps 可消费 stranded implementation | must-fix(交付/依赖真相·declared-mode v1 实现中) | ADR-036：edge-aware target/evidence/requirement 三层 contract；历史 exact legacy，strict 只读 dry-run，retry 清 current proof，零隐式网络 |
| 99 | `ccm worker run --cwd <相对>`(如 `.`)被 cwd 校验在 executable 解析**之前**硬拒,吐 `executable:null`+`request_rejected` 秒退——被误诊为「kimi driver executable 解析回归」,实为四 harness 通用 cwd 处理 bug(help 路径用 `process.cwd()` 恒绝对故不触) | should-fix(worker run 通用可用性·已修) | ✅ 已修(worker-process.ts 把相对 --cwd 按 process.cwd() 解析成绝对·同无 --cwd 默认;+回归测试;四 harness `--cwd .` 均恢复 exit 0)。附:kimi k3 404 不复现(ccm 从不注入 -m·raw passthrough·provider 侧瞬态) |
...
## Finding #92 — handler-runtime 提前失败先删 barrier 临时树、却未收走 runtime worker child，导致 polling orphan 与进程计数膨胀 · should-fix(测试基础设施生命周期·已修)

- **现象 / evidence**:`handler-runtime.test.ts` 的 no-replace publish race 直接 `spawn()` 两个 `runtime-publish-worker.ts`；若两个 worker 到达 barrier 后断言提前失败，或其中一个永不发布 ready handshake，成功路径不会写 `go`，原 `afterEach` 却立即递归删除含 barrier 的临时树。worker 持续轮询已删除、再也不可能出现的 barrier，测试 parent 退出后仍可留在宿主。确定性 RED 分别观测到 assertion path 的 owned PID `111403`/`111408` 与 readiness-timeout path 的 `111514`/`111515`；RED 外层只按捕获到的精确 `ChildProcess` 紧急终止并 reap，随后 worktree 路径审计为零。
- **两层计数 dogfood**:同场 UI 在 board `9/16` 时显示过 `25` 个 terminal/process entries；这不等于 board WIP 超出 9 个。board 层应数**独立 in-flight leaf / accountable root handle**，而 OS 层一个合法 root 可展开为 `shell → launcher → binary → test` descendants。承重 lifecycle invariant 是「每个 in-flight leaf 恰有一个可问责 root handle；完成 / cancel 后该 root 的 owned descendants 必须归零」——不能拿 descendant 数冒充 WIP，也不能因 root 已退出就把仍存活的 descendants 当作已收口。
- **根因 / 机制**:test fixture 持有临时目录生命周期，却没有持有由它派生的 child 生命周期；销毁顺序反了——先删 child 所依赖的 barrier，再期待成功路径上的 `waitForExit()` 自然收尾。这个缺口不只存在于 publish race：同文件若干 activation/runtime async spawn 也没有 suite-level ownership fallback，任何中途 assertion 都可能绕过各自的 success-only wait。
- **影响**:泄漏 child 会抬高后台 process/terminal 计数，污染后续 suite 的宿主环境，并把真实测试失败叠加成 hang 或跨任务假归因。它是 test harness 的资源所有权缺陷，不是 production runtime supply-chain 行为变化。
- **处置(含蒸馏判定)**:**已修**——在测试文件内建立精确 owner：所有 async runtime worker spawn 当场登记；POSIX 下每个 root 以独立 process-group leader 启动并按精确 PGID 持有整棵 subtree，publish race 自身用 `try/finally` 收走本次 trees，async `afterEach` 再作 suite-level fallback，且严格在 root `close` **并且 group 归零**后才删临时树。teardown 只对 owned group 发 `SIGTERM`，condition-based bounded 等待，超时再只对同一 group 发 `SIGKILL` 并 bounded reap；并行 cleanup 用 `allSettled` 等完全部 owned tree 后聚合报错，body 与 cleanup 同时失败时用 `AggregateError` 保留两份因果，不 broad `pkill`、不靠固定 sleep、也不吞 cleanup error。合入 latest main `#113` 后保留其全部 20 个新增 handler-runtime cases，并把它新增的 15 个 async materialization roots 一并收口到唯一 `spawnOwned()` 入口；显式 parent-SIGKILL 测试仍允许其 native descendant 完成既定 self-clean，只有早退 fallback 才收整组。fault fixture 让每个 publisher 再生一个 descendant，两条 assertion / ready-timeout 早退路径都捕获 2 roots + 2 descendants 共 4 个 PID 并断言全部归零；原 race 语义不变，post-#113 handler-runtime 45/45、final-head 20× focused subtree stress 每轮 4 tests 全绿。**蒸馏判定 = 回流测试 harness + finding，不回流 runtime skill prose / production contract**：这是仓内测试资源所有权的确定性实现纪律；两层计数只澄清 root-handle / descendant 生命周期，不新增 board node、scheduler feature 或用户侧运行时语义。
- **严重度 / 来源**:should-fix(测试隔离、宿主资源泄漏与失败可诊断性·已修)/ 一手(handler-runtime fault-injection RED、精确 PID 审计与 focused stress,2026-07-14)。

## Finding #93 — watchdog 对象存在不等于真实唤醒已存在；缺 `job_id` 仍可 arm 让三宿主误判健康 · should-fix(静默失败安全网假武装·candidate 已修)

- **现象 / evidence**:clean main `474b185` 的真实 CLI 对 `ccm watchdog arm --mechanism shell --fire-at 2099-...` 返回 0 并写入没有 `job_id` 的 `board.watchdog`；传空白 `--job-id '   '` 甚至加 `--force` 也同样能落盘。`ccm watchdog status` 只看对象存在就输出「已武装」，Claude Code / Codex / Cursor 的 verify-board 也把 future `fire_at` 对象当 healthy，因而压掉 in-flight 静默失败提醒。
- **根因**:`registry` 把 `--job-id` 声明成 optional，`watchdogArm()` 又只在调用方提供时机械复制；写入关卡没有「armed ⇒ real accountable handle」不变式。读侧只验证对象 / `fire_at`，把 board 里的意图记录错当外部 scheduler/loop/monitor/shell 已创建的地面事实；旧 `watchdogDisarm()` 还写 `watchdog:null` 且不清 legacy `wakeup`，使退役后的兼容读存在分叉。
- **影响 / blast radius**:所有 `cron|loop|monitor|shell` mechanism、CLI direct handler / `--force` 路径和三 host Stop gate 都受影响。最坏结果是 orchestration 认为有安全网而合法等待，但根本没有可追踪、可 recon、可退役的外部 job；legacy 缺 handle 记录若直接升 hard invariant，又会让任何无关 board 写入死锁。
- **处置(含兼容路径)**:**candidate 已修**——registry 将 `--job-id` 设为 required，mutation 对 trimmed nonblank handle 再守一次（direct handler / `--force` 不可越），失败发生在落盘前且 board byte-identical。engine 对 canonical `watchdog` / legacy `wakeup` 缺或空 handle 只发 `FMT-WATCHDOG` warn；`status --json` 保留原对象字段并派生 `health`，missing/blank → `missing-accountable-handle`、过期 → `expired`，都 `armed:false` 且给出 `disarm → 创建真实 wakeup → 带 --job-id 重 arm` action。三 host hook 同口径 fail-closed。`disarm` 删除 canonical `watchdog` 与 legacy `wakeup` 整字段，结果 ABSENT，不留 null/空对象。CLI help、using-ccm、master-orchestrator、pacing overlays 与 ADR-011 同步为「先创建真实机制拿 handle，再 arm」。**蒸馏判定=可执行不变式进 engine/CLI/hook，操作真相进 using/master references；不新增 judgment-bearing 红线**，因为是否选择哪种 mechanism 仍是现有判断，而 handle 是否真实可由写入关卡机械强制。
- **验证**:SDD 先改 host-neutral verify-board CONTRACT / capability card；RED 稳定命中 engine 2、CLI source 9、真实 binary 2、Claude/Codex/Cursor 各 2 个缺/空 handle 断言。GREEN 后 focused engine 40/40、CLI 116/116、真实 binary 35/35（含 legacy status + disarm 双字段 ABSENT endpoint）、Claude 36/36、Codex 17/17、Cursor 23/23；full gates、投影同步与进程/临时夹具审计见同候选交付报告。
- **严重度 / 来源**:should-fix(静默失败安全网的正确性与可退役性；未触碰 narrow waist)/ 一手(dogfood 问题复现 + clean-main SDD/TDD,2026-07-14)。

## Finding #94 — board-guard 只记“本段有重定向”却不记目标，把 `ccm --board REAL >/dev/null` 误判成 shell 手写 board · should-fix(合法写关卡被硬拦·candidate 已修)

- **现象 / evidence**:Codex dogfood 的两条合法多行命令都被 `board-write-single-path` 硬拒：① 先用 `FIRE=$(date ...)` 生成时间，再执行 `ccm watchdog arm ... --board <abs-board> >/dev/null` 和 `ccm watchdog status`；② `ccm task update ... --log '... PR #124 delivery; ...' --board <abs-board> >/dev/null` 后接 `ccm board lint ... | sed ...`。去掉第二条 log 里的 hash-sign/分号会让安装中的旧 `0.16.0` 缓存偶然放行，但 current-main 三宿主 quote-aware lexer 仍稳定误拒两个完整命令。
- **根因**:旧缓存先有 quote-unaware segment split，quoted `;` 会把 ccm argv 错切；current-main 已关闭该层，但 `lexShellCommands()` 仍只产出 `hasRedirection:boolean`。`commandWritesBoard()` 看见同一 ccm segment 同时有 `--board <real-board>` 与任意 `>`，就把 argv 中的 board 错当成 redirect target；它无法区分 `ccm ... --board board >/dev/null` 与真正危险的 `ccm ... > board`。`FIRE`、`#124` 本身都不是 production 根因，只是现场命令形状的一部分。
- **影响**:board-guard 阻断本应唯一合法的 ccm 写关卡，使 orchestrator 无法静默运行 watchdog/task 更新；若为消误报整体豁免 ccm，又会放过由 shell 执行的 `> board` / `>> board` 真手写，破坏 ADR-025。
- **处置(含蒸馏判定)**:**candidate 已修**——三宿主 bounded lexer 在既有 words/quote/segment 模型上只增加 `redirectionTargets[]`；仅当真实 command word 是 `ccm` 时按 redirect target 判定，board 仅作为 `--board` argv 且输出去 `/dev/null` 放行，任一 redirect target 是真实 board 仍拒。非 ccm 的保守规则不动，`tee`、Node/fs 真写和 nested `bash -c` counterfeit 继续 fail-closed。两个现场命令以 temp home/abs board 在 Claude Code、Codex、Cursor 同一 focused suite 精确 RED→GREEN；source-to-adapter 三端生成物同步。**蒸馏判定=回流 hook CONTRACT + 三宿主 executable teeth，不进 runtime skill prose**：用户侧“board 只走 ccm”纪律未变，本次只是 guard 把 shell effect target 与 ccm argv 混淆的实现缺陷。
- **严重度 / 来源**:should-fix(合法 ccm writer 被 PreToolUse hard deny；安全闸不应靠用户改写日志逃生)/ 一手(本 board watchdog arm 与独立 review task update 真实命令,2026-07-15)。

## Finding #95 — `done + verified` 只证明 candidate acceptance，不能证明 target delivery，deps 可消费 stranded implementation · must-fix(交付/依赖真相·declared-mode v1 实现中)

- **现象 / 证据**:对冻结 main `01dc8967e0be691d4ae1b38ce31bdf4a48bf63a3` 与同一运行 ledger 的只读审计中，五个 `status=done + verified:true + artifact` task 有两个 accepted runtime stack 仍不在 target main：native-attempt 的五 commit candidate 与 synthetic supervisor 的六 commit candidate；它们分别已有 19/21 个 downstream descendants。相邻 three-origin、Cursor contract-only、quota reservation 则分别以 exact、contract-only、reviewed/hardened 形态实际进入 main。审计材料 SHA-256 为 `d573fa93156ac3a95d0ca86746956b9f39042eb63a0027a807b6a3b5eeb7febf`。
- **根因**:`taskTrulyDone` 正确表达 candidate endpoint acceptance，但 legacy `dependencySatisfied` 对普通 task 只看 `status=done`；`readySet` 与 `reconcileGating` 复用该谓词。board 当前没有 target registry、current candidate delivery observation 或 downstream edge requirement，因而无法区分“候选验收通过”和“已进入消费者基线”。branch/worktree 只描述工作位置，不是 immutable delivery proof。
- **影响**:downstream 可把只存在于 topic branch/worktree 的实现当 target baseline 消费；worktree 删除、target ref 漂移、squash/rebase 或对象 GC 又会掩盖/改变真实可消费性。错误会被自动 reconcile 固化成 `ready`，跨 compaction 后尤其难追溯。
- **处置(含蒸馏判定)**:用户已批准 D1–D4，采用 [ADR-036](../adrs/ADR-036-declared-delivery-dependency-truth.md) 与 tracked contract：board `delivery_contract`、upstream `delivery`、downstream `dependency_requirements` 均为 ✎ flexible；candidate-complete 保持 `taskTrulyDone`，target-delivered 只认本地 immutable containment/digest/review binding，dependency-qualified 是三值派生结果；review reject 优先 fail-closed，retry 原子归档并清 current proof，waiver 仅 user-authorized + exact edge/target + expiring 且永不冒充 delivery。历史 board exact legacy；本轮只实现 declared 与 strict read-only dry-run，绝不打开 strict-default、迁移真实 board或隐式 fetch。**蒸馏回流 = engine/CLI contract + `using-ccm` 派生手册，不新增 master-orchestrator skill 纪律**：本次缺的是确定性机制与操作面；若后续 pressure evidence 证明 agent 仍把 done 合理化为 delivered，再另按 skill TDD 加最小决策锚。
- **验收**:同一语义 fixture 中 native/supervisor 的 main-required edge 保持 blocked；exact、contract-only、reviewed/hardened delivery 按各自声明 scope qualified；target drift/missing object/review reject/retry/non-git digest/waiver authority+scope+expiry/cycle+malformed+cap/no-network/dry-run+lock 均有 RED→GREEN，legacy output 与 ready/reconcile 逐字兼容。
- **严重度 / 来源**:must-fix(交付与依赖真相；错误 ready 可扩散到后续 integration)/ 一手(2026-07-14 冻结 main + ledger 只读审计，用户批准 D1–D4)。

## Finding #96 — PHIP SSOT content contract 把 gitignored plan 当发布权威面，本地审阅草稿可污染全仓验收 · should-fix(测试输入边界·已修)

- **现象 / 证据**:`tests/content/xh-c3-subscription-phip-ssot-contract.test.mjs` 递归扫描 `design_docs/` 下所有 Markdown；本地 gitignored 的 `design_docs/plans/xh-c3-subscription-phip-ssot-alignment-independent-review.md` 含 adversarial alternate-authority marker 后，根测试 106/107、唯一失败为该草稿制造的 unapproved marker、duplicate authority 与 exclusive-anchor 重复。该文件按仓库契约不会进入版本控制，也不是可发布 authority surface。
- **根因 / 影响**:测试以“工作树目录里存在”为输入成员资格，混淆了临时审阅证据与 tracked/publishable 合同面；结果依赖开发机残留草稿，干净 CI 与本地端点不再同构。若简单按 marker 或固定路径豁免，又会弱化正式文档中的重复权威 fail-closed 检查。
- **处置(含蒸馏判定)**:**已修**——authority discovery 改由 Git 枚举 `design_docs` 与 `plugin/src/hooks` 下 tracked 文件和非 ignored 的 untracked 文件；因此待提交正式文档仍立即受检，ignored plan 不进入候选集。mutation fixture 初始化真实 Git ignore/tracking 语义，并锁住两侧回归：tracked alternate owner 仍同时触发 unapproved/duplicate 拒绝；ignored hostile plan 即使含同一 subject 与 exclusive anchor 也不影响通过。**蒸馏判定=回流 content-test harness + finding，不回流 runtime skill prose**：`AGENTS.md` 已明确 `design_docs/plans/` gitignored、与正式 `design_docs/` 分离；缺口在测试未消费既有边界，不是 orchestrator 行为指导不足。
- **严重度 / 来源**:should-fix(本地全量验收稳定性 + authority contract 输入真实性·已修)/ 一手(cross-harness board dogfood 全量测试，2026-07-16)。

## Finding #97 — `kimi -p` headless 单发与 `-y`/`--auto` 权限 flag **互斥**，harness 事实文档记反了：`-p` 自身即自动执行工具，叠权限 flag 反而秒退 exit 1 · should-fix(harness worker driver 事实失真·已修)

- **现象 / 证据**:master orchestrator 实操用 `ccm worker run --harness kimi-code -- ... -p '<prompt>'` 派 kimi 做开发时，按 `design_docs/harnesses/kimi-code.md` 旧记载给 `-p` 叠权限 flag 反而失败。本轮在临时目录亲测复现三个对照(2026-07-17，本机 kimi 0.26.0 / ccm 0.21.0，prompt 均为「在当前目录创建 probe.txt 写一行 hello」)：① `kimi -p '…'` **单独**(不加权限 flag)→ exit 0，`probe.txt` **被真实创建**、内容 `hello`；② `kimi -p '…' -y` → `error: Cannot combine --prompt with --yolo.`、exit 1、**不产出任何文件**；③ `kimi -p '…' --auto` → `error: Cannot combine --prompt with --auto.`、exit 1、不产出文件。即 `-p` headless 单发**互斥于** `-y`/`--auto`，且 `-p` 单独就以非交互模式自动执行工具(含写文件)。
- **根因**:`-y`/`--yolo`(全自动批准)与 `--auto`(auto permission mode)是 **交互(TUI)模式**下的自动批准开关；`-p` headless 单发无 TTY，kimi CLI 直接把二者判为与 `--prompt` 互斥并在参数解析阶段秒退。旧文档(§旗标表 + §10 Approval + §13 worker driver 落点)沿用了「`-p` 非交互需批准的工具调用会阻塞/被拒，须叠 `-y`/`--auto` 或 config allowlist」这一**未经实测的假设**，与 kimi 实际行为(headless 自动执行工具、且拒绝叠权限 flag)完全反向。
- **影响**:harness 事实是 ccm worker driver 组装 kimi provider argv 的权威依据；照旧文档给 `-p` 叠 `--auto`/`-y` 会让每次 kimi headless 派发**秒退 exit 1、零产出**，orchestrator 误判为「kimi 不可用 / 任务失败」，实则只是多叠了一个互斥 flag。反向地也会让人误以为 `-p` 不叠 flag 就不会执行工具、白等一轮。
- **处置(含蒸馏判定)**:**已改** `design_docs/harnesses/kimi-code.md`(旗标表 `-y`/`--auto` 行 + §10 Approval 节 + §10 结论行 + §13.1 worker driver headless 调用形态)，准确记载「`-p` 单发自身即以非交互模式自动执行工具(含写文件)，无需也不能加 `-y`/`--auto`(组合报 `Cannot combine --prompt with --yolo/--auto` 且 exit 1)；config `[permission.rules]` allowlist 仍是可选的额外约束层」，逐句标 `[tested]`。**分发 skill 无需改**:`using-ccm/references/command-catalog.md` 的 `ccm worker run` 形态用 `<按 worker help 组装的完整 provider argv...>` 占位、并显式声明 ccm 不解析/补写 provider 的 permission/prompt flags(抗版本漂移设计)，从不硬编码 kimi `-p`/`-y`/`--auto` 组合，故无错误形态可改。**蒸馏判定=回流 harness 事实文档，不回流 runtime skill prose**:缺口是 harness 机制事实记错、非 orchestrator 行为指导不足;skill 已正确地把 provider argv 组装外包给 `ccm worker help`(读本机真实 agent help)。
- **严重度 / 来源**:should-fix(harness worker driver 事实失真:照旧文档派 kimi 必秒退零产出·已改)/ 一手(orchestrator 实操派 kimi 开发暴露 + 本轮临时目录三对照亲测复现，2026-07-17)。

## Finding #98 — agent 花名册只教「凡派发皆登记」前半段、缺「凡收割皆收口」后半段：收割 agent 产出后漏 `agent terminal`，roster 永久 `running` 堆积污染 recon;pressure baseline 强模型天花板(RED 未复现)·should-fix(生命周期闭环指导缺位·结构性补齐)+ ✅正向(诚实不造红线)

- **现象 / 证据**:一场真 orchestration(board `20260716T162115Z-175610`)的花名册里多个 agent 长期停在 `running`/`starting`,实际工作(PR merged、父 task `done+verified`)早完成;本 session 手工 `ccm agent terminal` 收掉 6 个(agt-019/026/027/028/029/030)才止血。task 侧干净(6 个 task 全 `done+verified`),agent 侧却全 `running`——`ccm agent list` 因此说谎(声称 6 个 worker 在跑,实为 0)。
- **根因**:分两层。① **by design(非 bug)**:task/agent/attempt 三层刻意解耦,`agent terminal ≠ task done`——ccm **故意不**在 task done 时自动 terminal agent(防 agent 终态被误读为 task 验收)。这一层正确,**不改**。② **真 gap(结构性缺位)**:「凡派发皆登记」(create/bind/link 前半段)在 SKILL A cross-harness 热路径 / 镜头3 / using-ccm §C.6 都重点讲,但 `agent terminal` **只在 spawn 失败**语境出现(「spawn 失败则收掉 `starting` 登记」),**成功收割后的收口后半段无指导**——没把「收割 agent 产出即 terminal」纳入决策程序的 recon 环节。于是 orchestrator 登记了却收割后忘收口,`running` 永久累积,污染 recon 的 in_flight/phantom 判定。关键误区:`ccm agent probe` **只 reconcile 死活**(`gone→orphaned`/`silent→uncertain`/`alive→running`),**永不产生 `terminal`**——「probe 会自动清」是假的。
- **pressure baseline(TDD-for-skills·RED 未复现·强模型天花板)**:2/2 capable-model run 在三压(exhaustion + sunk cost + deadline)下被强制 A/B/C 选择「收割完 6 个 agent 后要不要 `agent terminal`」,**都选 A(收口)** 且**主动驳斥** B(「roster 只是簿记、跳过省时」)与 C(「probe 会自动清」)——两个受试都从既有三层解耦模型自洽推出「roster 在说谎、probe 永不 terminal、收口是我的判断」。即**没有一条 rationalization 在强模型下稳定把「跳过收口」合理化掉**(不同于 [[Finding #46]] phantom in_flight、[[Finding #44]] 假串行边那类压力下反复复发的真红线)。据此**绝不编造一条强模型不需要的红线 / Rationalization-Table 行**(编造未发生的失败 = skillsmith Iron Law 下的谎·同 [[Finding #82]] 天花板处置)。真失败的机制是**长 session 流程中的注意力遗漏 / 跨 compaction 蒸发**(念头压根没在忙碌的 recon 里浮现),**非**受压下被辩护的 rationalization——对应 pressure-testing §7 诊断三(组织缺口):正确修法是**在每回合重注的 recon 步骤里加一个结构性对账锚**让念头必然浮现(同 SKILL A 已给 phantom in_flight 的结构化待遇),不是加纪律红线。
- **影响**:roster 永久 `running` 僵尸让 viewer / resume 后的自己 / recon 的 in_flight/phantom 判定全部数错在跑的 agent;长程编排里越积越多,污染「谁真在跑」的地面真相。
- **处置(含蒸馏判定)**:**已补**(结构性锚 + 机制兜底,均不造红线)——① **SKILL A**(每回合重注的魂·最小锚):决策程序 §4.0 recon 项 (e) 加「反向闭环」同拍对账(收割 agent 产出即 `ccm agent terminal` 收口·`agent terminal ≠ task done` 只挡正向·probe 永不 →terminal·漏收口污染 in_flight/phantom 判定),cross-harness 热路径的 terminal 指导句从「只 spawn 失败收口」扩为「成功收割同理收口」(对称的另一半);细则下沉不塞魂。② **using-ccm**(操作手册·详):`references/board-model-guide.md` §C.6 加「收口闭环纪律:凡派发皆登记,凡收割皆收口」段 + probe-永不-terminal 误区 + 批量收口安全段;`references/command-catalog.md` `agent terminal` / `agent list` 节同步;`SKILL.md` footgun 表加两行。③ **ccm 机制(defense-in-depth·已实现)**:`ccm agent list` 新增只读 stale-running advisory——active-state agent 的 linked task 全 `done` 就列为「疑似收割未收口」候选(json `stale_candidates`·human advisory 行),**绝不自动 terminal**(保持三层解耦·收口终态判断仍归 orchestrator);落在 recon 的 roster-rebuild 触点(`ccm agent list`)把候选直接交到手上,机械兜住注意力遗漏。**回流落点**:SKILL A(master-orchestrator-guide)recon 锚 + using-ccm 操作手册 + ccm `agent list` advisory。**不回流为红线**(RED 缺席·守 Iron Law·天花板诚实不造纪律;结构锚 + 机制兜底针对的是注意力遗漏,非受辩护的 rationalization)。
- **附带(batch-race 措辞精确化)**:同 PR 精确化 using-ccm 的锁竞态措辞——`ccm agent create/bind/link` **复合三连**才需 one-at-a-time(bind/link 吃 create 返回的 `agent_id`·靠数据依赖定序;跨 agent 一个三连做完再下一个);**同一命令重复**(已知 id 的批量 `agent terminal`/`probe`)**顺序 bash 背靠背跑天然无 race**——每条 ccm 写各自抢一次 O_EXCL board 锁串行化,不必也别 `&` 后台并行(并行 ccm 写争锁 exit 4)。避免把「所有 agent 操作都必须逐个小心防锁竞态」的过度保守心智传下去(落 board-model-guide §C.6 + footgun 行)。
- **严重度 / 来源**:should-fix(生命周期闭环指导缺位·结构性补齐)+ ✅正向(强模型天花板下诚实不造红线·结构锚 + 机制兜底替代纪律膨胀)/ 一手(真 orchestration 花名册 6 agent 卡 running 手工止血 + 本轮 2/2 pressure baseline RED 未复现,2026-07-17)。

## Finding #99 — `ccm worker run --cwd <相对>`(如 `.`)被 cwd 校验在 executable 解析**之前**硬拒 → 吐 `executable:null`+`request_rejected` 秒退,被误诊为「kimi driver executable 解析回归」;真因是四 harness 通用的相对-cwd 拒绝(help 路径用 `process.cwd()` 恒绝对故永不触发,制造「help 能跑 / run 挂」的假不对称) · should-fix(worker run 通用可用性·已修) + ✅正向(端点复现纠正误诊)

- **现象 / 证据**:master orchestrator 派 kimi worker 失败,backlog 记为「kimi driver `executable:null` 回归」,靶心猜作「`worker help` 能解析 kimi executable、`worker run` 路径解析成 null」。本轮端点复现纠正误诊——① `ccm worker run --harness kimi-code --cwd . -- --version` → `executable:null` + `error:{code:"request_rejected",message:"--cwd must be an absolute path"}` + exit 1(installed rc0 与当前源同);② **改用绝对 cwd** `--cwd /data/qiwei/repos/cc-master` → `executable:"/data/qiwei/.kimi-code/bin/kimi"` + exit 0,真实跑起;③ 同一 `--cwd .` 对**四个 harness 全部**复现同一 `executable:null`+`request_rejected`(claude-code / codex / cursor-agent / kimi-code 逐个证),**非 kimi 专属**;④ `worker help` 之所以「能跑」是因它在 `handlers/worker.ts::commonRequest` 里硬用 `process.cwd()`(恒绝对),`worker run` 才透传用户 `--cwd`——这就是「help 路径 vs run 路径差异」的真身:差在 **cwd 来源**,不在 executable 解析。
- **根因**:`worker-process.ts::validate` 首行 `if (!path.isAbsolute(request.cwd)) return '--cwd must be an absolute path'` 在 executable 解析**之前**硬拒任何相对 cwd;拒绝走 `fail(result,'rejected','request_rejected',...)` 早返回,此时 `result.executable` 仍是初始 `null`。于是「相对 cwd」这个真错被包装成一个 `executable:null` 的信封——调用方只看到 `executable:null`+exit 1,把它误读成 executable 解析回归。而 `worker run` **省略 `--cwd` 时默认 `process.cwd()`**(绝对·可跑),却把**显式 `--cwd .`**(语义等价「当前目录」)拒掉——**同义两种写法一个能跑一个秒退**,这个不一致本身就是 bug。executable 解析(`resolveExecutable('kimi')` → `executableOnPath` 搜 PATH)全程无回归:kimi 在 PATH 上,绝对 cwd 下四 harness executable 全部正确解析。
- **影响**:任何按直觉传 `--cwd .`(或任何相对路径)派 worker 的调用都秒退零产出,且错误被 `executable:null` 掩盖成「driver/executable 坏了」,诱导往错误方向排查(查 harness driver、查 PATH、查 kimi CLI),浪费一整轮诊断。真实 orchestration 里后台 shell 常以相对 cwd 起 worker,命中面不小。
- **处置(含蒸馏判定)**:**已修** `ccm/apps/cli/src/worker-process.ts`——`runWorkerProcess` 入口把 `request.cwd` 经 `path.resolve()` 解析成绝对(相对则按启动 ccm 的进程 cwd 锚定,与「省略 --cwd 默认 process.cwd()」同源),`validate` 改校验解析后的绝对 cwd(仅剩「须是存在的目录」),child 仍拿到 realpath 归一的绝对 cwd。core 的「须绝对」契约不再对用户可见地咬人,却仍对「解析后仍非存在目录」保留 `request_rejected`(防跑错目录)。**回归测试**加在 `worker-raw-passthrough.test.ts`:新增 `worker run resolves a relative --cwd against the process cwd instead of rejecting it`(四 harness `--cwd .` 均 executable 非空 + `state:exited` + exit 0 + spawn cwd = realpath(process.cwd())),并把旧 `bad cwd` 用例从 `'relative'`(语义已变)改为解析后不存在的目录名以保持「坏 cwd 仍拒」的意图清晰。**端点自证**:patched 源经 tsx 真跑,四 harness `--cwd .` 全恢复 exit 0;`pnpm -C ccm typecheck`/`lint` 绿 + worker 测试 13/13 绿。**蒸馏判定=纯代码修 + 一条镜头素材,不回流红线**:这是 CLI 边界「同义写法一致性」的 correctness bug,不涉及 agent 判断力;但它是「**看似 X 实为 Y** / **诚实端点复现纠正 backlog 误诊**」的又一活证据([[Finding #61]]/[[Finding #79]] 家族)——backlog 的「executable 解析回归」假设若照单全收去改 harness driver 就南辕北辙,唯端点亲复现 + 逐 harness 对照才逼出真因。
- **附带(kimi model k3 处置)**:backlog 另记「`kimi -m kimi-code/k3` 报 `provider.api_error: 404 Not found the model k3 or Permission denied`」。本轮查证:① **ccm 从不注入 `-m`**——`worker run` 是 raw argv 透传(commit #133),`-m kimi-code/k3` 是调用方 argv,ccm 忠实透传、无组装 bug;② 端点复现:经 `ccm worker run --harness kimi-code` 真跑,**default 模型(不带 -m)与显式 `-m kimi-code/k3` 现均 exit 0**、返回有效响应(`• KIMI_OK`),**404 不复现**——属 provider 侧瞬态权限/可用性(账户 managed OAuth 侧 k3 可用性变化),非 ccm 代码问题、非可 touch 面(禁碰凭证/config)。config `~/.kimi-code/config.toml` 的 `default_model="kimi-code/k3"` 映射 provider model `k3`;同 config 另有 `kimi-code/kimi-for-coding`(-highspeed)可选。**无 ccm 代码改动**;若 k3 再 404,是账户/provider 权限层面事,与 worker driver 无关。
- **严重度 / 来源**:should-fix(worker run 相对 cwd 通用可用性·四 harness 通用·已修 + 回归测试)+ ✅正向(端点复现纠正 backlog 误诊「executable 回归」→真因「相对 cwd 拒绝」)/ 一手(reset 后自驱验证四 harness worker run + 端点逐 harness 对照复现,2026-07-20)。

## Finding #100 — `ccm <verb> --board <裸ID>`(非完整路径)按 cwd-相对路径解析并 exit 5;`--json` 读命令虽返回 `{ok:false,exit:5,error}` 但只取成功形状的解析器把它当「空 roster」→ 误读成 agent 不存在 · should-fix(CLI board-ID 解析 UX footgun·backlog 候选) + ✅正向(recon transcript-ground-truth 抓到 phantom in_flight 生效)

- **现象 / 证据**:watchdog recon 复查 board `20260716T162115Z-175610` 两个 in_flight,先用 `ccm agent list --board 20260716T162115Z-175610 --json` / `ccm agent show <handle> --board <裸ID>`——经只取成功形状的 node 解析器**打印为空**,初判「agent 不在注册表」。直跑 `ccm task done ... --board <裸ID>` 才暴露真错:`error: --board path is missing or not valid board JSON: /data/qiwei/repos/cc-master/ccm/20260716T162115Z-175610`(exit 5)——ccm 把裸 ID 当**相对 cwd** 路径。`--json` 形态实为 `{"ok":false,"exit":5,"error":"--board path is missing…","violations":[]}`(非协议级静默,是解析器只认成功 shape 才显空)。改用完整路径 `--board /data/qiwei/.cc_master/boards/20260716T162115Z-175610.board.json` 后 `task done`/`task update`/`watchdog arm` 全 exit 0 落盘。设 `CC_MASTER_HOME=/data/qiwei/.cc_master` 后裸 ID **仍不解析**(同样 cwd-相对失败)。
- **根因**:`--board` 是**路径参数**、无 home-相对的 board-ID 解析层;裸 ID 拼到 cwd 下找不到即 exit 5。二次陷阱在**消费侧**:`--json` 返回结构化 error 对象,但常见「只取 `.agents`/成功字段」的解析器把 error 形状读成空集合,把「board 没解析到」误读成「roster 空 / agent 不存在」。
- **影响**:任何按直觉用裸 board-ID 的 ccm 调用都失败;写命令 exit 5 尚 loud,读命令经天真解析器却显「空」→ 可诱导「agent 不存在→当 phantom→重派」制造真重复派发。长 session / recon 高频用 board-ID,命中面不小。SessionStart 注入的永远是完整路径,照注入路径走不踩,手敲裸 ID 才踩。
...
## Finding #106 — viewer agent-stream 无源文案把「绑定缺口」误报为「agent 类型不支持」:codex agent 以 task-id handle 登记(未走 `ccm worker dispatch`)、无 transcript_ref → 无源可 tail,旧文案「no readable stream source for this agent type yet」诱导用户误判「只有 claude 的 stream 正常、codex/kimi 解析渲染坏了」· should-fix(归因文案误导) + ✅正向(e2e 先证伪解析/渲染 bug 再定位真因)

- **现象 / 证据**:用户报告「只有 claude harness 的 stream 正常显示」。e2e 复盘:空项目 + 隔离 home,`ccm worker dispatch` 真实派发 codex(`exec --json`)/kimi(`--output-format stream-json`)/claude 三 worker → `/agent-stream.json` 三 harness 全部正确归一化(11/6/5 events)→ 浏览器实测三个 stream 抽屉全部正常渲染。真因:用户真实 board 全部 20 个 agent 里唯一的非 claude agent(codex)handle 是 `task-id` 且 `transcript_ref:null`——viewer 拿 task-id 定位不到 codex rollout 文件,显示旧文案,读者按字面理解成「codex 类型不支持」。
- **根因**:①捕获侧:codex worker 未经 `ccm worker dispatch` 派发(经 origin 后台 shell + 手工 `agent bind --handle task-id:…`),session-id 从未被 ccm 观测到 → 无从定位原生 transcript;②文案侧:`buildAgentStream` 无源兜底 reason 把「这条记录没绑上源」(操作者可修)与「该类型无可读源」(能力缺口)合并成一句偏后者的话——归因错位放大成对整条 harness 能力的误判。叠加历史 #178(same-version 缓存遮蔽 VIEW STREAM)加重了「codex/kimi 一直坏」的印象。
- **影响**:多 harness 编排下,任何非 dispatch 路径登记的 cli worker 在 viewer 里都呈现为「类型不支持」,操作者既不知道能修、也不知道怎么修;对产品的「跨 harness 可观测」承诺构成假阴性。
- **处置(含蒸馏判定)**:**已修**(ccm patch·PR #206)——`noSourceReason` 归因分流:可流式 harness 点名绑定缺口(含 handle kind)+ 双操作出口(`ccm worker dispatch` 自动绑 / `ccm agent amend` 补绑 session-id 或 transcript);cursor 系点名 SQLite `state.vscdb` 不可 tail + `CURSOR_TRANSCRIPT_PATH`/`--transcript` 出口;AgentInspector fallback 同步。**蒸馏判定:回流 A(master-orchestrator-guide)的 dispatch 指导已覆盖**——cli worker 一律经 `ccm worker dispatch`(自动绑 session/transcript),不回流新增 prose;文案层修复即根治,本条入账留痕。方法论正向:先用受控 e2e 证伪「解析/渲染 bug」假设,再回到数据(真实 board 的 agents[] handle 形状)找真因,避免了在解析器里瞎修。
- **严重度 / 来源**:should-fix(归因文案误导·假阴性)+ ✅正向(e2e 证伪先行)/ 一手(stream e2e 排查,2026-07-23)。

## Finding #107 — native subagent 的 viewer 流式登记配方是 guidance 黑洞且曾错误全局化:分发 skill 只教了 session-id(CLI worker)/pid(bg shell)/task-id(泛「有 transcript 的派发」)三形,没教 host-specific 的父锚 + task-id 派生契约；初版补丁又把 Claude Code 的 `subagents/agent-*.jsonl` 规则复制给 Codex/Cursor/Kimi → orchestrator 会自创坏 handle 或套错 host 布局,子 agent stream 全黑/串成父流 · should-fix(guidance 缺口 + host 事实泄漏 → 观测面假阴性/误归因)

- **现象 / 证据**:真实 board(20260717T181340Z)上全部 origin/session-id agent 打 `/agent-stream.json` 均「transcript file not found yet for this session」;其 handle value(如 `a2cdfca607de307f2`·17-hex agentId)被定位器当会话文件名去 `projects/<slug>/` 下找,永远不存在。同机以正确形状直调 `buildAgentStream`(`task-id:<agentId>` + `--transcript <父.jsonl>`)立即返回 16 个事件的完整子转录流——机制无 bug,登记形状错。
- **根因**:Claude Code 的派生契约(子转录落 `<父转录去.jsonl>/subagents/agent-<agentId>.jsonl`,需「父转录 + agentId」两原料)只写在 `ccm/apps/cli/src/agent-probe.ts` 注释里；`using-ccm` 的 handle.kind 表与 command-catalog agent bind 节都没有 native subagent 条目。agent 面对「subagent 有个 id」自然套了最像的 `session-id` 形状。初版修复直接写 canonical，又违反 host 事实本地化：Kimi 的真实布局是同 session 下 `agents/main/wire.jsonl` + `agents/<agentId>/wire.jsonl`，Codex/Cursor 的父→子映射尚未实证，不能共用 Claude 规则。另有运行时启动竞态错误：派生子文件不存在时回退父 transcript，把 master 事件错误标成 child。
- **影响**:经 native Task 派生并手工登记的 subagent 可能恒无源，也可能在启动竞态中显示父 agent 内容；Kimi 即使给了父 main wire，也只会渲染 main agent 而非目标 Task 子 agent。与 Finding #106 的误导文案叠加成「只有 claude 主会话正常、其他都坏」的整体误判；「凡派发皆登记」的观测承诺对 subagent 形同虚设。
- **处置(含蒸馏判定)**:**已修**(runtime + plugin·同 PR #206)——canonical 只保留两个 SAP slots，四 host 分别投影登记契约：Claude Code 以父 session JSONL + `task-id:<agentId>` 派生 `subagents/agent-<agentId>.jsonl`；Kimi Code 以父 `agents/main/wire.jsonl` + Task 返回的完整 agentId 派生 `agents/<agentId>/wire.jsonl` 并复用 typed-wire parser；Codex/Cursor 对未实证父→子布局 fail closed，只允许精确子 transcript/log。定位器不再用父 transcript fallback，子文件未落盘时返回明确无源，避免父事件泄漏；新增 runtime fixtures 与四 host projection content contract。**蒸馏判定:已直接回流 D(using-ccm)两份 reference 的 per-host slots**,不另设 skill；深层教训是机制注释里的调用契约必须进入操作手册，但 host-native 路径只能进 adapter overlay，不能写 canonical。
- **严重度 / 来源**:should-fix(观测面假阴性·guidance 缺口)/ 一手(stream e2e 排查 · 真实 board 数据 + buildAgentStream 直调对照,2026-07-23)。

## Finding #108 — K3-01P 的最小分发边界被派生为 public topology、independent oracle 与 sealed release protocol blocker，连续 review 扩张让当前 slice 停在自造 acceptance 上 · high(范围失控·交付阻断)

- **现象**：K3-01P 的 acceptance 已明确要求“四 host dist、package staging、最终 archive 无 `knowledge/**` + required CI 机械扫描”，并明确写着“dist Markdown 不重建同图；independent oracle / sealed release protocol 不作为本任务 blocker”。但同一 board 随后新增 TX0–TX5：要求 P1–P8 transaction、独立 snapshot oracle、sealed four-host release manifest，以及 private/public 双图各自的 H gate；独立 review 的 `REQUEST-CHANGES` 又直接生成下一轮双 oracle / sealed CI blocker。原本已有简单扫描可验的边界任务，被连续改写成新的协议与产品面。
- **根因**：当前 slice 没有锁住 acceptance 的授权来源，也没有把 non-goals 原样交给 reviewer。实现者把“更完整的信任模型”当成用户需求，reviewer finding 又未被要求逐条映射原 acceptance；连续 `HIGH` / `CRITICAL` 后也没有 restart threshold，于是每轮 finding 都扩大系统而非收敛最小交付。
...
## Finding #116 — Agent 的 worktree 隔离从 main 开而非调用方当前分支,派出去的两个 worker 都看不见已提交的分支改动;一个停下质疑(诊断可回收),一个自行绕过(产物报废) · should-fix(派发前提失配) + ✅正向(质疑型 worker 的落点分析原样回收,省一轮摸索)

- **现象 / 证据**:在 `feat/failure-mode-schema` 上提交 `8aa35c28`(schema 加必填 `failure_mode` + 47 个模块文件 250 个点全部落字段)之后,派两个 `isolation: "worktree"` 的 agent 做下游任务。两者都报前提不成立。`git worktree list` 逐个查 HEAD:两个 worktree 都停在 `0a3b0aeb`(**main**),不是我所在分支;在 worktree 的 schema 里 grep `failure_mode` 命中 **0**。两个 worker 的反应截然相反——**A 停下质疑**:「schema `$defs.point` 无此字段、250 点 0 标注、`failure_mode` 全仓只出现在三份设计文档里」,并附「这是设计提案未落地,不是已落地字段」的判断;**B 未质疑**,自己在 `graph.mjs` 图构建阶段解析母本散文、运行时给 `point.failure_mode` 赋值,把字段「造」了出来继续往下写。
- **根因**:两层。表层是 harness 行为——**worktree 从 main 开,不是从调用方当前分支开**,已 commit 但未合并的改动一律不可见,这不是配置项。深层是**派发单的写法**:我写的是「字段已落在 …,250 个点全覆盖」——这句对主树成立、对 worktree 不成立,而单子里**没有一条要求 worker 先自证前提**。缺了这条约束,B 的绕过在它的处境里甚至是「合理」的:任务要它按字段分桶而字段不存在,它就去把字段弄出来。**前提失配时,worker 会自动补齐前提,而不是停下问。**
- **影响**:两轮 agent 的 token 白烧。更实的是 B 的产物:运行时从散文解析字段,与「字段落在模块 JSON、母本只留理由」这条设计决策直接冲突——若未察觉而合入,等于亲手造出第二条真相源,正是本轮改造要消灭的东西。
- **处置(含蒸馏判定)**:**已改走主树串行派发**(多写者时串行,避免互相读到中间态);B 的产物整个弃用、worktree 清理;**A 的落点分析原样回收进重派单**——`INVARIANTS` 是 `Array.from` 生成的(改数字而非改写成手写数组)、`graph.mjs` 里三条不变式可作结构范本、新检查该落 K1 及其理由(图不变式只在 `stage !== 'K0'` 时跑,而读该字段既不需要 git 清单也不需要对账新鲜度)——省掉重派后的一轮摸索。**蒸馏判定:回流 A(`master-orchestrator-guide`)的派发纪律**——现有指导讲了「派发要给足上下文」,没讲「**派发单必须让 worker 能自证前提成立,且明令前提不成立时停下报告、不许自行补齐**」。两个 worker 面对同一个坏前提给出优劣两分的反应,差别恰恰就在这一句的有无。落点:`references/dispatch.md` 的派发单一节(本轮未改 skill,记为待办)。与 [[Finding #111]] 同族——都是**不能问执行者要保证,得由派发侧给出可机械自证的判据**。
- **严重度 / 来源**:should-fix(派发前提失配) + ✅正向(质疑型 worker 诊断可回收)/ 一手(2026-07-31 失效类型落 schema)。

## Finding #117 — ✅正向:把一道假门变真,第一个被拦住的必然是自己;处置选「登记」而非「放水」,并给清单装上只许变短的反向检查 · ✅正向(门真了+债可见,两者兼得)

- **现象 / 证据**:收口 [issue #213](https://github.com/nemori-ai/cc-master/issues/213)——`run-tests.sh` 被 AGENTS.md §10 立为必过的第一道门却不在任何 workflow 里。把快层接进 required check 后,CI 当场红:4 个文件 14 条失败。独立复核(main `0a3b0aeb` 的干净 worktree 逐个跑)确认**全部是既有基线红**,与本次改动无关。**门刚变真,第一个拦住的就是引入这道门的 PR 自己。**
- **根因**:这不是意外,是必然——一道从未被执行的门,背后必定积着从未被看见的债。真正的岔路在处置:① 当场修(与引入门的改动无关,scope 爆炸) ② 让 CI 不 required(门又假了,回到原点) ③ 登记。
- **影响**:选错就前功尽弃。选 ② 等于把 issue 关掉却什么也没改变;选 ① 会让一个本该聚焦的 PR 膨胀成不可 review 的大杂烩。
- **处置(含蒸馏判定)**:**选 ③ 登记**,因为它是唯一同时满足「门是真的」与「债是可见的」的选项——这 14 条在 main 上已红很久,本次改动没让情况变坏,反而**第一次让它们有名有姓地被登记**。关键在于登记必须带牙齿,否则 quarantine 就是垃圾桶:落地物是 `tests/quarantine.txt` + `run-tests.sh --quarantine` 的**反向检查**——真跑清单里的文件,任何一条**变绿**却还留在清单上就是硬错误。**「通过」在这里是错误信号**,由此保证清单只许变短;nightly 每天执行。正负两向都验过(4 条全红→`QUARANTINE UNCHANGED`;混入一个绿测试→精确点名并报 `REGISTRY STALE`)。同期还立了第二份清单 `tests/heavy-tests.txt`(收「慢」而非「红」),两份语义互斥、同一文件进两份即硬报错。**蒸馏判定:回流 A(master-orchestrator-guide)的验收纪律**——现有「只信端点验收」讲的是别信自报,本条补的是另一面:**当你把一道声称存在的门真正接通,要预备好它立刻拦住你自己;此时唯一不倒退的选项是让债有名有姓,并给登记装上「只许变短」的机械保证。** 落点:`references/resume-verify.md` 端点验收一节(本轮未改 skill,记为待办)。与 [[Finding #114]] 互为正反面——那条记的是「门是假的会藏住真回归」,本条记的是「把门做真之后该怎么办」。
- **严重度 / 来源**:✅正向(门真了 + 债可见)/ 一手(2026-07-31 收口 issue #213)。

## Finding #118 — 派出去的 worker 在多 session 共享的工作树里放 process-wide `pkill` 模式匹配,等于隔空开枪 · should-fix(共享环境的越界副作用) + ✅正向(未造成实际损害,且 worker 自己招认了)

- **现象 / 证据**:一个 worker 收尾清理自己的等待器时,跑了 `pkill -f "until ! ps -eo args"` / `pkill -f "until ! pgrep -f"` 两条**进程全局**的模式匹配。它在报告里自己写明:「两个 skill-knowledge 测试进程仍在跑且**不是我的**……我的模式应当只匹配到自己的等待器 shell,但我**没有在杀之前验证这一点**,现在也无法排除误杀了兄弟会话的等待器。」实测未误伤——我当时起的基线对比用的是 `timeout 3000 node --test`,与它的 `until` 轮询模式不匹配,两个进程都活着。
- **根因**:`pkill -f <pattern>` 的作用域是**整台机器**,而 worker 的心智作用域是**它自己的任务**。这个落差在单 session 下不可见,在多 session 共享一个 repo 时立刻变成隔空开枪。与 [[Finding #111]] 同源——都是「worker 的自我认知边界小于它实际的作用范围」。
- **影响**:本次侥幸没中。但若模式再宽一点(例如 `pkill -f node --test`),会直接杀掉正在跑的基线对比,而那份数据当时正是判断「14 条失败是否我引入」的唯一依据——被杀后我很可能因缺证据而误判。
- **处置(含蒸馏判定)**:worker 自己给出了正解并已入其记忆:**按记录下来的 PID 精确杀,不要按命令行模式杀**。派发侧的补强:凡涉及清理后台进程的任务,派发单里明写「只杀你自己启动并记录了 PID 的进程;禁止 `pkill -f` 之类的全局模式匹配」。**蒸馏判定:回流 A 的派发纪律**,与 [[Finding #116]]「派发单必须让 worker 能自证前提」同属一族——**派发单要框住 worker 的副作用半径,不能假定它知道自己身处共享环境**。落点同 #116(`references/dispatch.md`,记为待办)。✅正向的一面:worker 主动招认了这个它本可以隐瞒的操作,这种自陈比事后追查可靠得多,值得在派发单里继续鼓励。
- **严重度 / 来源**:should-fix(共享环境越界)/ 一手(2026-07-31)。

---

## Finding #119 — 协调机制建成两侧:发的一侧自动化了,读的一侧零指导;实测收件箱积压 12 条,一条未读,中介兑现的价值当前为零 · high(设施空转·全静默无信号) + ✅正向(缺口被一句与协调无关的通用自检偶然撞上)

- **现象 / 证据**:另一场真实长程编排的 transcript(2026-07-28)。三块 board 并发活跃、心跳全是秒级,该 session **只盯了自己那块**,第三块正在改的面与它自己在改的面重合,它完全不知道其存在。`coordination inbox` 积压 **12 条通知,从未被读过一次**——全是前一日的配额警告(写入时 5h 95% / 7d 86%,被发现时实际已是 56% / 67%,全部失效)。缺口是被 `verify-board` 那句**与协调毫无关系**的通用完整性自检偶然撞上的:「is every to-do actually done — including any NOT yet listed on the board」。全仓能撞上跨板盲区的只有这一句。发现同侪之后,它做的最值钱的一步是算重叠面:`#525 ∩ #524` 十五个文件两侧都改,而 #525 当时正在解二十九个文件的冲突——**#524 若抢先合入,那二十九个里的十五个要再解一遍**。该 session 自己的结论:「这一轮自检查出的是跨 board 的调度盲区,比板内的任何一处都值钱。」
- **根因**:`coordination inbox` / `peers list` / `arbitrate` 机制层建模完整,`notify` 连消息 TTL 都想到了。**但决策侧对它的全部覆盖是 4 处命令名列举,零处教何时读。** 生产侧(arbiter 与 hook)一直在自动写,消费侧从未出现。先前的判断写成「inbox 永远是空的」是**错的**——inbox 是满的,没人读;补强方向因此从「教它发」翻转为「教它读」(而「教它发」这一半随后被点对点协商否决整体删除:编排者之间不谈判)。
...
## Finding #121 — 一轮测量里六次「装置缺陷冒充发现」,其中两次以「已复核的事实」上报给用户;共性不是粗心,是**每次都在对错误的层做比对** · high(假发现比无发现更贵·且会驱动错误决策)

- **现象 / 证据**:汰换台账 D 层(环境事实与接口的确定性比对)单轮内六次:①`cmd 2>&1 | head; echo rc=$?` 取到的是 `head` 的退出码(恒 0),差点写下「母本断言 exit 3 但实测 rc=0」;②只抓命名空间层 help,把 `ccm task done --help` 里的 `--review-verdict` 报成缺失;③正则从散文粘出 `--graph--biz` / `--legacy-wakeup` 等不存在的 flag;④用「看起来像命令语法」代理「被 help 覆盖」,把带意图注释的**动作序列**(`hotpath-flows`)算进重复,得出「19% 逐字重复」;⑤看 `head -12` 的截断表格,断言母本 `jc add` 漏了 `--set`/`--set-json`,**实为超集**,并据此写下「有损副本比没有副本更糟」上报;⑥拿**母本源码**对引擎 registry,把合法的投影占位符 `{{USING_CCM_FMT_POLICY_ROW}}` 判成「唯一一条真漂移」,**而已分发产物里那一行就在**。⑤⑥两次都写进了文档并口头上报为已核实事实。
- **根因**:六次的共性**不是粗心**,是**比对层错位**——每次的测量本身都正确执行了,只是指向了错误的对象。管道尾命令 vs 被测命令 / 命名空间 help vs 子命令 help / 散文 vs 代码 / 形状 vs 语义 / 截断视图 vs 完整视图 / **源码 vs 投影产物**。最后一条尤其结构性:SAP 架构下 `plugin/src` 不等于分发内容,中间隔着 adapter 投影,凡「分发内容是否包含 X」的问题**必须查 `plugin/dist`**,查源码必假。
- **影响**:①—④被复核挡住,只浪费时间;⑤⑥漏出去,成了向用户陈述的「已验证缺陷」,并被当作迁移决策的论据。修正后**本轮零漂移**——两次找漂移都以「无漂移」告终,`ccm` ⟷ `using-ccm` 锁步在 84 条规则上一条没漏。**这意味着「副本会骗人」这条论据在本仓没有任何实例支撑**,迁移的理由只剩结构性的那条。假发现不只是噪声,它会把决策推向一个证据并不支持的方向。
- **处置(含蒸馏判定)**:**蒸馏判定:回流两处**。①**回流 A 的验证纪律**——「只信端点验收」要补一句同型的:**先确认你在对哪一层做比对**;凡形如「X 里有没有 Y」的判断,先问「X 是源码、产物、还是运行时输出」,三者在本仓从不等价。②**回流本仓 dev 纪律**:凡「分发内容是否含 X」一律查 `plugin/dist/<host>/`,查 `plugin/src` 必假(SAP 投影 + slot 占位符)。**另记一条方法论**:①—④被挡住是因为每条结论都跑了复核,⑤⑥漏出去是因为恰好那两次没跑——**复核不是可选的收尾动作,它是结论成立的构成部分**;省掉它的那次,就是会漏出去的那次。
- **严重度 / 来源**:high(假发现驱动错误决策)/ 一手(2026-08-05,汰换台账 D 层)。

---

## Finding #122 — 「改一段已被 skill 消费的正文」这条路当前走不通:两条路各断在不同的地方,而 `plugin/src/knowledge/changes/` 零记录说明**从没有人走通过** · high(阻断所有 prose 维护·且缺口不显形)

- **现象 / 证据**:目标是把 `ccm.cmd.json-shape` 里 7 个已被 `--schema` 样例取代的小节换成指针(减 85 行)。两条路都试了,都断:
  - **路 A(直接改 + 对账)**:改母本与文章 → `reconcile-passages --write`(爆炸半径正确:3 文件)→ 但 `check` 仍 `ok=false`,`SKG-COMPOSITION-NOT-ADMITTED`,子诊断 `SKG-ANALYSIS-WITNESS-MISMATCH` + `SKG-ANALYSIS-VERDICT-MISMATCH`。候选分析里冻着改前的图指标,而 `analyzeSkillCandidate` 是**校验器不是生成器**——存档与重算不符时它拒绝产出新分析,于是没有任何入口能刷新它。
  - **路 B(typed change 事务·官方路径)**:`change begin` 现在能开了(#221 修的),但 `change.draft.json` 的 `wording` 操作必填 `before_sha256` / `after_sha256`(marker span 的哈希),而**公开 helper `hashMarkdownSpan(原始 span 正文)` 算出的值与图里记录的 `span_hashes` 对不上**(实测 HEAD 版:helper 得 `3fb23236…`,图记录 `2a48914f…`)。也没有任何命令替你算或填这两个值。
- **根因**:两条路缺的是同一样东西——**一个能把「派生物」重新算出来的写入口**。图把 span 哈希、候选分析witness、composition 准入三样都冻在源码里,却只提供了**校验**它们的能力,没提供**重算**它们的能力。`reconcile-passages` 只覆盖了三样中的一样(span 对账)。
- **影响**:**当前任何对「已被 composition 消费的文章」的正文修改都无法收口**——改完必然把该 skill 的 composition 打成未准入,而恢复手段不存在。这解释了一个此前当作巧合的事实:`plugin/src/knowledge/changes/` **零记录、零提交历史**——不是没人需要改,是**没人走通过**。上一轮 `FMT-POLICY` 那次(见 [[Finding #121]] 的撤回记录)也是撞在同一堵墙上,当时归因为「一次性意外」,现在第二次撞上同一处,可以确认是结构性缺口。
- **✅ 后续(同日修复)**:补了 `skill-knowledge refresh-analysis [--composition <id>] [--write]`,墙已破。修的过程中扒出了这堵墙的**精确成因**,比原判更具体——上游把裁决算成 `deriveVerdict(scoresheet, diagnostics.length === 0 && gates.ok)`,而 `diagnostics` 里**包含失配诊断本身**。于是「存档陈旧」这一个事实同时充当了「裁决应为 reject」的依据:**陈旧的分析永远推不出 admit,刷新在构造上不可能成功**。这是自指式的死锁,不是缺一个入口那么简单。refresh 在剔除陈旧性这一项之后用**同一个** `deriveVerdict` 重算,判据没放松(`admission_gates.ok` 仍须为真)。两条安全闸都做了变异验证:①任何实质性诊断(成员关系坏 / 身份对不上 / 准入真没过)→ 拒绝并原样报出;②重算裁决非 admit → 拒绝。**一个能把任何失败刷绿的 refresh 不是修复,是旁路。** 另修一处:`analyzeAgainstGraph` 返回的是**运行时** witness(比持久化形态多带定位键),整块写回会让文件 schema-invalid——只该覆盖 `WITNESS_METRIC_KEYS` 那一批。路 B(typed change 的 span 哈希填不出来)**仍未解**,但路 A 通了之后它不再是阻塞项。
- **处置(含蒸馏判定)**:~~本轮不修,登记为 ccm/toolkit 侧的独立工作项~~ —— 已修(见上)——它需要给 toolkit 补两样能力:① 重算并写回候选分析 witness(或让 `analyzeSkillCandidate` 支持 `--write` 刷新);② 一个算 span 哈希、填 `change.draft.json` 的辅助命令(否则 typed change 的必填字段人工填不出来)。**蒸馏判定:入账不回流 skill**——这是工具链缺口,不是编排纪律。**同时它是 json-shape 迁移的硬前置**:接口侧(`--schema` + 样例)已经就位,文档侧删不掉,所以那 7 节继续留着,不是判断变了,是路被堵着。
- **严重度 / 来源**:high(阻断 prose 维护)/ 一手(2026-08-05,两次独立撞墙)。

---

## Finding #123 — 复核提示词库时,拿探针答案去比对「当初那条被判为猜错的猜测」,而不是真考点,于是把 5 条未泄误判成泄、得出「7 抽 7 泄」并当场上报 · high(与 [[Finding #121]] 同一条纪律,在新场景下重犯)

- **现象 / 证据**:抽查 `tier0-prompts.json` 入库的 15 条,独立复跑盲测探针。第一轮逐条对比时用的靶子是记录里的 `leak_probe_guess` 字段,7 条全部「命中」,遂判 **7 抽 7 泄**,并向用户表述为「结论已经不是这批题写得不够好了」。用户质疑「你怎么定义泄题的」,回查才发现:**`leak_probe_guess` 记的是当初探针猜了什么——它猜错了,那条才 pass**。真考点在 `bait` 字段的「漏掉时产物会怎样」一句里。对着真考点重判:**1 真泄 + 1 半泄 + 5 未泄**,闸基本是好的。
- **根因**:两层。① **比对基准错位**——拿「已知为错的旧猜测」当靶子,任何与之相似的答案都会被判为命中,这个比对在构造上不可能得出「未泄」。② **概念混同**——把「第三方答得对」等同于「泄题」。二者相反:题简单而模型自发答对,正是第 0 层要测的**正面结果**;泄题是第三方**看穿了这是一道在测什么的题**、把诱饵指认成诱饵,从而使被测者的通过变成应试而非自发。
- **影响**:若未被质疑,会据此判定「承重任务与不点题在同一道题上不可兼得」,进而把整条第 0 层路线判为结构上跑不成,并把 `capability_gap` 78 条**全部**划出删除授权范围。实际正确结论是范围缩小(13 条可跑、约 60 次运行)而非路线作废。**一个错误的比对基准,差点作废一条还能走的路线。** 方向与 [[Finding #121]] 相反但同源:那六次是把「无缺陷」误报成缺陷,这次是把「装置有效」误报成装置失效。
- **处置(含蒸馏判定)**:**蒸馏判定:回流一处 + 改一处字段名**。① 回流的不是新纪律,而是[[Finding #121]] 那条既有纪律的**适用面扩大**:「先确认你在对哪一层做比对」此前的例子全是源码/产物/运行时输出三层,本次证明它同样适用于**记录字段**——一份记录里同时存着「真判据」与「已被否决的候选」时,拿错哪一个都不会报错,只会静默给出反向结论。② 可执行改动:`leak_probe_guess` 字段名有误导性(读起来像考点,实为猜错的东西),下一版记录格式改名为 `probe_guess_rejected`,或与 X 并排存放,让基准无从拿错。
- **严重度 / 来源**:high(错误基准差点作废一条有效路线)/ 一手(2026-08-06,提示词库抽查),详见 [`design_docs/disposition-ledger/tier0-prompt-spotcheck.md`](disposition-ledger/tier0-prompt-spotcheck.md)。

---

## Finding #124 — 闭卷装置把「模型幻觉出一次工具调用然后停住」记成 `ok`:退出码 0、stdout 非空、没有答案;它混进 counted 变成凭空捏造的「模型不知道」 · high(假数据·且比掉出更隐蔽)

- **现象 / 证据**:`scripts/exam-closed-book.mjs` 用 `--tools ""` 禁掉全部工具保证闭卷,但模型有时仍会尝试用工具——吐一句「我先看一眼当前仓库」再编出几段 `**Bash**` 调用(有的连假的 `Tool result` 回显都编了)然后结束。进程退 0、stdout 非空,装置照记 `status:"ok"`。第 1 层实测首轮命中 **10/114(约 9%)**;本表作者加检测重跑后,端点抽查用更灵的判据**又从「已清洗」批次里捞出 3 份残留**。
- **根因**:装置把「进程正常退出且有输出」当成「答了题」。这两件事在禁工具场景下**系统性地不等价**——模型的默认反射是先探环境,探不成就地停住,而这个停住在 shell 层面是完全正常的退出。
- **影响**:**比掉出危险**。掉出会进 `dropped` 计数、看得见、会拉高 degraded 率;这个混进 `counted`,评分时变成一个**凭空捏造的「模型不知道」**。方向上偏保守(把会的判成不会,不推向删除侧),但仍是假数据——它让 `routing.executor-vs-target` / `routing.qualification-and-fallback` 两条被误判为 `unknown`,而后者恰是三条「档位串档」待重判点之一,一个假的 unknown 会顺着影响它的重判。
- **处置(含蒸馏判定)**:**已回流进装置**——新增 `no-answer` 状态(判据 = 含工具调用痕迹 **且** 全长 < 400 字符;两条合取,不用纯长度阈值以免误杀真正简短的正确回答)+ 最多三次自动重试,**绝不当 `ok` 混进分母**。变异验证:5 份已知污染 5/5 命中、第 0 层 65 份正常产物 0/65 误杀。上线第一跑即当场生效(补跑时同一条又撞上,自动重试后拿到真答案)。**蒸馏判定:不回流 skill**——这是测量装置缺陷,不是编排纪律。**但它有一条通用形态值得记进验证纪律**:凡「子进程跑完了」这类成功信号,都要问一句**它证明的是「跑完了」还是「做成了」**;`exit 0` 只承诺前者。与 [[Finding #121]]「先确认你在对哪一层做比对」同族——那条讲比对对象,这条讲成功判据。
- **严重度 / 来源**:high(假数据混入分母)/ 一手(2026-08-06,第 1 层实测 + 端点抽查),详见 [`design_docs/disposition-ledger/tier1-run-2026-08-06.md`](disposition-ledger/tier1-run-2026-08-06.md) 的端点更正节。
