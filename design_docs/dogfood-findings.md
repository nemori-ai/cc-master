...
| 91 | provider supervisor 把全部 timeout/reap timer `unref()`,本地全量被旁路 handle 假绿、干净 CI 提前结束 pending promise | must-fix(CI 阻断·进程生命周期正确性·已修) | timer 在 supervision settle 前保持 ref,终态统一 clear;独立无旁路 handle 子进程回归 + Node 22.23.1/24.18 双验 |
...
- **处置(含蒸馏判定)**:**已修**——supervisor-owned timers 在 supervision promise resolve/reject 前保持 Node 默认 ref 状态，既有 `clearAllTimers()` 仍在所有 terminal path 清除 startup/idle/hard/termination/reap/poll timer；没有新增常驻 keepalive wrapper、没有放宽 timeout、没有改变 TERM→KILL/process-tree authority。`supervision keeps its timeout lifecycle alive without unrelated refed handles` 回归通过外层 test spawn 一个纯净子进程，要求它仅靠 supervisor 自己到达 `startup_timeout + termination.reaped=true`，避免 test runner 或其他并发文件再次提供假梯度。**蒸馏判定=回流产码不变式注释 + deterministic lifecycle test，不回流 runtime skill prose**：面向 orchestrator 的 supervisor/工业化指导没有缺口，本次是 ccm 实现违反既有合同；重复写 skill 只会把内部 timer 机制泄漏给用户 agent。
- **验证指针**:`ccm/apps/cli/src/provider-child-supervisor.ts`（ref 生命周期与统一清理）、`ccm/apps/cli/test/provider-child-supervisor.test.ts`（无旁路 handle probe）；修复后 Node 22.23.1 下原 startup timeout + 新 probe 2/2 green，Node 24.18 新 probe green，后续 full ccm/root/plugin gates 以同 commit 最终证据为准。
