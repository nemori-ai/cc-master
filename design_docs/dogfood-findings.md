...
| 91 | provider supervisor 把全部 timeout/reap timer `unref()`,本地全量被旁路 handle 假绿、干净 CI 提前结束 pending promise；三轮 fresh review 又依次抓到 long-delay owner、reentrant late-arm、callback 返回后继续副作用 | must-fix(CI 阻断·进程生命周期正确性·candidate 已关闭已知反例，待 fresh 终审) | ref timer + slot owner；可重入 effect postcondition guard；operational scheduler fail-closed；isolated / long-delay / callback matrix，Node 22.23.1/24.18 双验 |
...
- **处置(含蒸馏判定)**:**candidate 已关闭当前已知反例，待 fresh endpoint 终审**——supervisor-owned timers 在 supervision promise resolve/reject 前保持 Node 默认 ref 状态；`scheduleAt()` 接收所属 slot writer并在 lifecycle 已 terminal / failing 时拒绝 initial arm 与 recursive re-arm。`beginTermination()` 在 `signal()` 返回后复核 `settled`，未终结才在 cleanup poll 前登记 grace timer。更上层建立统一 `canContinueProviderWork()` postcondition：`markSpawned()` 显式返回 caller 能否继续，`collect()`、activity/idle arm、finalizer、`onClose` 在每个外部 callback/tree probe 后复核；`treeGone` 只可 false→true 单调推进，reentrant primary failure 不再被 `owned_tree_survived` 覆盖或重复 TERM。这里仍不越过证据声称“所有可能 terminal path 已证明”：当前机械证据覆盖 normal、long-delay、signal true/false、first-byte / spawn `onStarted`、stdout parser、abort、child error、`onClose` success/incomplete UTF-8 failure、tree probe 与 poll/reap seam，最终全称判断交给独立终审。没有新增常驻 keepalive wrapper、没有放宽 timeout、没有改变 TERM→KILL/process-tree authority。**蒸馏判定=回流产码不变式注释 + deterministic lifecycle test，不回流 runtime skill prose**：面向 orchestrator 的 supervisor/工业化指导没有缺口，本次是 ccm 实现违反既有合同；重复写 skill 只会把内部 timer 机制泄漏给用户 agent。
- **验证指针**:`ccm/apps/cli/src/provider-child-supervisor.ts`（ref 生命周期、slot owner、callback postcondition、单调 tree proof 与终态清理）、`ccm/apps/cli/test/provider-child-supervisor.test.ts`（无旁路 handle + long-delay + synchronous callback matrix）；Node 22.23.1 与 24.18 下，long startup / idle / hard 均从 active orphan=1 收敛为 active=0；signal/poll late-arm 从 grace=1 收敛为零；first-byte `onStarted` 从 idle=1 收敛为零，tree probe 从重复 TERM 收敛为一次，所有 matrix typed primary error 保留且 fake clock 推进后无 late SIGKILL / poll；full ccm/root/plugin gates 以同 commit 最终证据为准。
- **严重度 / 来源**:must-fix(CI 阻断 + provider timeout/process-tree 生命周期正确性·candidate 已关闭已知反例，待 fresh endpoint 终审)/ 一手(PR #105 两次 GitHub Actions clean-run 失败 + 本机 Node 22.23.1/24.18 deterministic RED→GREEN + 三轮独立终审反例,2026-07-13)。
