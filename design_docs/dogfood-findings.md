...
| 91 | provider supervisor 把全部 timeout/reap timer `unref()`,本地全量被旁路 handle 假绿、干净 CI 提前结束 pending promise；fresh review 又抓到 >Node timer limit 分段 re-arm 的 startup/idle owner 丢失 | must-fix(CI 阻断·进程生命周期正确性·已修) | timer 在 supervision settle 前保持 ref；统一 slot owner 跟踪每次 re-arm 并在终态 clear；独立 liveness + fake-clock long-timer 回归，Node 22.23.1/24.18 双验 |
...
## Finding #91 — provider supervisor 的 pending promise 没有自己的 event-loop liveness,本地全量假绿而干净 CI 提前退出 · must-fix(CI 阻断·已修)
...
- **处置(含蒸馏判定)**:**已修**——supervisor-owned timers 在 supervision promise resolve/reject 前保持 Node 默认 ref 状态；`scheduleAt()` 接收所属 slot writer，初次 arm、每次分段 re-arm 与到期触发都统一回写当前 handle / `null`，因此 `clearAllTimers()` 能在所有 terminal path 对称清除 startup/idle/hard/termination/reap/poll timer。没有新增常驻 keepalive wrapper、没有放宽 timeout、没有改变 TERM→KILL/process-tree authority。`supervision keeps its timeout lifecycle alive without unrelated refed handles` 通过纯净子进程锁住原 CI liveness；三条 fake-clock 回归分别让 long startup、long idle 与 long hard 进入第二分段，再验证 success/failure 终态 active timer 为零且 hard 仍 fail closed。**蒸馏判定=回流产码不变式注释 + deterministic lifecycle test，不回流 runtime skill prose**：面向 orchestrator 的 supervisor/工业化指导没有缺口，本次是 ccm 实现违反既有合同；重复写 skill 只会把内部 timer 机制泄漏给用户 agent。
- **验证指针**:`ccm/apps/cli/src/provider-child-supervisor.ts`（ref 生命周期、统一 slot owner 与终态清理）、`ccm/apps/cli/test/provider-child-supervisor.test.ts`（无旁路 handle probe + long-delay fake clock）；Node 22.23.1 与 24.18 下，long startup / idle / hard 三条回归均从 active orphan=1 的 deterministic RED 收敛为 active=0，原 isolated liveness 与完整 supervisor suite 保持 green；full ccm/root/plugin gates 以同 commit 最终证据为准。
- **严重度 / 来源**:must-fix(CI 阻断 + provider timeout/process-tree 生命周期正确性·已修)/ 一手(PR #105 两次 GitHub Actions clean-run 失败 + 本机 Node 22.23.1/24.18 deterministic RED→GREEN,2026-07-13)。
