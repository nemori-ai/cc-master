...
| 91 | provider supervisor 把全部 timeout/reap timer `unref()`,本地全量被旁路 handle 假绿、干净 CI 提前结束 pending promise；两轮 fresh review 又依次抓到 long-delay owner 丢失与 reentrant terminal 后 late-arm | must-fix(CI 阻断·进程生命周期正确性·candidate 已关闭已知反例，待 fresh 终审) | timer 在 settle 前保持 ref；统一 slot owner 跟踪 re-arm；signal 后 settled guard + poll 前登记 grace timer；isolated / long-delay / reentrant fake-clock 回归，Node 22.23.1/24.18 双验 |
...
- **处置(含蒸馏判定)**:**candidate 已关闭当前已知反例，待 fresh endpoint 终审**——supervisor-owned timers 在 supervision promise resolve/reject 前保持 Node 默认 ref 状态；`scheduleAt()` 接收所属 slot writer，初次 arm、每次分段 re-arm 与到期触发都统一回写当前 handle / `null`。`beginTermination()` 在 `signal()` 这个可重入 effect 返回后先检查 `settled`，未终结才登记 grace timer，且登记发生在 cleanup poll 之前；于是 signal 同步 terminal 不会 late-arm，poll 同步 terminal 则能看见并清除已登记 handle。这里不再越过证据声称“所有可能 terminal path 已证明”：当前机械证据覆盖 normal、long-delay、signal true/false 同步 close、`onClose` 成功 finalization、incomplete UTF-8 finalization typed reject + poll/reap seam，最终全称判断交给独立终审。没有新增常驻 keepalive wrapper、没有放宽 timeout、没有改变 TERM→KILL/process-tree authority。**蒸馏判定=回流产码不变式注释 + deterministic lifecycle test，不回流 runtime skill prose**：面向 orchestrator 的 supervisor/工业化指导没有缺口，本次是 ccm 实现违反既有合同；重复写 skill 只会把内部 timer 机制泄漏给用户 agent。
- **验证指针**:`ccm/apps/cli/src/provider-child-supervisor.ts`（ref 生命周期、统一 slot owner、reentrant arm 顺序与终态清理）、`ccm/apps/cli/test/provider-child-supervisor.test.ts`（无旁路 handle + long-delay + synchronous terminal fake clock）；Node 22.23.1 与 24.18 下，long startup / idle / hard 均从 active orphan=1 收敛为 active=0；signal true/false 同步 close 与 finalizer typed rejection 也从 active grace timer=1 的 deterministic RED 收敛为 active=0，fake clock 推进后无 late SIGKILL / poll；full ccm/root/plugin gates 以同 commit 最终证据为准。
- **严重度 / 来源**:must-fix(CI 阻断 + provider timeout/process-tree 生命周期正确性·candidate 已关闭已知反例，待 fresh endpoint 终审)/ 一手(PR #105 两次 GitHub Actions clean-run 失败 + 本机 Node 22.23.1/24.18 deterministic RED→GREEN + 两轮独立终审反例,2026-07-13)。
