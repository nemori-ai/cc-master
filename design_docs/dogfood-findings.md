...
## Finding #83 — `ccm task update --set/--set-json` 裸 dotpath 落 **board 顶层**而非该 task,help 文本与命令语境双重暗示「设的是这个 task 的字段」→ orchestrator 把 `decision_package` 误写到 board 根,且 render 照报「task 已更新」零提示 · should-fix(CLI 易误用·文档密度)
...
- **处置(含蒸馏判定)**:蒸馏判定——机制层修复归 ccm 线 backlog(候选:①`task update` 的 `--set` 裸 path 默认 scope 到该 task、板级逃生口挪到 `ccm board` 下;或最小 ②help 文本 + render 提示补 scoping 说明+落点回显);文档层回流 `using-ccm` `references/command-catalog.md` 的 `--set/--set-json` 条目——**必须写明裸 path=board 顶层、task 字段须 `tasks[<id>].` 前缀、并给挂 decision_package 的正例**(§6 ccm⟷using-ccm 锁步:本条先记台账,随 ccm 侧修复 PR 同步)。本轮以 `tasks[HOOKPAR-PR].decision_package` 正确写法绕过 + 根级残留置 null。
- **严重度 / 来源**:should-fix(易误用面·静默错落点)/ 一手(2026-07-07 HOOKPAR-PR 收口实测,board 20260707T081546Z-4904)。
