...
- **处置(含蒸馏判定)**:**本轮不修,登记为 ccm/toolkit 侧的独立工作项**——它需要给 toolkit 补两样能力:① 重算并写回候选分析 witness(或让 `analyzeSkillCandidate` 支持 `--write` 刷新);② 一个算 span 哈希、填 `change.draft.json` 的辅助命令(否则 typed change 的必填字段人工填不出来)。**蒸馏判定:入账不回流 skill**——这是工具链缺口,不是编排纪律。**同时它是 json-shape 迁移的硬前置**:接口侧(`--schema` + 样例)已经就位,文档侧删不掉,所以那 7 节继续留着,不是判断变了,是路被堵着。
