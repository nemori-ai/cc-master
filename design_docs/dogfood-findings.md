...
- **现象**:`board-graph.js`(机器算临界路径/float/并行度/impact/rollup 的运行时带外 CLI)早已实现并 ship 到 `skills/orchestrating-to-completion/scripts/`,但 grep 全 prose **零命中**——它就是 `status.md` / `decomposition.md` 里那句「要真算 float 请走带外脚本」中那个**无名脚本**。PR-3 把它具名化、织进魂与 references 的指针网前,先按 §6 TDD-for-skills 跑 pressure baseline(三压:time + sunk cost + exhaustion),验「没有该 prose 时 agent 会不会选错」。**baseline 结果 = 强模型天花板,RED 没失守**:强 agent 会(a)自己发现 `board-graph.js` 这个工具存在、(b)用「拓扑复杂度」自发判心算 vs 机器算(大钻石图升级机器算、平凡单链心算)、(c)不在平凡图上仪式性跑 CLI(自带反 busywork 直觉·镜头 4)。即:加判据 prose 前,强模型本就大致选对。
...
## Finding #72 — board v2 hook 收编(bash→node):JSON.parse 根除 awk-深扫盲区(一整类)+ 并行 subagent 移植 + cksum 指纹字节级复现 + 端点亲验 · ✅正向(收编真红利 + 行为等价纪律)
...
## Finding #73 — Track A 触发 eval 对「建议型」cc-master skill 是结构性死信道(loaded-env floor):不可作 description 发布门,质量靠 pressure baseline(定性)+ 兄弟 Do-NOT-use 定性 review 守 · 中(工具效率·eval 可信度)
...
## Finding #74 — 面向「一次写对」的派生操作手册,校验规则要给**全集**(从引擎 lint SSOT 逐条对账)而非抽样;且全集表是机械派生物、必进抗漂移锁步面 · ✅正向(效率根因 + 抗漂移闭环)
...
- **处置(含蒸馏判定)**:蒸馏——**「面向『一次写对』的派生操作手册,校验/约束规则给全集(从引擎 lint SSOT 逐条对账生成),不是抽样;全集表是机械派生物,必进抗漂移锁步面」**。已回流:AGENTS.md §6 锁步段(把 `board-model-guide.md` 规则表列入 ccm 改 lint 规则时的同 PR 同步面)+ `using-ccm/DESIGN.md` 演化锚。✅机制验证。
