...
## Finding #107 — in-session subagent 的 viewer 流式登记配方是 guidance 黑洞:分发 skill 只教了 session-id(CLI worker)/pid(bg shell)/task-id(泛「有 transcript 的派发」)三形,没教「Task tool subagent = task-id:<agentId> + --transcript <父转录>」派生契约(只活在 ccm agent-probe.ts 源码注释里) → orchestrator 实操自创 `session-id:<agentId>` 且漏 transcript,真实 board 上 19 个 origin agent 的 stream 全黑 · should-fix(guidance 缺口 → 观测面假阴性)
...
- **根因**:派生契约(子转录落 `<父转录去.jsonl>/subagents/agent-<agentId>.jsonl`,需「父转录 + agentId」两原料)只写在 `ccm/apps/cli/src/agent-probe.ts` 的注释里;`using-ccm` 的 handle.kind 表与 command-catalog agent bind 节都没有 in-session subagent 条目。agent 面对「subagent 有个 id」自然套了最像的 `session-id` 形状——guidance 没写的地方,agent 就会发明。
- **影响**:所有经 Task tool 派生并手工登记的 subagent 在 viewer 里不可观测(stream 恒无源),与 Finding #106 的误导文案叠加成「只有 claude 主会话正常、其他都坏」的整体误判;「凡派发皆登记」的观测承诺对 subagent 形同虚设。
- **处置(含蒸馏判定)**:**已修**(plugin 侧·同 PR #206)——`command-catalog.md` agent bind 节新增「in-session subagent 登记配方」(两原料 + 反模式:agentId 当 session-id 登 / 漏 transcript = stream 恒无源);`board-model-guide.md` handle.kind 表 `task-id` 行显式列 in-session subagent 用法并回指 catalog。**蒸馏判定:已直接回流 D(using-ccm)两份 reference**,不另设 skill;深层教训并入 #106 的方法论面:机制注释里的契约若要求调用方配合,必须同步落进操作手册,否则就是「手册骗人」的镜像(§6 锁步精神的 guidance 版)。
