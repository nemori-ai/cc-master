...
The executable oracle independently binds the last successful write event to the inode currently named by the operation target,
then requires both a later regular-file sync event for that same inode and a later directory-sync event for the inode of that
target's containing directory, for every `committed` mutation. Counts and object identity alone are insufficient: syncing the
exact current target while it still contains old bytes, syncing its exact parent, and only then writing the final bytes without
another target-file and parent-directory sync is rejected.
...
satisfy the oracle, and observed calls against the correct objects cannot satisfy it when their event order precedes the final
target write. The ordered event evidence remains oracle instrumentation, not a new public filesystem operation or receipt field.
...
3. V1, absolute/parent paths, symlink escape, unknown rename/remove, phase-forbidden operations, forged results, stale CAS,
   wrong-prefix/partial append, wrong-target/missing/unsafe/pre-final-write durability, post-publication false-safe errors and
   adapter bypass are rejected;
