...
> 状态：**实施中；当前实现已落 tracked candidate/affinity registry、`ccm model-policy show|advise`、三 origin 统一 skill 消费与机械 content contract，尚未取得 role certification/live target admission，也未接自动 routing enforcement 或付费 canary**
> 日期：2026-07-16 UTC
...
### 0.1 当前实现边界（2026-07-16）

| 能力 | 当前状态 | 诚实边界 |
| --- | --- | --- |
| tracked model-policy data | **implemented** | role candidate 与 task-affinity registry 已进 ccm 包；当前所有 model-role entry 仍是 `candidate`，blocker 明确要求 project role certification 与 live target admission |
| `ccm model-policy show` | **implemented** | 三 origin 共用同一 read model；只读 tracked facts/evidence，不 probe provider、不证明 entitlement/quota |
| `ccm model-policy advise` | **implemented** | 只排序调用者提交的、已 qualification candidates；不 qualification、不 reserve、不 spawn、不写 board |
| plugin runtime guidance | **implemented and mechanically gated** | `master-orchestrator-guide`、`pacing-and-estimation`、`using-ccm` 共用 target policy；usage signal、真实 dispatch、completion 与 watchdog 仍由 origin/target adapter 处理 |
| role certification / live admission | **not implemented** | Fable 5、GPT-5.6 Sol、Cursor Grok 4.5 只是 O candidates，不是 certified / automatically eligible |
| board-driven automatic fallback / maintenance routine / production canary | **not implemented** | `ccm worker run` 仍是显式 raw wrapper；ample/tight policy 不会自动执行，付费与账号 mutation 未获任何新增授权 |

## 1. 结论

本项目在现有 `economy / balanced / frontier` 之外建立一条**正交的模型资格轴**，而不是增加一个更贵的价格档：
...
- **`role_grade=T3`**：格式化、确定性提取、测试重跑、机械迁移等有强机器闸的叶子；它不是当前政策的承重档，只作为 tight posture 的最低成本补充。
...
这里必须把**组织角色**与**模型资格**分开：`executor=master-orchestrator` 表示持有全局上下文、HITL、scope 与 board authority 的责任主体；`role_grade=O` 只表示某个 `model × surface × effort × version` 有资格承担 orchestrator-grade 的设计、架构和高风险判断。O subagent 仍是 `subagent`，不会因此继承 master 的用户授权或写入 authority；前台 master 当前所用模型也不会因组织位置自动获得 O certification。
...
因此无需新增 executor enum，也无需新造第九个 runtime skill。当前实现已闭合 tracked read model 与三 origin skill 消费；剩余缺口是角色认证、target-bound live admission、生产校准与自动执行，而不是继续复制更多模型表。

### 2.2 当前主要偏差

| Gap | 当前表现 | 风险 / 应改方向 |
| --- | --- | --- |
| 通用 tier 与角色资格混为一谈 | **当前实现已拆开**：provider facts 保留 Fable 5 的通用 `economy` tier，独立 role-candidate registry 承载其 `O` candidate；两条轴互不补证 | `economy` 既不授予也不否定 O certification；所有 role entry 当前只到 `candidate` |
...
| origin-local overlay 限制全局视角 | **当前实现已删除** master allocation / pacing target model 的三份 origin-local overlay，三端投影共用 canonical target policy | 仅 origin-native usage、launch、completion、watchdog 与 surface 证据保留 host-specific；projection/idempotence gate 机械守护投影等价性 |
...
### 3.3 当前事实 registry 的实现与剩余边界

当前实现保留 Fable 5 的通用 provider tier `economy`，并在独立 role-candidate registry 中把它登记为 `O` candidate；同时增加其能力主页面 provenance、建立 task-affinity registry，并在 normalized read model 中保留 `hard_facts / project_role_evidence / community_advisory` 分层。通用 provider tier 与项目角色资格正交：`economy` 既不是 O 的证明，也不是拒绝 O 的理由。

仍未实现、不得由 prose 补写成已完成的部分：

- Cursor provider facts 仍需以 live probe 分别证明 `cursor-agent-cli:first-party`、`cursor-ide-plugin` 与 **Cursor Agent third-party-model route, payer unknown**；同品牌或同 identity 不共享 entitlement/quota。
- 当前 role registry 只有 candidate grades、state、surface、provenance 与 blockers；尚无通过本项目 eval 的 `certified_at/valid_until`、score/variance/seeds 或版本范围，因此任何 candidate 都不能写成 certified。
- 官方 benchmark、社区 affinity 与本项目 role eval 必须继续分层；`ccm model-policy show` 的 read model 不等于 automatic eligibility。
...
按四个可独立集成、逐步扩面的纵向阶段推进；当前交付范围只包含 V0/V1 的 read-model + guidance 薄片，不把后续证据与执行闭环伪装成完成：

1. **V0 — walking skeleton（implemented）**：tracked registries + `ccm model-policy show|advise` + master 决策入口已形成可演示只读薄片；命令稳定不 spawn、不付费、不 enforcement。它不含 board 自动 route，也不证明任何 live target。
2. **V1 — 三 harness 能力与场景扩面（partial）**：三端共用 O/T1/T2/T3、task taxonomy、affinity provenance/TTL/contradiction 与有界 tie-break；`master-orchestrator-guide`、`pacing-and-estimation`、`using-ccm` 已接入。`slicing-goals-into-dags` taxonomy 增量与真实 role eval 不在本薄片完成声明内。
3. **V2 — 可信维护闭环（not implemented）**：补 role eval 与版本绑定的认证/过期/撤销；remote routine 只产出候选 issue/draft PR，machine-local probe 独立验证 catalog/auth/admission；生成 registry/projection attestation，并用 fixture 证明 remote 事实不能替代 live admission、routine 不改 production routing 且不 auto-merge。
4. **V3 — 生产证明与最后执法（not implemented）**：在明确授权、合格主机与 payer 边界内做各 surface 的最小 canary，先 shadow 比较并校准 ample/tight 与 affinity cap；只有 registry、eval、read model、skill、route evidence 和 canary 全闭合后，才考虑把 effect-floor vocabulary 与 role-certification revision 变成 routing hard gate；legacy board 保持兼容。
...
- `claude --version` = `2.1.211`；`claude --help` 明确接受 `fable` / `claude-fable-5`。
- `codex --version` = `0.144.4`；本机模型 cache/config 可见 `gpt-5.6-sol`。
- `cursor-agent --version` = `2026.07.09-a3815c0`；通过 `ccm worker run --harness cursor-agent -- --list-models` 可见 Cursor Grok 4.5 first-party selectors，以及 Fable 5 / GPT-5.6 Sol selectors。随后只对 first-party `cursor-grok-4.5-high` 做了一次只读 pressure probe；wrapper 在 transcript 返回前以 `owned_tree_survived` 失败，因此该尝试不证明模型行为、payer、quota、调用成功或自动准入，也没有调用任何 third-party/API route。
