...
> 状态：**实施中；本分支已落 tracked candidate/affinity registry、`ccm model-policy show|advise`、三 origin 统一 skill 消费与机械 content contract，尚未取得 role certification/live target admission，也未接自动 routing enforcement 或付费 canary**
> 日期：2026-07-16 UTC
...
| 能力 | 当前状态 | 诚实边界 |
| --- | --- | --- |
| tracked model-policy data | **implemented in branch** | role candidate 与 task-affinity registry 已进 ccm 包；当前所有 model-role entry 仍是 `candidate`，blocker 明确要求 project role certification 与 live target admission |
| `ccm model-policy show` | **implemented in branch** | 三 origin 共用同一 read model；只读 tracked facts/evidence，不 probe provider、不证明 entitlement/quota |
| `ccm model-policy advise` | **implemented in branch** | 只排序调用者提交的、已 qualification candidates；不 qualification、不 reserve、不 spawn、不写 board |
| plugin runtime guidance | **implemented in branch，待全套门禁** | `master-orchestrator-guide`、`pacing-and-estimation`、`using-ccm` 共用 target policy；usage signal、真实 dispatch、completion 与 watchdog 仍由 origin/target adapter 处理 |
...
因此无需新增 executor enum，也无需新造第九个 runtime skill。本分支已闭合 tracked read model 与三 origin skill 消费；剩余缺口是角色认证、target-bound live admission、生产校准与自动执行，而不是继续复制更多模型表。

### 2.2 当前主要偏差

| Gap | 当前表现 | 风险 / 应改方向 |
| --- | --- | --- |
| 通用 tier 与角色资格混为一谈 | **本分支已拆开**：provider facts 继续保留通用 tier，独立 role-candidate registry 承载 `O/T1/T2/T3`；Fable 5 的 provider tier 已纠正为 `frontier` | 仍不可把 `frontier` 当 O certification；所有 role entry 当前只到 `candidate` |
...
| origin-local overlay 限制全局视角 | **本分支已删除** master allocation / pacing target model 的三份 origin-local overlay，三端投影共用 canonical target policy | 仅 origin-native usage、launch、completion、watchdog 与 surface 证据保留 host-specific；全套 projection/idempotence gate 仍是合并前条件 |
...
本分支已经：把 Fable 5 的 provider tier 从 `economy` 纠正为 `frontier`、增加其能力主页面 provenance、建立独立 role-candidate 与 task-affinity registries，并在 normalized read model 中保留 `hard_facts / project_role_evidence / community_advisory` 分层。
...
按四个可独立集成、逐步扩面的纵向阶段推进；当前分支只交付 V0/V1 的 read-model + guidance 薄片，不把后续证据与执行闭环伪装成完成：

1. **V0 — walking skeleton（本分支 implemented）**：tracked registries + `ccm model-policy show|advise` + master 决策入口已形成可演示只读薄片；命令稳定不 spawn、不付费、不 enforcement。它不含 board 自动 route，也不证明任何 live target。
2. **V1 — 三 harness 能力与场景扩面（本分支 partial）**：三端共用 O/T1/T2/T3、task taxonomy、affinity provenance/TTL/contradiction 与有界 tie-break；`master-orchestrator-guide`、`pacing-and-estimation`、`using-ccm` 已接入。`slicing-goals-into-dags` taxonomy 增量与真实 role eval 不在本薄片完成声明内。
