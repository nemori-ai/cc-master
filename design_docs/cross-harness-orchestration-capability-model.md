...
并配套 opt-in RED fixtures；这只证明 spec/fixture readiness，collector、store、reservation manager 与 live
admission runtime 仍不存在，不能把本段或 RED evidence 晋升为 current/shipped。
...
- opt-in RED 的 future GREEN 必须穿 `@ccm/engine` 公共 contract、existing CLI router/registry 与 owner-only
  filesystem store 三条 production seam；fixture lookup evaluator 不构成证据。可执行 oracle 已冻结 10-way
  refresh single-flight、0600 atomic publish、event replay crash recovery、同 idempotency key 10-way 单 reservation，
  但这些 seam 当前仍 absent，故本能力仍是 `target`。
...
| Reservation | 无 | quota v1 contract + opt-in RED engine/CLI/store/concurrency oracles 已冻结；seams/lock/live admission 未实现；pacing 不是 reservation | account+pool lease/live recheck/orphan retention | S4 |
