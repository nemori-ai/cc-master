...
| Task CLI | add/update/start/done + planning/routing 与五个 native-attempt dedicated writer | native verbs 只写 ledger，其中 bind/terminal/reconcile 消费 ccm 私有 evidence；不会调用 host tool，generic/`--force` 不能修 active native projection | dispatch/run control 与受信 runtime composition | S0/S6 |
