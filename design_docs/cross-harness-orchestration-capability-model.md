...
| Provider execution | Codex、Claude Code、Cursor Agent 的 resolver-backed session-bound raw wrapper 为 `current`：真实 help、caller-owned raw argv/stdin、generic process envelope | ccm 只管理 process lifecycle；不解析 provider terminal，不证明 provider flags、model/quota admission、permission 或 safety | normalized provider adapter、automatic route/admission 与 provider-specific result semantics 均为 `target` | S3/S6/S8 |
