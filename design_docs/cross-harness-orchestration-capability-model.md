...
> 状态：**cross-harness 纵向成熟度与目标设计台账；current / partial / target 分层描述**
> 日期：2026-07-23 UTC
> 当前产品边界：以 [`design_docs/feature-manual.md`](feature-manual.md) 为面向人的 current 索引；本文保留经日期标记的演进切片，不能把旧切片里的“三路/target”快照反推为今日能力
> 正式版同步：Claude Code、Codex、Cursor、Kimi Code 四个 origin adapter 与四路 session-bound raw worker 已交付；`ccm worker dispatch` 已将本机 worker 的 Agent Registry create/bind/link/terminal 和 transcript/attach evidence 收成 tracked current。自动选路、durable remote supervisor、统一 provider sandbox 仍不是 current
...
> 适用范围：cc-master master orchestrator 在 Claude Code、Codex、Cursor、Kimi Code 任一 origin harness 中，对本机 host-native 与 cross-harness agent workers 的规划、选择、派发、管理、恢复和验收
...
3. Claude Code、Codex、Cursor、Kimi Code origin 如何用不同 host surface 表达同一决策语义；
...
- 当前 host 事实和易变 probe 以 [`design_docs/harnesses/`](harnesses/) 为准，尤其是 [Claude Code](harnesses/claude-code.md)、[Codex](harnesses/codex.md)、[Cursor IDE Agent](harnesses/cursor.md)、[Cursor Agent CLI](harnesses/cursor-agent-cli.md)、[Kimi Code](harnesses/kimi-code.md) 与 [compatibility matrix](harnesses/compatibility-matrix.md)。Cursor 两个 surface 同品牌也不得合并事实或 role；Kimi origin plugin 与 `kimi -p` worker 同样是独立 bounded context。
...
## 5. 四 origin delivery 与 Track A/B

具体事件、payload 和版本事实可能变化，必须回读 [`design_docs/harnesses/`](harnesses/)；本节只锁定等价 intent 与诚实降级。

| Timing | Claude Code origin | Codex origin | Cursor IDE origin | Kimi Code origin | 共同 acceptance / degradation |
| --- | --- | --- | --- | --- | --- |
| ARM confirmation | `UserPromptSubmit` context | prompt-first `UserPromptSubmit` context | `beforeSubmitPrompt.user_message` | plugin `UserPromptSubmit` message | 只有 bootstrap 可 ARM；ccm 缺失不建半武装 board |
| Session start/resume | SessionStart startup/resume/compact | SessionStart additional context | 目标版本 probe 通过才用 SessionStart；否则 alwaysApply 只放静态 role/ref | manifest `sessionStart.skill` + prompt-first ARM | cached-only；missing/corrupt → 空或 unknown；零 provider probe |
| Compaction role | SessionStart compact 重注 | host resume/compact substrate | `preCompact` 不能注入；alwaysApply Track B 保底 | `sessionStart.skill` 在 compact 后重触发静态基座；动态板上下文不宣称 1:1 | Cursor/Kimi 保留各自 declared divergence；动态 snapshot 不塞静态规则 |
| Mid-turn delta | PostToolBatch 只在边变化 | 无 PostToolBatch；不用每次 PostToolUse 伪装 batch | 仅目标版本 probe 通过时使用有界 PostToolUse delta | 无 PostToolBatch / mid-flight pacing hook | 无等价 event 就 inbox/下次 start；routine delta 静默 |
| Stop action | block/advisory/inbox | block 或 systemMessage/inbox | followup_message 只给真实 decision/action | Stop hook 的 deny/message envelope | host 输出协议不同，但只有真实 directive 才要求 continuation |
| Native completion | task notification | 已发现的 subagent handle/completion | subagentStop/AwaitShell/notify_on_output | 内置 subagent/background handle；无自定义角色 | native handle/result 逐 host probe；没有 handle 不 running |
| Cross-run completion | ccm coordination delta + reconcile | 同左 | 同左 | 同左 | tracked dispatch fact 来自 ccm，不依赖 origin event；terminal ≠ task done |
| Handoff/resume | board resume + run attach | 同左 | 新 conversation 重新 ARM 后同左 | 新 session 重新 ARM 后同左 | 先 classify handle，再 drain/attach/poll/orphan |
...
- Codex/Cursor/Kimi 无 PostToolBatch 是负能力，不应由逐工具事件伪装。
...
board 新增 ✎ 段 `agents[]` 与 `ccm agent` namespace，构成跨所有派发类型（sub-agent / background shell / workflow / cross-harness CLI worker）的统一运行时登记簿。它的成熟度定位是 **L1→L2 之间的 registry 切片**：native/sub-agent 仍由 master 经 ccm verb 显式登记 + ccm probe reconcile；cross-harness CLI worker 则由 `ccm worker dispatch` 机械执行 idempotent create/bind/link/terminal。它**不是 durable L2 supervisor**——不含跨 session lease/journal、自动 route/reservation/live gate 或 remote transport，任何一项都不得由登记与 tracked local dispatch 外推。今天登记的 agent 在 §6.2 四分类下仍是 `legacy_session_bound`。
...
**probe 写权边界与证据分级**：`ccm agent probe` 只写 `agents[]` 自己的 probe / lifecycle 字段；观测 reconcile（`gone→orphaned` / `silent→uncertain` 的降级与 `alive` 的恢复）也只改 agent 自身 state，**绝不写 `task.handle` / attempt 投影**——那是 native-attempt dedicated writer / 未来 active-version reconciler 的独占地盘（§9.2 `board_write_mode=none` 精神）。判死与复活都按证据强度分级：`gone` 只出自确定性证据——pid kill-0 判死，或 mtime 类方法（session-file / transcript）的 seen-before 判死（上次同方法观测 `alive`/`silent`、本次完整扫描确认文件消失；从未见过 / 扫描不完整只出 `unknown`，启动竞态不判死）；`orphaned` 只被身份强的 mtime 类 `alive`（按 sid / 路径寻址）复活为 running，pid 的 `alive` 不够格（kill-0 不验进程身份·pid 复用假 alive）。reconcile 提议的转移写盘前再过引擎状态机闸（合法转移含 `starting→terminal` 启动失败收口），不合法保持原态并在 `--json` 以 `reconcile_rejected` 标注。拿不到即 `unknown`，不由相邻字段推导补齐。

**逐字复用的既有铁律**：`starting→running` 必须交真实 handle 证据——无真实 handle 不算 running（§6.1）；agent `terminal` 只是 worker 事实，terminal ≠ task done，父层独立验收（§10），`agent terminal` 绝不碰 task status。段形状与登记完备性由 `FMT-AGENTS` / `BIZ-INFLIGHT-AGENT` 两条 warn 级 lint 兜底；cross-harness `worker dispatch` 已把登记变成机械路径，其他 native 派发的「凡派发皆登记」仍是软提示纪律而非全局机械闸。
...
| L1 Machine visibility + shadow | surface/auth/model/quota facts、freshness、四层 context、planning/chains、shadow advice | master 可看见全机组合和解释建议 | 真实 spawn、quota guarantee | 四 origin cached summary 等价；unknown 保真；hook 零 credential/network |
...
| L3 Durable multi-provider writer | isolated worktree/permission、supervisor/journal/lease/attach、immutable runtime、多 provider drivers/fallback | 可恢复真实 writer orchestration | 自动学习即最优、中央公平 daemon | origin crash/upgrade/handoff 不中断 managed run；security 负例全零 |
...
以下保留 2026-07-10 批准方向下的纵切编号，作为演进与未完成工业化能力的台账；其中“三路”措辞已按当前四 host 校正，但单个切片晋升事实仍以 §13 的最新快照和真实 contract 为准。进入尚未完成的切片前仍需对应 implementation spec/ADR/contract，把 schema、migration、tests 和 rollback 锁定。
...
| S1 Machine read-only skeleton | surface inventory、capability/quota schema+home store、cached context；Cursor headless发现 | S0 types | 四 origin 同 revision；hook 零 probe/secret；shadow only |
...
| S8 Claude + Cursor + Kimi | Claude quota unknown保真、Cursor headless/dual-pool、Kimi rolling windows、四路 safe fallback | S4+S7 | auth≠quota、pool不混、account mutation=0、taxonomy不绕权 |
| S9 Plugin/hook/read model | canonical policy、four-origin delivery/native ledger、using-ccm、viewer/report、worker role | S1+S6+S8 | 任意 origin 同事实/同 run；Track A/B等价；frontend零推理 |
...
| SG6 parity | 四 origin 各自发明判断或假 parity | Capability INTENT + Track A/B equivalence + host probe |
...
| Task CLI | add/update/start/done + planning/routing 与五个 native-attempt dedicated writer；`ccm worker dispatch` 是独立的 tracked local process writer | native-attempt verbs 仍只写 ledger/authority transaction、不调用 host tool；tracked dispatch 不写 task status/acceptance，generic/`--force` 不能修 active native projection | automatic route→admission→durable run control 与 live canary | S0/S6 |
| Machine registry | 四 host adapter-owned install/session/usage descriptors；Cursor IDE plugin 与 `agent|cursor-agent` CLI 有独立 read-only descriptors，Kimi origin/worker 同样分面；auth/model/quota unknown 保真与 negative capabilities | 易变 provider entitlement、permission/sandbox 和 exact-model 仍须逐 surface live proof，不能由 installed 推断 | 更完整的 surface-level live capability snapshots | S1 |
| Account boundary | Claude token-blind vault/policy；Codex、Cursor、Kimi account mutation/autoswitch unsupported | Cursor/Kimi 仍需保持 origin/worker credential 边界；无统一 opaque auth snapshot | provider-neutral identity + non-Claude hard negative | S1/S8 |
| Usage/pacing | 四路 reader、multi-pool usage、engine pacing/forecast、monitor sweep；Kimi 5h/7d managed collector | provider signal coverage/TTL 各异；monitor 非持久 authority；unknown 不得补绿 | home machine quota read model + honest derivation | S1/S4 |
...
| Provider execution | Claude Code、Codex、Cursor Agent、Kimi Code 的 resolver-backed session-bound raw wrapper 与真实 PID lifecycle 由四 harness hermetic tests 证明 `current`；`worker dispatch` 在同一路径上提供 idempotent tracked registry lifecycle | ccm 只管理本机 session-bound process lifecycle；各 provider exact model/payer/task success 仍按 live probe 独立证明。Cursor native identity/SQLite transcript unsupported，但可接显式可读 transcript；无 automatic route/fallback | normalized automatic route/admission、durable/remote supervisor 与更强 provider-specific control semantics | S3/S6/S8 |
...
| Agent registry | board ✎ `agents[]` 登记簿 + `ccm agent` 七 verb（登记/探测/读取 noun）+ 按 handle 分级 probe/reconcile；`worker dispatch` 为 cross-harness CLI worker 机械执行 create/bind/link/terminal | native/background 派发的登记完备性仍靠 orchestrator 纪律（warn 软提示）；probe 只有 pid/mtime/transcript 级证据，无跨 session supervisor/journal/lease；handle 仍是 `legacy_session_bound` | run store 落地后 `agent_ref` 升级映射 `run_ref`；durable handle/attach 归 S3 | S1↔S3（registry 切片） |
...
| Plugin substrate | 四 host packages、SAP/PHIP/commands/hooks；四 origin 投影同一显式 raw/tracked-worker 决策、命令与验收指导，并消费同一 cached shadow context | host events 不 1:1：Codex/Cursor/Kimi 无 PostToolBatch，Cursor/Kimi reinject 各有 declared Track B 差异；plugin hook 不授权 automatic dispatch | canonical automatic policy + host landing + durable worker attention + equivalence | S9 |
| Native subagent | 四 host 都有各自指导/工具面；精确 Codex create/bind/cancel/terminal/reconcile ledger 为 `partial` | Kimi 无自定义角色；无通用受信 one-shot native invoke、跨 session durability；默认 spawn 为零 | 经 live probe 晋升的 native invoke + durable handle/result/cancel | S0/S9 |
...
| Viewer/report | board status/report/web viewer；server 侧 join `agents[].links` 派生 roster/node refs/单 agent 钻取；已登记 actor 可 tail harness-specific raw transcript（Claude/Codex/Kimi native，Cursor explicit external path）；frontend 零推理 | 只覆盖已登记 actor；Cursor native SQLite stream deferred，仍无完整 planning/route/quota-at-selection/operator attention | one read model；frontend render-only | S9 |
...
1. 同一 frozen board+machine revision 在四 origin 生成语义等价、脱敏、≤4 KiB context；坏 cache/collector sleep 时 hook RC0 且零 network/credential read。
2. Codex/Cursor `auth=true, quota=unknown`从不 automatic eligible；Cursor IDE/headless 独立安装/认证正反 case 正确。
3. 同 task 在四 origin 得到同 ccm eligibility/reason；真实 native capability/overhead 可改变候选事实，但 host 不改 core semantics。
...
- [ ] 四 origin 共享的是 intent 还是字节形状？Track A/B 是否诚实？
