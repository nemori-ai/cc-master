...
严禁进入 agent-visible projection：token、refresh token、email、credential path、raw private response、完整 argv/env、不可比较的精确余额、全量 transcript。大证据只留 owner-only home store，以 opaque ref 暴露。
...
| Machine registry | 已知 host 安装/session/usage descriptors | Cursor偏IDE launcher；无surface/auth/model/permission快照 | surface-level capability snapshots | S1 |
