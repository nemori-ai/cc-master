...
### 6.6 全量审计的执行结果：判据两次返工，最终一致度 80% / 88%【已执行·2026-07-30】

6.4 的 81% 是在 36 点上取得的。把同一份措辞铺到全部 256 点的审计上，它塌了——第二评分者一致度只有 **68%**，且分歧是系统性的：8 条分歧里 5 条都是「我们判 `motivation_conflict`／对方判前三类」，全量分布里 `motivation_conflict` 占到 **99/256 = 39%**。

根因不是标注员马虎，是**判据本身的兜底规则太松**。顺序过滤器规定「平局取靠后」，而几乎每段纪律性文字都会在结尾夹一句「别为此找借口」——于是最后一档把什么都吞了。一个吞掉 39% 的类别，撑不住「哪些知识随模型变强而更该留」这条分析。

判据因此改了两版，每版都在留出样本上实测：

| 判据 | 与第二评分者一致度 | 问题 |
|---|---:|---|
| 顺序过滤器（6.2 版） | 68% | `motivation_conflict` 吞噬一切 |
| v2「动机完美反事实测试」 | 76% | 矫枉过正——把「既是洞见又是约束」的点全推给 `capability_gap` |
| **v3「先判主体：行为约束 vs 事实方法」** | **80%（留出样本）** | 残余分歧全部集中在「主体」这一个轴上 |

v3 的措辞是两步：**先问这段知识的主体是行为约束还是事实方法**（结尾那句反合理化是附加条款，不改变主体）；主体是行为约束时再问「告诉一个动机完美的执行者，它会不会遵守」。实测里第二步几乎恒真——**鉴别力全在第一步**，这让判据比它看上去更简单、也更可审计。

**80% 是在留出样本上测的**：v3 与旧判据在同一批 25 个点上对比（旧判据在这批上只有 56%），而这 25 个点没有参与任何一版判据的调校。这是 predict-then-validate，不是在同一批上爬坡。

#### 双重性质点的裁决规则

v3 落地后仍有 25 个点，其标签与第一轮写下的理由行自相矛盾。读下来发现它们是同一类：**既是方法／事实、又是行为约束**（测试先行的五步循环、组合优于继承、一次只问一个问题）。判据迭代三版、每版换一类错，说明这不是措辞问题，是这批点真有双重性质。

用**目的本身**裁——分类学存在的理由是回答「哪些知识随模型变强而更该留」：

> 把方法／事实那部分拿走，交给一个能力更强的模型，**它自己能不能补回来？**
> - **能**（公共工程共识）→ 那部分会随模型变强失效，留下的是约束 → `motivation_conflict`。
> - **不能**（本项目独创的框架、本工具的字段语义）→ 那部分才是承重结构 → 归前三类。

这条规则在这 25 个最难的点上拿到 **88% 的双人一致度**——高于全量的 80%，因为它直接问到了要害。剩下 3 条分歧逐条人读定案。

#### 最终结果

| | 6.2 判据 | v3 + 裁决 |
|---|---:|---:|
| `motivation_conflict` | 99（39%） | **51（20%）** |
| `capability_gap` | 58 | 86 |
| `environment_fact` | 83 | 110 |
| `prosthetic` | 16 | 3 |

（右列以落盘后的 250 个母本实测，非估算。）

`prosthetic` 从 16 掉到 3 值得单独记一笔：在 v3 下，「跨压缩会忘」很少能压过「这本来就是本工具的字段事实」。这个类别近乎空了——**是否该保留它，留待下一轮 portfolio 判定**，本轮不动。

**方法论教训（比数字更值钱的两条）**：① 36 点上验过的判据，不等于全量语料上站得住——小样本会让判据的兜底规则来不及暴露；② 判据每改一版都必须在**没参与调校的留出样本**上测，否则「一致度上升」只是过拟合。
...
#### 6.3.1 覆盖率实测：假设成立，且不对称的方向自带信息【已执行·2026-07-31】

上面那句「多数只写了一侧」原标着**待核假设**。现已对全部 51 个 `motivation_conflict` 点逐条通读验证（判据：正文里能找到**具体句子**分别描述两侧的失败才算齐全，抽象地说「要把握分寸」不算）：
...
**② 不对称的方向比总量更有信息** —— 省事侧 29 : 过度侧 5，接近 **6:1**。写纪律时我们天然想得到「别偷懒」，极少想得到「别过度」。这与第七部分实测的**过交付结构性不对称**互为佐证：**不只是 agent 会过交付，连我们写用来约束它的纪律，本身也系统性地偏向防省事那一侧。** 规约要求两侧齐全，堵的正是这个盲区——而盲区大小现在有数了。

**③ 补的成本是「补另一侧」，不是「从零写」** —— 零个点两侧全缺，说明这批点的作者都至少想到了一侧，不存在空转文档。

**处置**：**这条闸本轮不上。** 上闸即飘红 34 个（占该类三分之二），那是一摊独立的内容补写工作——且补的是纪律型正文，每条都要有品味，批量糊只会逼出「为过闸而写的敷衍两侧」，反而稀释规约本身。留作独立议题。

顺带：`outside.low-risk-exception` 作为「双侧走廊成熟先例」的地位经本次逐条核验**确认成立**（两侧齐全）。
