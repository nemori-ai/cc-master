...
2. retry mutation 把 task 转为 `ready`，删除旧 `started_at`/`finished_at`/`artifact`，并把 `verified`
   重置为**布尔** `false`（不得写字符串 `"false"`）。随后既有写入关卡仍执行 dependency
   `reconcileGating`：deps 未全 done 时最终持久态为 `blocked`，否则为 `ready`；human/JSON 输出都必须按
