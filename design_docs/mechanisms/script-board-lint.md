...
> 类别：运行时带外 node 脚本（独立手动 board lint·NOT a hook·随 skill 分发）。源码：`skills/orchestrating-to-completion/scripts/board-lint.js`。复用 hook 同一份 lint 核心（`hooks/scripts/board-lint-core.js`），补 PostToolUse hook 看不见的编辑路径（尤其 Bash 改 board）。
