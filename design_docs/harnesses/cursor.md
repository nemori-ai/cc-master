...
本页只覆盖 **Cursor IDE Agent**（canonical surface id：`cursor-ide-plugin`；本机 Composer / Agent Chat / Cmd+K Agent 操作）。
...
- Cursor Agent CLI / headless one-shot ——它是独立的 `cursor-agent-cli` worker surface，不等于本页的 IDE plugin / hook adapter；两个 bounded context 的角色、独立事实轴、负能力、pool 禁止推导、RED fixtures 与迁移/rollback 以 [`cursor-dual-surface-contract.md`](cursor-dual-surface-contract.md) 为正式合同，mode-specific admission/result 子合同见 [`cursor-agent-admission-contract.md`](cursor-agent-admission-contract.md)，易变 binary/auth/model/quota/sandbox/plugin-host/invoke/result/cancel/resume 事实在 [Cursor Agent CLI facts](cursor-agent-cli.md) 维护；完整 spawn / poll / cancel / resume transport 仍未交付。

## 同品牌 surface 边界【本仓】

Cursor IDE plugin 与 Cursor Agent CLI 是两个独立 bounded context。任一侧的安装、认证、model、quota、sandbox 或 plugin-host 状态都不得推出另一侧状态。正式角色与生命周期合同见 [`cursor-dual-surface-contract.md`](cursor-dual-surface-contract.md)；本页只维护 IDE/plugin facts，CLI 的 executable alias、只读 probes、手动认证、两池 quota、sandbox qualification 和 provider runtime 边界见 [Cursor Agent CLI facts](cursor-agent-cli.md)。
...
`ccm harness list` 将 IDE/plugin 报为独立 descriptor `cursor-ide-plugin` / `ide-plugin`：继续沿用 `cursor` executable、Cursor config dir 或 cc-master local plugin dir，顶层 harness `installed` 只供 plugin install/upgrade 消费。CLI binary 存在不能把它翻真；CLI descriptor 与只读 inventory 细节见 [Cursor Agent CLI facts](cursor-agent-cli.md)。
...
- `tests/content/capability-host-coverage.test.mjs` — 四 host 均有 strategy 占位。
