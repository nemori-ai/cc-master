...
kimi-code（Moonshot AI CLI）事实来源：[`kimi-code.md`](kimi-code.md)（2026-07-16 本机 v0.26.0 probe + binary 取证，2026-07-21 按现行 adapter/collector 对账）。它是 origin + headless worker 双角色；因 manifest/hook 形态差异较大，仍在文末专表按同一维度列出。

| Surface | Claude Code | Codex | Cursor (IDE Agent) | cc-master 决策 |
| --- | --- | --- | --- | --- |
| 当前支持状态 | 已发布 adapter。 | Codex adapter 已有 dist：skills + hooks + prompt-first bootstrap；install/upgrade 通过本地 marketplace 注册 Codex plugin；commands 不投影原生 slash-command artifact。 | **已发布 adapter**（`plugin/dist/cursor`、probe 完成、install `--harness cursor`）。 | 四 host 独立 release zip（ADR-022；Kimi 见文末专表）；source 保持多 host adapter 边界。 |
...
| `kimi-code` (`kimi -p` headless one-shot) | `kimi -p "<prompt>" --output-format stream-json`；exit 0；OpenAI-message JSONL；session `session_<uuid>` 落 `session_index.jsonl`；`kimi export <sid>` 拉 transcript zip；resume `kimi -S/-c` | CLI 本身无 headless usage 命令，但 ccm 已通过 `kimi-usages-api` 读取 current-login 5h/7d；`-p` 自身非交互执行工具且不能叠 `--auto`/`-y`；退出码全谱未穷举；ACP / server 更强 transport 未深 probe | kimi origin plugin 与 `kimi -p` worker 是独立 surface：installed/auth/model/quota 不跨 surface 推导；详见 [`kimi-code.md`](kimi-code.md) §10/§13 |
...
| Surface | kimi-code 事实 | 与其他 host 的关键差异 |
| --- | --- | --- |
| 当前支持状态 | **已发布 adapter**：`plugin/dist/kimi-code` 含 skills、6 个 host-native commands 与 5 个注册 hook；`install.sh` / `upgrade plugin` 支持 `--harness kimi-code`。 | origin plugin + `kimi -p` headless worker 双角色；两者仍是独立 bounded context。 |
...
| Project memory | 项目根 `AGENTS.md`（>32KB 告警）、`.kimi-code/AGENTS.md`、`~/.agents/AGENTS.md`；manifest `sessionStart.skill` 在 compaction 后原生重触发。 | `PostCompact` hook output 会被丢弃；静态 role substrate 可重注，但动态板上下文仍是 Track B 缺口。 |
| Dist artifact | `plugin/dist/kimi-code` 由 sync 生成；`cc-master-plugin-kimi-code-*.zip` release asset；无 `validate` 等价（doctor 只校 config/tui）。 | 靠 projection + hook probe；dist 不手改。 |
| 主要风险/缺口 | 无自定义 subagent / Workflow / PostToolBatch / 非阻断 Stop advisory；account pool 与 external statusline unsupported。只读 5h/7d 已由 ccm dashboard collector 补齐。 | 见 Capability Cards 与 `kimi-code.md` §12/§13。 |
