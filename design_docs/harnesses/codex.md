...
## Candidate / Provider / Driver Contract Addendum（2026-07-13）

`ccm provider inspect codex --request @file --json` 已实现为受资格门控的**read-only
candidate**；v1 contract 位于
[`../2026-07-13-codex-candidate-provider-driver-contract-v1.md`](../2026-07-13-codex-candidate-provider-driver-contract-v1.md)：

- installed、auth、entitlement、live model catalog、registry 与 multi-bucket quota 各自有独立
  freshness/provenance；evidence id、payload digest、source revision/path 与每个 quota window、
  rolling-24h derivation、requested/resolved/actual identity 做 canonical 绑定，非空假 id 不算证据；
  版本字符串不等于 behavioral capability probe；
- `model:auto` 禁止，requested/resolved/actual model + effort 分栏留证，缺 actual 或任一 mismatch
  都 fail-closed；
- headless 编译固定 read-only + approval never，login/logout/switch/auth-write/credential mutation
  为零；
- quota unknown/tight/hard-stale 不得 automatic eligible；Codex 只以 7d 作 hard ceiling，rolling
  24h 仅 advisory，5h 不参与 eligibility/pacing；
- opt-in RED 不把 fixture catalog、scenario id、trace path 或动态 proof nonce 暴露给 handler；
  必须穿过未来 CLI router endpoint，发出带 correlation id 的真实 app-server 请求，并从响应、
  JSONL 与 structured result 推导 evidence digest / actual identity；粗粒度 superficial spawn、
  validator、registry lookup 或 parser-only 均不足以变绿；
- protected workspace/board-home/Codex/Cursor roots 与 parent env 由外部 snapshot 验真；child env
  是 exact allowlist，不从 parent denylist 过滤；handler 的 process/network authority 必须走注入的
  provider-runtime port，测试在加载 handler 前拦截 direct socket/HTTP/fetch/process/worker escape；
  `side_effects.*=0` 只是 telemetry，不能自证 read-only。

该 endpoint 的实装边界是：只经 `provider-runtime` port 解析/启动本机 `codex`，不读取或写入
credential、不做网络请求、不切换账号，且仅在 live entitlement × local registry × 7d quota 全部
fresh/complete/一致时执行。它不宣称现有 `ccm usage` 的历史 5h 字段迁移已经完成；后者仍按上文
target addendum 另行收敛。
