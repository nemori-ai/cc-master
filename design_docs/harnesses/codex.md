...
- `Stop` hook 输出 `{"decision":"block","reason":"..."}` 会触发反复 Stop 重入；90s probe 中记录 43 次 Stop，最终 timeout（rc 124），`stop_hook_active` 从第二轮起为 `true`。
- `Stop` hook 退出码 2 也会触发反复 Stop 重入；90s probe 中记录 64 次 Stop，最终 timeout（rc 124），`stop_hook_active` 从第二轮起为 `true`。
- 因此 Codex adapter 的 `Stop` hook 生产策略应先做 advisory（如 `systemMessage` 或已另行验证的 context 注入），不要用 block / exit 2 来模拟 Claude Code 的 Stop 阻断或续跑提示。
...
Codex adapter 不把 Stop hooks 投影成 Claude Code 的 hard Stop gate。原因是实测 `Stop` 上的 `decision:block` 和 exit 2 都会导致重复 Stop re-entry。

当前 Codex `verify-board` 是 replacement advisory：

- dormant-until-armed：只有当前 session 的 active board 匹配时才输出。
- 输出：`systemMessage`，非阻断。
- 覆盖信号：空 active board、ready / uncertain 任务、仍在等用户的 blocked task、in-flight 但无 armed watchdog、最终目标 self-check。
- 不实现 Claude Code 的 fingerprinted self-check sidecar gate；这是 host-specific hard-gate 能力，Codex Stop 当前不安全。
...
| Subagents | Codex subagent workflows | CLI 和 App 当前支持；只在用户显式要求 subagents / parallel agents 时启动；Codex 负责等待并汇总结果。 |
...
- Claude Code `Task/Agent run_in_background`：Codex 有 subagents，但本仓尚未验证出可从 adapter 文本稳定映射的 handle / completion notification / permission 语义。
...
因此 Codex adapter 当前只能把这些记录为“官方能力 / 待适配能力”。在完成 host-native probe 之前，`master-orchestrator-guide` 这类依赖后台派发、完成通知、watchdog 和 hooks 的 runtime skill 继续对 Codex 使用 unsupported stub，不能复制 Claude Code 指令。
