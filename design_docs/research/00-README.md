...
*本目录的研究在 worktree `research-dynamic-workflow` 中产出。四份报告 + 本导读已全部完成。报告 1 由主线程基于一手素材（含本 agent 自身 `Workflow` 工具契约）撰写；报告 2/3/4 由三个并行后台研究 agent 检索、主线程综合落盘——这一编排过程本身即"主线程不空等"原则的现场演示。*
