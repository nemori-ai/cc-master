...
- `eligible_for_automatic_selection`, which is true only for a fresh structurally valid snapshot;
...
`future-invalid`; after `valid_until` it is `hard-stale`. Both are ineligible but remain observable
with exit 0 so callers can explain the denial.
...
2. `observed_at > valid_until`, or an observation in the future at projection time;
