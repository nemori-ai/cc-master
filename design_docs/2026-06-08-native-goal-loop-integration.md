...
为让 agent 认回"我在冲哪段",board 柔性边新增 `phase` 段:`current_phase` + 阶段 goal 条件原文 + 本阶段 task 范围。`reinject.sh` 重注时带出,提醒 agent:认回阶段、核对 goal 是否还挂着;**若 goal 丢了则按 board 记录的条件重设**(hook 读不到 goal 状态,只能提醒 agent 自核)。这是 cc-master "board 扛 compaction"既有套路的自然延伸。
...
| 2 | `skills/orchestrating-to-completion/SKILL.md` | 文档 | ① 决策程序补「识别自驱区段 → 起点设阶段 goal → 第 6 步结论+验收证据显式落对话与 board」;② 七镜头/红线加 goal 纪律:**goal 是分段发令枪非全程枷锁,条件必带正当等待逃生口**;③ board 协议要点提 `phase` 段。 |
| 3 | `skills/orchestrating-to-completion/references/async-hitl.md` | 文档 | 写透 goal × HITL 咬合:阶段边界=HITL 点=goal 清除点;逃生口=正当等待判据;两个 Stop hook 相容性(见线三)。 |
| 4 | `skills/orchestrating-to-completion/references/board.md` | 文档 | `phase` 柔性边 schema:`current_phase`、阶段 goal 条件原文、本阶段 task 范围。 |
| 5 | `hooks/scripts/reinject.sh` | **动代码** | 重注文案加:读 board `current_phase`,认回阶段、核对 goal 是否还挂着;goal 丢了则按 board 记录条件重设。 |
| 6 | `skills/orchestrating-to-completion/assets/board.template.json` | **动代码** | 加 `phase` 字段骨架。 |

### 线二:`/loop` 后台 shell 消解

| # | 文件 | 性质 | 改什么 |
|---|---|---|---|
| 7 | `skills/orchestrating-to-completion/references/dispatch.md` | 文档 | 「三机制 · shell」加范式:等外部不可追踪状态 → 后台 shell 轮询(`until …; do sleep N; done` + `run_in_background`)靠完成通知重入。点明 **别用 `/loop`/`ScheduleWakeup`,理由 ship-anywhere**。 |
...
| 10 | `tests/hooks/test_reinject.sh` | **动代码(测试)** | 补断言:含 `current_phase` 的 board 被重注时输出带阶段提醒。(board 模板加字段,捎带核 bootstrap test。) |
...
- compaction 后 `reinject` 带出 `current_phase`,agent 认回阶段、续编排不丢角色。
