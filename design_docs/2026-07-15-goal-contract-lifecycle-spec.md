...
- 实质歧义或高风险：写 pending contract/Brief → 给用户最小 decision package → 不派发依赖该答案的执行任务。
...
| 与当前 contract 冲突且尚未获授权 | `pending` + decision package |
...
| verify-board | 完成前校验 contract integrity；pending 仅在结构完整的 user decision package 等待态合法；完成自检绑定当前 revision |
...
- **GC-02 Framing first**：agent 在切 DAG 前完成 Goal Framing Test，并写入 normalized goal；有实质歧义时停在 pending decision package。
...
- **GC-06 Scope-creep rejection**：面对“顺手优化”，agent 用 Goal Trace Test 拒绝或单列 out-of-scope，不把它偷渡进 DAG。
...
- **GC-13 Skill behavior**：pressure baseline 在 copy、scope creep、amendment、local-green/global-wrong 场景先失败；加指导后独立 Codex 评审通过且无越界/复述。
