...
//   ——未武装静默 exit 0（复用 hook-common 的武装闸 boardMatches）。红线2：只读 narrow-waist（owner.active/
...
function main() {
  ...
  try {
    ...
    return; // 非法 stdin → 静默（兜底，main 整体也在 try/catch 内）
  }

  // session_id / agent_id 只从 stdin 根对象顶层读（JSON.parse 让 tool_results[] 内同名字段天然不污染·CODEX10）。
  const sid = (typeof obj.session_id === 'string') ? obj.session_id : '';

  // ── SUB-AGENT 闸（红线4：指挥不演奏）─────────────────────────────────────────────────────────────────
  // PostToolBatch 在 sub-agent（Task 派生子 agent）上下文内部也触发；官方 stdin 此时带顶层 `agent_id`（主线缺席）。
  // 注入进 sub-agent 的 additionalContext 进的是 leaf worker 自己的 context——主编排者专属的 WIP 软警告绝不能
  // 泄漏给单元 worker（破红线4：把指挥的乐谱递给乐手）。只认带引号的字符串 agent_id：`"agent_id":null` 或
  // 字段缺席 → 非字符串 → 视为主线（与 v1 sed「只认带引号值」字字对齐）。非空字符串（sub-agent）→ 静默早退
  // （在武装闸之前，最早可静默处）。
  ...
  // 本 session 的 active 板（武装闸 boardMatches）。按文件名排序 = 与 v1 glob `*.board.json` 同序（确定性）。
  // 无匹配板 → 休眠静默（dormant-until-armed）。
  const boards = listMatchingBoards(HOME_DIR, sid)
