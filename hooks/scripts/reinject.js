...
function main() {
  ...
  if (boards.length === 0) return; // 无 active orchestration → 休眠静默（exit 0）
  ...
  if (danglingEntries.length) {
    ctx += ` Note on resume: your board has unresolved node(s) needing attention — stale/escalated: ${danglingEntries.join(', ')}. ` +
      `Reconcile these (re-run stale, re-altitude escalated) before scheduling new work.`;
  }

  process.stdout.write(`{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":${jsonEscape(ctx)}}}\n`);
}
