...
- **Project website (`website/`)** — a bilingual (EN default + `/zh/`) Astro static site, deploy-ready for Cloudflare: a landing page that leads with the design philosophy (the five deaths, the seven lenses, the ambient/advisory/directive attention protocol), real ccm web-viewer screenshots and a live interaction loop (captured from a real `ccm web-viewer` serving the i18n fixture board via `website/tools/capture.mjs`), a five-step full-chain story, the three-era evolution timeline, and a curated docs section (getting-started + concepts + usecase guides) that deep-links the repo instead of mirroring it. Design tokens are ported from the web-viewer (OKLCH mission-control, light-primary with dark toggle). Docs source lives in `website/src/content/docs/`; deployment config is `website/wrangler.jsonc` (Workers Static Assets).

### Changed

- **README rewritten around the top-level narrative** — new architecture diagram (`docs/images/architecture.svg`: four harnesses → plugin adapters → master orchestrator → process boundary → ccm engine → boards/workers/viewer), a "human ↔ agent interaction paradigm" section (attention reallocation, layered judgment, explainability), and the three-era evolution story with a timeline diagram (`docs/images/evolution.svg`: plugin era → engine era → meta-harness era). Viewer screenshots refreshed to the i18n fixture board so the README story, the sample orchestration, and the site all tell the same run. `README_zh.md` rewritten in lockstep.
...
- **kimi-code harness adapter (4th host)** — cc-master now projects a fourth adapter for Moonshot AI's kimi-code CLI agent, alongside Claude Code, Codex, and Cursor. The kimi-code adapter ships the distributed skills (with `${KIMI_SKILL_DIR}` path-token support), host-native **namespaced** commands (`cc-master:as-master-orchestrator`, `discuss`, `distill`, `handoff-to-new-session`, `retro`, `stop` — `cc-master:stop` is namespaced and does not collide with any built-in), and the core orchestration hooks (bootstrap, board-guard, board-lint, verify-board, and reinject via `PostCompact`) registered inline in the `kimi.plugin.json` manifest. `install.sh --harness kimi-code` performs a managed-directory install into `$KIMI_CODE_HOME/plugins/managed/cc-master/` with an `installed.json` entry, and the release workflow emits `cc-master-plugin-kimi-code-<version>.zip`. Documented gaps are honest: no custom subagent roles (only built-in coder/explore/plan/general + Agent Swarm), no Workflow equivalent (authoring-workflows is an unsupported stub), no PostToolBatch batch-boundary event, and no CLI quota signal at all (`ccm usage` returns `available:false` for kimi, so there is no window pacing and no account auto-switch). The advisory Stop nudges (usage-pacing, coordination-inbox, identity-nudge) and the SessionStart orchestrator-context reinjection are also not part of this initial adapter — the kimi-code hooks ship the core skeleton (bootstrap, board-guard, board-lint, verify-board) plus PostCompact reinject only.
- **Agent-registry lifecycle guidance — both halves** — the orchestrator skills now teach the full agent lifecycle loop. `#177` surfaced "register on every dispatch" (create/bind/link) as a top-level `master-orchestrator-guide` anchor and closed the `using-ccm` guidance gap; `#184` adds the symmetric reverse-closure — once an agent's output is harvested/verified, the orchestrator explicitly `ccm agent terminal`s it (success-harvest is the other half of spawn-failure closure; `agent terminal ≠ task done` guards only the forward direction; `agent probe` reconciles liveness but never →terminal). `using-ccm` documents the closure plus a batch-race precision (repeated same-command `agent terminal` runs race-free in sequence; only create→bind→link needs one-at-a-time by data dependency). Pairs with the ccm-side read-only `agent list` `stale_candidates` advisory shipped in ccm-v0.22.0-rc.0.
...
- **Goal Contract lifecycle and drift guards** — `as-master-orchestrator` now treats its free-form argument and GitHub issue as source evidence instead of copying them into `board.goal`. Fresh runs start with a pending contract, then use revisioned `ccm goal set|confirm|amend|show|check`; complex context can be stored as an immutable, hash-verified Goal Brief linked from the board. Claude Code, Codex, and Cursor entry/resume/compaction/periodic/completion surfaces align work to the current revision, reject malformed or tampered briefs, and permit a pending handoff only through a complete `blocked_on:user` `decision_package`. Legacy boards retain their prior behavior.
...
- **Cross-harness worker MVP** — `ccm worker` launches a locally installed Claude Code, Codex, or Cursor Agent CLI, passes target-native arguments through without lossy normalization, exposes target help, returns structured lifecycle evidence, and owns the child process tree through terminal settlement. This is a local session-bound transport; durable remote transport and stronger isolation remain post-MVP work.
...
- **Agent Registry v1** — `agents[]` separates runtime actors from task planning and execution attempts. `ccm agent create|bind|link|terminal|probe|list|show` records cross-harness lifecycle evidence; the web viewer adds an agent roster, inspector, task links, and probe state. Terminal agent state never implies task acceptance.
- **Machine-wide quota posture and notifications** — all three origin hosts consume cached, provider-scoped quota posture for all locally supported harnesses through orchestrator pre-context and coordination notifications. Signals are cross-session and provider-silent; Codex uses the official seven-day ceiling plus rolling-24h advisory, Cursor Agent contributes its subscription signal, and no Codex/Cursor account auto-switching is introduced.
- **Unified model-policy guidance** — every origin now sees one cross-harness O/T1/T2/T3 model taxonomy, orchestrator-tier candidates, task-role floors, ample/tight fallback chains, current admission facts, and bounded community task-affinity taste. Architecture/design uses orchestrator-capable models; implementation defaults one tier down when the contract is complete; review prefers a heterogeneous family; read-only research prefers two tiers down.
- **Provider boundaries and admission evidence** — add fail-closed Codex exact-model admission, native-attempt evidence, quota reservation authority, and offline Claude/Cursor provider fixtures. Cursor IDE and Cursor Agent are modeled as independent surfaces.
- **Declared delivery/dependency truth** — optional candidate, delivery target, edge qualification, immutable artifact, and reviewed-reconciliation contracts distinguish “candidate passed” from “landed in the baseline consumed downstream.”

### Changed

- **Linux/macOS transactional installer publication** — `install.sh` now copies the SEA and every Claude Code / Codex / Cursor plugin tree into a target-adjacent stage, validates checksum/manifest/executability, fsyncs, and activates by atomic rename or immutable-version pointer. Injected copy/checksum/exec/rename/`EXDEV`/activation/backup-barrier/rollback/rollback-barrier/cleanup failures return honest nonzero structured state; recovery names and successful rollback renames cross a parent-directory durability barrier before cleanup, while barrier failure retains resolvable old/new artifacts for recovery. Binary target symlinks (including dangling links) fail closed, and spaces, Unicode, modes, ownership, and safe relative symlinks are covered by a Linux + Darwin CI matrix. The installer now documents Node.js 22+ as an unconditional prerequisite, including pinned and local/offline modes.
- **Cross-harness identity becomes the default** — `master-orchestrator-guide`, `as-master-orchestrator`, Codex entry skill, Cursor rules, dispatch/reconcile/handoff guidance, quota consumption, and model allocation all start from a machine-wide worker pool rather than an origin-local model list. Origin overlays now retain only genuine host invocation differences.
- **Web viewer becomes execution-aware** — the shipped viewer exposes Goal Contract, planning/routing snapshots, quota posture, Agent Registry, linked tasks, and route-aware filters/URL state while remaining local, token-gated, and read-only.
- **Portable runtime and service lifecycle** — executable/path resolution, runtime launchers, statusline install boundaries, launchd/systemd serialization, monitor/web-viewer reconcile, and persistent stores use explicit Linux/macOS contracts instead of hard-coded machine paths.
- **Skill portfolio stays at eight** — `master-orchestrator-guide`, `authoring-workflows`, `using-ccm`, `slicing-goals-into-dags`, `dev-as-ml-loop`, `engineering-with-craft`, `pacing-and-estimation`, and `distilling-lessons-into-assets`; `using-ccm` is synchronized with the current CLI and all **82** board validation rules.

### Fixed

- **Accountable watchdog handles** — `ccm watchdog arm` now requires a non-blank real scheduler/loop/monitor/shell handle and rejects missing or blank values atomically even with `--force`. Status preserves legacy object fields while deriving explicit health for missing-handle and expired records; verify-board treats those records as unarmed across Claude Code, Codex, and Cursor. `watchdog disarm` removes both canonical `watchdog` and legacy `wakeup` fields completely, while warn-only lint keeps legacy boards writable during migration.
...
- **Attempt/review correctness** — retries archive old evidence, invalidate stale review verdicts, and keep downstream review gates blocked until the current attempt records `APPROVE`.
- **Worker/process cleanup** — owned descendant trees are reaped and allowed to settle before terminal evidence is returned; runtime launcher materialization, locks, and persistent writes are crash/concurrency safe.
- **macOS runtime and service evidence** — Darwin invocation uses an attested final-path tier instead of Linux fd pseudo-path assumptions; launchd deactivation/uninstall and evidence reruns report partial failure honestly.
- **Clean-worktree and generated-fixture regressions** — content mutation tests create their ignored scratch root, Agent Registry fixtures carry matching runtime actors, and generated adapter projections remain deterministic.
...
- **Hook parity contract layer (HOOKPAR-DEC / ADR-028)** — added `plugin/src/hooks/<hook>/CONTRACT.md` (host-neutral business-rule SSOT, one per dual-implemented hook: bootstrap-board / board-guard / board-lint / reinject / identity-nudge / usage-pacing / verify-board), a generated `design_docs/hook-parity-matrix.md` (via `scripts/gen-hook-parity-matrix.sh --check`, wired into `run-tests.sh`), a minimal normalization bridge in `hook-common.js` (`normalizePayload`/`ctx.normalized`, additive-only, zero behavior change to existing hooks), a behavioral fixture parity test (`tests/hooks/test_parity-fixtures.sh`, covering FUSE / `segmentTouchesRealBoard` / handshake dedup across both hosts), a structural CONTRACT.md PARITY-anchor check extending `tests/content/hook-injection-contracts.test.mjs`, and a PR-diff existence check (`scripts/check-hook-parity-touch.sh`). See `adrs/ADR-028-hook-parity-contract-and-normalization.md`.
...
- **Codex hook parity (HOOKPAR-DEC)** — Codex's `verify-board-core.js` gained the FUSE anti-infinite-loop circuit breaker and the `ccm board lint` rollup-consistency reminder (both previously only on Claude Code); `board-guard-core.js`'s `bashWritesBoard()` now ports `segmentTouchesRealBoard` and drops its own whole-command fallback branch (aligning it with the Claude Code fix above — the Codex-side fallback was an independent, undeclared source of the same false-positive shape); `verify-board-core.js` / `board-guard-core.js` / `usage-pacing-core.js` / `identity-nudge-core.js` now wrap their agent-facing messages in the ADR-018 `directive`/`advisory` tag protocol, matching Claude Code's `hook-common.js` wrappers. `hooks.yaml`'s stale `verify-board.host_coverage.codex: implemented-advisory` corrected to `implemented-blocking`.
...
#### board v2 数据模型 + ccm 引擎解耦

- **board v2 完整数据模型 + 三档建模（ADR-013·schema v1→v2）** — board 从「被动 JSON + 各消费者各自解析」演进为「**完整数据模型 SSOT + 三档字段标注**」：一份模型把全部命名枚举（status / executor / taskType / role / refKind / askType / logKind / judgment_calls / watchdogMechanism / acceptance 等）、**每字段六要素元数据**（tier 🔒load-bearing / 👁observed / ✎flexible · type · 缺省 · 谁读 · 谁写 · 何时写 · 缺失降级）、**不变式注册表 INVARIANTS**（每条规则 id/级别/家族 SSOT）、status 状态机、跨消费者共享谓词收口到一处，lint / graph / CLI / viewer 全部派生。narrow-waist 演进为三档建模——红线 2 真正保护的仍只是 🔒 子集。
- **board v2 task 实体 + 敏捷新模块** — task 升级为完整 DAG node：`executor`（5 值，取代 v1 `mechanism`+`assignee`，含 `external`）· `type`（8 值开放枚举，未知值降 warn 不硬拒）· `role`（normal/fill-work）· `references`（`{kind,ref}` 数组·`ref` 须绝对路径或 URL）· `estimate`（估点）· `acceptance`（**两态目标函数**：一句话 DoD 或 `{criteria:[…]}`，ML 比喻 objective）。新增三个顶层模块：`cadence`（节奏 / timebox 策略层）· `judgment_calls`（agent 自决诚实台账·HITL 第三姿态）· `scheduling`（`wip_limit`+`owner_wip_limit`）。`wakeup`→`watchdog` 重命名；`num_account`/`accounts[]` 退役（effective-N 改从 accounts.json registry 读）。
- **board 引擎解耦为独立安装的 `ccm` CLI（ADR-014·工业化 TS monorepo）** — board 状态逻辑（数据模型 / lint / graph / lock）从「插件内嵌的零依赖 CJS」解耦为**独立安装的 `ccm` CLI**：源在 `ccm/`，pnpm/Turborepo monorepo——`@ccm/engine`（board 引擎 SSOT·TS 库）+ `apps/cli`（`ccm` 命令）+ 未来 desktop/web 平行消费方。cc-master plugin **降为消费方之一**：hooks 经**进程边界** `spawn ccm` + JSON 契约访问 board、**绝不 import `@ccm/engine`**（TS/npm 依赖锁在 ccm 内·进程边界 = 红线1 新落点）；webview 吃 vendored `@ccm/engine` IIFE。**分发模型** = per-OS Node SEA 单文件可执行（startup 2× 快于直跑 node）、主机预装；hooks 解析顺序 `$CCM_BIN`（绝对路径覆写）> PATH 上的 `ccm`。配 GitHub Actions CI（碰 `ccm/**` 即 build/typecheck/lint/test）+ 多平台 SEA release + changesets。
- **`using-ccm` skill（SKILL D·一体两面手册）** — `ccm` CLI 操作手册：面 1 = 命令面（`command-catalog.md` 全量命令逐条对 registry）、面 2 = board 模型与字段取值指南（`board-model-guide.md`：领域概念解释 + 字段取值判断 + 决策树 + **全部 FMT/GRAPH/BIZ 校验规则速查**），让 agent 操作 CLI 一次写对、不因不知规则反复撞 `exit 3`。**board 协议 canonical SSOT 归位 `@ccm/engine` 的 board-model**，`master-orchestrator-guide` 的 `board.md` 降为 orchestrator 侧派生叙事视图、非 SSOT。AGENTS.md 新增「`ccm` ⟷ `using-ccm` 锁步」抗漂移硬约束（ccm 命令面 / lint 规则 / 字段档位改动必同 PR 同步 skill）。
- **方法论分发 skill 三件（敏捷切分 + 执行循环 + 工程手艺）** — `slicing-goals-into-dags`（SKILL E）= 敏捷切 DAG 方法论（目标→纵切薄的端到端可 ship 增量 / walking skeleton / 粒度品味 / 价值 × 风险排序，「切」先于 A 的「排」）；`dev-as-ml-loop`（SKILL F）= 执行侧 dev loop 心智（agentic-loop 开发 = ML 优化过程：验收 = 目标函数 / 迭代-测量-收敛 / 局部最小值 = 钻牛角尖→restart / 收敛即停别过拟合）；`engineering-with-craft`（SKILL G）= 设计/开发/测试的工程手艺（DDD/SDD/TDD/OOP 整合成五条共享根 + 工程红线·深度进 ddd/oop/sdd/tdd 四个 reference）。三者与 A/B/D 正交不重叠（红线 3）。

#### 估算 + 配速引擎（ADR-015）

- **`ccm baseline` namespace + EVM 计划基线字段（P1·估算引擎地基）** — 新增两个 ✎ 字段 `board.baseline`（EVM plan 基线 SSOT·`task_estimates`/`dag_snapshot`/`bac_h`/`history`）+ task `model`（派发/完成时记录模型档），均 agent-shaped 非窄腰（红线 2 不破）。配套 `ccm baseline` 写 noun（`snapshot` 快照基线 / `show` 只读 / `reset` re-baseline 旧入 history 只增不删），是后续只读 namespace 的 EVM/SPI 计算消费源。
- **`ccm usage` + `ccm estimate` 两个只读 advisory namespace（P2/P3·OR/ML 估算 + 配速引擎集成层）** — 在 hand-roll 算法层（双通道 Monte Carlo / EWMA 分层校准 / split+Mondrian conformal / EVM+Earned Schedule / SLE / CCPM·**0 新 dep**·约束过滤现代 SOTA、重型 ML 逐条排除）之上接 CLI：**`usage`（配额侧·charter ②/⑤）** = `show`（当前号 + 全备号 5h/7d `used%`/`resets_at` registry 快照）/ `advise`（双侧走廊 verdict `throttle|accelerate|hold|hard_stop` + 推荐 lever + `switch_candidate`）/ `task-cost`；**`estimate`（工作侧·charter ④/⑥）** = `show` / `forecast`（双通道 MC → P50/P80/P95 ETA + makespan + 敏感度）/ `evm`（EVM + Earned Schedule SPI(t)·消费 baseline）/ `velocity` / `risk`。**两 namespace 纯只读硬不变式**（全 verb 走 `runRead`、零写、不抢 board-lock）。**5% 硬墙**：所有预测 `p95` 绝不算到 100%（真上限是 session hard-stop）。**诚实字段贯穿**（`source`/`confidence`/`as_of`/`coverage_pct`/`snapshot_stale`/`history_n`/区间）；信号不可得 = exit 0 + `available:false`。**home 级跨板历史语料**（`--scope home|this-repo|this-board`·默认 home 多层收缩）。**红线 3**：ccm 出 verdict/数据，**不替 orchestrator 决策**。
- **`ccm` 成本轴：偿付力仪表（ENG-B·配额% = 唯一权威账本）** — 把资源问题建成「时间存量 × 成本流」双变量受限优化的成本侧：`usage burn-rate`（配额%-burn-rate · 5h+7d 双窗口 · %/h）· `usage runway`（剩余走廊空间 ÷ burn → 距触顶 vs 距 reset 的 headroom · verdict `ample`/`will-exhaust-before-reset`/`unknown`）· `estimate cost-to-complete`（剩余 backlog × 每单位工作的配额%增量 · throughput-MC · 回答「剩余预算装得下吗」）。核心纪律 **配额% 是唯一权威单账本**（捕获全体消费含 master 前台 + cache 经济），token 仅作派活相对 sizing 辅助。三个 verb 全只读、零写、优雅降级（账户信号不可得 → `available:false`）。
- **`pacing-and-estimation` skill（SKILL H）** — 把「消费 ccm 只读 advisory（`usage`/`estimate`/`baseline`）配速 + 估算」升成顶层可发现 skill（动机：estimate 整轴此前 out-of-mind 从不被 router 召回）：四档模型相对成本 / 按难度选档 / 主线为何固定模型（model-tiers）· 5h/7d 配额信号源链 + 诚实天花板（usage-signals）· 双侧走廊 verdict→lever + effective-N 缩放（pacing-levers）· estimate verb 消费 + baseline 生命周期 + 诚实字段（estimation）。**ccm 出 verdict、A 决策**——只教消费层，决策回 A。
- **`ccm statusline` self-contained status line + 无感知自动安装** — ccm 自带一条完整 status line（`ccm statusline`·= bare noun 默认 verb `render`）：读官方 status-line stdin JSON → 渲染**单行 ANSI** 状态行（context 进度条 10 格 + 5h/7d 配额用量·按阈值绿/黄/红变色·缺段优雅省略·null context 不显 ctx），**同时**把 `rate_limits.{five_hour,seven_day}` 落用量 sidecar（subsume 退役的带外 `statusline-capture.js`·account-authoritative pacing 信号源）。配 `ccm statusline install`（幂等把它写进**全局** `settings.json`·绝对路径绕过 Finding #39·备份你原有的 statusLine）/ `uninstall`（从备份恢复 + opt-out 标记让自动安装不再覆盖回去）。**无感知自动安装**：首次跑任意非-`statusline` ccm 命令时 marker 守、静默、幂等地装一次（kill-switch `CC_MASTER_NO_AUTOINSTALL=1` / opt-out 标记 / 已装 marker 任一在即 skip；坏 JSON 的 settings.json 绝不覆写）。退役 `skills/master-orchestrator-guide/scripts/statusline-capture.js`（采集已内置进 ccm·README EN+ZH 删手接指导改为「自动配置·会覆盖·可恢复」诚实提示）。

#### board.policy 自主权限 + 多账号负载均衡 + 无重启换号

- **`board.policy` + `ccm policy` namespace + 切号机制硬闸（ADR-016·board 框定 orchestrator 自主权限）** — board 新增可扩展 **`policy` 段**（✎ 非窄腰·hook 不读·红线 2 不破）框定本块板 orchestrator 自主权限，首条键 `autonomous_account_switch`（`allow`/`deny`）门控**是否允许自主换号**；缺省 = allow（opt-out·向后兼容）。强制力 = **纵深防御**：① 建议层（SKILL A 换号决策点先读 policy + 「绝不自授权」红线）+ ② 机制硬闸（切号前经进程边界 `ccm policy show --board <确定性目标板>` 校验·`deny`→拒绝换号 exit 7 + log·fail-open 分两类堵 discovery-failure 绕过）。配套顶层 `ccm policy` 写 noun（`show`/`set`·写为用户所有·非 TTY 须 `--user-authorized`）。
- **账户管理迁进 ccm `account` namespace（号池 CRUD + 无重启换号·token-blind）** — 换号号池机制整体迁进 ccm `account` 引擎：`add`（录入当前登录号完整 OAuth blob 进 vault + accounts.json·身份 guard）/ `refresh` / `delete` / `list` / `switch`（无重启覆写官方三存储 + trap 恢复 + 切出 token 抢救）。token-blind 铁律贯穿（token 全程活在脚本子进程 + OS keychain / 0600 file vault，绝不进 agent context / log / registry）；账户权威 5h/7d `used_percentage` 是 pacing 的权威信号源（ADR-008）。
- **LOADBAL 多账号负载均衡 + LBHOOK 自主切号（ADR-016/017 联动）** — `account switch` 选号引擎扩为**负载均衡**：inactive 账号配额恢复**预测**（`SNAPWIRE` 切出快照 wiring 喂语料）+ select 按 reset-proximity / reserve-floor 择最优切入号 + `WHEN` 触发判定。`usage-pacing.js` hook 接 LOADBAL **自主换号**（LBHOOK·切号执行 / 选号 / `board.policy` 硬闸都委托 ccm·hook token-blind·冷却防抖·`CC_MASTER_AUTOSWITCH=0` kill-switch）。

#### 多-orchestrator 协调（ADR-017）

- **多-orchestrator 协调感知层 + 跨板只读花名册（COORD）** — 多块板同时活跃时的 orchestrator 间协调：`ccm peers` 读 verb（扫 home 全 active 板出 goal / owner / 进度·不抢锁）+ board 新增 ✎ `coordination` 块（agent-shaped·hook 不读·红线 2 不破）。协调走**感知 + 板级优先级 + 机械 floor**（换号协调溶解进 LOADBAL 选号、配速协调走各板独立感知），**砍掉显式通信通道**（无 message-passing·避免分布式复杂度）。

#### hooks 体系

- **hook→agent 标签化消息协议（ADR-018·AGENTS.md §13）** — 立一套标签化协议约束**所有 hook 往 agent context 注入的文本**——洞察「没有中性注入，凡注入即潜移默化塑造行为」：三类 closed-set taxonomy（`<ambient source>` 背景·决策归 agent / `<advisory source strength="weak|strong">` 建议·喂判断·agent 最终拍 / `<directive source>` 指令闸·决策归 system·绑定遵从 + why）+ 所有标签必带 `source` + P1–P6 作者纪律。作者侧落 §13、读者侧契约进 SKILL A（先跑 pressure baseline）。纯文本注入·不动窄腰·不改武装闸（红线 1/2/5/6 不破）。
- **runHook harness（phase-1b·武装收口为结构性保证）** — 五个 node hook 的武装闸收口进 `hook-common.js` 的 `runHook(spec)` harness（武装是它的固定环节·真代码级保证「每 hook 入口先过武装」，非靠各 hook 自觉）：reinject / posttool-batch / usage-pacing 用 `arm:'boards'`，board-lint / verify-board 用 `arm:'custom'`。红线 6 dormant-until-armed 从「各 hook 自觉」升级为「harness 结构性保证」、不弱化。
- **周期身份提示 hook（IDNUDGE）+ board 参数区 `runtime` + `ccm board set-param`（ADR-020·hook 经 ccm 写 ✎ board）** — 松绑 §12 此前「hook 永不碰 board」pre-ccm 保守默认——许可 hook 经 `ccm` 带锁字段级 setter 写特定 ✎（非窄腰）board 字段，受六硬约束框死（只写 ✎ `runtime.*` / 进程边界 spawn / 带锁 + lint / 武装后 + 确定性目标板 / ccm 缺降级 / token-blind）。落地：第 7 个 hook `identity-nudge.js`（**Stop 周期提示**·默认 6h 重申「你是 master orchestrator」+ 漂离提示重温 SKILL A·advisory weak·读 `runtime.last_identity_remind` 判阈值、经 `ccm board set-param` 写回·**写回成功才注入**杜绝 spam）+ `@ccm/engine` ✎ `board.runtime` + `FMT-RUNTIME` warn + `ccm board set-param` least-privilege 写 verb（收窄 `runtime.*`·白名单 + 值校验）。**窄腰一字不动**（runtime 是 ✎·hook 不读·红线 2 不破）。
- **hooks 增强 v2 三块** — ① **bootstrap ccm install-presence 硬前置（ADR-021）**：`bootstrap-board.sh` 触发后建板前硬查 `command -v ccm`（`CCM_BIN` 覆写则 `[ -x ]`·纯 bash·不 spawn ccm·红线1 floor），缺则**拒 arm**（不建板·不武装）+ 注 `<directive source="bootstrap">` agent-relay 提醒用户装 ccm + exit 0，框定「装没装」（二元·起点 fail-loud·用户可修）vs 运行时「装了但这一下没响应」（瞬态·软扛·绝不动）的边界。② **临界路径周期提示 critpath-nudge**：并进 `identity-nudge.js` 周期提示表 `[identity, critpath]`（默认 2h·advisory weak），报临界路径进度 X/Y（`chain ∩ tasks[].status` 纯计数·读窄腰）+ 可选 `ccm estimate evm` 出 on-track/behind（无 baseline 优雅降级）·读 `runtime.last_critpath_remind`。③ **PostToolBatch pacing 中途采样**：`usage-pacing.js` 同时挂 Stop + PostToolBatch，把配速感知从「轮末才知」前移到「回合中途也查」（band 跨档升 OR 冷却满才注入·节流 sidecar·只报临界侧不报 underuse·不换号·sub-agent 闸早退）。

#### 文档 / 手册

- **产品功能手册 `design_docs/feature-manual.md`** — 面向人的功能索引 + 状态总账：分层铺开各功能面（编排决策 / board 模型 / `ccm` 命令面 / pacing 成本轴 / 换号号池 / 武装闸 / webview …）+ 每项状态标记（✅ 已 ship / 🔨 实现中 / 📐 仅设计）+ 用户价值视角，并把「设计前沿 / 正在长成什么」单列一节诚实区分已 ship 与雄心方向。
- **README 产品化重写（EN + ZH 同步）** — 两份 README 从功能堆叠重写为面向使用者的产品介绍（多受众分层披露 + 信息差放大 + 诚实划界），安装段改「`ccm` 必须先装」（ADR-021 硬前置·两份同步）。

#### 跨平台分发与安装（dual-OS macOS + Linux）

- **发版 CI 自动产「解压即装」plugin 制品 + 补 Linux arm64（`ccm-release.yml`）** — release workflow 在多平台 ccm SEA 二进制（`ccm-darwin-arm64`/`-x64`、`ccm-linux-x64`、**新增 `ccm-linux-arm64`**·`ubuntu-24.04-arm` runner·原生不交叉编译）之外，新增 `package-plugin` 作业:经 `scripts/package-plugin.sh`（allowlist 打包·dev-only·repo 根）产出 `cc-master-plugin-<tag>.zip`（解压即 plugin 根·含 `.claude-plugin/` + 7 个分发 skill + commands/hooks/docs·排除 `ccm/`/design_docs/tests/scripts 等非分发物），解压跑 `claude plugin validate` 自证后随同一 GitHub release attach。每个 release 四个 ccm 二进制 + plugin zip 配套同 tag。
- **README 版本感知安装指引 + 人类使用指南（EN + ZH 同步）** — `Get started` 重写为具体可执行两步（`uname` 判 OS/arch → 下对应 `ccm-<os>-<arch>` → 重命名 `ccm`/chmod/放 PATH/`ccm --version` 验；再下同 tag `cc-master-plugin-<tag>.zip` → 解压 → `claude --plugin-dir`），任意 agent 照着即可装对版本到对位置；新增 `Everyday use` 人类常用命令速览（as-master-orchestrator / status / view / discuss / stop / handoff / `ccm account`）。

### Changed

- **hook 收编为 node + 经进程边界调 `ccm`（ADR-013 §2.4 / ADR-014）** — `reinject` / `verify-board` / `posttool-batch` 从 bash 收编为 node（经共享 `hook-common.js` + `board-model` 派生·`JSON.parse` 取代 v1 的 awk 深度扫描，根除「归档板嵌套 active」「task-local log stale」等一整类全文 grep/awk 误读盲区）；board-lint / verify-board 改经进程边界 `spawn ccm`（不再 in-process require 引擎·缺 `ccm` 静默降级不 block），webview 改吃 vendored `@ccm/engine` IIFE。随之**删除插件内旧的内嵌 `cli/`**——board 引擎 SSOT 现归 `@ccm/engine`。`bootstrap-board.sh` 仍为 bash（唯一豁免的 ARM 动作）；红线 6 dormant-until-armed 字节级保持。
- **home 从 per-repo 迁到全局（HOME-A·升级用户旧 board 自动迁移）** — home 从 per-repo（`$CLAUDE_PROJECT_DIR/.claude/cc-master/`）迁到全局：home = `$CC_MASTER_HOME`（默认 `$HOME/.claude/cc-master/`），`*.board.json` 集中落 `<home>/boards/`，home 根放全局 `accounts.json` 号池 registry + decision sidecar + coordination channel。bootstrap 时做一次性、非破坏的最佳努力迁移（旧 per-repo 板复制进 `<home>/boards/`·保留原件·同名跳过·吞错），让升级用户旧板 `--resume` 仍找得到。hook 武装闸只读 narrow-waist `owner.active`/`session_id`（红线 2/6 不变）。
- **`usage-pacing.js` / `cc-usage.sh` 走廊数学收口进 `ccm` + 去 python3（ADR-015 P4）** — Stop hook 不再自算双侧走廊数学，武装后优先经进程边界 `ccm usage advise --json` 取 verdict（引擎 `pacing.ts` 是走廊数学唯一真相源），`ccm` 缺 / 失败 / `available:false` → 回退本地计算路径（绝不丢提示能力）。带外 `cc-usage.sh` 从「python3 解析本地 JSONL 反推」改为薄 shell 包装委托 `ccm usage show/advise`，**零 python3 依赖**（本地反推已撤·引擎不含反推·Finding #37 证反推 reset 失真到数量级；此带外信号现仅账户权威）。
- **全栈 `~/.claude` 路径跟随 Claude 的 `CLAUDE_CONFIG_DIR`（不再硬编码）+ dual-OS 兼容收口** — 新增 `@ccm/engine` 的 `paths.ts` 共享 SSOT（`resolveClaudeConfigDir = $CLAUDE_CONFIG_DIR || $HOME/.claude` + 派生 home/credentials/rate-cache/projects/`.claude.json`）；ccm（15 处）+ hooks（bootstrap/hook-common/usage-pacing/board-lint·纯 env 读·红线1 不 spawn）+ skills 脚本全栈改为跟随 Claude Code 自己的配置目录重定位。优先级链 = 显式 flag/env（`--home`/`CC_MASTER_HOME`/`CLAUDE_JSON_PATH`/`CRED_PATH`/`CC_MASTER_RATE_CACHE`）> `CLAUDE_CONFIG_DIR` 派生 > `$HOME/.claude` 派生（向后兼容）；credentials 走 macOS Keychain / Linux 明文 `.credentials.json`（随 configDir）·`.claude.json` 双路径容错（`<configDir>/.claude.json` → `$HOME/.claude.json`）。bootstrap 的 ccm-missing directive 同步补具体装法（下载 `ccm-<os>-<arch>`→PATH）。dual-OS 尾巴:`codex-review.sh` `mktemp` 去 BSD/GNU `-t` 分歧语义、3 个 bash 脚本经 bash 3.2 审计（无关联数组/`${var,,}`/`mapfile` 等·clean）、Linux 换号写路径（无 keychain→file `.credentials.json`）核正确。
...
- **两 skill 分离原则扩为三（ADR-005）** — `account-management` 作为第 3 个**分发** skill 经 curating 闸纳入，红线 3「三个分发 skill 各自自洽、互不重叠」成形：SKILL A（`master-orchestrator-guide`，编排决策 + 换号**决策**）/ SKILL B（`authoring-workflows`，workflow 脚本写法）/ SKILL C（`account-management`，换号**机制**层：号池管理 / 选号 / 切号 / vault token 安全）。换号的「何时换、谁拍板」归 A、「怎么选号切号管 vault」归 C，A 在 pacing 决策点**单向引用** C 而不复述其机制。`master-orchestrator-guide` 的 SKILL.md（镜头 5 换号 lever 改为「无重启」）+ `references/cost-and-pacing.md`（换号 lever 4 步决策序列）+ `references/external-coordinates.md` 同步。
...
- **dispatch 指导优化（反过度工程护栏 + parallel-vs-pipeline smell-test 指针 + 机制选择软指针 · G1.5）** — 收紧机制选择这一段，防「为并行而并行」的过度工程：
  ...
  - `master-orchestrator-guide/OBJECTIVE.md` 新增**「机制选择正确率」`strict_dim`（O-4）**——把「机制选对没有」纳入成功契约 J 的可度量维度。
...
- **魂新增反过度串行纪律 + `master-orchestrator-guide/OBJECTIVE.md`（Finding #44）** — 反假串行的承重纪律此前全住 references（`decomposition.md` / `dispatch.md` / `cost-and-pacing.md`），而每次 compaction 整篇重注的常驻手册 `SKILL.md` 无显式护栏——常驻的反并行压力（lens5「限 WIP / 瞄 75%」）对上非常驻的反串行纪律，天平系统性倒向串行。现把三处承重护栏上提进魂：**lens2 加「边即债务 / 默认并行 / 逐边举证」**、**Rationalization Table 加「窗口紧 / 预算紧 → 串起来省」行**（真相：串行**不省 token**、只拉长 makespan，省预算靠降档 / 控 WIP / 推迟 float）、**Red Flags 加「画了边却说不出被下游消费的上游产物」条**；`decomposition.md` §2 加反向指针（规则 SSOT 在魂、论证 SSOT 在 reference，勿互抄）。新增 `skills/master-orchestrator-guide/OBJECTIVE.md` 把**并行度**纳入成功契约 J。Track B 重跑：魂-only 臂从「靠推导默认并行」变为「逐字引用三处新规则 + 具名删 4 条假边」（价值在强模型天花板下的 legibility + 跨 compaction 一致性，非宽度跃升）。
...
- **两个分发 skill 补 `DESIGN.md` 设计宪法** — `skills/master-orchestrator-guide/DESIGN.md` 与 `skills/authoring-workflows/DESIGN.md`（6 段模板：one-liner / craft 自分类 / value triad / 责任边界 / 触发与反例 / 演化锚），补齐「每个站得住的 skill 都配一份 DESIGN.md」此前只覆盖 dev-only meta-skill 的不对称。
...
- **全仓 self-contain scrub（去保密场景 + 去外部出处痕迹）** — ① 把示例/demo/fixture 用的一个真实保密项目场景全部替换为通用合成的 **i18n 国际化场景**（README×2 / `examples/sample-orchestration/{walkthrough.md,smoke.sh}` / `skills/master-orchestrator-guide/{references/board.md,assets/board.example.json}` / `design_docs/{spec.md,eval/track-b-benchmark.md}`，三者对齐、`smoke.sh` 真跑过、`board.example.json` 结构测试绿）;② **彻底清除所有外部出处 / 上游项目的字眼与暗示**，让本仓所有文档项目内自洽（涉 `AGENTS.md`/`CHANGELOG.md`/`design_docs/` 多篇/`skills/.../references/async-hitl.md` 分发 ref/dev skills），repo-wide grep 两轴零残留。详见 dogfood Finding #29（must-fix 泄密，用户 review catch）。
- **distributed skills 内容母语化打磨** — 按 `cc-master-skillsmith` craft + `curating-skill-portfolios` 结构,对 `master-orchestrator-guide`（SKILL.md + 6 reference）与 `authoring-workflows`（SKILL.md + 3 reference）做纯中文母语化 + clarity 收紧;**牙齿零改动**（决策程序 dot-graph byte-identical、红线/Rationalization/Red Flags 语义与结构保留、所有代码块/公式/API 签名/board 契约 byte-identical，机器校验）。
...
- **`master-orchestrator-guide`（SKILL A）按六愿景重组** — `SKILL.md` 加**六愿景索引地图** + 「当 hook 对你说话」共鸣小节（把运行时 hook 注入与七镜头 / 决策程序接上，告诉编排者 hook 提示该如何消化）；六个 reference 各打**愿景 tag**；`DESIGN.md` §2/§6 同步。reinject 重注友好原则下，索引地图入主文件、深细节仍留 references。
...
- **`commands/` 全中文化 + `as-master-orchestrator` 整体重构** — 三个命令(`as-master-orchestrator` / `status` / `stop`)正文中文化;点火命令 `as-master-orchestrator` 重构为**薄点火层**(点火 + 指向 `master-orchestrator-guide` skill,不复述七镜头 / 红线 / 决策程序——红线 3 + reinject 重注友好),保留三步骨架 + 「指挥不演奏」收尾;bootstrap sentinel(`<!-- cc-master:bootstrap:v1 -->`)byte-exact 首行触发机制与所有技术字面量(`$ARGUMENTS` / `owner.session_id` / `tasks[]` / `blocked_on:"user"` 等)原样保留。
...
- **Model tiering & usage-aware pacing (SKILL A reference)** — new
  `skills/master-orchestrator-guide/references/cost-and-pacing.md`: the four
...
- **codex as a second endpoint reviewer** — `scripts/codex-review.sh` wraps
  ...
  `skills/master-orchestrator-guide/references/resume-verify.md`.
...
- **Skill A — `master-orchestrator-guide`** — the main-thread orchestration method:
