...
- **Agent registry operating manual in `using-ccm`** — the distributed skill gains a full `ccm agent` namespace chapter in the command catalog (create/bind/link/terminal/probe/list/show: evidence-gated bind, idempotent agent-side link, graded probe/reconcile semantics with seen-before death evidence and evidence-strength orphan recovery, exit codes) plus a board-model guide section for the `agents[]` runtime roster (agent-vs-executor layering, lifecycle state machine, handle kinds, ccm-owned probe fields) and quick-reference entries for the two new lint rules `FMT-AGENTS` / `BIZ-INFLIGHT-AGENT`.
