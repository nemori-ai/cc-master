...
- **Hooks 增强 chunk v2（三块）：③ PostToolBatch pacing 中途采样 + ② 临界路径周期提示 + ④ bootstrap ccm 硬前置（ADR-021）** — 三块互不耦合的 hook 增强：
  - **④ bootstrap ccm install-presence 硬前置（ADR-021·拒 arm 姿态·用户拍板）** — `bootstrap-board.sh` 在触发门之后、建板之前硬查 `ccm` 装没装（纯 bash `command -v ccm`·`CCM_BIN` 覆写则 `[ -x ]`·**不 spawn ccm**·红线1 floor）。缺则**拒绝 arm**：不建 board + 注一条 `<directive source="bootstrap">` additionalContext（agent-relay 提醒用户先装 ccm·ADR-018 directive·硬前置缺失是系统级硬约束）+ **exit 0**（不 `decision:block`·否则 agent 收不到 directive 无法转告用户）。不建板 → 不武装 → 所有 runtime hook 续休眠（红线6 自然成立）。幂等可重试（装好 ccm 后重跑即正常 fresh-arm）。框定 **bootstrap 硬查（「装没装」·二元·起点 fail-loud·用户可修）vs 运行时软降级（「装了但这一下没响应」·瞬态·**绝不动**·不让一次抽风崩长程编排）的边界**。把 ccm（ADR-014 已定的主机前置）从「运行时静默降级才暴露」提前到 ARM 入口 fail-loud——不新增依赖（不破 ship-anywhere）。立 **ADR-021** + README/README_zh 安装段改「ccm 必须先装」（硬前置·两份同步）。
  - **② 临界路径周期提示 critpath-nudge（并进 `identity-nudge.js` 周期提示表·hooks-enhancements-v2 ②）** — `identity-nudge.js` 概念升级为 **periodic-prompts hook**：顺序跑一张周期提示表 `[identity, critpath]`（共享 `hook-common.periodicNudge` helper·同一武装 / 单 active 板守卫 / 各自独立计时）。新增 **critpath-nudge**（默认 2h·env `CC_MASTER_CRITPATH_INTERVAL_SEC`·advisory weak·`source=critpath-nudge`）：报临界路径整体进度 **X/Y**（`chain ∩ tasks[].status` 的**纯计数**·读窄腰·红线2·hook 不算图·红线3）+ 可选 `ccm estimate evm` 出 on-track/behind 判定（无 baseline 优雅降级·省按期/落后从句·诚实记账·ccm 出 verdict·红线3）。临界链 `chain` ← `ccm board critical-path --json`（进程边界）。读 `runtime.last_critpath_remind` 判阈值、注入后经 `ccm board set-param` 写回（写回成功才注入·anti-spam）。为此把 `runtime.*` 白名单加第二键 `last_critpath_remind`（ISO·`@ccm/engine` `RUNTIME_PARAM_KEYS` + `FMT-RUNTIME` 各加一条·`using-ccm` command-catalog + board-model-guide 锁步·§6）——正是 ADR-020 预留的「加一个键 = 加一条 ✎ 字段说明 + 复用同一条 warn lint」同形扩展。两条 nudge 都 due 时同回合注两条 advisory（各 weak·语义正交）。
  - **③ PostToolBatch pacing 中途采样（`usage-pacing.js` 多事件·hooks-enhancements-v2 ③）** — `usage-pacing.js` 同时挂 **Stop + PostToolBatch**（`hook_event_name` 分流·`hook-common.runHook` 的 `event` 支持函数形态让 envelope hookEventName 与触发事件一致）：Stop 走现有完整路径（双侧 pacing + LBHOOK 换号 + 号池 ambient·零回归）；PostToolBatch 走新的**中途采样轻路径**——把配速感知从「轮末才知」前移到「回合中途也查」。节流（防通知风暴·本块命门）：注入 ⟺（band 跨阈值升档·normal/throttle/hard_stop）OR（距上次中途注入满冷却·默认 15min·env `CC_MASTER_PACING_SAMPLE_COOLDOWN_SEC`·且仍在临界带以上）；5h 窗口 reset 清 band 记忆。先做**廉价本地预闸**（`readRateCache` 算 band·零 spawn），只在该注入时才 spawn `ccm usage advise`（ccm spawn 被绑定到实际注入·罕见）。节流状态落 hook-owned sidecar `<home>/.cc-master-pacing-sample.json`（非 board·transient + home-global·语义同换号冷却 sidecar）。中途**只报临界侧**（throttle/hard_stop·不报 underuse·方向性服务减速/换号侧）、**不含 autoswitch**（换号留 Stop-only·带网络 POST 的重操作不放高频 PostToolBatch）、PostToolBatch sub-agent 闸（stdin 顶层 `agent_id` → 静默早退·红线4）。`hooks.json` PostToolBatch 数组追加 `usage-pacing.js`。
  - 三块全过两道门（`run-tests.sh` ALL PASSED + `plugin validate`）+ 红线1/6 grep + ccm engine/CLI 测试全绿（392/460）。AGENTS §2/§14/§N + ADR-020 关联段 + adrs/AGENTS.md 表锁步。
- **周期身份提示 hook（IDNUDGE）+ board 参数区 `runtime` + `ccm board set-param` 写 verb + §12 松绑（ADR-020）** — 新增第 7 个 hook `identity-nudge.js`（**Stop 事件周期提示**）：长会话里隔默认 6h（env `CC_MASTER_IDNUDGE_INTERVAL_SEC`）轻量重申「你是 master orchestrator」+ 漂离时提示重温 SKILL A（advisory weak·ADR-018），补 `reinject`（compaction 边界）够不到的「无 compaction 时缓慢漂移」盲区。它读 board 上新增的 ✎ 参数区 `board.runtime.last_identity_remind`（ISO-8601 UTC）判阈值、提示后**经进程边界 `ccm board set-param` 把时间戳写回**（**写回成功才注入**·杜绝 ccm 缺时每回合 spam）。为此新增：① `@ccm/engine` ✎ board 字段 `runtime`（hook-owned 参数区·与 policy/coordination/baseline 同档非窄腰）+ `FMT-RUNTIME` warn（规则全集 46→47·缺/坏一律 graceful-degrade）；② `ccm board set-param <key> <value>` **least-privilege scoped 写 verb**（候选 B·用户拍板·作用域收窄 `runtime.*`·键白名单 + 值校验〔非白名单 key / 非法 ISO → exit 2〕·走 runWrite 带锁管线）；③ **ADR-020** 松绑 AGENTS §12 此前「hook 永不碰 board」的 pre-ccm 保守默认——许可 hook 经 ccm 带锁字段级 setter 写特定 ✎（非窄腰）board 字段，受六硬约束框死（只写 ✎ `runtime.*` / 进程边界 spawn / 带锁 + lint / 武装后 + 确定性目标板 / ccm 缺降级 / token-blind）。clobber（lost update）走**轻解**：hook 独占 `runtime.*` + agent 永不写它 + board.md/SKILL.md 一句澄清「整文件 Write 须先 re-read 合并保留 hook-owned `runtime.*`」（**不**退役整文件 Write·那是独立长期议题）。**narrow waist（红线2）一字不动**（runtime 是 ✎·hook 武装闸不读它）·hook 仍 bash+node/JS（红线1）·dormant-until-armed（红线6·arm:'boards'）·token-blind·ccm 缺优雅降级（红线5）。`using-ccm` 两份 reference（command-catalog + board-model-guide）+ AGENTS §2/§12/§14/§N 锁步。
- **Skill portfolio 重排：新增 `pacing-and-estimation`（SKILL H）+ 退役 `account-management`（SKILL C）·分发 skill 7→7（ADR-019）** — 把「消费 ccm 只读 advisory（`usage`/`estimate`/`baseline`）配速 + 估算」从 SKILL A 的 reference 升成**顶层可发现 skill** `pacing-and-estimation`（覆写 2026-06-26「做 A 的 reference」判定·动机 = estimate 整轴此前 out-of-mind 从不被 router 召回·B.2 触发覆写）：四档模型相对成本 / 按难度选档 / 主线为何固定模型（model-tiers）· 5h/7d 配额信号源链 + 诚实天花板（usage-signals）· 双侧走廊 verdict→lever + effective-N 缩放（pacing-levers）· 5 个 estimate verb 消费 + baseline 生命周期 + 诚实字段 + usage⊗estimate 张力（estimation）。**ccm 出 verdict、A 决策**——只教消费层，决策回 A。配 DESIGN.md/OBJECTIVE.md/evals。同时**退役** `account-management`：换号机制已全迁 ccm `account` 引擎（add/refresh/delete/list/switch·policy 硬闸 `deny`→exit 7）+ 操作面归 `using-ccm`（command-catalog namespace account + 新 reference `account-pool.md` 概念叙事）；**`/cc-master:accounts` 命令直接退役删除**（账号操作全归 `ccm account` CLI·用户直接敲·token-blind·自动切号在 usage-pacing hook 机械触发 → 一个 agent-facing 命令零增量零覆写 = 装饰）。SKILL A 留魂瘦身：镜头 5/2 正文不删（指针改指 H）、决策程序 §(f) 7d 闸牙齿不动、换号决策锚下沉新 `references/cost-decisions.md`、删旧 `references/cost-and-pacing.md`。退役 C 同删测其旧 bash 脚本的 6 个 plugin-side 测试（机制已由 ccm CI 测）。AGENTS §2/§3/§6/§12/§N + skill-lint.sh 七名单同步。**hook 一字不动**（pacing/换号是 ✎/带外·hook 不读 skill 边界）。
...
- **board 引擎解耦为独立 `ccm` CLI（ADR-014 · 工业化 TS monorepo）** — board 的状态逻辑（数据模型 / lint / graph / lock）从「插件内嵌的零依赖 CJS」解耦为一个**独立安装的 `ccm` CLI**：源在 `ccm/`，是 pnpm/Turborepo monorepo——`@ccm/engine`（board 引擎 SSOT·TypeScript 库）+ `apps/cli`（`ccm` 命令）+ 未来的 desktop/web 平行消费方。cc-master plugin **降为 ccm 的消费方之一**：插件的 hooks（board-lint / verify-board）经**进程边界** `spawn ccm` + JSON 契约访问 board，**绝不 import `@ccm/engine`**（TS/npm 依赖锁在 ccm 内——这条进程边界是红线1 在新架构的落点）；board webview 改吃 vendored 的 `@ccm/engine` IIFE（零网络、自包含）。**分发模型**：`ccm` 走 per-OS 的 Node SEA 单文件可执行、主机预装。**优雅降级**：`ccm` 缺失时这两个 hook 静默 no-op（不 block agent 流程），插件仍能在只有 Node+bash 的环境加载并基本工作——只少了 board lint 自动注入 + rollup stop-gate 两项增强；故 **`ccm` 是推荐安装的增强项，不是用插件的硬前置**。hooks 解析 `ccm` 的顺序：`$CCM_BIN`（绝对路径覆写）> PATH 上的 `ccm`。新增的 `ccm` 可选安装前置见 README「Optional but recommended — install the `ccm` CLI」一节。
- **ADR-014** —— `ccm` CLI 解耦为独立产品/引擎（`@ccm/engine` TS SSOT），plugin 降为消费方之一、经进程边界 shell 调全局 `ccm` 二进制 + JSON 访问 board（绝不 import 引擎）；ship-anywhere 口径从「单件自包含 + 零依赖」改为「主机预置 per-OS Node SEA 二进制 + 进程边界」（「宿主 vs 模型后端」之分同 ADR-006）。修订 ADR-013 的 CLI 定位 + ADR-002 的 ship-anywhere 口径。
...
- **`dev-as-ml-loop` skill（第 6 个分发 skill·Craft B 心智模型）** — 给**执行 agent** 的心智模型:把 agentic-loop 开发当成一个 **ML 优化过程**——验收=目标函数（board `acceptance` 字段）、dev loop=迭代优化（提议→测量→调整）、测试=测量仪器/梯度、explore vs exploit、**局部最小值=钻牛角尖→restart 换方向**（把"钻牛角尖"重 frame 为可操作的 restart 信号）、收敛即停别过拟合、拟合意图非用例、简单性=正则。**执行侧 plane**（与 A「指挥不演奏」、E「只切」互补)。诚实记账:Craft B 框架 skill **不走 pressure baseline**（skillsmith 铁律只 gate 纪律 prose;两个早期 baseline GREEN 正说明价值在统一框架、非纪律覆写）。两个方法论 skill（E/F）在关键锚点补了与 ccm 字段/状态的串联（纵切片=带 `--accept` 的 task、收敛=`done --verified`、restart=`failed`/`escalated` 状态机等）。红线 3「五→六个分发 skill」。
- **`engineering-with-craft` skill（第 7 个分发 skill·Craft B 框架 + 工程红线）** — 给在 **design → build → test 动手**的执行 agent 的工程手艺:把 **DDD / SDD / TDD / OOP** 整合成**五条共享根**（不变式即锚 / 契约即 SSOT / 组合优于包装 / 分层思维 / 证据优于声称）+ 几条工程红线（no-silent-failure / spec 不漂移 / test-first / constraint parity / 无 idle wrapper）。深度进 4 个 reference（`ddd.md` / `oop.md` / `sdd.md` / `tdd.md`，从 omne-next 对应 skill 蒸馏出**项目无关**的通用智慧、剥掉项目专属；SDD 无现成源、从第一性原理原创）。**与 F 不同 plane**:F 给执行循环的**形状**、G 给循环里那双手的**手艺内容**（领域 / 类 / 合约 / 测试**本身**怎么建得好）。诚实记账:跑了两个纪律向 pressure baseline（TDD 铁律 / no-silent-failure），**双双 GREEN**——强 agent 单步默认守住,故按 skillsmith 铁律**不伪造** Rationalization Table、红线段按业界既有工程硬规则陈述（价值在统一框架、非纪律覆写,同 dev-as-ml-loop）。红线 3「六→七个分发 skill」。
- **`using-ccm` 升级为「一体两面」+ board 协议 SSOT 归位 `@ccm/engine`** — using-ccm 从「主要是 CLI 操作手册」补全为**一体两面**:面1=命令面(`command-catalog.md`)、**面2=board 模型与字段取值指南(新 `references/board-model-guide.md`,807 行)**——系统性补上领域概念解释(task 字段 / status 八态 / executor 五种 / judgment_call / cadence / parent / watchdog)+ 字段取值判断(acceptance 怎么写 / estimate 怎么估 / deps 怎么连 / executor 怎么选)+ 决策树 + **全部 43 条 FMT/GRAPH/BIZ 校验规则速查**(每条 id / level / 触发 / 怎么一次写对,从引擎 `board-lint-core.ts` + `INVARIANTS` 逐条对账),让 agent 操作 CLI 一次写对、不因不知规则反复撞 exit 3。SKILL.md 加两面阅读指引 + description 补面2触发 + Pointers。**配套 SSOT 归位**:明确 **board 协议 canonical SSOT = `@ccm/engine` 的 board-model**(enums / 字段 tier / 不变式 / 状态机),`orchestrating-to-completion` 的 `board.md` 降为 **orchestrator 侧派生叙事视图、非 SSOT**——同步修正 AGENTS.md §3 红线2 + §N 表、board.md 自述、using-ccm/slicing/dev-as-ml-loop 里所有「board.md = 协议 SSOT」的旧措辞。顺带修 board.md 残留的 `schema cc-master/v1` 陈旧(引擎 + 全部测试 canonical 为 `cc-master/v2`)。
- **`ccm baseline` namespace + EVM 计划基线字段（ADR-015 P1·估算引擎地基）** — 新增两个 ✎ board-model 字段：`board.baseline`（EVM plan 基线 SSOT·`task_estimates`/`dag_snapshot`/`bac_h`/`history`）+ task `model`（派发/完成时记录模型档·#34 档位成本效益），均 **agent-shaped 非窄腰**（红线 2 不破·hook 不读）。配套 `ccm baseline` 写 noun——`snapshot`（从当前 tasks 的 estimate+deps 快照基线，已存在则 exit 3、`--force` 覆盖）/ `show`（只读，无基线也 exit 0）/ `reset`（re-baseline：旧入 `history[]` 只增不删 + 建新，非 TTY 须 `--yes`）；**board 内唯一写 noun**，后续 `usage`/`estimate` 只读 namespace（P2/P3）的 EVM/SPI 计算消费它。lint 补 `FMT-BASELINE`/`FMT-MODEL`（warn），board-model-guide 校验规则全集 **43 → 45**。同步锁步 `using-ccm`（board-model-guide 字段速查 + 规则全集 + command-catalog baseline namespace·红线 §6）。
- **`board.policy` + `ccm policy` namespace + 切号机制硬闸（ADR-016·board 框定 orchestrator 自主权限）** — board 新增可扩展 **`policy` 段**（✎ 非窄腰·hook 不读·红线 2 不破）框定本块板 master-orchestrator 的自主权限，首条键 `autonomous_account_switch`（`allow`/`deny`）门控**是否允许 orchestrator 自主换号**；**缺省 = allow**（opt-out·向后兼容旧板·保 v0.8.0 自主换号现状）。强制力 = **纵深防御**：① 建议层（SKILL A 在换号决策点先 `ccm policy show` 读，`deny` → 不自主换号、surface 给用户拍）+ **「绝不自授权」红线**；② 机制硬闸——`switch-account.sh` 覆写凭证前经进程边界 `ccm policy show --json` 读 policy，`deny` → **拒绝换号 exit 7**（不取锁/不覆写任何官方存储/registry 原封不动）+ board.log 留痕，ccm 缺 / 非法 JSON / 字段缺 → **fail-open** 放行（不误锁未接 ccm 的环境）。配套顶层 **`ccm policy` 写 noun**——`show`（只读 `policy` + `effective` 有效值·`.data.effective.autonomous_account_switch` 钉死契约路径）/ `set --autonomous-account-switch=allow|deny`（**写为用户所有**·非 TTY 须 `--user-authorized` 否则 exit 2·append `decision` 到 board.log），刻意置于只读 namespace 之外（同 baseline 写 noun 定位）。lint 补 `FMT-POLICY`（warn·`policy` 非对象或值不在 `{allow,deny}` 枚举），board-model-guide 校验规则全集 **45 → 46**。同步锁步 `using-ccm`（board-model-guide 字段速查 + 规则全集 + command-catalog policy namespace·红线 §6）+ SKILL A `cost-and-pacing.md` 换号 lever 决策语义 + `account-management` 切号机制文档。**诚实记账**：agent 有 shell，故是纪律 + 审计、非硬锁。
- **`ccm usage` + `ccm estimate` 两个只读 advisory namespace（ADR-015 P2/P3·OR/ML 估算 + 配速引擎集成层）** — 在已实现的 `@ccm/engine` 算法层（双通道 Monte Carlo / EWMA 分层校准 / split+Mondrian conformal / EVM+Earned Schedule / SLE / CCPM·P2/P3 引擎）之上接 CLI 集成层，落地两个 charter 能力面：**`usage`（配额侧·charter ②/⑤）** —— `show`（当前号 status-line sidecar + 全备号 5h/7d `used%`/`resets_at`·备号 = **只读** `accounts.json` registry 生命周期快照·标 `as_of`/`snapshot_stale`）/ `advise`（双侧走廊 verdict `throttle|accelerate|hold|hard_stop` + 推荐 lever + `switch_candidate`·收口 usage-pacing 走廊数学·引擎 `pacingAdvice` SSOT）/ `task-cost`（读 board `observability.tokens`·shell/无遥测 = `N/A`·`coverage_pct`·`--group-by task|executor|type|tier`）；**`estimate`（工作侧·charter ④/⑥）** —— `show`（estimate + EWMA 校准乘子覆写 + conformal 区间）/ `forecast`（双通道 MC：估算-DAG ① + 吞吐 ② → P50/P80/P95 ETA + makespan + 敏感度 CI/CRI/SSI + consistency warning·`--mode`/`--runs`/`--seed`/`--as-of`）/ `evm`（EVM + Earned Schedule SPI(t)·消费 `board.baseline`·无 baseline 降级 warn·`--ac-source duration|token`）/ `velocity`（历史吞吐 + SLE cycle-time P85/P95）/ `risk`（CI/CRI/SSI + WIP-aging SLE + CCPM buffer_health）。**两 namespace 纯只读硬不变式**：全 verb `read:true` 走 `runRead`、零写、不抢 board-lock、不落状态（唯一写例外是已实现的 `baseline`）。**5% 硬墙**：所有预测 `p95` = 95% 分位、**绝不算到 100%**（真上限是 session hard-stop）。**诚实字段贯穿**：`source`（account / registry-snapshot / observability / local-derived-approx）/ `confidence`(high/medium/low) / `as_of` / `coverage_pct` / `snapshot_stale` / `history_n` / 区间 `{p50,p80,p95}`；信号不可得 = **exit 0 + `data.available:false`**（非 exit 1），无 registry → 单账号 `effective_n=1` 优雅降级。**home 级跨板历史语料**：`--scope home|this-repo|this-board`（默认 home·多层收缩·只读）。registry test EXPECTED 8 → **10 namespace**；同步锁步 `using-ccm`（command-catalog 两 namespace 段 + Namespaces 表 + Reserved 摘除 + `--json` 形状·红线 §6）。**红线 3**：ccm 出 verdict/数据，**不替 orchestrator 决策**（真动作归 SKILL A）。
...
- **`usage-pacing.js` hook 走廊 verdict 收口进 `ccm`（ADR-015 P4·pacing 数学 SSOT 归位）** — Stop hook 不再自算双侧走廊数学，武装后**优先经进程边界 shell 调 `ccm usage advise --json`**（透传 `CC_MASTER_HOME`/`CC_MASTER_RATE_CACHE`/`--effective-n`），把 verdict（`throttle`/`accelerate`/`hold`/`hard_stop`）映射成本 skill 词汇的非阻断提示——引擎 `pacing.ts` 是走廊数学的唯一真相源（同 board-lint/verify-board 的进程边界范式）。**优雅降级**：`ccm` 不在 PATH / 调用失败 / 非法 JSON / `available:false` → **回退到 hook 既有的本地计算路径**（account-authoritative sidecar + 本地反推 floor），绝不丢失提示能力。`CCM_BIN` 是 dev/test 覆写口。武装闸（红线6·shell 调 ccm 全在 armed gate 之后）、never-blocks（红线4）、注入嗓音不变。
- **`cc-usage.sh` 去 python3 依赖（ADR-015 P4·plan §10 line 174）** — 带外 usage 信号脚本从「系统 python3 解析本地 JSONL 反推」改为**薄 shell 包装委托 `ccm usage show/advise --json`**（account-authoritative sidecar + registry + 双侧走廊 verdict），reshape 成一行 JSON，**零 python3 依赖**。**诚实降级**：无 sidecar / `ccm` 缺失 → `source:"unavailable"` + `available:false`（本地 JSONL 反推已撤——引擎不含反推、Finding #37 证反推 reset 失真到数量级；此带外信号现仅账户权威。Stop-hook `usage-pacing.js` 仍保留其**自带的**本地反推 floor 作为 in-session 撞墙启发式，与带外信号正交）。`cost-and-pacing.md` 读取方式 re-point 到 `ccm usage advise`。

### Fixed

- **`ccm usage` 当前号信号读错 sidecar 路径（P4 sidecar bug·INTEG 遗留）** — `usage show`/`advise` 的当前号 5h/7d 信号此前读 `${home}/usage-snapshot.json` / `statusline-usage.json`——一条**任何 writer 都不写**的路径，导致账户信号**永远** `available:false`。修为读 `${CC_MASTER_RATE_CACHE:-$HOME/.claude/.cc-master-rate-limits.json}`，与 writer（`statusline-capture.js`）+ readers（`cc-usage.sh`/`usage-pacing.js` hook）三者钉死的同一路径对齐；`normalizeSignal` 正确归一真实 sidecar 形态（`five_hour`/`seven_day` + `used_percentage` + epoch-秒 `resets_at`/`captured_at`）。补两条回归测试（真路径读通 + 旧错路径不再被采纳）。
